...
## R192 — **同じマークは同じ画素数／「分からない」を「陸」と答えるな／震度をその定義で計算する／波を海の上で解く** (tag `#R192`)

指示は6系統。うち1系統（航空機マーク）は**4回目の再報告**で、今回ようやく誰も測っていなかった量が
出てきた——**大きさ**である。通しの主題は **「主張は測れる形にして初めて主張になる」**：マークの大きさ、
陸か海か、震度がいくつか、波がいつ着くか、どのスレッドが詰まっているか——今回はどれも先に測った。

### 1. 航空機マーク——**4ラウンド誰も大きさを見ていなかった**

#R187 がグリフを原典に戻し、#R190 が立体側に同じ輪郭を与え、#R191 が色と白ストロークを合わせた。それでも
報告が続いた理由は、**画面上の大きさが一度も一致していなかった**から。実測（半長・CSSピクセル）:

| | z5 | z9 | z12 | z15 | z17 |
|---|---|---|---|---|---|
| 平面グリフ（`icon-size` ランプ） | 11.0 | 14.8 | 14.8 | 14.8 | 14.8 |
| 立体（#R190 の寸法） | 13.0 | 13.0 | 13.0 | **40.0** | **160** |

フランクフルト上空の実機を z15 で撮って比較: 立体の外接箱 **41 × 82 px** に対しグリフ **19 × 26 px**。
原因は `half = max(60, 13·mpp)` の **metre floor**。「最小の大きさ」に見えるが `13·mpp` はどの縮尺でも
ちょうど 13 px なので、この床は**深いズームでだけ効き**、そこでマークを 120 m の実機サイズに固定して
無制限に膨らませていた。

→ 大きさの規則は **`_PLANE_SIZE` に1回だけ**書き、シンボル層の `icon-size` 式と立体側の
`half = 19·iconSize(z)·mpp` の**両方がそこから読む**。floor は削除。実測後: z2 7.60/7.60・z4 9.88/9.88・
z9〜z17 14.82/14.82（立体/グリフ）。**そして厚みは 0.35 px**（従来 2.2 px）——傾けたカメラが見ていた
「壁」は立体化の副作用であって設計ではない。白ストロークは**穴あきリング**（外周 `_PLANE_RIM`／穴
`_PLANE_CORE`）にした: 0.35 px しか離れていない2枚の面は深度バッファに解けない引き分けを出す。
`IntMapPlanes3D.state()` が `halfPx / glyphHalfPx / thickPx` を返す＝**次からは測れる**。

### 2. 「たまに海もべた塗」——**陸海判定が他人のタイルに依存していた**

再現できた。`buildFar()` の陸海マスクは **z3 の DEM タイル64枚**で、#R191 は「半分未満しか届かなければ
マスクを使わない」と書いていた。その分岐の帰結が報告そのもの——**マスク無し＝全セルを塗る**、つまり大洋が
一面の色になる。「たまに」なのは、それがタイル取得だから: 冷えたキャッシュ、遅い回線、大陸規模の場が
自分で起こす `_demCacheTrim` の追い出し。どれも利用者からは見えない。

陸と海は**昨日と今日で変わらない**ので、答えを同梱した: **`data/land-mask.png`**（2048×1024・1bit・
**19.3 KB**、Natural Earth 1:50m 陸ポリゴンを `scripts/build-land-mask.mjs` でラスタ化）と
**`js/land-mask.js`**。1画素 19.6 km＝遠方場の52 kmセル（携帯104 km）より細かい。`isLand()` は
**true / false / null** の3値で、**「分からない」を「陸」と答えない**ことがこのモジュールの存在理由。
そして**フェイルクローズ**: マスクが復号できなければ遠方場は**描かない**（絵が無いのは絵が無いだけ、
海が塗られているのは間違った絵）。細かい場も同じ扱い——DEM の無いセルはマスクに訊き、それも無ければ塗らない。

⚠ **本番で確かめたら1画素で引っかかった**。ラスタライザは最初、行の**中心**の緯度1本で走査していた。
19.6 km の画素にとって「中心が陸か」は「画素が陸か」ではない——**東京**（139.76, 35.68）が海で返った。
その行の中心緯度 35.60°N が東京湾に落ちるからである。¼・½・¾ の3本を走査して**多数決**にした
（＝画素が画素について答える）。⚠ ただし多数決にしても `isLand(139.76,35.68)` は **false のまま**で、
これは**正しい**: その19.6 km セルは過半が東京湾である。だから DEM が答えられる所では誰もこれに訊かない。
検証は17地点（東京内陸・仙台・大阪・京都・ロンドン・NY・シカゴ・モンゴル・サハラ・アマゾン・シドニー／
太平洋・大西洋中央・南大洋・カスピ海・インド洋・東京湾）を `tests/r192-checks` が**PNGを復号して**当たる
——マニフェストだけ見ていると、全ゼロのマスクでも他の全アサーションが通ってしまう。

### 3. 震度——**PGV は「積分の下限」という定数が決めていた**

「現在の分布は過大評価すぎに思える。」測ったら、過大評価は**帯域**の問題だった。`rvt` は 0.02 Hz から
積分する。Brune ω⁻² 源の**速度**スペクトルはコーナー周波数で最大になり、M9 では fc = 0.0075 Hz（周期133秒）。
実測: M9 の速度エネルギーは **0.02 Hz のビンが 1 Hz の100倍**——100 km でも 7,000 km でも。つまり震度に
渡していた PGV は**積分の下限**が決めていて、それは誰も感じない50秒のうねりであり、どんな強震計も PGV とは
呼ばない量だった。7,000 km で 0.1 Hz 以上は経路で消え、そのうねりだけが残り、**大陸3つに震度1が塗られていた**。

階級は**それぞれの定義の帯域**から計算する:

* **震度＝気象庁「計測震度の算出方法」そのもの**。`jmaFilter(f)`（周期補正 f^-0.5／10 Hz ハイカット／
  0.5 Hz ローカット）を加速度スペクトルに掛け、`jmaA0()` が **合計0.3秒間超える加速度 a₀** を確率振動論で
  解く（3成分ベクトルはマクスウェル分布、V/H=2/3 で等方化——近似であることを明記）。
  `I = 2·log₁₀a₀ + 0.94`。**藤本・翠川の PGV 回帰は廃止**（#R189 の注記が自分で「換算にすぎない」と
  書いていた）。
* **MMI** は Worden et al. 2012 のまま、渡す PGV を **0.1 Hz・4次ハイパス**（強震記録が実際に出せる帯域）で
  求める。PGA は全帯域のまま——PGA を運ぶ周波数ではフィルタは 1 である。
* 震源スペクトルを **Atkinson & Silva 2000 の2コーナー**に。単一コーナーは大地震で中間周波数を
  過小評価するために公表された式で、まさにその欠陥が出ていた。

**採用前に公表 GMPE で検証**（BSSA14、Vs30 760、応力降下量は既定 3.0 MPa のまま）:

| | PGA 模型 / 公表 | PGV 模型 / 公表 |
|---|---|---|
| M6 R10 km | 0.163 / 0.20 g | 11.4 / 12 cm/s |
| M6.5 R20 km | 0.118 / 0.16 g | 11.2 / 13 |
| M7 R30 km | 0.101 / 0.11 g | 13.0 / 14 |
| M7.5 R100 km | 0.038 / 0.035 g | 8.3 / 7 |

——両軸とも±25%以内。⚠ **#R191 が記録した「PGA が公表値の半分」という欠陥は、応力降下量を触らずに
消えた**（触らないという#R191の判断が正しかった。足りなかったのは源スペクトルの形）。

**2011年東北地方太平洋沖（M9.0・深さ24 km・岩盤・点震源）で観測震度と比較**:
...
近距離は依然 1〜1.5 階級低い。**理由は分かっていて書いておく**: (a) 実際の破壊は 450×200 km で仙台の
Rrup は 50 km 程度なのに、模型は震央の点震源、(b) これらの都市は堆積層の上（模型は岩盤）。どちらも
パネルの「震源域を描く」と地盤クラスで利用者が埋められる。**遠方は正しくなった**——最下位クラスの終わりは
M9・最軟地盤で **7,122 km → 1,621 km**、これは東北地震で震度1が実際に届いた範囲そのものである。

### 4. 津波——**「波の伝播のわかるアニメーション」は伝播を解くという意味**

既存の「津波」は `js/sims.js` の**浸水**モデル（26 km 四方を波高まで満たす）で、伝播については何も
言わない。新規 **`js/tsunami.js`**: 球面スタッガード格子（Arakawa C）上の**線形長波**をリープフロッグで
解く——JMA・TUNAMI-N2・COMCOT が沖合で解くのと同じ式。水深は terrarium DEM。屈折も浅海での減速も
**式から出てくる**（当てはめではない）。

初期条件は **Okada (1985)** の海底鉛直変位。断層寸法は Wells & Coppersmith 1994（逆断層）、そして
**走向は海底そのものから読む**——沈み込み帯の境界面は等深線に沿うので、局所的な水深勾配が傾斜方位を与える。
⚠ **公表検証例で確かめてから使った**（L=3, W=2, d=4, δ=70°, 観測点(2,3) の dip-slip uz = −3.5639e-2 に対し
公表値 −3.564e-2）。⚠ そして**それでも1つ罠があった**: `arctan(ξη/qR)` は**主値**であって `atan2` ではない。
`atan2` にすると Chinnery の差分が分岐の跳びを残し、断層の 400 km 背後に**一定 −5.14 m の「沈降」**が
出る（＝ slip·sinδ ちょうど）。有限の震源から離れれば変位はゼロに収束しなければならない。

⚠ **もう1つ**: 太平洋規模の領域は**日付変更線をまたぐ**（日本からだと 208°E まで）。`demSnapshot` は
±179.999 で切るので、実測 **102,400 セル中 19,840（東太平洋全部）が水深不明**で既定値の 3,800 m を
使っていた——このモデルの主戦場が丸ごと。東西2枚のスナップショットにして **no-depth セル 0**。

**検証（2011年東北・M9.1・9時間・320²・36 km セル）:**

| | 模型 | 観測 |
|---|---|---|
| 海底の隆起／沈降 | +6.04 m / −3.53 m | 海溝近くで隆起 5〜10 m、沿岸沈降 ~1 m |
| DART 21418 最大波高 | 2.07 m | ~1.8 m |
| グアム 第1波到達 | 3h15 | ~3〜4 h |
| ミッドウェー 第1波到達 | 4h30 | 長波理論で ~4.5〜5 h |
| 沿岸波高（グリーンの法則） | 16.1 m | 三陸で 10〜20 m |

⚠ 沿岸波高は**水深200 m 以深のセルからしか**外挿しない。最初の版は一番浅いセルを使って三陸で 27 m を
出したが、そこは η/h = 0.16 で**線形理論がすでに壊れている**——模型自身の算術を有効範囲の外まで走らせた
数字であって予報ではない。最後の1 kmは従来の浸水モデルへボタン1つで引き渡す。

### 5. 衛星画像——**重かったのはスレッドの選択だった**

先に測った。東京 z12 で6秒間パンしたときの実測:

| | frames/s | ロングタスク | 合計ブロック | 最悪 |
|---|---|---|---|---|
| デスクトップ（@2x） | 4.8 | 75 | 8.9 s | 275 ms |
| 携帯（@2x 無し） | 22.6 | 7 | 0.40 s | 63 ms |

**驚いたのはデスクトップ**。6秒のパンの中で 8.9 秒ぶんブロックしている＝#R178 の @2x は、1タイルにつき
4取得・4デコード・4描画・1転送を**地図を描くのと同じスレッド**でやっていた。仕事の量は正しい、乗っている
スレッドが違う。

→ **`src/sat-worker.js`**: 取得・プレースホルダ判定・祖先の遡上・切り出し・@2x 合成の**全部**をワーカーで
やり、**転送可能な ImageBitmap** を返す。判断は #R158/#R178/#R179 のまま（2.5 KB のプレースホルダ、13段の
遡上、z10近傍ごとの `have`/`stop` メモ）。深さメモは返信のたび**メインスレッドへミラー**するので
`IntMapSatProto.depth()/wouldStitch()` は同期のまま（テストがそう訊く）。ワーカー／OffscreenCanvas／
転送可能ビットマップが無い環境では**従来のメインスレッド実装がそのまま**動く。

**Cesium も同時に速くなる**——`js/cesium-layers.js` の `AppImageryProvider` は同じ `imapsat` プロトコルを
呼ぶので、両エンジンが1つのワーカーを共有する。

⚠ **そして最初の仮説は外れていた**。ワーカー化のあとプロファイラで見ると、残っている時間は JS ではなく
`(program)` 7,429 ms と `texImage2D` 298 ms＝**画像デコードとテクスチャ転送**、つまり @2x が4倍の画素を
GPU に上げている代金である。これは #R178 が意図して払った代金なので触っていない。**測って「触らない」も
結論**（#R191）。

**携帯も同じ場所で重い。** CPU 4倍スロットリング（＝実機に近い条件）で同じパンを測ると 4.6 fps・
8秒間に 9.7 秒のブロック。プロファイルの内訳も同じ——`(program)` 6,391 ms が支配的で、我々の JS は
ワーカーへ出たあと。ここで **`setImageConcurrency` を 48→16 にする案を実際に試した**（同時デコードの
バッファ数を減らす、#R20 と同じ論拠）。3回で「4.6→8.6 fps」と出たが、**5回に増やすと中央値 4.4 fps・
9,590 ms＝差は無い**（個々の値は 2.1〜9.5 と、主張したい効果より広く散らばる）。
**測れない差は報告しないし、入れない**（#R190）——変更は戻した。

### 6. 起動——**10 MB を先に落としていた**

実測（クリーン起動、種別ごとの転送量）: **その他 10.36 MB**。中身は `data/cshapes.js` **5.5 MB**（t=1,243 ms）、
`ne_10m_admin_0_countries.geojson` 4.3 MB、Köppen ラスタ 2.2 MB。cshapes は #R122 が「最初の時間旅行を
速くする」ために 900 ms の `setTimeout` で先読みしていたもので、**5.5 MB の `<script>` はメインスレッドで
パースもされる**。画面上の何もそれを待っていない。

→ **`requestIdleCallback`（上限6秒、地図の `idle` 後）**まで待つ。#R122 の理由は生きているので先読み自体は
残す。Data Saver / 2G では**先読みしない**。そして `index.html` に、最初の描画が必ず使う4ホストへの
`preconnect`（Carto ×2・OpenFreeMap のグリフ・arcgisonline）。

**実測: 起動時の追加ダウンロード 10.36 MB → 4.77 MB（携帯 10.28 → 4.69 MB）、ヒープ 228 → 159 MB
（携帯 190 → 141 MB）。**

### 7. テスト

`tests/r192-checks.test.mjs`（6件・ソース契約）と `tests/r192.spec.js`（4件・実レンダラ／実ネットワーク／
実モデル）。⚠ 既存の3つのアサーションが R192 の変更で偽になったので更新した（r176 の `PGV_FELT`、
r191 の `pgvAtJMA`／リムの高さ／DEM 陸海マスク）——**置き換えた実装を pin しているアサーションは、
置き換えと同じコミットで直す**。

---

## R191 — **レンダラが返す色を要求する／凍らせた DEM／最下位クラスの終わりまで** (tag `#R191`)

指示は8系統。うち3系統（航空機マーク・震度分布の到達範囲・津波）は**再報告**。今回の通し主題は
**「宣言した値と、実際に出てくる値は別物である」**——航空機の色も、DEM の標高も、`npm run test:profile`
という命令そのものも、書いた通りには動いていなかった。

### 1. 航空機マーク——**`fill-extrusion` は与えられた色を決して出さない**

「元に戻せと言っているのに、色を勝手に変えるな」。#R190 は立体側に**輪郭**を戻して終わっていた。
実測（フランクフルト z10.5・866機・画素統計）:

| | 機体の色 | 機体1画素あたりの白縁画素 |
|---|---|---|
| 平面グリフ（原典） | rgb(30,144,255) = `#1e90ff` | 0.037 |
| at real altitude (#R190) | rgb(35,141,245) | 0.012 |

原因は MapLibre の頂点シェーダそのもの——`color += 0.03`（全チャンネル一律の環境光）に
`directional = mix(1−I, max(1−luminance+I,1), clamp(dot(normal/16384, u_lightpos),0,1))` を掛ける。
**レイヤー単位の逃げ道は 5.24 に無い**（paint プロパティは opacity/color/pattern/translate/height/base/
vertical-gradient だけ。emissive-strength は存在しない）。`light.intensity = 0` にすると全投影で
宣言色ちょうどが出る（実測 rgb(38,152,255)：z8 globe・z13 Mercator・pitch 62° すべて同一）が、
**3-D 建物が完全に平坦になる**（ミッドタウンでスクリーンショット比較）ので不可。

残る道は「出てほしい色になる色を要求する」こと。`directional` は自由変数ではない——白い機体で実測して
**globe 0.933（z4→z10・画面外・pitch 55°・bearing 90° で不変）／Mercator 1.0**。よって補正は
z11.5→13 の2段ズームランプ（レンダラ自身の投影遷移帯）。結果は **globe rgb(30,144,245)・Mercator
rgb(30,144,255)**——青だけは globe で天井（255 には 1.075 チャンネル必要）に当たるが、残り2チャンネルは
完全一致。

**そして原典の白縁も戻した**（利用者に確認済み）。`ensurePlaneIcons` は塗った後に 1.6 px の白でストローク
する＝パスの内外に 0.8 ずつ。だから立体側も **本体＝輪郭を 0.8 内側にオフセット／縁＝0.8 外側**、縁は
本体より 8% 低く押し出して先に積む（重なりは深度テストで本体が勝ち、はみ出した 0.8 だけが白く見える）。
⚠ **#R185 の「中心から拡大したリム」ではない**——拡大では機首と翼端だけ太くなる。頂点法線のマイター
（限界 2.6）で**どこでも 0.8**。実測: 本体18頂点すべてが縁の内側・オフセット 0.803〜2.60（2.60 は機首の
マイター限界）・面積 320→437。

### 2. 「ズームインで全域表示」が消えない——**答えが古かった**

`updatePlanesZoomHint` は `_planeCover.clipped` を読んでいた。これは**走っている掃引の計画**で、次の掃引
まで置き換わらない＝128円の掃引なら最大 **154 秒**古い。ズームインしても、利用者がもう離れた視界の答えが
出続ける。プランナ自体は128セルで安い（`planeCircles(false)` で**採用せずに計画**）ので、通知は毎回
**今の視界**について訊く。パンでも訊く（`moveend`）。切り取られた掃引は必ず**中央ブロック**を覆うので、
その内側にズームした視界は本当に完全である。

### 3. デスクトップの🔍を撤去／レイヤーサイドバーが無条件に透過していた

🔍 は i18n の航空機・船舶ヒント（5言語×3）とレイヤー検索プレースホルダから削除。**モバイル専用の
`#ms-btn::after`（丸FABに文字が入らない）だけ残す**。

`#layer-sidebar-r` は `background:var(--sidebar-bg)`（= rgba …,0.85）＋ blur を**全モードで**当てていた。
設定「サイドバーの外観」は #R33 から「全てのUIはこの設定に従うように」であり、左 `.sidebar` は #R104 から
従っている。このパネルだけ配線されていなかった。不透過モードは左と同じ recessed `--panel-bg` を塗り、
blur は切る（不透過の下のぼかしは誰にも見えない GPU 仕事）。フロスト2モードは無変更＝
**「無条件で不透過」でもない**。

### 4. 震度分布の縞々——**行が計算された時刻に絵が依存していた**

塗りは約83,000セル、8行ごとに `await` して％を出す。その隙間に**共有 DEM キャッシュは同じ物ではない**:
① `warmDEMTiles` のタイムアウトは「届いた分」で返るので、**残りは描画中に届き続ける**（DEM 無し＝パネルの
一様地盤、DEM 有り＝勾配由来。増幅率が丸ごと違う）② `_demCacheTrim` が**追い出す**（予算はデスクトップ
560・スマホ140、大陸規模の場は自分だけで最大520、その上マウス移動が別ズームのタイルを挿す）。
どちらも**縞の幅＝譲る間隔**になる。

→ `demSnapshot(w,s,e,n,z)`（js/map-readout.js）が**復号済みタイルバッファへの強参照の集合**を返し、場は
そこからしか読まない。追い出しは届かず、新規要求も増えない。**ループ内の `demElevAt` も削除**——あれは
*要求*であり、この場が読んでいるタイルを追い出していた張本人でもある。実測（M9.0 沖合）: 169タイルを
凍結・欠損56・`noDem` 0・構築 312 ms・縞なし。

### 5. 震度分布が震度1の終わりまで届いていなかった（再報告）

実測（最軟地盤・最下位クラスが終わる半径）:
...
#R190 の 1,500 km は M7 の 58%・M9 の 21% しか出していなかった。#R190 が「3,000 km の箱は駄目」と測った
のは正しい（欠損71%・20.5秒）——**間違いは1枚の絵に両方の仕事をさせた前提**。数は3つになった:

* `MMI_CALIB_KM` 1,000 km — 減衰式の較正範囲（表は依然ここまでしか数値を出さない）
* `MMI_TERRAIN_KM` 1,500 km — **細かい場**の範囲。DEM についての言明（外側では1セルが地形変化より広い）
* `MMI_MAX_KM` 8,000 km — 最下位クラスが終わる所

外側は地形が読めない＝**距離だけの関数**なので、**全世界ラスタ**として描く。箱が無いので**日付変更線も
極も壊れない**（#R189 が波面で記録した破綻の族）。正距円筒の格子なので `cos(Δλ)` は列ごと・`sin/cos(φ)` は
行ごと、セルあたり乗算2回と `acos` 1回＝**全世界で約40 ms**。

⚠ **そして海を塗ってはならない**。最初の版は塗って、**リング群の真ん中に灰色の長方形**が空いた——細かい
場の箱（海を塗らない）と、その周りの遠方場（塗る）。1枚の絵に規則が2つあるのは、どちらの規則より悪い。
陸海判定に標高は要らず符号だけなので **z3（64タイル・約20 km/px、遠方セル52 kmより細かい）**を読む。
半分未満しか届かなければマスクは使わない（半端な海岸線はデータに見える穴を開ける）。
実測（M9.0 沖合・震度）: rEdge **6,900 km**・遠方場 40,534セル描画・143,950セルは海。

### 6. 地動モデル——引用のある4つの置き換え

「精度、忠実性を根本的に大幅に…高めて」。すべて**出典のある公表モデル**で、パネルと注記（5言語）に名前を
書いた。

* **等価点震源深さ**（Yenier & Atkinson 2014, BSSA 104(3)）`log₁₀ h = −0.405 + 0.235·M`。M8 は 200 km
  級の断層で、点震源には**近距離の飽和が無い**＝震央で誰も観測したことのない値を出す。⚠ **震源域を
  描いた時は加えない**（Rrup が実断層距離なので二重計上）。実測 M8/深さ10 km 震央: PGV 209→49 cm/s。
* **経路継続時間**（Atkinson & Boore 1995 — **三折れ幾何減衰と同じ論文**）。`0.05·R` という出典の無い
  直線を、公表の4区間へ。実測 100 km で 5.0→8.7 秒、1,000 km で 50→42.6 秒。
* **ピークファクター**（Cartwright & Longuet-Higgins 1956, Boore 2003 式(43)–(46)）。Davenport 漸近形は
  **狭帯域**の極限で、地震加速度は広帯域。**第4モーメント**を足して厳密積分に。
* **MMI 換算**（Worden et al. 2012 = ShakeMap の GMICE）。Wald et al. 1999 は MMI V–IX の1本線。
  ⚠ **検証してから採用**: USGS が階級境界に印刷している PGV（1.1 / 3.4 / 8.1 / 16 / 31 / 60 / 116 cm/s）を
  入れると 3.84 / 4.56 / 5.76 / 6.69 / 7.60 / 8.51 / 9.42 が返る＝各境界の 0.3 以内。Wald は同じ 1.1 cm/s を
  MMI 2.5 と言う（1階級半低い）。
* ⚠ **`at()` に MMI 換算の3つ目のコピーがあった**（#R190 は2つ見つけて1つ見落とした）。まさにこれが危険な
  理由で、モデルを変えた瞬間に表と地図が1階級ずれるところだった。

**変えなかったもの**: 応力降下量の既定 3.0 MPa。公表 GMPE（M7・30 km・岩盤で PGA≈0.11 g）に対し本モデルは
0.049 g と低いが、**能動地殻域の確率論的手法に単一の公表値は無い**（EXSIM の ENA は 140 bar、改訂版は
250 bar 相当）ので、出典の無い数字に差し替えることはしない。パネルの調整項目として見えている。

### 7. 日本語では気象庁震度を既定に

`scale` は言語から決まる（`scaleForLang()`）。**利用者が選んだ瞬間に `scaleSet` がラッチ**し、以後の言語
変更は既定を動かさない。パネル・`setParams`・`setScale` の3つの入口すべてが `pickScale` を通る。

### 8. 衛星画像——**切り出したタイルは2回トランスコードされていた**

#R178 の注記は「この経路もビットマップを返すようになった」と書いてあるが、**実際は違った**: 祖先を復号
→部分描画→**JPEG 0.92 で再エンコード**→MapLibre がもう一度復号。1タイルにつき余分なコーデック2回、
どちらもメインスレッド、しかも**海・砂漠・Esri が浅く終わる所＝ほぼ全タイルがこの経路**。MapLibre の
protocol は ImageBitmap をそのまま受ける（#R178 の @2x はずっとそうしていた）ので、持っているビットマップを
返す。切り出し結果の**バイト保持もやめた**——保つ価値があるのは祖先で、それは `_satRaw` が既に持っている。

モバイル: `_satCache` 800→200・`_satRaw` 1200→300。**JPEG バイトの Map が2つで 20〜60 MB**、MapLibre 自身の
タイル予算（#R21 がスマホで 640〜1,024 に削ったもの）の隣に座っていた。デスクトップ用の数字が一度も
読み直されていなかった。

**読み込みの継ぎ目**: `raster-fade-duration` を 0 → **180 ms**（衛星ラスタと全球の床だけ。他のラスタ
オーバーレイは 0 のまま——データ層であり、半分溶けた熱異常値は誤った値）。#R147 が 0 にしたのは
MapLibre の既定 300 ms が指の下で遅く見えたからで、それは正しい。しかし 0 は**タイル単位のハードスワップ**
で、ズームすると鮮明と不鮮明の市松模様が1枚ずつ入れ替わる＝報告された「点滅」。0 では親を保持する物も無い。
#R190 が床と本番の色を合わせてあるので、クロスフェードが「1枚の絵が鮮明になる」ように読める。

**Cesium は測って触らなかった**。#R184 の掃引を実行（⚠ `npm run test:profile` は**まだ動いていなかった**——
Windows で `spawnSync('npx.cmd')` はシェル無しで解決できず status null、`||0` がそれを正常終了に変えて
**何も出力せず成功を報告**していた。#R190 が直したつもりの沈黙の1階層下。修正済み・実行して確認: 1 passed
(2.8m)）。結果: sharpness は MSAA 4/2/1 すべて **9.21** で同一、FXAA を入れると 6.39（#R184 の再現）、
SSE を 2→3→4 に緩めても 9.14 / 9.06＝**1〜2%しか失われない**。つまり現行設定は既にこの計器で見える上限に
あり、フレーム時間はこの機械では同一設定の再測定が 294 ms と 476 ms に割れる（62%）ので**取引の材料に
ならない**。測れない数字は報告しない（#R190）。

---

## R190 — **同じマークを両方の表示で／自分のオリジンから取る／床の色を実タイルに合わせる** (tag `#R190`)

指示は8系統。うち5系統（航空機マーク・既定レイヤー・水・シム開始・起動）は**再報告**なので、今回も
まず**再現と測定**から入った。最大の発見は、5件のうち3件が **「直したこと」ではなく「直したものが
届く経路」** の問題だったこと——保存されたセッション、CORS を返さないホスト、そして
「トリムの包絡線」という1行のクランプである。

### 1. 航空機マーク——**2つの表示が別の絵を描いていたのが全部の原因**

「at real altitude 中も昔のものに戻して」。#R187 は 2-D グリフを原典に戻し、**立体側は #R183/#R185 の
8部品エアライナー（白リム＋暗ハロー）のまま**にした。つまりトグル1つでマークが別物に変わる状態が
残っていた——それが報告そのもの。立体側を **#R172 の描画**（`_PLANE_ORIG` を1枚だけ押し出す＋地上への
細い柱）へ戻し、サイズも `half = max(60, 13×mpp)` に戻した。`_P_LEVELS` / `_PLANE_PLAN` /
`DETAIL_MAX_AIRCRAFT` / リム・ハロー板は削除。**これで「昔のマーク」はトグルに依存しなくなった**ので、
#R187 が既定 OFF にした理由（グリフを画面に出すため）も消えた → 既定 **ON**。

⚠ 既定を変えるならキー世代も上げる（#R189 の教訓を #R189 自身に適用）。`intmap_planes3d2` は
**既定 OFF の時代に書かれた** ので、その `'0'` は「チェック→解除」の副産物であることが多い。読めば
報告された不具合をそのまま返すことになる。`intmap_planes3d3` へ移し、旧2キーは削除。
⚠ `_PLANE_ORIG` の宣言は `_PLANE_OUTLINE` と `ensurePlaneIcons` の**両方より上**へ（#R167/#R183/#R189
が3回続けてモジュールごと落とした TDZ）。

### 2. 既定レイヤー——**「片方しかつかない」の残りは、まだ他人のサーバだった**

#R188 は Cache API を、#R189 は `defv` 世代印を入れた。どちらも本物で、どちらも**2回目以降**の話である。
**1回目の訪問は依然として見知らぬサーバに頼っていた**：submarinecablemap.com は ACAO を返さないので、
ボランティア CORS プロキシ3つのどれかが生きているときだけレイヤーが出る。

そこで #R145（Street View カバレッジ）と同じ形にした：**自前の Edge Function**
`supabase/functions/cable-geo`——2つのURLだけの許可リスト、`Access-Control-Allow-Origin: *`、
features の検証、s-maxage 1日。**データ源も出典も不変**で、変わったのは「誰に訊くか」だけ。
デプロイ済み・ページから実測：**200 / 718 features / ACAO `*` / 1.9 s**。

そして**この環境の実プロファイル**を読んだら、まさに #R189 が警告した形が出た：
`{"defv":189,"layers":[…,"dl-climate"]}` ——**印つきで、ケーブルだけが無い**。#R189 の `imAutoOff` は
「一度ついた箱をアプリが外した」場合しか守れず、**最初から一度も点いたことのないプロファイル**には
守るものが無かった。障害の原因が構造的に消えた今、もう1世代だけ癒す：**`defv:190`**。
⚠ 例によって `playwright.config.js` と `tests/r172|r173|r186` の種も 190 に上げた
（#R189 が置いた汎用検査がこれを強制する——外すと落ちることを確認済み）。

**実測（クリーンプロファイル）**：`dl-climate` / `dl-subcables` とも **checked かつ painted**。

### 3. 衛星画像の色——**床と本番が別の衛星画像だった**

「読み込み前と読み込み後で全然色味が違う」。原因は設計そのもの：床は **NASA Blue Marble**（同梱・
公有・極まで届く唯一の正距円筒）で、上に載るのは **Esri World Imagery**。ページから z2 タイル16枚で
実測すると——

| | R | G | B |
|---|---|---|---|
| Esri World Imagery | 85.5 | **121.3** | 125.9 |
| Blue Marble（従来の床） | 72.4 | **84.3** | 101.1 |

**緑が 37 も違う**。16組の最小二乗でチャンネル毎アフィンが `R²=0.97/0.98/0.96` で当たるので、
デコード直後に**1回だけ LUT を当てて** canvas に焼く：`[[1.1011,5.81],[0.9225,43.53],[0.9787,26.94]]`。
ファイルは変更していない（＝出典も権利も不変、Cesium にも `bitmapUrl()` で同じ canvas を渡す）。

**実測 RMSE：29.46 → 11.85**（平均の緑差 37.0 → 2.5）。
色が合ったので `maxzoom` を **4 → 6** にした——**ぼけた「正しい画」は穴より良い**。実測：
サンチャゴ z7 / モスクワ z8 / シドニー z9 へ飛んだ 300 ms 後の空白画素は **0 / 0 / 0.27 %**。

### 4. 水——**「海面下」で止めていたのは、海だと確認できなかった場所だった**

「陸地にもかかわらず、0m以下になったから描画を終了するのはやめて」。#R186 が正しい検査
（連結性で海と閉塞盆地を分ける）を作り、#R189 がそれを繋いだ。**両方とも、その答えの使い方を間違えて
いた**：検査が「海ではない」と答えたとき、トレースは `end='sink'` で**やはり止まっていた**。
オランダ、ポー川デルタ、日本の海抜ゼロメートル地帯——実在の川が数百 km 走る場所である。

「海ではない 0 m 以下」は**ただの陸**として扱い、`belowSea` を立てて歩き続けるようにした。止まるのは
**連結が確認できた海**か、洪水窓が越えられないと判定した盆地だけ。検査は 1 トレース 8 回・20 km 以内
では再検査しない（1回につき洪水窓1枚のコスト）。

**実測**（アーネム 5.90E/51.98N から）：**187.1 km・`end='sea'`・`belowSea=true`**、終点 4.955E/51.863N
＝ロッテルダム（ニーウェ・ワーテルウェフ）。以前ならポルダーに入った時点で「海面下の閉じた窪地」。

もう2件：**編集のたびに再トレース**（`solve()` の末尾で 320 ms デバウンス。⚠ 発火したのに `tracing` を
捕まえられず「動いていない」と誤診したので、`retraceState()` に**発火/不発火の理由**を出すようにした
——実測 `why:'fired'`）。そして **「この範囲の標高データを読めません」を出さない**：欠損3割超は
**場所の性質ではなくレベルの性質**なので、`build()` は **z を1段下げて取り直す**（z7 まで、最大6回）。
1段下げれば同じ地面をタイル1/4で覆える。それでも駄目なときだけ、原因である**ネットワーク**を名指しする。

### 5. フライトシム——**「水平で強制的に始まる」は本当だった。犯人は `computeTrim` のクランプ**

実測（z9.5 / pitch45 / bearing70 で START）：カメラは確かに 9.50/45.0/70.0 から始まる（#R189 のシードは
効いている）。しかし **t=2.0 s で pitch 90.5、以後ずっと水平**。理由は `computeTrim(gammaWant)` が
要求された経路角を**定常飛行の包絡線**（アイドル滑空角〜全推力上昇角）へ丸めるから——45°の見下ろしは
約 −4° の降下になり、機首は1秒で水平になり、コックピット視点は機首なので水平になる。シードは
**すでに水平にされた飛行機の上を 1.2 秒だけ飾っていた**。

利用者に確認：**「機体を実際にその角度で飛ばす」**。トリムは推力・昇降舵・速度を解いたまま、
**姿勢だけ**を地図の傾きにする。機体はトリムから外れる——それが要点で、実機と同じように
自分で引き起こす。⚠ 限界は丸い数字ではなく**引き起こしに要る高度**：
`h = V²(1−cos γ)/(g(n−1))`（n = min(gLim,4)、余裕 200 m、上限 85°）。既定の地図は真上から見下ろし
（pitch 0 → γ=−90°）なので、これが無いと毎回垂直降下で始まる。地形読み取りの**後**に適用する。

**実測**（同条件）：

| t | 機体の経路角 γ | 画面の pitch |
|---|---|---|
| 340 ms | **−44.9°** | 47.6 |
| 1.5 s | −42.9° | 51.7 |
| 4.0 s | −41.3° | 54.4 |
...
＝押した画角のまま始まり、**飛行の結果として**水平へ戻っていく。あわせて「高度が一律」への答えとして、
プリフライト画面が押す前に言うようにした：`Start altitude 15 km (view is at 68 km — limited by this
aircraft's ceiling) · flight path -45°`。

### 6. 起動画面——**直列に待って、終わる前に上がっていた**

「ローディングが遅い。終了後も読み込みが終わっていない」。#R186 の順序は
`idle待ち → 既定レイヤー発火 → 380ms → idle待ち`。**レイヤーは基図が静まるまで要求されない**ので、
2つの取得が重なるはずのところが端から端まで直列だった（＝遅い）。そして2つ目の待ちには3秒の逃げ道が
あり、**その逃げ道が要る遅い読み込みでこそ、まだ届いていないのに画面が上がる**（＝終わっていない）。

いまは style-ready で**即発火**し、上がる条件は「idle **かつ** 既定レイヤーが実際に地図に載っている」
（`window.__imLayerPainted` ＝ #R81 のレイヤー監査の答えを再利用。チェックの入った箱ではない）。
**実測（クリーンプロファイル）：html 127 / dom 296 / renderer 392 / load 884 / style 2,183 /
layers 3,276 / idle 4,725 ms**、両レイヤーとも painted。

### 7. 地震波——**9項目。うち2つは「測ってはいけない物を測っていた」**

- **不透明度**：0.55 は薄すぎるという報告どおり。スライダー（`setOpacity`）＋既定 **0.85**。
- **計算開始ボタン＋LOS 方式の進捗**（実パーセント＋バー）。パネルの設定変更は `touch()` で
  「要再計算」を立てるだけにした（以前は数値スピナーを1回動かすたびに DEM 読みが走っていた）。
- **カーソル地点の震度**を左下の常時表示欄に**階級色つき**で。⚠ そのために場の保存を
  **震度から PGV へ**変えた——塗った時の階級が残っていると、MMI→JMA に切り替えた瞬間に
  **MMI の 8.63 を「震度7」と読む**（実測）。換算は1本（`mmiOf`）に統一。
- **フリー描画中に Draw パネルを出さない**：`DrawTool.start(null,{silent:true})`。共有ツール側に
  1行のフラグを置いた（他人の DOM に手を伸ばさない）。
- **震度1の範囲まで塗る**：`MMI_MAX_KM=1000` が最外クラスを切っていた（実測 M7.2：MMI II は岩盤で
  699 km、軟弱地盤なら 1,000 km 超）。「較正範囲」と「描画上限」を分離し
  `MMI_CALIB_KM=1000` / `MMI_MAX_KM=1500`。表は従来どおり較正範囲外で震度を出さず、塗りは最下位クラスの
  端まで届き、**1,000 km を超えたセル数を明示**する。⚠ 3,000 km も試して**捨てた**：M9 で場が 6,000 km
  になり、**DEM が届かないセルが 71 %**、塗った 91 % が外挿、構築 20.5 s——広いだけの絵は忠実ではない。
- **精度**：①**Q を周波数依存**へ `Q=Q₀·f^η`（Q₀=180・η=0.45、Raoof, Herrmann & Malagnini 1999）。
  定数 Q=300 は近距離で高周波を潰し遠距離で残しすぎる＝**場の形**が違っていた。両値はパネルで可変。
  ②**Vs30 の勾配を DEM の実解像度で測る**——Wald & Allen の proxy は ~900 m 基線だが、大陸規模の場では
  1 DEM 画素が km 単位なので、±900 m のペアは**同じ画素の中**に落ちて平滑化された勾配を返す。
  小さい勾配＝最軟の Vs30 ビン＝最大増幅なので、**粗い場ほど世界全体が軟弱地盤になる**系統誤差だった。
  基線は `max(900, 実間隔×1.25)`、実間隔 2 km 超では proxy を使わず `coarse` として数える。
  ③格子 176² → **288²**。実測（M7.2・東京・JMA）：z8・DEM 497 m・基線 900 m・`coarse 0`・`noDem 0`・
  **rEdge 1,061 km**（＝震度1の外縁に到達）・較正範囲外は 4 セルのみ。
- **津波**：Mw≥6.5・深さ≤100 km・震央が海底（実DEM）——津波情報の運用しきい値——を満たすときだけ
  ボタンが出て、既存の浸水モデル（`IntMapDisaster`）へ `hazard:'tsunami'` と波高を渡す。波高は
  Abe の津波マグニチュード関係 `log10 H = 0.5Mw − 3.3`。**モデルは増やしていない。**
  実測（M9.0・三陸沖・深さ24 km）：`{waveM: 15.8}`。`sims.js` の `open()` は hazard とパラメータを
  **位置と同時に**受けるようにした（後から設定すると前の災害種別で1回走ってしまう）。

### 8. 両エンジン——**残っていた安全な knob は2つ**

`preloadSiblings` は #R189 が実測で撤回済み（`globe.tilesLoaded` の意味を変える）。SSE と imagery の
半解像度は #R185 が解決済み。今回入れたのは **Cesium `tileCacheSize` 512→768 / 256→320**（誰も待って
いない旗を変えない knob）と、**床の `maxzoom` 4→6**（§3）＝カメラが飛んだ直後の空白が実測でほぼ消えた。
⚠ `npm run test:profile` は `playwright.config.js` の `testIgnore` に阻まれて**現状どのコマンドでも
走らない**（既存の不整合。今回は触っていない）。フレーム時間は隠れた preview ペインでは測れないので、
**測れない数字は報告していない**。

**テスト**：`tests/r190-checks.test.mjs`（10件）。#R183/#R185/#R176/#R189 の固定文字列を新実装に追随
（部品構成・`part==='body'`・`MMI_CALIB_KM`・`tileCacheSize`）、`tests/r172|r173|r174|r186` と
`playwright.config.js` の `defv` を 190 へ。ユニット **515 件グリーン**。

## R189 — **8系統の指示：残っていたのは「保存された過去」と「1行のTDZ」だった** (tag `#R189`)

指示は8系統。再報告組（航空機マーク・既定レイヤー・Cesium極・水の無反応・シム開始）は今回も
まず**再現と読解**から入った——そして最大の発見は物理でも描画でもなく、**`const` の位置**だった。

### 1. 航空機マーク——グリフは最初からバイト一致、残っていたのは**保存されたキー**

R187 が戻した `_PLANE_ORIG`・色・サイズランプを R170 の原典と突き合わせ：**バイト一致**。本番の
クリーンプロファイルでも 2D グリフが出る。残る経路は1本だけ——**R172〜R186 は planes3D の既定が
TRUE** で、その時代にトグルへ触れた利用者の localStorage には `'1'` が残り、R187 の既定 OFF 化を
**永遠に上書きする**（#R188 の「状態を選択として保存するな」そのもの）。キーを
`intmap_planes3d2` へ世代交代し旧キーは削除。トグル機能は不変。

### 2. 既定レイヤー——**書き込み側を直しても、ストレージに残った毒は読めば戻る**

R188 は snapshot/imAutoOff を直したが、**R188 以前に書かれたセッション**は `dl-subcables` を
含まないまま残り、`_restore()` が毎回「利用者の選択」として外し続ける。スナップショットに世代印
`defv:189` を導入し、**印の無いセッションは一度だけ既定レイヤーを取り戻す**（以後の absent は
本物のオプトアウトとして尊重）。実測：毒入りセッションを書いて再読込→**両レイヤー ON・実体あり**；
`defv:189` でケーブル absent→**OFF のまま**。さらに2件：`sw.js` の activate が**ケーブルの
Cache API キャッシュをデプロイごとに削除**していた（`intmap-subcables-*` を残すよう修正）、
`build()` が12回で黙って諦める経路（チェックON・レイヤー無し）に imAutoOff＋トースト。

⚠ **この移行は「保存された全セッション」に届く——テストスイートが自分で書く4本を含む**。
`playwright.config.js` の `storageState` は #R186 で置いた種で、「このプロファイルは既定レイヤーを
既に外してある」と宣言して**約350件のテストを 9,160 ms でなく 3,192 ms で起動**させるためにある
（多MBの気候ラスタ＋CORS リレー越しのケーブル網）。世代印が無ければ移行はそれを毒と見なして
**両レイヤーを全テストで復活させる**。CI 実測：r174「傾き上限を外してもズームで視点が動く」が
**3回とも 180 s タイムアウト**、r174×2・r175×1 がフレーク化、シャードが 28.5→31.1 分。R189 は
`tests/r186.spec.js` だけ印を付けて `playwright.config.js`・`tests/r172`・`tests/r173` を落として
いた。4本すべてに `defv` を付け、**`js/app-body.js` から世代を読んで全種にそれを要求する静的検査**
を追加（印を外すと落ちることを確認済み）。教訓：**既定値を治す移行は、その既定値を抑止している
自分のテスト基盤も治してしまう**。

### 3. Cesium の極——**生きた URL は、届かないネットワークでは黒**

R188 の帯と色調整はこの環境では正しく測れている。残る黒の経路は **GIBS が届かない場合**：地図
ビューでは同梱フロアが `show=false` なので `baseColor` の黒に戻る。帯の `errorEvent` が一度でも
発火したら、同梱フロアに**地図の装いを着せて極冠だけ見せる**フォールバックを常設（Mercator 基図は
±85.05° を覆うので、フロアが覗けるのは正確に極冠だけ）。GIBS が答える限り何も変わらない＝R187 が
消したにじみは退役のまま。あわせて未設定だった `tileCacheSize` 100→512（携帯256）。
⚠ **`preloadSiblings` は同じラウンドで試して撤回**：読み込んだタイルごとに隣8枚をキューに積むので
`globe.tilesLoaded` が静まらなくなる——アプリ自身の settle もテスト（r180⑥・r184-fs②）もその旗を
待っており、CI で3リトライ全滅を実測。「地球が静かになった」の意味を変える knob は性能設定ではない。

### 4. 水——**「結果が出ないことがある」の正体は TemporalDeadZone だった**

#R188 は `collectPond` を**呼び出しより下の `const`** に書いた。盆地で終わるトレース（まさに
宣伝している 「水は流れなくなる地点」 の結末）は**毎回 ReferenceError で reject** し、呼び出し側に
catch は無い＝画面には何も出ない。しかも r188 テストが**壊れた順序そのものを固定化**していた。
宣言を使用より前へ移し、トレース全体に catch（`end='error'` として描画・報告）。実測：
Death Valley で `end='sink'`（海面下の閉じた盆地）・**未処理拒否ゼロ**。

精度も3段：
- **解像度ラダー**：源流10 kmまで z13（約19 m）、50 kmまで z12、以遠 z11。R188 自身の測定
  「454/498 断面が DEM 解像度未満」への答え——いちばん細い上流を、いちばん細かいデータで測る。
  `spac[]` を点ごとに記録し、断面・描画・belowRes 判定が各点自身の解像度を使う。
- **タルウェグ**：窓内経路を洪水の脱出親鎖から **MFD 集積の最大枝**へ（channelChain）。合流で
  横谷の鞍部へ抜ける誤りが消える。平坦はスピル親鎖を**一括**通過（1歩ずつだと集積側と往復する
  ——千曲で実測した偽 'lake' の原因）。
- **エスカレーション**：窓より広い平地では **3× 窓**を焚いて出口を探す（最大6回）。落とし穴が2つ
  実測で出た：①5×5 タイルを温めないと粗窓が欠損40%超で null＝逃げ道が見えない、②粗い標本間隔では
  **狭い峡谷が消える**（立ヶ花は幅~200 m、276 m 標本の間に落ちる）ので粗窓の充填はサンプリング
  人工物を含む——ゲートは 25 m でなく **75 m**（実在の深い閉塞盆地はそれでも止まる）。効果の実測
  （千曲川・同一始点）：**139 km の偽 'lake'（川中島平）→ 354 km**、立ヶ花峡谷を抜けて越後平野の
  **実在の海抜ゼロメートル地帯**まで到達し「海面下の閉じた低地」として正直に終端（55窪地通過）。
加えて：**流量 Q (m³/s) を設定可**（`A=Q/(K√S)`、K=40 ≒ 0.1% 勾配で 1.3 m/s；未設定は #R188 の
体積連続のまま）——実測：千曲 Q=500 で幅 **214〜990 m**・断面635。DEM が3割超欠損なら**造らずに
断る**（平らな虚構の上の「海に到達」を返さない）、seaCheck 不能は「未確認」と明記、補間セル数を
パネルに常時表示。パネルは不透過（--card-bg）。

### 5. フライトシム——**「一律」の正体は上限クランプ、「画角」の残り半分は構図**

`o.alt=min(eye.alt, ceil)` は z≲13.5 で常に**機体上限（F-35 15,240 m）に飽和**＝一律の正体。
そして R188 が運んだのは角度だけで、**ズーム（視距離）は一度も**運ばれていなかった——コックピットは
固定 1,800 m 先を見るので、初フレームの構図は必ず切り替わる。シードに**地図の視点位置と
視点→中心距離**を積み、イントロ窓の間 eye と look-arm を smoothstep で機内へ運ぶ
（定常状態の初期化子は #R158 の定数そのもの；1フレーム跳躍ガードはイントロ中だけ停止）。
実測（z9.5・pitch42・bearing71・視点高度 59,989 m から START）：

| t | zoom | pitch | bearing |
|---|---|---|---|
| **237 ms** | **9.50** | **42.0** | **71.0** ＝押した画そのもの |
| 974 ms | 10.40 | 84.0 | 70.8 |
| 1,927 ms | 14.69 | 90.0 | 70.8 |
| 以降 | 14.99 | 90.6 | 70.7（コックピット定常） |

### 6. 衛星・3D——**残っていた knob は3つ**

MapLibre 側は R158〜R185 でほぼ限界（プロトコル修復・@2x・深度メモ）。残り：**z19 でも @2x を
組む**（旧ガード `z>=19` は最大ズーム＝いちばん凝視する画面だけを半解像度のまま残していた；都市は
Esri に実 z20 があり、無い所は深度メモが1回で学ぶ）。Cesium 側は `tileCacheSize`（既定100＝約
1画面分→512/256。`preloadSiblings` は §3 の⚠のとおり撤回）。

### 7-8. 地震波——**同心円をやめる方法は、距離と地盤を本物にすること**

- **等倍再生**：旧ループは 90 ms ごとに +10 s（≈111倍）で、どこにもそう書いていなかった。
  壁時計×レート（×1 既定、×1〜×300 select）。波面 tick は震度場・表と独立（どちらも t に依存
  しないので、再生中に RVT を回さない）。
- **震度は塗った場**：Vs30 を実 DEM の**地形勾配から推定**（Wald & Allen 2007・活動域テーブル・
  ±900 m 標本）し、1/4 波長則へ。PGV は増幅係数に**線形**なので、RVT は 1 次元プロファイル1本＋
  セル毎1乗算。実測（M7.2・東京）：30,976 セル・塗り 4,969・海 19,299（塗らない）・
  **富士山腹 Vs30 802 / 関東平野 300**＝場は地形に従う。DEM 不能時は一様地盤へ落として**そう表示**。
- **気象庁震度**：切替式。計測震度換算 I=2.68+1.72 log₁₀PGV（藤本・翠川 2005）と**明記**し、
  配色は気象庁基準。MMI の「気象庁震度階級ではありません」も従来どおり残る（出典2件を追加）。
- **フリー描画の震源域**：共有 DrawTool の `currentGeometry()`（#R141）から取り込み、
  M₀=μAD̄（μ=ρβ²）→Mw。実測：**10,934 km² × D̄5 m → Mw 8.1**（東海地震スケールに一致）。距離は
  **Rrup**（面内0）になり——実測：断層縁 13 km で計測震度 5.7、243 km で 4.7——分布は断層形に従う。
  波面は各点の破壊伝播遅延（Vr=0.75β）つき**包絡**。
- **極のバグ**：ring() が頂点ごとに経度を [-180,180) へ丸めていた＝反対子午線で全世界を横断する
  線・極で子午線に退化。`HOST.diskOutlineLines`／`_splitLineToWindows`（#R176 の実績ある機構）へ
  委譲し、gcDelta は haversine へ。パネルは不透過。

**テスト**：`tests/r189-checks.test.mjs`（15件——うち1件は §2⚠のセッション種の世代検査）。
r158/r173/r174/r187/r188 の固定文字列を新実装に追随（**壊れた順序を守っていた r188 の1件を含む**）。
全ユニットグリーン。

## R188 — **測って消した容疑者／「一度失敗した」を「そう決めた」と記録していた／線をタイルにしても線** (tag `#R188`)

指示は6件、**全部が R187 の再報告**。だから今回もまず測った——そして今回いちばん効いたのは、
**やろうとした対策を測って先に捨てたこと**である。増やせるはずだった経路（第2のADS-Bホスト、
速いポーリング）は2つとも実測で死に、残った1つ（詰め方）が3.4倍を出した。

---

### 1. 航空機：足りなかったのは要求の数ではなく**詰め方**だった

「もっと増やして」に対して、まず**増やせそうな道を3本とも測った**。

| 試したこと | 実測 |
|---|---|
| 2つ目のフィード（adsb.fi / adsb.lol） | ページのオリジンから **`TypeError: Failed to fetch`**。どちらも `Access-Control-Allow-Origin` を返さない＝**ブラウザからは使えない**。Node からは 200 で airplanes.live と同じ 554 機（和 561）を返すので、CORS だけが理由 |
| 速いポーリング | 1,200 ms 間隔の34連射は **34/34 が 200**。同じ34発を 700 ms にすると **12回成功のあと16連続で切断**（#R186 が測った遮断そのもの）。**1.2 秒は動かせない** |
| 大きい円 | r=250 は 200、r=300 / r=500 は **403**（#R187 から不変） |

残ったのは**格子**である。#R186/#R187 は円の**内接正方形**の歩幅で並べていた——円が払った πr² のうち
2r² しか使っていない。等円による平面の最適被覆は**三角格子**（Kershner 1939）：隣を r√3、行を その√3/2、
1行おきに半歩ずらす。実測値（r=463 km・重なり4%）で **中心間770 km・行間667 km**、
**1リクエストの担当面積 378,700 → 513,600 km²＝1.36倍**。

円の予算 48 → **128**（モバイル 12 → 24）。**長期のリクエスト頻度は1ミリも上がらない**——ポーリング間隔は
以前から「1円あたり3.5秒」なので、大きな掃引は**速く叩くのではなく、たまにしか回らない**だけである
（上限は 180→600 秒。ここを切ると実測より速く叩くことになる）。

| | R187 | R188 |
|---|---|---|
| 格子 | 内接正方形 | **三角（最適被覆）** |
| 1円の担当面積 | 378,700 km² | **513,600 km²** |
| 円の予算 | 48 | **128** |
| 実測の被覆 | 4,920 × 3,690 km ＝ **18.2 M km²** | 11,548 × 5,334 km ＝ **61.6 M km²（3.4倍）** |
| 掃引の発行時間 | 58 秒 | 154 秒 |
| 長期リクエスト率 | 0.29 /秒 | **0.29 /秒（不変）** |

⚠ **154秒の掃引には、それ用の作法が2つ要る。**
- **途中で publish する**（4秒ごと）。終わりにまとめて出すと2分半なにも変わらない。運んだ機体を消してよいのは
  **その空をもう一度訊いたとき**だけなので、格子セルを O(1) で引ける `planeCellOf()` を置き、
  「訊いたのに返ってこなかった」機体だけを落とす。年齢は格子の外にいる機体のための保険。
- **カメラが去った掃引は捨てる**。#R186 が「捨てるな」と決めたのは*集めたものが失われるから*で、
  途中で publish する今その理由は消えた。中心が被覆の半分より遠くへ動いたときだけ交代する。

⚠ **並び順が結果を決めた。** 行優先だとブロックの角から始まる——z3 のヨーロッパで実測したら
**最初の4円が大西洋のど真ん中で、機体1機**。フランクフルトはまだ訊かれていない。**視点からの距離順**に
並べ替えるだけで、最初に埋まるのが画面の真ん中になる。円の集合は同一。

### 2. 「片方しかつかない」——ケッペンとの非対称は**同梱かどうか**だった

R187 は本物の欠陥（拒否された addSource を握り潰していた）を直した。それでも再報告されたので、
**ページのオリジンから直接叩いた**：

```
fetch('https://www.submarinecablemap.com/api/v3/cable/cable-geo.json')
    → TypeError: Failed to fetch      ← ACAO なし。毎回
```

つまり**海底ケーブルは最初から一度も直接取れたことがなく**、ボランティアのCORSプロキシ3つのどれかが
生きているときだけ点いていた。ケッペンは**自分のオリジンに同梱されたPNG**なので、この依存が無い。
「片方」が毎回こちらを指す理由はこれである。

2つ直した。**どちらもデータ源は変えていない。**
1. **成功した応答を保存する**（Cache API）。次回は**ネットワークより先に**それで描き、裏で更新して
   `setSourceData` で差し替える。プロキシの不調は**レイヤーではなく更新1回**の損失になる。
2. ⚠ **失敗を「利用者の選択」として記録するのをやめた。** 取得に失敗すると旧コードはチェックを外し、
   `_snapshot()` は**チェックの入った箱**を保存する——だから次に何かを1つ切り替えた瞬間、
   「このレイヤーはOFF」というセッションが書かれ、以後の復元が**毎回わざわざ外す**。
   corsproxy.io の悪い1日が**恒久的な「片方しかつかない」**になっていた。
   アプリ側が外すときは `imAutoOff` を立て、スナップショットはそれを**まだ望んでいる**として保存し、
   **本物のクリック（`isTrusted`）が来た時点で**印を消す。取得は 5/15/45 秒で3回リトライしてから諦める。

### 3. Cesium の極——**衛星ビューだけ直しても、既定は地図ビューである**

変更前に測った：

| | 平均チャンネル値 | 暗い画素 |
|---|---|---|
| 衛星ベース・88°S・z3 | **239.3 / 255** | 0 % |
| **地図ベース・88°S・z3** | **0.56 / 255** | **99.4 %** |
| **地図ベース・88°N・z3** | **12.0 / 255** | **99.99 %** |

R187 の GIBS は効いている。そして R187 は**それを衛星ベースマップに結び付けた**（街路地図の上に
衛星写真が覗くのは別の地図だ、という理屈で）。アプリが開くのは**地図ビュー**で、そこが 0.56/255 だった。
直した半分は、誰も見ていない半分だった。

答えは「結び付けを外す」ではなく**極冠に地図の服を着せる**こと。Cesium の `ImageryLayer` は
アプリ自身の基図ラスタと同じ色調整（明度・コントラスト・彩度・ガンマ）を持つので、
**極の2帯は常に表示**し、衛星ビューでは無加工、地図ビューでは**テーマごとに**調整する。
⚠ **同梱の全球1枚画像は別物**として衛星ビューに残す（1行を極冠全体に配ると R187 が消した放射状のにじみに戻る）。

⚠ **その4つの数字は掃引して決めた。** 一律に暗くすると**氷は光り、海は消える**——2つの極は同じ絵ではない。
生タイルを実測すると**南極 143 / 北極 48.9**（しかも 48.9 は Blue Marble 3製品で**いちばん明るい北極**。
`NextGeneration` と `ShadedRelief` はどちらも 19 なので、`_Bathymetry` のままが正しい）。
最初の設定（明度0.62・コントラスト1.06）は南極を**151**にし——地表が 9.4 の暗い地図の上でサーチライト——
北極は **9.4 のまま＝まだ黒**。両者を近づけるのは**コントラスト**なので、明度と一緒に掃引した：

| 設定 | 88°S | 88°N |
|---|---|---|
| b .62 / c 1.06 | 151.1 | 9.4 |
| **b .42 / c 0.90** | **103.5** | **20.1** |
| b .34 / c 0.80 | 90.9 | 30.8 |
| b .26 / c 0.72 | 80.8 | 39.5 |

2行目を採用。**最終実測**（変更後・8視点）：

| | 平均 | 暗い画素 |
|---|---|---|
| 地図/ダーク 88°S | **105.0** | **0 %**（旧 0.56 / 99.4 %） |
| 地図/ダーク 88°N | **20.3** | **0.9 %**（旧 12.0 / 99.99 %） |
| 地図/ライト 88°S | 252.9 | 0 % |
| 地図/ライト 88°N | 27.2 | 0.2 % |
| 衛星 88°S | 238.6 | 0 %（不変） |
| 衛星 88°N | 23.5 | 1.3 % |

北極が 20 前後なのは**そこに写っているのが北極海だから**で、暗い画像を明るく偽ることはしない。
変わったのは「99.99% が黒」から「0.9% が黒」＝`baseColor` ではなく**実際の画像**が出ているという一点である。

### 4. ウィジェット——**どれ一つガラスではなかった**

3度目の報告。既定の外観設定で測ったら、理由が1行で出た。

```
--glass-fill              = #1c1c1e   ← 完全不透明（Solid では --card-bg に解決される）
.wgt-card の背景          = rgb(28,28,30)
AQI カードの背景          = rgb(255,204,0)   ← 不透明な単色の板
```

`backdrop-filter` は**全カードに掛かっていて、何もしていなかった**——地が不透明だからである。
R187 は「AQI/UV を例外にするのをやめる」と読んで設定に従わせた。**例外を逆向きに消しただけ**で、
全部が等しく「ガラスでない」状態になった。**今回ユーザーに確認**して決定：
**ウィジェットは設定に関係なく常にガラス**。`.wgt-card` 専用のマテリアル `--wgt-glass` を与え、
AQI/UV は**同じ地に固定 α (0.52) の色味を乗せるだけ**にした（R187 の「平坦にする」半分はそのまま）。
⚠ **`.wgt-card` にだけ効かせてある**——他のすべての面は #R33/#R153 のまま設定に従う。
実測（既定の外観のまま）：全カード `rgba(30,30,38,0.44)`＋`saturate(1.8) blur(24px)`、AQI `rgba(255,204,0,0.52)`。

### 5. 水——**線をタイルにしても線である**

追記が正しい。R187 は 2.6 px の折れ線を消して、**同じ折れ線を一定幅のラスタで**描いた。峡谷でも
氾濫原でも 92 m の帯なら、それは水ではなく**太さのある経路**で、地形は一切画に届いていない。

届くのは**横断面**である。course 上の各点で**進行方向に直交する側へ実DEMを±1.8 km 読み**、
水面を上げて**濡れ断面積**が通すべき量に一致するところで止める。その濡れ区間の両端が左右の岸。
どれだけ通すかも当て推量ではない：**連続の式**（同じ量がどの断面も通る／断面積は通過速度に反比例／
速度は√勾配）で `A(s) ∝ 1/√S(s)`、S は**トレース自身の標高プロファイルから実測**、比例定数は
**利用者が置いた水量**（「1クリックの水量」）で1回だけ決まる。**自由なパラメータは利用者の数字だけ。**

実測（千曲川上流・111.8 km・498断面）：

| 置いた水量 | 平均幅 | 最大幅 | 最大水深 | DEM解像度未満 |
|---|---|---|---|---|
| 1×10⁶ m³ | 92.7 m | 183.7 m | 25.5 m | 454 / 498 |
| 5×10⁸ m³ | **833 m** | **2,204 m** | 44.5 m | **18 / 498** |

大きい方の幅の並びは **92 → 92 → 230 → 1,102 → 2,204 → 2,204 → 2,204 m**——峡谷で締まり、
氾濫原で広がる。一定幅はどこにもない。⚠ 2,204 は**±1.1 km の断面サンプリングそのもの**だった＝
数字が水ではなく計測系だったので、±1.8 km へ広げ、**それでも端に届いた断面は `atLimit` として数える**。
データより細い断面も `belowRes` として数え、両方パネルに出す（#R185「黙って切るな」）。

⚠ **最初の版は「無い水」を15,755セル描こうとしていた。** 窓の `filled − surf` は
**「この窓から出るのにどれだけ埋まる必要があるか」**であって水ではない——#R186 が
「125 km の川で157個の偽の湖」と測って書いてあったことである。**深くて広い**という #R186 自身の判定を
通ったプール、およびトレースが**終わる盆地**についてだけ、その水位以下の連結セルを塗る（6,018セル）。

⚠ **キャンバスの解像度は数字ではなくデータで決める。** R187 の固定 1,024 px は 600 km の course で
92 m のセルを**0.16画素**にする＝上の縁を全部捨てる。1 DEM サンプル＝1 画素になる長辺（上限 2,560）にした。

**「結果が出ないことがある」の2つ目の原因**も潰した。#R186 は矩形の外のクリックを直したが、
ハンドラの**1行目** `if(!opened||!G) return;` が残っていた——`G` は DEM を読み終えるまで null で、
読めなければ**ずっと** null。その間のクリックは**一言もなく捨てられていた**。今はグリッドが無ければ
**作ってから置き**、読めなければ**そう言う**（5言語）。

### 6. フライトシム——「画角」は**方位のことではなかった**

R187 は「方位＝今見てる画角の片割れ」と書いたが、方位は #R95 以来ずっと合っていた＝**何も変わっていない**。
一度も渡っていなかったのは**傾き**で、同じ場所で眺めを別物にしているのはそれである。
**今回ユーザーに確認**して決定：**姿勢もカメラも合わせる**。

- **姿勢**：地図の pitch（真下=0°／水平=90°）はシムのカメラ規約と同じなので、視線の含む経路角は
  `pitch − 90°`。⚠ それを**機体が保てる範囲にクランプ**する——上は**全推力で登れる角**
  `asin((T−D)/W)`、下は**アイドルの滑空角** `−atan(CD/CL)`、どちらも**水平トリムと同じ極曲線**から出す。
  真上から見下ろしていても垂直降下にはならず、その機体がいちばん急に降りられる定常降下になる。
  引数なしの `computeTrim()`＝これまでの全経路は**1バイトも変わらない**。
- **カメラ**：機体の姿勢に任せると START を押した瞬間に水平線へ切り替わる。地図自身の
  方位・傾き・ロールで平滑化器を**種付け**し、**1.2 秒**かけてコックピットへ移る。窓が終われば種を捨て、
  #R174 が調整した定数がそのまま戻る。

実測（アルプス上空・zoom 13.2 / pitch 62° / bearing 118° / 視点高度 6,626 m から START、rAF ごとに採取）：

| START からの経過 | カメラ pitch | bearing |
|---|---|---|
| **37 ms** | **62.0**（＝押した瞬間の地図そのもの） | 118.0 |
| 299 ms | 72.1 | 118.2 |
| 653 ms | 78.8 | 118.2 |
| 987 ms | 81.6 | 118.2 |
| 1,356 ms | 83.8 | 118.2 |
...
**最初のフレームは押した画そのもの**で、1.36 秒かけてコックピットへ移る。落ち着いた 83.9° は
機首 6.1° 下げ＝地図の 62°（要求 −28°）が **F-35 の滑空角にクランプされた結果**であり、
「真下を見ていたら垂直降下」にはならない。

---

**テスト**：`tests/r188-checks.test.mjs`（7件）。CI は既存の8シャード並列のまま。

## R187 — **「黒い」は数値で測れる／`rank` は重要度ではなかった／描くべきは線ではなく水** (tag `#R187`)

指示は12件。**そのほとんどが R186 が「完了」と書いた項目の再報告**で、追記が付いたものは
**追記のほうが本題**だった。だから今回いちばん効いたのは、実装ではなく **「まだ足りない」を数で言い直す
こと**である——「真っ黒」は 0.16%、「店ばかり」は 74ラベル中バス停24・店14、「片方しかつかない」は
コンソールに1行で書いてあった。

---

### 1. 「真っ黒」は感想ではなく測定値だった——0.16%・平均 0.396/255

R186 は Yale Bright Star Catalogue（9,096個・V≲6.5）を同梱し、歳差・恒星時・実測B−Vまで正しく実装した。
それでも「背景は真っ黒」と返ってきたので、**画素を数えた**：globe 背後の星キャンバスで
**光っている画素 0.16%・平均チャンネル値 255 中 0.396**。視野の星は **331個**。

ここで**明るさ曲線をいじるのは間違い**である。331個の点は、どれだけ明るくしても
**331個の明るい点**にしかならない。足りないのは輝度ではなく**数**だった。

同梱カタログを **Hipparcos（ESA 1997・CDS I/239）**へ差し替え、V≤9.5 で **98,887個・593 KB＝13倍**。
深さには密度以外の意味もある：**天の川は「描く帯」ではなく星が実際に集まっている場所**なので、
この深さならカタログ自身が天の川を描く。BSC はビルドのフォールバックとして残した。

| | R186 | R187 |
|---|---|---|
| カタログ | 9,096 | **98,887** |
| 視野内に描いた星 | 331 | **4,104** |
| 光っている画素の割合 | 0.001554 | **0.008567**（5.5倍） |
| 平均チャンネル値 | 0.396 | **2.185**（5.5倍） |
| 1フレームの描画時間 | — | **6.8 ms** |

⚠ **99,000個を毎フレーム射影する経路は作り直した。** `projectDirection()` は**契約のまま**——tests/r186 が
`map.project()` と突き合わせているのはこれ——で、一括ループだけが**合成した3×3行列**を使う。根拠は
恒等式2つ：`sphereVec(ra−gmst, dec) ≡ Ry(−gmst)·sphereVec(ra, dec)`（恒星時は極軸まわりの回転そのもの）と
`Ry(−lng)·Ry(−gmst) ≡ Ry(−(lng+gmst))`。単位ベクトルは歳差更新時（地図時間で1日1回）に前計算する。
**信じずに測った**：tests/r187 が合成行列と `projectDirection()` を**3カメラ状態×3 GMST×25方向で比較し
最大誤差 1e-6 px 未満**。

### 2. ⚠ `rank` は重要度だと思っていた。ヒストグラムを見たら通し番号だった

追記「店以外もいろいろ地点を追加して」。R186 は OpenMapTiles の `poi` を `rank ≤ 40` で絞っており、
コメントには「rank はタイル自身の重要度順」と書いてある。**東京都心 z16 で実測したら、rank の
ヒストグラムは平坦だった**——72, 72, 71, 71, 71 …。つまり **rank は点数ではなく通し番号**である。

その恣意的な切片が何を通していたか：視野の 12,134件から 2,769件、最大勢力は**店 723 とバス停 372**、
レストランは 1,903→41。**画面に残った74ラベルはバス停24・店14**、あとは銀行1・パン屋1・学校1。
コンビニとバス停——報告そのものだった。

窓を **「その場所が何であるか」を言う唯一のフィールド `class`** に掛け直した。4段階をズームで開き、
`symbol-sort-key` にも同じ式を渡すので、**混雑した角で残るのは病院であって自動販売機ではない**。
`rank` は段内の同値決着として残る（平坦な通し番号に唯一ふさわしい仕事）。

| | R186 | R187 |
|---|---|---|
| 描かれたラベル | 74 | **136** |
| 種類の数 | 17 | **28** |
| 内訳の1位 | バス停 24 | 店 25（バス停 **0**） |
| 博物館／名所／飲食 | 0／0／0 | **6／8／10** |

### 3. 「片方しかつかない」はコンソールに書いてあった

冷たい初回読み込みで**再現**したら、`addSubcables Error: Style is not done loading.`。
`whenStyleReady()` は約6秒で**ハード解決する**——#R41 が意図的に置いた脱出弁で、無いと永久にぶら下がる。
遅い初回（**R186 自身が既定レイヤー2枚で ready を 3.2秒→9.2秒に伸ばしたと測っている**）ではハード解決が
勝ち、MapLibre が addSource を拒否し、**旧コードは警告を出して止まっていた**。チェックは入ったまま、
行は点いたまま、レイヤーは存在しない。ケッペンは自前の5秒ポーリングで生き延びる＝**2枚のうち1枚**。
追加が拒否されたら**待って作り直す**（12回・約9秒・チェックを外した瞬間に放棄）。

### 4. ⚠ 正しいものを計算して、間違ったオブジェクトを描いていた

追記「一本の補助線はいらない。余計な機能を追加するな。」——R186 は水の行き先を実DEM上で窓ごとに
海または脱出不能な盆地まで追跡していた（それ自体は正しい）。そして **2.6 px のシアンの折れ線**を描いた。
元の指示は逆を求めている：「**水は**流れなくなる地点または海に到達した地点まで…**描画すること**」。

`tw-flow`/`tw-flow-case` を撤去し、同じ追跡結果を**同じ水のパレットの画像オーバーレイ**として焼く。
⚠ **幅は画素ではなく地上メートル**（`trace.stepM` ＝追跡した窓の標本間隔）——**画素幅は、ズームした瞬間に
水路であることをやめる**。計算は1行も増えていない。作業矩形・湖・終点ラベルは objection の対象ではないので
残した。実測（千曲川上流）：**183.1 km・1,629点・16窓・33の窪地を通過・stepM 91.6 m**、線レイヤーは不在、
ラスタは存在。

### 5. 全強度の大気は「正確」ではなく「飽和するまで掛けた」

追記「質感がチープ」＋「日光当たってる側が明るくしすぎ」。R186 は `atmosphere-blend` を z0 で **1.0＝全強度**に
していた。⚠ **チャンネルが飽和した時点で色は失われる**——だから光学的に正しいレイリー＋ミー散乱が
**安っぽい白い襟**に見え、日照側が白飛びする。**0.55 へ半減し減衰も速く**、`style.light` の `intensity` も
**0.5 → 0.3**（同じ light が fill-extrusion の陰影も決めるので、この指示は両方に効く）。散乱モデル自体は
一切触っていない——**触れる knob は「どれだけ混ぜるか」だけで、2つの報告はどちらもそれについてだった**。

### 6. 極の「にじみ」——1枚の画像には極冠を描く行が1行しかない

R186 の診断（メルカトルは±85.05°で終わる／見えていたのは `baseColor`）は正しく、処方が半分だった。
正距円筒画像は**最上段の0.176°分が1行**なので、それを極冠全体に配れば**放射状のにじみ**になる。しかも
常時オンだったので、**ダークな地図モードにも明るい青のにじみが乗っていた**。正解は**本当にタイル化されて
いて本当に±90°に届く**出所＝**NASA GIBS の EPSG:4326 WMTS**（500 m・9レベル・`ACAO: *`・レベル0が
2×1タイル＝Cesium の `GeographicTilingScheme` 既定と一致）。同梱画像は**オフラインの床として下に残す**。
両方とも `setWorldBase()` で**基図に追従**する（呼び出し元は `js/world-base.js` 一箇所＝両エンジンを1人が駆動）。

### 7. 残り

- **航空機のマーク**：最初のデザインへ（44単位の輪郭・1.6 px の白線・ramp 0.4/0.58/0.78）。⚠ #R185 が
  「既定で見えているのは2Dグリフではなく立体の機体」と測っているので、**`planes3D` の既定も OFF** に
  しないと画面は変わらない（トグル自体は不変）。⚠ **唯一戻していないのはラスタ解像度**：`pixelRatio` を
  渡さない状態は**欠陥であって外観ではない**（同じ44単位を dpr で描けば画面上の大きさは同じでボケない）。
- **航空機の範囲**：**16→48円**（モバイル6→12）＝2,460×2,460 km → **4,920×3,690 km**、上限2万→5万機、
  下限 z3→z2。API 上限は再実測（r=250 は200・r=300/500 は**403**・`/v2/all` は**404**）。⚠ **ペースは
  上げていない**：1.2秒は #R186 の実測値。ポーリング上限だけ 120→180秒（**上限で切ると実測より速く叩く**）。
- **ウィジェット**：AQI/UV の柔らかい影は**カードの外ではなく中**にあった——158°のグラデーションが右下を
  14%暗くしていた。**平坦な半透明の色味**へ。不透明だったせいで `--glass-fill` を上書きしており、
  **この2枚だけがガラスでなかった**ことも同時に解ける。⚠ 文字色は**合成後の面**で判定する。
- **フライトシム**：「current map view」の空中開始は `camera.eye()`＝視点そのもの（`keepAlt` で +1,500 m の
  クリアランスを無効化）。⚠ **機体の実用上昇限度でクランプ**する（z2 の globe では視点は1万km上空）。
- **海面変動**の既定不透明度 0.62 → **0.60**。

### 8. ⚠ テストが見つけた2件——「既定を変える」は機能を道連れにしてよいという意味ではない

全件（360件）を走らせて7件が落ちた。内訳を取ったら、**2件は自分が入れた本物の欠陥**だった。

**(a) 既定を2Dに戻したら、機体をクリックしても何も起きなくなっていた。** `pickPlane` は
`if(!planes3D) return null` で始まっていた——立体表示しかピックしない前提である。フラットなグリフを
既定に戻した瞬間、**選択もされず、詳細カードも開かず、軌跡も描かれない**。見つけたのは
**tests/r175＝「クリックで詳細カードが開く」**という、立体とは何の関係もないテストである。
#R174 の規則（「描画と違うオフセットで探すピックは、機体がいない場所を探す」）がそのまま答えで、
**描画中のレンダリングに合わせて高度を返す** `_planeDrawAlt()` を1つ置き、ピックと
「どこに描かれているか」診断の**両方**をそこに通した。⚠ **既定値の変更は、その既定に依存していた
コードを全部連れて行く。**

**(b) 極冠の画像が、地球の裏側でも取りに行っていた。** 新しい GIBS レイヤーに**地球全体**の rectangle を
渡していたので、カメラがどこにあってもタイルを要求する——アルプス上空3 kmでフライトシムが走っている
最中でさえ。そこでは全部アプリ自身の衛星画像の裏に隠れていて、**1枚もこのレイヤーの存在理由ではない**。
実測：GPUの無いCIランナーで **tests/r184-cesium-fs が3回中3回失敗**（main は緑）。メルカトルは
±85.0511°で終わるので、**その外側の2本の帯だけがこのレイヤーの仕事**であり、Cesium は rectangle と
交差するタイルしか要求しない——極から離れれば費用はゼロになった。

**残り5件の帰属**：4件（r172/r173/r174/r185）は**立体そのものを検査するテスト**なので、動いた既定に
頼るのをやめて `IntMapPlanes3D.set(true)` で明示的に立体を要求するようにした（トグルも描画も不変）。
1件（r184-satellites ②）は **CelesTrak の絞り込み**——同じコミットで**CIでは通っている**。同ファイルは
27行目に「CelesTrak は答えるのをやめる権利がある」と書き、48行目で `test.skip` する作法まで持っている
のに、②だけその防具が無い。今回の変更とは無関係なので別タスクとして切り出した。

---

## R186 — **測るまで「海」と「海面下の窪地」は同じ／黒い空はCSSの色だった／同じ面の2つの標本は境界で必ず食い違う** (tag `#R186`)

指示は11件。共通していたのは「**そこにあるはずのものが無い**」——星空、大気、極の画像、読み込み画面、
POI名、水の行き先。実装より先にやったのは、**無い理由を一つずつ測ること**だった。

---

### 1. 「地球の背景が真っ黒」——それはCSSの色で、宇宙は最初から透明だった

`#map{background:var(--bg-color)}`。ダークテーマで `--bg-color` は `#000` である。そして MapLibre の
globe は、**深宇宙をアルファ0で描く**（大気シェーダは大気に当たらないレイで `1.0-color.a` を 0 にし、
フレーム自体も `Color.transparent` でクリアされ、canvas は `alpha:true`）。つまり**空は空いていた**
——黒はCSSが塗っていた色だった。

だから星は canvas に描いてレンダラの canvas の**下**に敷いた。この置き方には決定的な利点がある：
**地球の裏側の星は、地球そのものが隠す**。深度バッファも地平線の計算も球とレイの交差判定も要らない。
大気は α ブレンドで描かれるから、リムの近くの星は**大気の不透明度ちょうどのぶん減光する**——実際の
空がそうであるように。

**星は本物**。Yale Bright Star Catalogue 第5版（Hoffleit & Warren 1991）の 9,096 個を同梱（55 KB、
`data/stars.bin`、`scripts/build-star-catalogue.mjs`）。J2000 からアプリ自身の時計まで歳差補正し、
グリニッジ平均恒星時で地球固定系へ回す。大きさは実測V等級、色は実測B−Vから Ballesteros の色温度式と
プランク軌跡でRGBへ。検証：**GMST(J2000)=280.46062°（公表値と誤差0秒角）**、春分の太陽赤緯 0.002°、
夏至 23.435°、視直径 0.5246°、Sirius/Vega/Arcturus/Betelgeuse/Polaris の位置が最大6秒角、B−Vも一致。

⚠ **無限遠の点がどこに写るかは、自分で導くな。転写しろ。** #R182 はカメラ規約を取り違えて「動いて
しまう」値を渡した回である。今回は MapLibre 自身の `Transform._calcMatrices` が組む
`P·T(0,0,−c2c)·Rz(roll)·Rx(−pitch)·Rz(bearing)·T(0,0,−R)·Rx(lat)·Ry(−lng)·S(R)` をそのまま使い、
無限遠では並進が消えることを使った——そしてそれは、MapLibre の大気シェーダの `getSunPos()` が光源
方向に掛けている鎖と**同じもの**だった。信じずに測った：**同じ経路に並進を戻して地表点を射影し、
`map.project()` と比較。5つのカメラ状態・245点で最大誤差 0.000 px。**

### 2. 大気は「実装されていなかった」のではなく「0倍されていた」

MapLibre v5 の globe には**本物のレイリー＋ミー散乱の積分**が入っている——惑星6,371 km、大気6,471 km、
波長別散乱係数 (5.5, 13.0, 22.4)e−6、ミー21e−6、スケールハイト8 kmと1.2 km、トーンマップとガンマ補正。
「実寸・実色」はレンダラ側の持ち物である。ところがそれは `atmosphere-blend` 倍されており、**`sky` を
持たないスタイルはこの値が 0**。このアプリはずっと `sky` を持っていなかった。`set3D()` の
「globe は自前の大気を描く」というコメントは、**一度も走ったことのないものの説明だった**。

太陽方向は `style.light` から来る。既定は視点固定のランプなので、**利用者が見ている方向に明暗境界が
できてしまう**。⚠ そしてこの light の規約は罠だらけである（`position` は [半径, 方位°, 極角°]、方位は
+90°オフセット、**直交形は使用前に符号反転**、そのあと globe 行列と同じ鎖で回される）。#R182 の教訓
どおり、呼び出し側には**規約を持たない一つの事実**だけ言わせた——`scene.setSunDirection({lng,lat})`＝
「太陽が真上に来ている地点」。変換はレンダラの隣（`js/geo-engine.js`）に置いた。検証：light 仕様から
方向を復元して実際の直下点と比較し、**誤差0.077°（＝設定から読み出しまでの18秒ぶんの自転）**。

⚠ 副作用は隠さない：大気の太陽は `style.light` であり、それは fill-extrusion の陰影も決める。衛星
ベースマップの間だけ実太陽になり、切ると既定のランプに戻る。日照シミュレータ（`IntMapSun`）が開いて
いる間は**そちらが勝つ**——「今」より「利用者が調べている瞬間」のほうが具体的な要求だからである。

### 3. 極が黒いのは、タイルが失敗したのではなく**存在しない**から

Web メルカトルは ±85.0511° で定義が終わる。Cesium は地球を ±90° まで描く。だから極冠には**どの
ソースにも要求できるタイルが無い**（アプリのラスタ源はすべて `WebMercatorTilingScheme`）。見えていた
のは `globe.baseColor` である。

答えはメルカトルでない源：同梱の正距円筒 NASA Blue Marble（`data/world-basemap.jpg`、2048×1024、
284 KB）を `SingleTileImageryProvider` で −180…180 / −90…90 に貼り、imagery コレクションの**最下段**へ。
`_reorderImagery` はアプリのレイヤを全部その上へ上げるので、順序の管理は要らない。南極の実写が出た。

**同じ1枚が、8番目の指示（「毎回衛星画像を再読み込み／何もないタイル」）も答える。** MapLibre 側では
`imapworld://` プロトコルを登録し、**ネットワークに一切触れずに**低ズームの衛星タイルをこの画像から
生成する（メルカトルの出力行1つ＝緯度1つ＝正距円筒の行1つなので、タイルは512回の1行 drawImage）。
maxzoom は 4：元画像は赤道方向2,048 px＝512pxタイルでは**z2が等倍**、それ以深は拡大でしかない。
本物のEsriタイルが届くまでの**床**であって、置き換えではない。

### 4. ⚠ 同じ面を2通りに標本化すると、境界で必ず食い違う

水の行き先（指示10の後半）は最初、16方向の最急降下を1点ずつ歩き、下がる先が無ければ窪地脱出、と
書いた。**92メートルで止まった。** 測ると：歩行は**半径92 mの連続曲面上のリング**を見て「ここが最低」
と言い、脱出側の priority flood は**92 mの格子**を見て「窪地ではない（充填深さ0.00 m）」と言っていた。
どちらも正しい。一つの面の二つの標本化は、閾値の近くで必ず食い違う。

だから**質問をやめた**。priority flood の `parent` は「そのセルがどこから到達されたか」＝下流を指す
ので、**どこからでも parent を辿れば窓の縁まで完全な流路になり、窓の中の窪地は全部解決済み**である。
窓を焼いて鎖を歩き、抜けた場所へ窓を移す。1つの窪地ごとに2回floodしていたのが、**約9 kmの川ごとに
1回**になった（測定：295歩101 kmで**95回の窪地脱出**→ 窓15枚で183 km）。

⚠ **さらに窓は前を向く。** 直前の出口をそのまま中心にすると、窓は「来た道」と「これから」を同量
含み、窓ローカルの minimax は来た道から出ていける。実測で2本の長い川が折り返し検出で止まった
（167 km / 119 km）。半幅の34%だけ進行方向へずらすと、来た道は窓の後端に落ちる（183 km / 「湖」判定）。

### 5. ⚠ 「海面下」は「海」ではない——Death Valley が海だと報告された

トレースは15 kmで−4.8 mに達し「海に到達」と言った。そこは**閉じた窪地**であり、水が止まる場所の
教科書的な例である。**標高だけでは海と閉塞盆地を分けられない。** 分けられるのは**連結性**：世界の海は
0 mで一つに繋がっているので、その中の点からは「一度も0 mを超えずに」どこまでも行ける。Death Valley
からは縁を越えねばならない。

⚠ しかもこれは priority flood で訊いてはいけない。最初の実装は「窓の縁まで出るのに何m上がるか」を
訊き、**外洋の太平洋を落とした**——深海平原の脱出路は途中の海山で3.4 m上がるからである。海底の排水は
無関係で、要るのは **≤0 m の領域**だけ。だから塗りつぶし（flood fill）1本。

窓は除きたい盆地より大きくなければならない：z7・約240 km（Death Valleyは長さ200 km——13 kmの窓は
**丸ごと谷の中に入ってしまい**、最初の版はそれで通した）。⚠ そして縁への到達だけでも足りない：
**死海が通る**（ヨルダン地溝の≤0 m回廊が250 kmの窓から出ていく）。海は縁に届くだけでなく**近傍を
埋める**ので、第2の指標は連結領域の面積比：

```
外洋 100%  ロサンゼルス沖 55%  チェサピーク湾口 53%  東京湾 49%  ボスニア湾奥 40%  ミシシッピ河口 70%  カスピ海 92%
死海 7%    Badwater/カッターラ = そもそも0 m超
```

15% は**両側に十分な余白がある谷**にある。

### 6. 残り——測って直した細かいもの

- **水の流下は D8 から MFD へ**（Freeman 1991／Quinn 1991、(勾配)^1.1 × 等高線長 0.5Δ/0.354Δ）。
  D8 の署名的アーティファクト＝開けた斜面で流れが1セル幅の平行線に潰れる、が消える。循環は不可能：
  分配先は必ず `filled` が厳密に小さいセルで、priority flood の pop 順は `filled` について非減少。
  **湖は段階的に**：窪地内のセルは水を隣ではなく**盆地**に渡し、その盆地の全セルを訪れ終えた時点で
  水位を解き、**溢れた分だけ**を出口に注ぐ。満杯にならない盆地は水を止める＝「流れなくなる地点」。
- **作業格子 256→384**（デスクトップ）。60 km表示でDEMはz13＝13.5 m標本なのに、256セルは234 m——
  **17標本を1セルに潰していた**。156 mへ。解は45 ms。
- ⚠ **「水を配置しても結果が出ないことがある」の真因**：格子は最大60 kmに切られるが表示はしばしば
  それより広く、外側のクリックは `cellOf` が null を返して**黙って捨てられていた**。青い点だけが出て
  何も起きない。作業矩形を**描き**、外側のクリックは**そこへ格子を作り直す**ようにした。
- **航空機は1つの円から円のタイル張りへ**。250 nmはAPIの硬い上限（300は403）。⚠ ペースは実測：
  4並列は20件中9件が**裸の接続失敗**（429ではない＝先方がこのIPに答えなくなる）、その後は再試行も
  全滅。**30秒の静止でブロックが解け、1.2秒間隔なら14連続すべて成功**。よって逐次1.2秒・16円・
  掃引の大きさに比例するポーリング間隔（0.29 req/s）。失敗した円は1回再試行し、それでも欠けたら
  **前回の位置を2分だけ引き継ぐ**（穴の空いた空を「空いた空」として描かない）。実データがある限り
  合成データには落ちない。z5→z3、上限1,800→20,000機。実測：1円494機 → 20円1,708機。
- **読み込み画面**は body の**先頭の静的マークアップ**（CSSは render-blocking な `<link>` なので
  最初のフレームで塗られる。JSで注入したら、覆うはずのものより後に出る）。進捗は本物の里程標
  （modules→renderer→style→layers→idle）で、20秒で開かなかったら理由をコンソールに書いて開ける。
- **POI名**は OpenMapTiles の `poi` レイヤー（z14〜）。`rank` の窓をズームで開き、`symbol-sort-key`
  で衝突判定にも同じ順序を渡す。⚠ `["zoom"]` をフィルタに使うと整数ズームでしか再評価されないが、
  タイルごとの rank 窓にはそれが**望ましい**。
- **海面変動**：数値入力が `input` で即時適用（Setは残すが誰も待たない）。⚠ **Opacity 100%が100%で
  なかった真因は色の側**——ランプが 0.60〜0.92 のαを持ち、レイヤの `color-relief-opacity` はそれに
  **掛かる**ので、スライダは最大でも0.92／岸では0.60にしか届かなかった。ランプを完全不透明にし、
  透明度はスライダ一本に。深さは色（濃紺→淡青）が担っていて、αではない。
- **ウィジェットの外側の影を全部撤去**（`inset` の1px ハイライトはガラスの縁で、周りの暈ではない）。
- **ケッペンと海底ケーブルが既定でオン**。⚠ チェック済みの箱は何も描かない（#R34と同じ）ので
  `change` を発火させる。⚠ そして**セッション復元は「既定オン」を切れなければならない**——さもないと
  「既定オン」が「二度と消せない」になる。

---

## R185 — **重いのはフレームではなく「動かしたとき」／半解像度の4つ目の扉／出所が1つだと空になる** (tag `#R185`)

指示は5件（両エンジンの衛星画像・3Dの高速高画質化／Cesium の 3D 衛星画像が重い／人工衛星レイヤーを
フルクオリティに／航空機アイコンをもっと目立たせ立体をリアルに／地名検索のズームを対象に合わせる）。
5件とも**再報告**——#R183/#R184 がそれぞれ「完了」と書いた項目である。だから最初にやったのは実装ではなく、
**同じ物差しで測り直すこと**だった。

---

### 1. Cesium が重い——測っていたのが「静止画」だった

#R183 は「同一シーンで Cesium は MapLibre の約4倍」と測り、#R184 は FXAA を切って両軸を改善した。
どちらも**カメラを止めて**フレーム時間を測っている。今回、同じ時計（`readPixels` で GPU を待つ）で
**カメラが動いているとき**を測ったら、話がまるで違った：

| | 静止 | ドラッグ1歩 | ズーム1歩 |
|---|---|---|---|
| MapLibre | 21 ms | **36 ms** | 42 ms |
| Cesium | 24 ms | **249 ms** | 120 ms |

静止時はほぼ互角で、**動かすと7倍**。40歩10.7秒の内訳は `_flush()` が **4,552ms＝43%**——
**毎フレーム 84 レイヤー・1,885 エンティティを作り直していた。**

真因は2つあった。ひとつは `_afterMove` が「カメラが動いた」というだけで全ベクタレイヤーを dirty に
していたこと。もうひとつが**面白い方**で、その判定に使う `getZoom()` は**地形メッシュにレイを当てて
逆算する読み取り値**である。DEM が精緻化するあいだ、**ズーム13で固定したパンなのに 13.12→13.14→
13.00→13.02 と揺れ**、0.01 の門を **19フレーム中5回**越えて「ズームが変わった」＝全レイヤー再構築に
なっていた。**門を掛けている量そのものが、測り直すたびに違う数を返していた。**

直したのは3つ、いずれも「出力が何に依存しているか」を言い直しただけ：

- **タイル被覆の署名**。`vt.want()` が被覆をキー列として返し、ベクタタイル層は**タイル境界を跨いだ
  ときだけ**再構築する。被覆が同じなら特徴も同じで、画面に出ている実体はもう正しい。
- **ズーム感度は文書の性質**。`filter`/`paint`/`layout` に `"zoom"` も `stops` も現れないレイヤーは、
  ズームが変わっても出力が変わらない（min/maxzoom の窓を跨いだ場合は別途判定）。84 のうち
  ズーム式を持つのは 18、窓を持つのは 11 だった。
- **スタイル評価のズームは「命令された値」**。`setCamera` が置いた位置にカメラが居るあいだは、
  `setCamera` が受け取った zoom を返す。読み取り値が収束する先と同じ数で、地形と一緒に揺れない。

ついでに3つ、同じ性質の無駄：`_centreCarto`/`_hpr` を**姿勢＋フレーム番号でメモ化**（`setCamera` は
冒頭で中心・ズーム・方位・傾きを読む＝**地形ピック4回で 7.7ms**、メッシュはフレーム内で変わらないので
1回で足りる）／`_features` を**レイヤ単位からソース単位**へ（10レイヤーが同じ被覆を10回組み立てて
10回走査していた）／ラベルの衝突判定を**フレームの行列を1回作って自前射影＋格子**に。

**249ms → 61ms。** `_flush` 100.6→7.4ms、`setCamera` 23.8→3.1ms、dirty レイヤー中央値 **84 → 0**。

### 2. 衛星画像が半解像度だった——同じ欠陥の4つ目の扉

#R178（衛星タイル）・#R179（基図）・#R180（描画バッファ）と同じ欠陥が、今度は**画像レベルの
選び方**から入っていた。同じカメラ（ユングフラウ真上 z13）で撮って測った鮮鋭度（平均|ラプラシアン|）：

```
MapLibre  22.7      Cesium  7.5
```

Cesium は「1テクセル＝地形タイルの幾何誤差1単位」になるよう画像レベルを選び、地形は
`maximumScreenSpaceError`＝**2デバイス画素**まで精緻化する。つまり**1テクセルが2画素**＝半分。
読み出して確認：地形タイル level 13 が **1,024 デバイス画素**を占めるのに、載っている画像は
**512テクセル**。`maximumScreenSpaceError=1` にすると 15.7 に上がるが、**地形メッシュまで4倍**になる
（18タイル→66タイル）——壊れていたのはメッシュではない。

カスタムプロバイダがこの選択について所有している入力は**宣言 `tileWidth` ただひとつ**なので、
そこに方針を式で書いた：`tileWidth = tileSize / screenSpaceError`（＝1テクセル1デバイス画素）。
**7.5 → 15.4**、地形タイルは 18 のまま、届く画像は #R178 の 512px 合成タイルのまま。
さらに1段深くすると 15.98（+3.5%）にしかならずタイルは3.4倍——**そこが限界だと測って確かめた**ので
1段にとどめた。携帯は `fullResolution:false` で従来どおり（#R20 の RAM 方針）。

⚠ **そして、画像が4倍になった途端に効いた欠陥がひとつ。** プロトコル経路（`imapsat://`）は
Cesium の `RequestScheduler` を通さず生の Promise を返していた——**上限も優先度も取り消しも無い**。
ドラッグ中のタイル復号は **316件・中央値303ms**（＝待ち行列）だった。Cesium が渡してくる Request を
そのまま使い、「保留なら undefined を返す」という Cesium 自身の作法に合わせて **227件・129.6ms**。

### 3. MapLibre 側は、測って触らなかった

canvas は dpr 等倍、衛星タイルは #R178 の 512px 合成で 1:1、傾斜時の異方性フィルタは #R180 で
全ピッチ有効、地形ドレープの RTT は `qualityFactor 2`＝dpr2 で 1:1。衛星以外の全レイヤーを隠す
インターリーブ A/B でも **−2.2ms（雑音）**で、捨てている画素も無駄な描画も見つからなかった。
ここで「何かする」ことは**表示解像度を超える画素を描いて速度を捨てる**ことにしかならないので、
しない。指示に対しては**そう報告する**（利用者に確認済み：「できないなら何もしなくていい」）。

### 4. 人工衛星レイヤーは、そもそも空だった

再現しようとしたら **CelesTrak が数時間まるごと到達不能**だった（DNS は引けるが接続タイムアウト。
既存の公開CORSプロキシ3種も Cloudflare 520/522＝先方の障害）。**単一ソースのこのレイヤーは、
何も描かない。** 利用者の回答は「何も表示されないのは論外。実際の位置に全衛星がリアルタイムで
美しく表示されるのが最低条件」。

- **出所を4系統に**：CelesTrak →公開CORSプロキシ2種→ **同梱カタログ `data/tle/`** → localStorage。
  ⚠ **同梱カタログは最後の砦ではなく「床」**にした。ネットワークのタイムアウトを待ってから読むと、
  最大1分間ずっと空＝結局「何も表示されない」。同一オリジンで数ミリ秒なので**先に読んで即描画**し、
  ライブが答えたら差し替える。どの出所を使ったかは画面にもう出ている。
- **同梱カタログは実物**（`scripts/build-tle-snapshot.mjs`。CelesTrak、不達時は SatNOGS DB＝
  Space-Track/CelesTrak 由来の要素）。捏造はしない——SGP4 は元期から実位置を出し、カードは
  **要素の経過時間**を出す。CI（`.github/workflows/tle-refresh.yml`）が1日2回作り直す。
  ⚠ **TLE の元期 `YYDDD` は数として並ばない**（1975=75042 が 2026=26212 より大きい）——最初のビルドは
  「最新の要素は1975-02-11」と報告した。絶対時刻へ直してから比べる。
- **既定を `active`＝全衛星に**。#R184 が 157天体を既定にした理由は2つとも測って消えた：要素は
  **TLE 形式**で取れば 6.8MB→約2.5MB（数値は同一）、SGP4 は**実測 1.94µs/天体**で1万1千天体でも
  21ms／tick。tick は天体数に応じて 1→2→3 秒。
- **美しく**：アイコンを**地上軌跡の方位へ回す**（慣性速度をECFへ回して地球自転 ω×r を引き、直下点の
  ENU で読む＝再伝播なし）。軌道帯（LEO/MEO/GEO/HEO）で色と大きさの違うハローを敷く。名前は
  600天体以下のときだけ（`text-optional` は候補から外してくれない）。

フィードを遮断した実測：**1,634天体・1,551描画・全部が実位置**（0,0 のダミー0件、最低高度>100km、
最高>30,000km、全天体が進行方向を持つ）。

### 5. 航空機アイコン——直した物が画面に出ていなかった

#R183 は2Dグリフを作り直した（デバイス解像度・白リム・ドロップシャドウ・実機の平面形）。
しかし `planes3D` は**既定 true** で、`applyPlanesMode` は「平面グリフ**か**立体」の排他である。
**既定で見えているのは立体のほう**で、そちらには輪郭も縁取りも影も無く（`fill-extrusion` に
outline は無い）、しかも**2Dグリフより小さかった**——半長 13px 対 19px、つまり立体にした瞬間に
アイコンが**3分の1縮んでいた**。東京 z11 の実スクリーンショットで、淡い基図の上の1機は目が滑る。

- **自前の対比**：白いリムと暗いハローを、機体より少し大きい**極薄のプレート2枚**として本体の**下**に
  敷く。真上から見ると本体が中心を隠すので**残るのはちょうど輪郭**、傾けると**明るい下面＋影の線**に
  なる（実機がそう見える）。2Dグリフが線で描いていることを、線を持てない立体で同じ結果にする。
- **大きさを 2Dグリフに合わせる**（19px）。
- **立体の作り**：胴体を2段（下部は幅広・上部は細く＝断面が丸に近づく）、エンジンを**主翼の下に吊る
  独立ナセル**、垂直尾翼を2段テーパー。8部品＋2プレート。真上からの輪郭は変えていない。
- ⚠ **部品の底面はもう機体の高度ではない**（プレートが下にあり胴体はその上に乗る）。`acAlt` を全部品に
  持たせ、`lifted`/`maxAlt` はそれを読む——#R183 の「カウンタが数えている物を疑え」と同じ罠で、
  地上の機体が「高度118m」になるところだった。

### 6. 地名検索のズーム——実測範囲を持っていない経路があった

#R183 の枠取り（実範囲 → 種別）は正しい。壊れていたのは**ローカル一致**——国名を打つと
`countryStats` の完全一致が**先頭に**出るが、そこには**範囲が無い**。実測：

```
バチカン市国 4.4   モナコ 4.4   シンガポール 4.4   日本 4.4   ロシア 4.4   フランス 4.4
```

国の実ジオメトリ（読み込み済み）から **bbox を持たせて**同じ階段に通した。⚠ すると今度は
**モナコが落ちた**——#R183 の「外れ地の判定」が**割合**（点が範囲の中央60%の外なら棄却）で、
モナコは fy=0.92。しかしその規則が探しているのは**飛び地**であり、飛び地とは**距離**である
（東京は範囲の中心から 6.3°＝700km、フランスは 20°、モナコは 0.10°＝11km）。絶対値の門
（1°≈110km）を併せて、東京とフランスは落ちたままモナコとバチカンは自分の範囲を保つ。

⚠ 分類表の穴も2件、実応答で見つけた：**グレートバリアリーフ**（`natural/reef`）と
**グランドキャニオン**（`natural/valley`）は**どちらも OSM ではノード**で範囲が無く、表に無かったため
**既定の 12.2＝8km 窓**になっていた（2,300km のサンゴ礁と 446km の峡谷）。

実測（すべて修正前は上記のとおり）：バチカン 16.5・モナコ 12.8・シンガポール 10.4・日本 4.0・
ロシア 3.2・フランス 4.4（仏領ギアナで範囲棄却＝正しい）・GBR 8.6・グランドキャニオン 9.6・
エッフェル塔 15.5・バイカル湖 6.0・マンハッタン 10.8・セントラルパーク 13.3。

### 7. 本番検証で1件出た——SGP4 は黙って発散する

デプロイ後に本番で測ったら、レイヤーの最大高度が **9,166,588 km** と出た（月まで384,400km）。
同梱カタログを全件伝播して外れ値を数えると3件で、うち**2件は発散**だった——ASTROCAST-0201 が
9,244,632km、MENUT が 238,361km で、**どちらも平均運動が16回転/日**＝90分周期＝高度数百kmの物体である。
残る1件 EXPLORER 50（IMP-8）は**本物**で、実際に270,956km にいる。
つまり**高度のしきい値では判定できない**。判定は**その物体自身の軌道**で行う：SGP4 が平均運動から
導いた半長軸が遠地点 a(1+e) を決めており、実在の位置がそこから大きく外れることはない（余裕1.5倍。
IMP-8 は自分の遠地点の1.04倍で残り、発散2件は36倍・1,400倍で落ちる）。80km 未満は再突入。
**落とした数は `state().diverged` に出す**（黙って消すのはそれ自体が不正直）。
実測：1,551 → 1,545 描画（6件が発散）。#R184 まで既定が157天体（visual）だったので見えていなかった。

### テスト

```
static checks             298 ファイル走査 · PASS
renderer-coupling gate    PASS
node --test               r185-checks 9件を追加
playwright                r185 4件（Cesium のパン＝dirty 中央値0／画像レベル＝地形+1／
                          フィード遮断下の衛星 1,634天体／航空機の輪郭と高度）
```

---
## R184 — **#R183 が持ち越した6件を全部やる：軌道は自作しない／継ぎ目を実データで埋める／位置カメラには位置を渡す／速度と画質を1つの物差しで測る** (tag `#R184`)

#R183 は指示11件のうち5件を終えて、6件を「未着手」として持ち越した——**人工衛星レイヤー／
ドローン航法10項目／経路検索9項目／Cesium でフライトシム／衛星画像・3D の高速高画質化（両エンジン）**。
このラウンドで**その6件をすべて実装した**。

各項目の詳しい設計判断は `Architecture.md` §19 #R184 補足に、ファイル単位の役割は §3 の一覧にある。
ここには「何をして、何が実際に壊れていて、何をどう測ったか」を書く。

---

### 1. 人工衛星レイヤー（`sats`）——「Live aircraft trafficの要領で、フルクオリティ」

航空機レイヤーの**形**を軌道に当てた：キー不要の実フィード（CelesTrak GP/OMM）、実伝播、
ホバーでツールチップ、クリックで詳細カード、選択天体の**フットプリントと地上軌跡**、
凡例の中のカタログ選択と「ここから見えるものだけ」フィルタ、不透明度、Atlas のアクションと SYS、
出典、5言語。新規 `js/satellites-live.js` + `js/satellite-detail.js`。

**軌道伝播は自作しない。それは正しさの判断である。** SGP4 の近地球分岐だけでは周期225分未満しか
有効でなく、**名前で探される代表格の GEO と Molniya がもっともらしく間違った場所に出る**。
`satellite.js`（MIT）を、Vallado の標準ベクトル（衛星00005・t=0）に対して**誤差 6.8e-9 km ＝
7マイクロメートル**で検証してから使った。実データの ISS でも 422.8km／7.66km/s／92.96分／51.631°。

**描画は直下点。高度は数値として持ち、高さにはしない。** #R172 が航空機を実高度へ上げたのは
11km が地図上の実距離だから。静止衛星は**地球半径の 5.6 倍**で、実尺は画面外、
「ほどよい高さ」は捏造（標準指示4）。代わりに**実際の幾何**を正しく描く——フットプリント
`acos(Re/(Re+h))` と前後1周の地上軌跡。

**⚠ ブランチに残っていた土台には、静かに死んでいる欠陥があった。**
`shadowFraction(sunEciAU, satEciKm)` は**太陽の位置**を取る関数で、時刻を取らない。
`Date` を渡すと **NaN** が返り、例外も警告も出ないまま**全天体の日照判定が null** になる——
それは「肉眼で見える衛星」というカタログを意味あるものにしている唯一の性質だった。
正しくは `sunPos(jday(t)).rsun`。ISS 1周で **57分照射／36分食**＝実際のデューティ比。

**⚠「取得中」を「無い」と答えると、後から呼んだ方が空になる。** `load()` の
`if(loading) return false` は、レイヤーと**レイヤー一覧のプレビュータイル**が両方カタログを
要求したときに、後から来た方へ空配列を返した（実測：プレビュー 0 件・レイヤー 157 件）。
合流させる。しかも**群ごとに**——「合流」と「無視」は別物で、`setGroup()` 直後の別カタログ要求は
自分のリクエストを出さないと永久に間違ったカタログを持つ。

**`paint()` は先に伝播し、後で描く。** 描けないあいだ伝播ごと止めると、157天体が計算可能なのに
`state()` が 0 と答える（凡例が「0 / 157」と出た）。位置はこのレイヤーの**データ**で、
レンダラは置き場所にすぎない。

`tests/r184-satellites.spec.js` 7/7。GEO 群は**全天体が SDP4 分岐**で高度中央値 35,786km、
ISS のフットプリント半径は閉形式 `Re·acos(Re/(Re+h))` に対し 2,258km、地平線フィルタは
**仰角が正の天体をちょうど過不足なく**残す。

### 2. ドローン航法——10項目

#R174 はプランナーを作り、`registerWindField` / `registerHazardSource` という**2つの継ぎ目を
宣言だけして埋めなかった**（データ源を配線していなかったから）。新規 `js/drone-ops.js` が
両方を実データで埋め、さらに「意見」ではなく「操作」である4件を足す。**プランナー本体は
書き換えていない**（外側から公開APIに刺す）ので、このファイルが無ければ従来どおり動く。

- **風**＝Open-Meteo のモデル面 10/80/120/180 m を**面と面の間だけ内挿**し、区間の **ETA の時刻**で読む。
  along-track 成分は対地速度・時間・電力へ。cross-track 成分は「傾けて保つ分の電力」として
  別の所見に（上の数値に含まれていないと明記）。
- **帰投分の電池**＝同じ風の中を戻る往復で使用可能容量と比べる。
- **通信可能範囲**と**視通の途切れ**＝同じ地形走査 1 回。曲率・大気屈折・第1フレネル・
  ナイフエッジ回折は `IntMapLOS._phys` を**再利用**（2つの道具が「遮られた」の意味で食い違わないため）。
- **飛行禁止・制限区域**＝OSM（空港・ヘリポート・軍用地・原子力・刑務所・保護区・国立公園）。
  **目安であって航空当局の許可ではない**とパネルに明記。
- **地形追従**は #R174 の `followTerrain` が既にあり、そのまま。
- **最短／最省電力／最安全**＝同じウェイポイント上の3つの**実計算**（比較後に元の経路へ戻す）。
- **緊急着陸地点**＝OSM の開けた土地 × **DEM で測った傾斜 8°以下** × 定格降下率で到達できるもの。
- **帰投経路**＝経路上の最高地形＋最低地上高まで上昇→水平移動→降下（＝実際の RTH の挙動）。
- **他機との干渉**＝水平・垂直・**時刻**の3次元での最接近。

**⚠ 検証して見つけた3件（どれも「動いているように見える」形）：**
1. `IntMapWx.metNo()` は `{current, daily}` を返し、**hourly 系列は存在しない**。
   風の fallback は undefined を読んでいて何も出していなかった。系列は**共有クライアント側に**
   公開した（別ファイルで2本目の fetch を書くのは #R183 が消した形そのもの）。
2. Overpass の **`out center` は bounds を返さない**ので、緩衝距離 0 の面（軍用地・保護区・国立公園）が
   「重心から 0 m 以内」という**一度も発火しない検査**に退化していた。`out tags bb` で実際の広がりが
   返る（チューリッヒ空港 3.25km）。
3. `windAt()` が 0.25°セルの**完全一致**を要求し、読み込んだ2セルの**間**の点で null を返した。
   最寄りの公開格子点（0.6°以内）へ落とす。

実測（ベルン・アルプス 14km・prosumer機）：2.4GHz は点検51点中40点で視通が切れ回折損失62dB、
9.8km で予算超過。往復 84.3Wh に対し使用可能 61.6Wh。OSM 区域53件、着陸候補40件中25件が到達可能。
`tests/r184-drone.spec.js` 10/10。

### 3. 経路検索——9項目

3件は**要求の組み立て方**を変えるので `js/routing.js` に、6件は**できた経路について訊く**ので
新規 `js/routing-ops.js` に。

- **経由地**＝`opts.via` は #R132 から効いていた。**無かったのは人がそう言う手段**。
- **通過禁止範囲**＝Valhalla の `exclude_polygons`。OSRM はどのモードでも表現できないので
  **能力による provider 選択**。実測（ミュンヘン・直線上の2km箱1つ）4.734km → 5.444km。
  徒歩・自転車でも効く。
- **フェリー／鉄道／徒歩の個別除外**＝MOTIS の `transitModes`（**許可リスト**なので1つ除くとは
  他を並べること）と `maxPre/maxPostTransitTime`。実測：120秒で候補0本・1800秒で10本＝本当に効く。
- **標高差**＝同じ terrarium DEM。**3m のヒステリシス**を明記（見えない定数に依存する合計は測定ではない）。
  実測ミュンヘン→ニュルンベルク +1,011m / −1,230m・正味 −219m＝2都市の標高差そのもの。
- **国境通過**＝順番に国名で、かつ**各国の通過距離**も（200m の「通過」は境界の解像度の話で、
  それを見分けるのが距離）。
- **沿道の天候・災害・ニュース**＝経路上の複数点を**その人が着く時刻で**引く。
- **到着時刻**＝出発起点でも「〜までに着く」起点でも。
- **経路の差**＝共通部分を除いた分だけを描く。
- **過去の路線網での経路計算**＝OSM 自身の年代記録でその年に存在した way だけを残し、
  実ジオメトリからグラフを作って Dijkstra。ベッドフォード→ケンブリッジは **1950年で 49.3km、
  現在は `disconnected`**（Varsity Line の 1967年廃止）。**経路自身のどれだけが「年代の記録がある線」か**を
  報告する＝どこまでが記録でどこからが推定か。

**⚠ 測って直した3件：** OSM は「もう無い線」を**名前空間**（`abandoned:railway`）**と値**
（`railway=abandoned`）の2形で書く（名前空間だけ見て Varsity Line を取り逃した）／回廊の padding を
距離に比例させると130kmの旅で1.2°＝イングランド南部全域になり返ってこない（0.35°で上限）／
**配列走査の優先度キューの Dijkstra は O(V²)** で25万ノードでは終わらない（二分ヒープ＋
バケット化した端点スナップで約5秒）。`tests/r184-routing.spec.js` 9/9。

### 4. Cesium でフライトシミュレーター

**まず再現した。** 大半は既に動いていた——起動し、6自由度モデルが回り、機体は実距離を進み、
ジェスチャは全部止まり、空が入れ替わり、ムービングマップが第2ビューとして開いた。
壊れていたのは1つで、しかも**「動いているように見える」形**：経度・緯度・方位はすべて正しいのに
**視点の高度だけ 3,174m ずれていた**。

原因はメソッドの欠落ではなく**2つのエンジンの実際の違い**。MapLibre のカメラは中心＋ズーム＋
ピッチ＋方位で、視点は**その帰結**——だから視点をコックピットに置くには前方の目標点を狙って逆算する
（`calculateCameraOptionsFromTo`＝#R174〜#R179 が6ラウンドかけて詰めた道）。Cesium のカメラは
**位置そのもの**。同じ道を通すと「目標点の高さ」と「視点の高さ」が混ざる。

契約に `capabilities.eyeIsPosition`（MapLibre `false`／Cesium `true`）を足し、シムは**自分が
すでに計算している視点**を `camera.setEye` でそのまま渡す。**MapLibre 側の経路は一切触っていない。**
⚠ 最後の砦の `jumpTo` に `!posCam` を足すのが要——位置指定パスでは `cam` が null・`okCam` が false で、
それは「FromTo が使えない」と**同じ形**なので、毎フレーム z14 へ引き戻していた。

修正後の誤差 **2ナノメートル**（3,174m から）。方位 90.05°で東進、ピッチ 95.6°で機首上げ。
`tests/r184-cesium-fs.spec.js` 3/3（MapLibre 側が無傷であることも含む）。

### 5. 衛星画像・3D の高速化と高画質化（両エンジン）

**「どちらか一方の犠牲はNG」を、1つの物差しで測れるようにするところから。**
#R183 が原因を特定できなかったのは、「速いか」と「見た目が悪くなったか」を**別々の器械で別々の時に**
訊いていたから。`tests/r184-imagery-profile.spec.js` は1つの凍結シーンで、フレーム時間の中央値の隣に
...
**MSAA の枚数は「画面」の関数**にした：dpr 1→4（従来どおり）、2→2、3以上→1。dpr 2 では CSS 1画素が
すでに 4 デバイス画素で、その上の 4×MSAA は CSS 画素あたり16標本＝**どの画面も解像しない**。
見えない標本の代金を払うのをやめただけで、画質は落としていない（1× では従来と同一）。
地形 LOD は**測って触らなかった**（SSE を上げても速くならず、上げれば detail だけ失う）。

**⚠測定そのものが2度嘘をついた。**
(a) `preserveDrawingBuffer` の無い WebGL canvas は**合成後にクリアされる**ので、1 setTimeout 後の
読み出しは真っ黒（黒率 1.00／postRender 内なら 0.00）——鮮鋭度の数字が全部「空のバッファの測定」に
なるところだった。(b) タイル到着中の A/B は**タイルの到着を測る**：最初の版は「FXAA を入れると
鮮明になる」と報告した。`tests/r184-imagery.spec.js` 4/4（#R180 の全デバイス解像度が両エンジンで
維持されていることも含む）。

### 6. 検査は「プログラムがどう動くか」に合わせる
...
import 束縛は**宣言**として数える（さもないと `import * as SAT` の全使用が「実行時に何にも解決しない
自由識別子」に見え、この検査が探している当のものの偽陽性になる）。

なお `satellite.js` 7 は**任意の WASM 加速器**を再エクスポートし、その Emscripten 入口は
トップレベル `await` と `node:module` を使う。パッケージに `sideEffects` 宣言が無いので Rollup が
グラフに残し、**ビルドがそこで落ちる**。`vite.config.js` で両方の subpath import を
**呼ばれたら throw するスタブ**へ向けた（黙って no-op にすると本当の回帰を隠す）。

### テスト

```
static checks             283 ファイル走査 · PASS
renderer-coupling gate    PASS（予算1・実測1）
node --test               443 pass / 0 fail（r184-checks 9件を追加）
playwright                r184-satellites 7 · r184-drone 10 · r184-routing 9 ·
                          r184-cesium-fs 3 · r184-imagery 4 · r184-imagery-profile 1
```
...
**このラウンドで完了したのは5件**（下記）。残り6件は未着手で、#R184 以降に持ち越す。
...
### 5. Line of sight — 「2点間」という、これまで答えられなかった問い

面のスイープは「このアンテナから何が見えるか」に答える。しかし人が見通し線に本当に訊きたいのは
**「この地点から**あの地点**へ届くか。届かないなら何が邪魔で、アンテナを何m高くすればよいか」**であり、
これには**構造上答えられなかった**——60kmの円を1,400セルのラスタで表すと地上分解能は43mだが、
リンクの成否を決めるのは「あと1m高いかどうか」の尾根1本だからである。

つまりこれは面の焼き直しではなく、**同じ物理を使った別の計算**である。

**精度。** 線のタイル数は面積ではなく**長さ**に比例するので、プロファイルは長いリンクでも
terrarium native の **z15** で走れる。面のスイープはタイルキャッシュ予算のため後退せざるを得ない
（#R176：超えると掃引中のタイルが追い出される）。実測：10kmで**3.9m間隔**、98kmで**24.5m間隔**、どちらもz15。

**追加したもの（すべて既存のヘルパーから）：** 曲率＋屈折を適用した地形断面（地球の膨らみが
「判定の中」ではなく**膨らみとして見える**）／**決め手となる地形**（どの尾根が決めているか・
何km地点か・何m余裕/不足か）／第1フレネルゾーンの**割合**（60%規則。yes/noではない）／
遮蔽時のナイフエッジ回折損失 dB（ITU-R P.526）／そして現場が本当に必要とする数字＝
**必要アンテナ高**（推測ではなく同じ判定を二分探索して**解く**）。

**障害物の順位は「素の余裕」ではなく「フレネル半径に対する比」で決める。** 第1ゾーンは中間点で最も広いので、
メートルだけで順位を付けると**手前の小丘**が、実際にリンクを決めている**中間点の尾根**を追い越してしまう。
判定語（clear / fresnel / diffraction / blocked）はラスタと**同じ4語**にした——2つの道具が
「blocked」の意味で食い違うのは、道具が1つしかないより悪い。

**実地形で検証（推測ではなく）：**
- 東京タワー（150m）→ 富士山頂 97.9km：**clear**。正しい（富士山は東京から見える）。
- **富士山を貫くリンク 49.9km**：blocked・決め手は **25.16km 地点＝ちょうど中間＝山頂**・
  **3,495m 不足**・回折損失 58dB。アンテナを400mにしても余裕は193mしか改善しない——これも正しい。
  50kmのリンクは片端を上げても中間点の光線はほとんど動かない。
- 12.2km で 15.6m 不足：「**回折で到達可**」・**必要高 130.6m**。

**そして null だった結果は、解法の壊れではなく物理だった**（UIで説明するようにした）：
9.3km のリンクで尾根が**経路の94%地点**にある場合、500mのマストでも見通せない。**手前**を上げても
障害物地点での光線の持ち上がりは `(1 − d1/total)` 倍しかなく、**29.5m 対 必要63.7m**。
パネルは「**相手側のアンテナを上げてください**」と言う——判定と**答え**の違いはここにある。

断面グラフは devicePixelRatio で描く（このラウンドで航空機グリフが学んだ教訓の、隣のパネル版）。

---

### 6. 余談: `js/weather.js` の改行コード
