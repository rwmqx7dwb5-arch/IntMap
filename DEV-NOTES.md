...
## R174 — **#R173 が壊したフライトシムの復旧（空・見上げ）／傾きの視点固定がズームまで飲んでいた／立体の高度単位／軌跡を消していたのはダブルクリック／ドローン航法** (tag `#R174`)
...
**外れた仮説を2つ潰した**: ①リボンの幅が縮尺依存で細くなる → 実測でズームインすると**太くなる**。②高ズームでカメラが
機体高度より下に来て視錐台から外れる → 低高度（4,000m）の機体で追試すると z14.5 でも 9本描かれていた。**どちらも違う。**

**真因**: **ダブルクリックは地図のズーム操作**で、MapLibre は自分の `dblclick` の前に**通常の `click` を2回**配る。
その click が機体ピッカーの「何もない所をクリック＝選択解除」に落ちていた。

**実測（スタブ ADS-B フィード）**:

```
ホイールで z11 → z12.86  : selected ABC123 / legs 6   （保持）
同じ場所でダブルクリック : selected null   / legs 0   （消える）
ズームボタン（＋）        : selected ABC123 / legs 6   （保持）
```

**直し方**: ①ダブルクリックの**2回目**（`e.originalEvent.detail >= 2`）は無視する（機体を選んだ直後に同じ機体を
トグルで外してしまうのも防げる）。②「何もない所」の解除は **320ms 遅らせ**、`dblclick` が来たらキャンセルする。
機体に当たったクリックは従来どおり即時（遅延しない）。

**ついでに直した実バグ**: 軌跡のリボン幅・厚みは縮尺から作られるのに、`zoomend` では**グリフだけ**作り直していた
（軌跡は 20 秒に一度のポーリング時しか作り直されない）。同じハンドラで軌跡も作り直す。
