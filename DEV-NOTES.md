...
- **Still sequenced (not this round):** universal object list, multi-point route optimisation (TSP), unified disaster simulator, slope/aspect analysis, sun/shadow, RF/coverage, Earth Replay. Live cameras ALREADY EXIST (`dl-webcams`, OSM Overpass `contact:webcam`/`webcam` — see Sources).
