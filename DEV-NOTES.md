...
（`BlockableMapEventHandler.reset()` が `_ignoreContextMenu` を立てる）。このエンジンは毎回出しており、
アプリはそれで右クリックメニューを開く。**実測：右ドラッグで1回回したあと、次のホイールと次の ctrl ドラッグは
メニューに当たって何も起きなかった**（`document.elementsFromPoint` の先頭が `DIV#ctx-menu`）。
ブラウザ既定のメニューは従来どおり抑止したうえで、**回したあとの離しはクリックとして通知しない**
（次の押下でリセット＝素の右クリックは今までどおり効く）。
...
- `tests/r182-cesium.spec.js`（**7件・実ブラウザ・差分方式**）：14ジェスチャを両エンジンで実入力し、
