...
**なぜテストが取り逃したか（重要）**: #R161 のテストは全て **register 済みデータが空**の状態で走っていた＝**本番と前提が違った**。`tests/r161-checks.test.mjs` に **#12b** を追加し、**本番同様に重複行を register した状態**で曖昧性解決・階層・新規curated地点の追加を検証する。ラベル付きコーパス/ホールドアウトは 100% のまま（回帰なし）。
