...
## 2b. どのモデルで走らせるか（Claude Code のみ）

**読むだけ・数えるだけ・掃くだけの役に、決める役の値段を払わない。**
⚠ 下げてよいのは**何を返すかが決まっている**役であって、**何が起きたかを判定する**役ではない。

| 役 | 既定 | なぜ |
|---|---|---|
| `intmap-scout` | `sonnet` | 成果物が `file:line` の数え上げ。正解が機械的に決まっており、漏れは網羅性の問題であって判断の問題ではない |
...
| `intmap-prod-verifier` | 継承（親と同じ） | 本番で**観測器の嘘**を見抜く役。ここを弱めるのは下の禁止に真っ向から反する |

正本は `.agents/roles/*.md` の `claude: model:` で、`node scripts/agent-sync.mjs` が
`.claude/agents/*.md` の frontmatter へ写す。綴りは `npm run check:agents` が照合する
（Claude Code は知らない名前を**黙って無視して継承に戻る**ので、宣言だけが残る）。

### ⚠⚠⚠ 昇格させる条件（呼び出し側が `model` で上書きする）

**`intmap-verifier` に「この失敗は環境要因か、本物の退行か」を訊くときは `model: "opus"` を渡す。**
これは節約の例外ではなく、節約が**成立する条件**である——`one-pass-or-a-reason.md` §2 が数える
繰り返しの原因の 1 番は「**観測器が嘘をついた**」で、実測は #R736（21 手・10分29秒）・
#R742（207 操作中 52 件）・#R768。**誤判定 1 回の再試行は、その役の全ラウンドぶんの節約より高い。**

昇格させるのは、次のどれかを訊いているとき:

- その赤は**環境要因**（改行コード・ポート衝突・並行実行・`node_modules/.bin` の shim）か、**本物の退行**か
- **緑だが実は死んでいる**のではないか（`intmap-recurring-lessons.md` の「計器が緑でも機能は死んでいる」）
- 失敗が**観測された事実**か、**観測できなかっただけ**か（`one-pass-or-a-reason.md` §5 が分けろと言う 2 つ）

単に「`npm test` を流して落ちた spec を挙げろ」なら既定のままでよい。

⚠ **迷ったら上げる。** 下げた既定は節約のためにあるので、
**節約のために判断を曇らせた瞬間に目的を失う**（`AGENTS.md` §5.0 の末尾と同じ理由）。
