...
/* ⚠⚠⚠ (#R700) WHICH LANGUAGES INTMAP STILL WRITES IN IS A POLICY, AND IT IS NOT WRITTEN HERE.
   scripts/lang-policy.mjs is the one place; this gate READS it so that restoring the nine stays the
   one edit the instruction asked for. See the gate block below for what the narrowing does and —
   more to the point — what it deliberately does NOT do. */
import { authoredLangs, carriedLangs } from './lang-policy.mjs';
import { coverageOf, floorProblems } from './i18n-floor.mjs';
...
/* ══ ⚠⚠⚠ (#R700) THE AMENDMENT, AND THE ONE THING IT MUST NOT BE ALLOWED TO MEAN ════════════════
   「9言語化という憲法は改正。いったん英語と日本語だけ」 narrows what IntMap WRITES NEXT. Every
   cell above is 100 % today for all nine (MEASURED #R700), and the cheapest way to honour the
   amendment would have been to stop asking the seven anything at all — which would let 421 keyed
   rows, 463 page strings, 7,336 call-site arguments and 6,000 inline rows per language rot out one
   deletion at a time with nothing red. That is a REDUCTION of what readers already have, and
   CONSTITUTION.md §0-3 forbids one without asking. Nobody asked for it; the user asked for the
   opposite («いつでもワン指示で戻せる» presumes there is something to come back to).

   So the seven are held to a FLOOR instead of a ceiling of 100 %:
     · a NEW string written only in en+jp lowers no count → green, which is the amendment.
     · a DELETED Korean row lowers one → red, which is §0-3.
   ⚠ AND IT FAILS IN BOTH DIRECTIONS, like PAIR_CEILING below and scripts/test-budget.mjs: a floor
   with headroom it no longer needs has stopped asserting anything (#R194). Translating more costs
   one edit here, and that edit is the record that the number moved.

   ⚠ THE COMPARISON ITSELF IS scripts/i18n-floor.mjs, NOT A BLOCK HERE. This gate costs 28.7 s
   (MEASURED #R700 — thirteen child instruments), so a regression that ran it to watch the floor
   fire would cost a minute of every `npm test`, and a regression that read this file's SOURCE for
   the words «lost» / «floor» is #R488 exactly: it pins a spelling and stays green the day the rule
   stops being called. The rule is a pure function; this file and the test both CALL it. */
const FLOOR_REL = 'tests/i18n-coverage-floor.json';
const FLOOR_FILE = join(ROOT, FLOOR_REL);
const AUTHORED = new Set(authoredLangs(ROOT));
const CARRIED = carriedLangs(ROOT);

if (process.argv.includes('--update-floor')) {
  const next = Object.fromEntries(CARRIED.map((c) => {
    const r = rows.find((x) => x.code === c);
    if (!r) throw new Error(`scripts/lang-policy.mjs carries «${c}» but the audit measured no such language`);
    return [c, coverageOf(r)];
  }));
  writeFileSync(FLOOR_FILE, `${JSON.stringify({
    '//': 'GENERATED by node scripts/i18n-audit.mjs --update-floor. (#R700) How many rows each CARRIED '
      + 'language has right now. scripts/lang-policy.mjs narrowed AUTHORING to en+jp; these counts are '
      + 'what the seven already have, and the gate fails if one drops — narrowing what is written next '
      + 'is not licence to delete what is written already (CONSTITUTION.md §0-3). It fails upward too: '
      + 'translate more and raise the number here, or the floor stops asserting anything.',
    langs: next,
  }, null, 2)}\n`);
  console.log(`✓ wrote ${FLOOR_FILE}`);
  process.exit(0);
}
...
if (wantTodo >= 0) {
  ...
  /* ⚠⚠⚠ (#R700) AND THE VERDICT HAD TO LEARN THE AMENDMENT, OR IT WOULD LIE THE OTHER WAY.
     This line said «--gate fails on «fr»» whenever any cell was short, which was the gate's exact
     predicate until CONSTITUTION.md §7. It no longer is: a carried language goes short the moment
     anyone writes an en+jp-only string, and the gate PASSES on that by design. Leaving the sentence
     would be #R403 in reverse — a line asserting a red that no run produces, which is just as
     unreadable as #R403's «nothing» printed under a list of somethings. `shortCells` still NAMES
     what is left, because a reader who wants to translate fr needs exactly that list. */
  if (short.length && AUTHORED.has(code)) console.log(`# and --gate fails on «${code}»: ${short.join(', ')}`);
  else if (short.length) console.log(`# and --gate does NOT fail on «${code}»: it is CARRIED, not authored`
    + ` (CONSTITUTION.md §7) — the gate holds it at its floor in ${FLOOR_REL}, so what is short above is`
    + ` text IntMap is no longer asked to write: ${short.join(', ')}. Translating it is welcome and costs`
    + ' one `--update-floor`; letting it SHRINK is what the floor refuses.');
...
if (process.argv.includes('--gate')) {
  const problems = [];
  /* (#R700) THE AUTHORED LANGUAGES ARE GATED EXACTLY AS EVERY LANGUAGE WAS BEFORE — 100 % of every
     surface. #R239's sentence still holds for them word for word. */
  const bad = rows.filter((r) => AUTHORED.has(r.code)).filter(shortOf).map((r) => r.code);
  if (bad.length) problems.push(`incomplete language(s): ${bad.join(', ')}`);
  /* …AND THE CARRIED ONES ARE GATED ON THE FLOOR (see the note above `FLOOR_FILE`). */
  if (CARRIED.length) {
    if (!existsSync(FLOOR_FILE)) problems.push(`${CARRIED.length} language(s) are carried rather than authored (scripts/lang-policy.mjs) and ${FLOOR_REL} does not exist, so nothing holds their ${rows.filter((r) => !AUTHORED.has(r.code)).reduce((n, r) => n + Object.values(coverageOf(r)).reduce((a, b) => a + b, 0), 0)} existing rows in place — run scripts/i18n-audit.mjs --update-floor`);
    else {
      const measured = Object.fromEntries(CARRIED
        .map((c) => [c, rows.find((x) => x.code === c)]).filter(([, r]) => r)
        .map(([c, r]) => [c, coverageOf(r)]));
      problems.push(...floorProblems({
        carried: CARRIED, have: measured,
        floor: JSON.parse(readFileSync(FLOOR_FILE, 'utf8')).langs || {}, file: FLOOR_REL,
      }));
      /* the =EN clause is the one half-measure that survives narrowing: a language whose rows were
         mass-reset to their English keys keeps every count above and has still been emptied. It
         stays here because it reads `identical`, which is not a coverage count. */
      for (const code of CARRIED) {
        const r = rows.find((x) => x.code === code);
        if (r && r.inline && r.identical > r.inline[1] / 2) problems.push(`more than half of «${code}»'s inline table is the English string again (${r.identical}/${r.inline[1]})`);
      }
    }
  }
  ...
  /* ⚠⚠⚠ (#R700) …AND THE SENTENCE HAD TO MOVE WITH THE GATE. «every language is complete» was the
     literal predicate until this round; now it is asserted of the AUTHORED languages and the rest
     are held to a floor, and a pass line that kept the old wording would be the #R372 defect again
     in the opposite direction — a claim wider than the measurement, read by every future round as
     «the translation is finished». Both halves travel in the same sentence. */
  console.log(`\n✓ i18n gate: ${[...AUTHORED].join(' + ')} complete on every MEASURED surface`
    + (CARRIED.length
      ? `; ${CARRIED.length} carried language(s) (${CARRIED.join(', ')}) are held at or above their floor`
        + ` rather than at 100 % — scripts/lang-policy.mjs narrowed AUTHORING, not what they already have`
      : '')
    + (pairs.total
