...
/* ══ (#R667) THE RING ASSEMBLER IS js/ohm-rings.js, EVALUATED — NOT COPIED HERE ═══════════════
...
(async function main() {
  ...
  if (fs.existsSync(tf)) { console.error('· tags (cached)'); tagJson = JSON.parse(fs.readFileSync(tf, 'utf8')); }
    ...
    console.error('· tags …');
    /* ══ (#R667) THE CLAIM IS THE LEVEL, AND A MISSING `boundary` TAG IS A MISSING TAG ══════════
  ...
  /* Which of them can EVER be on screen? The clock floor is `--since`, so a unit that
     ...
     ══ ⚠⚠⚠ (#R667) AN UNDATED RECORD IS NOT A PRESENT-DAY RECORD ═══════════════════════════════
  ...
  /* ══ ⚠⚠⚠ (#R667) THE CACHE IS KEYED BY THE RECORD, NOT BY ITS POSITION IN THIS RUN'S LIST ══════
  ...
  function absorb(w, el) {
    ...
    /* ══ (#R667) COLUMN 10 IS THE OHM RELATION ID, AND IT IS WHAT MAKES THE CLICK SHARP ═════════
