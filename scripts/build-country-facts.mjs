#!/usr/bin/env node
/* ============================================================================
 *  IntMap · THE FOUR FACTS THE COUNTRY CARD USED TO ASK A DEAD SERVER FOR  (#R452)
 * ----------------------------------------------------------------------------
 ...
 *  OFFLINE by tests/r452-checks.test.mjs, and that the rows actually REACH THE SCREEN is
...
/* the code js/countries-ui.js derives for a Natural Earth feature — copied deliberately, and
   tests/r452-checks.test.mjs ② compares this rule with the one the app uses. */
function neCode(p) {
...
async function build() {
  ...
  return {
    ...
    /* ⚠ THE ABSENCE IS DECLARED RATHER THAN LEFT AS AN ABSENCE — the whole point of the round.
       ...
       build dropped it. tests/r452-checks.test.mjs ⑤ holds the two apart. */
