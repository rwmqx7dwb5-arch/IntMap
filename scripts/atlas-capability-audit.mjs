/* ============================================================================
 *  IntMap · THE CAPABILITY AUDIT  (#R315)
 * ----------------------------------------------------------------------------
 ...
 *  DATA, and tests/r315-checks.test.mjs feeds each one a fixture with the defect deliberately
...
export function auditWith({ caps, docs, atlas, controls, capSrc, execSrc, stateSrc, plannerSrc, resultsSrc }) {
  ...
  /* ⑬ the nine languages reach the same capabilities */
  {
    ...
    /* the model-facing language name must be DERIVED, not a five-row literal (#R315's zh 英文 bug) */
    ...
    ['zh', 'zh-hans', 'fr', 'ko'].forEach((code) => {
      ...
      if (missing.length) bad.push(`ui.${code}.js is missing ${missing.length} #R315 result message(s), e.g. "${missing[0].slice(0, 40)}"`);
    });
