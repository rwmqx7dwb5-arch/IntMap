 ...
 *
 *  ── (#R600) THE ROW IS NOT DROPPED, THE CODE IS KEPT ────────────────────────────────────────
 ...
 *
 *  ── (#R600) …AND THE CURATED COLLISION IS A FLAG, NOT A DELETION ────────────────────────────
...
/* The languages js/newsgeo.js can actually match (see the header). `ja` is kept in its own column
   ...
   ⚠ (#R600) `en` IS DELIBERATELY NOT IN HERE AND MUST NOT BE ADDED. This list is the set of
...
async function alternateNames(keepIds) {
  ...
  /* ⚠ (#R600) 'en' is added HERE and only here — see the note on LANGS. It rides in the same map
  ...
  for (let i = 0; i <= buf.length; i++) {
    ...
    parse: {
      ...
      /* (#R600) English is read for the DISPLAY name only, and the header's rule for it is
...
/* ══ (#R600) WHAT KIND OF PLACE A FEATURE CODE IS — ANSWERED BY THE PUBLISHER, NOT BY A LIST ═══
...
async function main() {
  ...
  /* ⚠ (#R600) A CODE WITH NO PUBLISHED DESCRIPTION IS NOT QUIETLY A SETTLEMENT. If cities1000
  ...
  const admit = (r) => {
    ...
    /* ⚠ (#R600) a curated row owns this NAME, so the locator must not see this row — but the row
  ...
  const out = keep.map((r) => {
    ...
    /* ⚠ (#R600) THE DISPLAY NAME IS A DIFFERENT COLUMN FROM THE MATCHING KEY, deliberately.
