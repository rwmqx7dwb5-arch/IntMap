 ...
 *
 *  ── (#R620) THE ROW IS NOT DROPPED, THE CODE IS KEPT ────────────────────────────────────────
 ...
 *
 *  ── (#R620) …AND THE CURATED COLLISION IS A FLAG, NOT A DELETION ────────────────────────────
...
async function alternateNames(keepIds) {
  ...
  /* ⚠ (#R620) 'en' is added HERE and only here — see the note on LANGS. It rides in the same map
  ...
  for (let i = 0; i <= buf.length; i++) {
    ...
    parse: {
      ...
      /* (#R620) English is read for the DISPLAY name only, and the header's rule for it is
...
/* ══ (#R620) WHAT KIND OF PLACE A FEATURE CODE IS — ANSWERED BY THE PUBLISHER, NOT BY A LIST ═══
...
async function main() {
  ...
  /* ⚠ (#R620) A CODE WITH NO PUBLISHED DESCRIPTION IS NOT QUIETLY A SETTLEMENT. If cities1000
  ...
  const admit = (r) => {
    ...
    /* ⚠ (#R620) a curated row owns this NAME, so the locator must not see this row — but the row
  ...
  const out = keep.map((r) => {
    ...
    /* ⚠ (#R620) THE DISPLAY NAME IS A DIFFERENT COLUMN FROM THE MATCHING KEY, deliberately.
