...
/* ⚠ (#R650) THE TOTAL MOVED BY THE MEASUREMENT, NOT BY THE SHORTFALL. tests/r650.spec.js is the
...
const HISTORY = [
  ...
  ['#R753', 4882, "THE TOTAL CEILING MOVED BY THE MEASURED AMOUNT (+2 s) AND THE CORE CEILING DID NOT MOVE. "
    ...
    + "account panel that had NEVER been tested: tests/r168.spec.js says in its own header that currentUser is out of reach "
    + "because the hermetic policy blocks Supabase and credentials do not belong in a test — true of the NETWORK, and not "
    + "true of the SESSION, since supabase-js answers getSession() out of localStorage. One session object with a future "
    + "expires_at opens the real openAccountMenu(), and every request it then makes is blocked exactly as intended. "
    + "THE 1 s IS THE SPEC'S OWN MEASUREMENT, taken the way #R455 / #R620 took theirs: warm server, one worker, the "
    + "reporter's own duration for the TEST BODY, summed over the file's six tests — 594 / 1,970 / 574 ms over three consecutive runs, entered at the upper bound, 2. "
    + "⚠ IT WAS MADE CHEAP FIRST, NOT ROUNDED DOWN: the first draft was SEVEN fixed sleeps (5,798 ms of body), and every "
    + "one became a wait on the condition it stood in for (#R399's rule), which is where the other 5.2 s went. "
    + "⚠ NO SAVING WAS CLAIMED ELSEWHERE, for #R455's reason: this table is known to UNDER-charge (that round measured "
    + "smoke at 77.2 s against an entry of 8), and a table that under-charges is not one a round may take a saving out of."],
  ['#R620', 4722, "THE TOTAL CEILING MOVED BY THE MEASURED AMOUNT (+6 s) AND THE CORE CEILING DID NOT MOVE. Saying both plainly, because this file's own message is «never raise it». ⚠ THE 6 s IS THE SPEC'S OWN MEASUREMENT, taken the way #R416 / #R435 / #R494 took theirs: warm server, one worker, worker-scoped page, the reporter's own duration for the TEST BODY, summed over the file's two tests — 5,749 / 4,744 / 5,623 ms over three consecutive runs, upper bound 5,749, entered at the conservative 6. ⚠ AT 6 s AGAINST CORE_MAX_S = 1 THE FILE DOES NOT STAND IN THE GATE, and that is the price rule working rather than a demotion asked for: what gates a push for this round is tests/r620-checks.test.mjs, fourteen node checks that LOAD AND RUN js/atlas-query.js (only the gazetteer rows and the language picker are stubbed — no collaborator that decides anything, per #R552). Two mutations measured red. ⚠ THE CORE CEILING WAS NOT RAISED AND NO SAVING WAS TAKEN OUT OF THE CORPUS: the spec entered core as UNMEASURED (charged p75 = 43 s) and put the gate 0.5 min over; measuring it — not re-tiering it, not trimming another entry — is what brought the gate back, which is the direction this file allows. ⚠ AND THE SPEC WAS MADE CHEAPER FIRST, then measured: its second test originally re-ran the reported query (population + distance-to-the-ocean over 140,396 rows) three more times to compare RESULT KEYS, and a result key is an identity, not a row count — the coastline measurement it was paying for proved nothing the first test had not already proved on real data. Dropping to a population-only condition there took the file from 1.5 to 1.3 min of wall. ⚠ WHAT THE SPEC BUYS is the one claim no source-shape gate can reach: the reported defect was the SAME 34 CITIES IN TWO TABLES, and the two tables differ, so a string comparison of rendered HTML (the guard js/atlas-console.js already had) passes them both. Only running the shipped dispatch door twice with different `show` and reading the `meta.resultKey` that comes back can tell whether the engine declared what it RESOLVED — and only the real 5.29 MB gazetteer can show that Baghdad and Tehran, which were absent from every answer this engine ever gave, are in it now."],
