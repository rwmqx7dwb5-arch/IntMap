 ...
 *  own header is about.
 *
 *  ══ ⚠⚠⚠ (#R456) …AND IT WAS ASKING IT OF HALF THE APP ══════════════════════════════════════════
...
/* …and the DOM PROPERTY each one is spelled as when JavaScript assigns it instead (#R456) */
...
const EXEMPT = [
  ...
  /* ⚠ (#R456) …AND THE SAME PROPER NOUN WITH PUNCTUATION AROUND IT IS STILL A PROPER NOUN. The
  ...
  /* ⚠ (#R456) A LONE SYMBOL — one latin letter, whatever punctuation sits around it: `m` on the
  ...
  /* ⚠ (#R456) A BARE ALL-CAPS INSTRUMENT TOKEN — `PAPI` and `ILS` on the flight HUD. The header
...
/* ⚠ (#R456) ONE MORE CLASS, AND ONLY BECAUSE THE SECOND UNIVERSE CAN PRODUCE IT. A value a string
...
/* ⚠ (#R456) ONE SCANNER, TWO UNIVERSES. `index.html` is markup on disk and js/*.js builds markup
...
/* ⚠⚠⚠ (#R456) AND THE TAG DOES NOT HAVE TO FIT IN ONE STRING. A start tag this app builds is
...
/* ⚠⚠ DOES A TRANSLATION CALL REACH THIS VALUE? If one does, the string is in the system and a
   ...
   ⚠⚠⚠ (#R456) AND «A CALL» IS THE WHOLE TEST — NAMING THE LANGUAGE IS NOT ENOUGH. The first draft
...
/* ⚠⚠⚠ (#R456) THE JS PASS IS A FUNCTION OF A SOURCE, NOT OF THE REPOSITORY — and that is what lets
   ...
   defect lines as they were WRITTEN, this returns 3 — see tests/r456-checks.test.mjs. */
...
/* ── run directly: the list ──────────────────────────────────────────────────────────────────────
   ⚠ (#R456) GUARDED, because this file now EXPORTS the scanner it used to only run. An unguarded
   `process.exit(0)` at module scope kills whatever imports it — tests/r456-checks.test.mjs proves
