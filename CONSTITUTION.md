...
- `dev-notes/` … 日記形式の開発記録（時系列・根本原因の記録）。**1 回の作業＝1 ファイル**（`<日付>-<主題>.md`、
  番号で呼んでいた頃のものは `R<N>.md`）。`DEV-NOTES.md` は**固定の案内**で、新しい順の一覧は `node scripts/dev-notes.mjs --list` がその場で出す
  （`scripts/dev-notes.mjs`）。記録は**1 行も削らない**。
...
## 7. 言語 (Languages) — **2026-09-11 改正・2026-09-12 追補**

> 「9言語化という憲法は改正。いったん、英語と日本語だけやる方針に変更で。
>   いつでもワン指示で多言語体制に戻せるようにはしておくこと。」

**この §7 が言語方針の正本である。** `AGENTS.md` §3-5 はここを指すだけで、機械の正本は
`scripts/lang-policy.mjs`（コードを綴る唯一の場所。`return all;` の 1 行で 9 言語体制に戻る）。

- ⚠ **線は「どの画面に出るか」ではなく「誰がその言葉を書いたか」で引く。**
  - **出典が書いたラベル**（Wikidata・OpenHistoricalMap・上流の地名・ライセンス名）は、
    **出典が書いた全言語をそのまま運ぶ。** 取得済み・典拠つきで、運ぶ費用がゼロだから。
  - **IntMap 自身が書く文**（UI の文言・パネルの散文・応答・上流の英語説明の訳）だけが
    **en + jp**。改正が narrow したのはこちら＝**執筆**である。
- ⚠⚠⚠ **「これから書かない」は「もう書いたものを消す」ではない。**
  9 言語は今日すべて 100% である（実測 #R700: keyed 421/421・pages 463/463・positional 7,336/7,336・
  inline 6,000/6,000）。**既存の 7 言語を削るのは §0 の 3 の手続き**（提案 → 確認 → 実行）であって、
  この改正はその承認では**ない**。門（`npm run check:i18n`）は en+jp を 100%、残り 7 言語を
  **床**（`tests/i18n-coverage-floor.json`）で支える——新しい英語だけの文字列は数を下げないので通り、
  韓国語の 1 行を消すと落ちる。⚠ 床は**上向きにも落ちる**（#R194。余った余白は何も主張しない）。
- ⚠ **「9 言語」という数を文書に書き写さない。** 数の正本は `js/locales/_langs.js`（生成物）で、
  綴りは実装のコードそのまま（日本語は `jp`、繁体字は `zh`）。`npm run check:docs` が照合する。

### 2026-09-12 追補 — **凍結であり、優先順位でもある**

> 「いやIntMapの言語設定自体英語と日本語のみで、いったんは英語話者と日本語話者しか存在しない
>   ものとして扱うってはずですが。9言語体制は、廃止ではないが、完全凍結。」
> 「日本語翻訳も後回しでいい。言語の問題は後回し。実態を改善対象にしろ。」
> 「この方針は憲法に明記しろ。なんなら、英語単体でもいい。」

改正が narrow したのは**執筆の範囲**だった。追補が足すのは**優先順位**である。

- ⚠⚠⚠ **言語は、それ自体を目的とした作業の対象にしない。** 「◯◯語の被覆が低い」は、それだけでは
  着手の理由にならない。**読者に届いていないのは言葉ではなく実態か**を先に問う——地図が何を描いて
  いるか、その形が正しいか、触れるか、読めるか。**改善の対象は実態であって、実態の翻訳ではない。**
  ⚠ これは「翻訳するな」ではない。**言語の穴を見つけたら、直さずに記録して次へ行く**という意味である。
- ⚠ **いまは英語話者と日本語話者しか存在しないものとして扱う。** 残り 7 言語の被覆率は、
  設計判断の入力にしない（床が守っているので、腐りはしない）。
- ⚠ **英語単体でもよい。** 執筆を `en` だけに絞ることは**許可されている**。ただし
  「許可されている」と「今そうする」は別で、`scripts/lang-policy.mjs` の `NARROWED` を動かすのは
  **その指示があったとき**である。⚠ 絞った瞬間、門は日本語を 100% ではなく**床**で支えるようになる
  ——つまり日本語の行が 1 削除ずつ腐る側に回る。だから**絞るのは、そう言われたときだけ**。
- ⚠ **凍結は削除ではない**（上の 3 つ目の項目と同じ手続き）。9 言語の既存の行は 1 つも消さない。
  出典が書いたラベルは今までどおり全言語運ぶ——**運ぶ費用はゼロで、凍結しているのは執筆だから。**
- ⚠⚠⚠ **門は、方針が実行できることを保証しなければならない。** #R707 で実測: 改正から 1 ラウンド、
  `scripts/i18n-positional-audit.mjs` は位置引数のサイトに **5 個の引数を無条件で要求し続けていた**
  ——`authoredLangs` が en+jp に narrow されたあとも。改正どおり en+jp だけで書いた最初の文字列が
  **欠陥として報告された**。リポジトリの `short` が 0 だったので、誰も気づかないまま 1 ラウンド通った。
