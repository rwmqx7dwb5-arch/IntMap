...
- **本体は `index.html`（939行・84 KB）＋ `css/`（3本）＋ `js/`（231本・11.3 MB）＋ `src/`（10本）。**
...
- **プリセット送信文の主語は「視界」であって「中心画素が落ちる国」ではない**
  ...
  **`V`（視界）が `P`（国）／`W`（世界）より必ず強い**（重みは 12–17 対 上限 10）——街区を見ている
  ...
  ① **視界に入る国**（箱の 6×6 標本を国ポリゴンに当てる。`countryStats` の `bbox` は
  ray-cast を減らす**足切りにだけ**使い、答えには使わない）——2 か国なら国境、3 か国以上なら三国境、
...
- **両大戦の日ごとの勢力**（`js/war-fronts.js`・レイヤー行 `dl-wars`・既定 OFF）。ON の間だけ Chronos に
  ...
  **戦線の線で国の輪郭を切って導く**（`js/war-geom.js`）ので、線と面が食い違いようがない。
  ...
  滑らかに見せるための補間はしない。
  - **収録範囲**: 第一次大戦 **9 戦線 / 69 本の日付入り戦線 / 62 作戦**、第二次大戦
    **12 戦線 / 95 本 / 159 作戦**。戦線は西部・東部・イタリア・マケドニア・シナイ＝パレスチナ・
  ...
  - ⚠ **太平洋には戦線を引かない**——線が存在しなかったから。島嶼戦は「どの場所がいつ手を変えたか」
    ...
    集合そのもの**で、東経100度以西を除く 45 件以上が 1941–45 の各年に分布する。
