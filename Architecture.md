...
- `scripts/master-sync.mjs` — **原本（main worktree）が merge 後の状態かを見るツール**。
  ...
  ⚠ **`--sync` は branch を切り替えない。** 原本は「`main` の置き場」であって作業場ではなく
  （`CLAUDE.md` §6）、`main` 以外にいる場合は**何もせず報告する**。
  ⚠ **未コミットの変更を「邪魔かどうか」判定するのは git であってこのスクリプトではない。**
  早送りが触らないファイルの変更（他セッションの `.claude/launch.json` など）は素通りさせ、
  実際に上書きになる場合だけ `git merge --ff-only` 自身の理由を表示して止まる。
  `--check` も同様で、汚れは**警告として印字するが exit 0 を妨げない**——
  「遅れている」と「汚れている」は別の主張だから。
  **早送りしかしない＝冪等**なので、並行セッションが同時に走らせても結果は同じで、
  **排他ロックを必要としない**。1 回の実行がその時点で merge 済みの全セッション分を運ぶ。
