...
js/
  ...
  ocean-currents-field.js           (#R222) **流向の場の復号と間引き `IntMapCurrentField`**——`data/ocean-currents-*.bin.gz`
                                    ...
                                    海流レイヤーは2つ（`js/ocean-currents.js` の世界データ版と `js/data-layers.js` の #R208 版）
                                    あり、**どちらもこのファイル経由で同じ格子を読む**。
  ocean-currents.js                 (#R219 データ／#R220 描画) **世界の海流 `IntMapCurrents`**——**同梱の固定データを常時描画**する
  ...
  sky-model.js                      (#R202) **空そのものの色 `skyColour(sunElev, camAlt, relAz)`**——Rayleigh+Mie
...
- **UI言語は en/jp/de/ru/es の5言語＋ zh（繁體中文・beta、#R223）**。インライン文言は `L(en,jp,de,ru,es)` /
