...
- **国の状態は4つあり、主張は3つしかないので、4つ目は何も描かない。**
  ⑴ フィードが無い国＝**灰色斜線**（`fill-pattern`・`wp-alert-hatch`。`washTier` = 0）、
  ⑵ フィードがあり何も出ていない＝**灰色**（気象庁自身の `#c8c8cb`。= 1）、⑶ 発令中＝その階級の色、
  ⑷ **対応しているが、まだ読めていない／読めなかった＝何も描かない**（= −1。地の図がそのまま見える）。
  **「警報なし」と「データなし」は別。**
- ⚠⚠⚠ **斜線は「未対応」だけを意味する。** 灰色が「読んだうえで何も無い」という主張である以上、
  未読の国に灰色は塗れない——が、そこで**斜線**を塗ると、対応している国が未対応の国と**同じ見え方**になる。
  実測、レイヤーを入れて 45 秒の時点で **22 の対応国が斜線**（WMO 登録簿の輪番中 15・MeteoAlarm 5 ほか）、
  北マケドニアは**恒久的に**斜線だった（`feeds.meteoalarm.org` が `upstream_error` を返すため）。
  → `washTier` は `readState(iso)` が `ok` でなければ **−1** を返し、**塗りも斜線も出さない**。
  −1 は塗り式の既定値そのものなので、斜線（`== 0`）にも薄塗り（`> 0`）にも当たらない。
  **どの沈黙なのか**（未対応／順番待ち／取得不可）はタップした地点のカードが**言葉で**言う
  （順番待ちと取得不可は別の文言で、見本は**破線の空枠**——斜線ではない）。
...
- **凡例は「1レイヤー1枚」で、レンダラ自身の配色表から作る**（`IntMapECMWF.legend(variable)`）。
  箱の `<h4>` は**そのレイヤー自身の名前**（`data-legend-ec-<id>`）、中身はそのレイヤーの帯・目盛・説明だけ。
  ...
  **凡例の最大値が LUT と食い違うことは構造上起きない。**
  ...
  ⚠ **予報時刻は「鍵」ではなく「操作」なので、自分の箱**（`data-legend-ec-time`・
  「ECMWF 予報時刻」）に住む。ECMWF レイヤーが1つでも出ていれば出る。
...
- **風の配色は Windy 相当の独自表**（`WIND_ANCHORS`：静穏＝青紫 → 青 → 青緑 → 緑 → 黄 → 橙 → 赤 →
...
- ⚠⚠⚠ **SDK の配色表は<b>補間しない</b>ので、17 色の表は 17 本の帯になる。** 出荷されているコードで
  ...
  **凡例だけが滑らかで地図が階段**という、色と名前が食い違う状態になっていた。
  → `rampFrom(WIND_ANCHORS, 0.1)` が**同じ 17 色**を 0.1 m/s 刻みへ**再標本化**する（601 段）。
