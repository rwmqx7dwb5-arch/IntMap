...
js/
  ...
  limb-layer.js                     ⚠⚠ (#R227) **地球の縁（リム）を、このアプリ自身が描く custom レイヤー。**
                                    ...
                                    **中身**：js/sky-model.js と同一のモデルをフラグメントシェーダで行進する
                                    ...
                                    取るので、ピッチを付けてもズレない。⚠ **地球に当たる光線は描かない**
                                    （円盤の手前の空気は線形空間の合成が要る＝#R227 §5 の宿題）。
                                    ⚠⚠ **CPU ラスタライザには出さない**（ が
                                    ...
                                    ⚠ 出るのは **globe かつ視点が殻の外かつ太陽位置が既知**のときだけで、
                                    そのとき `atmosphere-blend` は 0（maplibre のパスは全レイヤーの**後**に描かれる
                                    ので、上から描くことはできない）。実測 `map._render` 中央値 4.7 ms／p90 7.9 ms
