...
js/
  ...
  terrain-water.js                  (#R176) **地形編集＋水流 `IntMapTerrainWater`**。実 DEM をブラシで盛る／削る、
                                    ...
                                    ⚠⚠⚠ (#R265) **これは定常状態（t → ∞）であって、描かれる水ではなくなった。**
                                    「経過時間に対する水の動きが、現実と乖離しすぎ。リアルなモデルにしろ。」——
                                    従来は注水のティックが**累積した総量を毎回この定常解へ解き直す**モデルで、
                                    水は **t = 0⁺ で最終地点に居た**（パネル自身が「波の到達速度は扱いません」と
                                    書いていた）。いま画面に出る水は **`js/water-dynamics.js`** が時間積分した
                                    浅水場で、この定常解は ⏭「定常状態まで」と下流トレースの入力として残る。
                                    実測（甲府盆地・5×10⁶ m³ を1回配置）: 先端は **60秒 2,276 m → 10分 2,810 m →
                                    50分 3,526 m → 1.8時間 3,914 m**、⏭ の定常状態は **5,979 m**——
