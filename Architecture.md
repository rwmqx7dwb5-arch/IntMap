...
js/
  ...
  tsunami.js                        (#R192) **津波伝播 `IntMapTsunami`**。球面スタッガード格子（Arakawa C）上の
                                    **線形長波**をリープフロッグで解く（JMA／TUNAMI-N2 と同じ沖合方程式）。水深は
                                    terrarium DEM、初期条件は **Okada 1985** の海底鉛直変位（断層寸法 Wells &
                                    Coppersmith 1994、走向は局所的な海底勾配から）。アニメーション再生・最大波高場・
                                    到達時刻・グリーンの法則による沿岸波高。格子は数十kmなので**外洋モデル**であり、
                                    最後の1 kmは `js/sims.js` の浸水モデルへ引き渡す。
...
**津波伝播（`js/tsunami.js`）。** 球面スタッガード（Arakawa C）格子上の**線形長波**をリープフロッグで解く。
水深は terrarium DEM（`demSnapshot`、**日付変更線をまたぐ領域は東西2枚**）。初期条件は **Okada 1985** の
鉛直変位（⚠ `arctan(ξη/qR)` は**主値**。`atan2` にすると Chinnery 差分が分岐の跳びを残し、断層の背後に
一定値の「沈降」が出る）、断層寸法は **Wells & Coppersmith 1994**（逆断層）、走向は**局所的な海底勾配**から。
格子 320²（携帯192²）・Δt は CFL、フレームは 1 mm 量子化の `Int16Array`。沿岸波高は**水深200 m 以深の
セルから**グリーンの法則で外挿（浅いセルは線形理論が壊れている）。最後の1 kmは従来の浸水モデル
（`js/sims.js`）へ引き渡す。Atlas: `{"type":"earthquake","tsunami":true,"hours":3-24,"maximum":bool,"play":bool}`。
