...
- **Atlas（自然言語コンソール, beta, #R42〜#R42c)** — 自然言語で指示するとAIが**JSONアクション計画**を返し、実ディスパッチャが
  ...
  国をハイライト＋一覧。**複合指示**対応（各アクションを順次await実行）。
  **全動作対応**: 個別アクションに無い操作も**汎用 `control` アクション**で実行可能 — `findControl()` が画面上の
  あらゆるボタン/チェック/ドロップダウン/スライダー/入力（関連 `<label>` も照合、閉じたパネル内も可）に名前/idでマッチし
  クリック/設定/切替する。AIにはDOM由来の**操作カタログ（約123件、名前+#id）**をシステムプロンプトで渡すため正確に指定可能。
  ウィンドウは**最小化・サイズ変更可**、×で地図上のハイライトも消去。起動: ツールバー `⌖` / 右クリック / **Ctrl・⌘+K**。
