...
js/
  ...
  sky-model.js                      (#R202) **空そのものの色 `skyColour(sunElev, camAlt, relAz)`**——Rayleigh+Mie の
                                    単散乱をマーチして返す純関数（DOM もレンダラも時計も読まない）。
  ...
  render-scale.js                   (#R202) **動いている間だけ解像度を落とす `IntMapRenderScale`**——
...
**宇宙探索（`js/space.js` ＋ `js/ephemeris.js`、#R197 → #R201 で入口を変更）。**
⚠ **(#R201) ボタンは無い。** 「宇宙を探索は、ボタンで押す形式ではなく、そのまま普段の状態から、
ズームアウトし続ければそのまま宇宙まで行き、画面も出てくる形式に。」ズームの床は**推測ではなく状態**
`camera.zoom <= camera.getMinZoom() + 0.06`。⚠ **レンダラは「できないズーム」のイベントを出さない**
（MapLibre は minZoom で黙ってクランプする）ので、「まだ引こうとしている」は**入力から**しか読めない：
ホイール／トラックパッド（`deltaY>0`、MapLibre と同じ 1 レベル≒300 単位で正規化）と、
**閉じていく2本指ピンチ**。どちらも `{passive:true}` で、`preventDefault` は一切しない
（＝床以外のズームでは挙動が1画素も変わらない）。**持続**が条件で、床で拒否されたズームアウトを
**ズームレベルで積分**し、`OVER_TRIGGER=1.6` で渡り、入力が `OVER_DECAY=900 ms` 途切れると 0 に戻る。
...
- ⚠ **月以外の衛星は入れていない**。公表要素だけでは元期の平均経度が無く、正しい半径・正しい周期の円軌道は
  「完璧に見えて木星の反対側にイオを置く」。「フェイク実装をするな」に従って、扱う天体を明記して出さない。
- **2つの尺度**: **実寸大**（1単位＝1 AU・半径も実寸——地球は 4.3×10⁻⁵ 単位で1画素未満になり、パネルが
  「いま何画素か」を表示する）と**モデル大**（軌道半径 ∝ r^0.42・天体 ∝ ∛r）。どちらも比率を明記する。
- **惑星のグローブ**: 「地球と同じ形式で。地名のラベル付き。」実表面画像（Solar System Scope, CC BY 4.0、
  NASA/JPL/USGS 由来）を貼り、その時刻の**実際の太陽方向**で照らし、**実際の IAU 極と本初子午線**で回す。
  地名は **USGS 惑星地名辞典（IAU 承認）2,772件**（`data/planet-names.json`）を公表座標に置き、裏側は球自身が隠す。
  ⚠ 冥王星だけ表面図が無い（配布されている準惑星の表面図は作者が「架空」と明記）ので**実測の色**で描き、理由を表示する。
- **検証（`tests/r197-space.test.mjs`）**: 自分自身ではなく**独立な言明**に対して測る——太陽は `js/space-sky.js` の
  USNO 系列に対し**最悪 0.63′**、各惑星の周期は `a` からと L の変化率からの2つが一致、水星・金星の最大離角は
  観測値 28.3°/47.3°、各天体の日心距離は公表の近日点・遠日点の内側、月は近地点/遠地点と**平均朔望月 29.53066 d**
  （公表 29.530589）、地球の W は独立な GMST 系列を追い、そして⚠**月の秤動**——正しい極と W₀ でなければ
  地球側正中点は±8°に収まらない（実測 8.04°/5.32°）。

**夜側と夜間光（`js/night-side.js`）。** 上記モジュール表を参照。ズームの傾斜は**スタイル式**で、
⚠ #R197 で**引いたときの暗さを 0.78 → 0.93**、影の色も `#03060f` → `#02030a` にした（不透明度だけ上げても
同じ灰色を濃く塗るだけで、残すべきものは**都市光**だから）。薄明の帯（太陽高度 0〜−12° の5枚）は変えていない。
...
**⚠ #R197 で津波は全球になった（`src/tsunami-worker.js` ＋ `js/tsunami.js` ＋ `js/bathymetry.js`）。**
領域は **−180…180 × −80…80 を 0.25°（1440 × 640）で固定**し、**経度は周期的**（列 nx−1 の東隣は列 0）。
箱が無いので端が無く、端からの反射も端への流出も無い——チリの地震が日本に**届いて、通り過ぎて、戻ってくる**。
① 解像度が**計算時間に依存しなくなった**（#R196 は到達距離から箱を切っていたので、同じ地震が 6 h では 22 km 格子・
24 h では 40 km 格子だった）。② 海底は **同梱の `data/bathymetry.png`**（0.25°・1440×720・1.26 MB・
`scripts/build-bathymetry.mjs` が terrarium z5 の 1,024 タイルから構築）で、R,G ＝そのセルの**海サンプルの平均水深**、
B ＝**海の占有率**。全セルに実測水深があるので、#R192 が測った「タイルの19%が届かず定数で計算」が構造的に起きない。
③ **コリオリ**（f = 2Ω sinφ）を追加——30時間・大洋横断のスケールでは f⁻¹ ≈ 3 h が効く。
④ ⚠ **極フィルタ**（Arakawa & Lamb 1977）: 60°より極側では緯度円に沿って cos60°/cosφ セル幅の箱フィルタを**2回**かける。
これで Δt が 60° のセルで決まる（かけないと 80° の 4.8 km セルが Δt を 1/6 にする）。近似なので**信用せず検査する**——
|η| が 500 m を超えたら絵を出さずに `diverged` を返す。⑤ **最大30時間**。⑥ モデルの構築（海底・断層・Okada）も
**すべてワーカー**に移った（主スレッドは 921,600 セルの三角関数を持たない）。⑦ フレームだけ `dec` で間引く
（デスクトップ 720×320・電話 480×214）——**解は両方とも 1440×640**で、#R193 のように電話で答えが変わることはない。

**⚠ 津波は災害シミュレータから消えた（#R197）。** 「災害シミュレータからは津波シミュレータを削除しろ」。
`js/sims.js` の `tsunami` ハザード（26 km 箱の浴槽塗り）は**削除**され、`setHazard` は知らないハザードを**拒否**する。
これを開いていた3つの経路——地震パネルの `openTsunami` のフォールバック、津波パネルの「最大波高地点の浸水域」ボタン、
Atlas の `{"type":"tsunami"}`——は**すべて伝播モデルへ向き直した**。このアプリの津波は1つだけになった。

**津波の検証（`tests/r197-checks.test.mjs`）。** 物理を**実行して**検査する: Okada は公表テストケース
（−3.5639e−2 対 −3.564e−2）、**平坦な海の伝播速度は √(gh) に対して 0.20 %**（198.5 対 198.1 m/s）、
そして**震源の東西で到達時刻が一致する**——西側の標本は日付変更線を越えないと到達しないので、これは周期性そのものの検査。

**津波の震源（`src/tsunami-worker.js`、#R197 で `js/tsunami.js` から移動）。** Okada 1985 を**テーパー付き小断層 8×3** で重ね合わせる。重みは
