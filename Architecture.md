...
- **70以上のデータレイヤー** — 気候(ケッペン/気温/降水/風/レーダー/雲/積雪/エアロゾル/一酸化炭素CO)、
  海洋(SST/海面水温偏差/海氷/海洋クロロフィル/EEZ/海底ケーブル/航空機)、
...
- **Atlas（自然言語コンソール, beta, #R42)** — 自然言語で指示するとAIが**JSONアクション計画**を返し、実ディスパッチャが
  地図を操作（レイヤー切替/移動/投影/ベース地図/比較/天気/AIブリーフ）。さらに `countryStats` 上で**実分析**：
  ランキング(top/bottom N)・比率(A/B)・**回帰残差(relate)**（例「GDP per capitaの割にHDIの低い国」＝HDIを log(GDPpc) に
  回帰した最も負の残差）→ 該当国を `countries` ソースでハイライト＋一覧。起動: ツールバー `⌖` / 右クリック / **Ctrl・⌘+K**。
- **天気ポップアップ（右クリック→「ここの天気」, #R40/#R42)** — Open-Meteo の現況＋5日予報（常に最新）。
  気温は設定の °C/°F/両方に完全対応（両方モードは日別予報も °F を併記, #R42）。ドラッグ移動・⟳更新可。
- **共有（Share this view, #R42)** — `🔗` ツールバー/右クリックから**共有パネル**を表示。位置・ズーム・投影・ベース地図・
  選択中の全レイヤー・時刻(タイムトラベル)・比較状態を **URLにすべて格納**した正規リンク（`self=1`なし）をコピー/ネイティブ共有。
