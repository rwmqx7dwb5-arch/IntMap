...
- **歴史的な政体名は、記録をまたいで 1 つの表 `data/histnames.json` が答える。**
  ...
  実測 2026-09-11: **655 名前**を尺度で決め、識別子で 283 QID、**101 の説明文**、合わせて 5,005 の訳語。
...
- **現代名の記載がない歴史地名も、出典自身の地点として描く**（`js/hist-places.js` の
  ...
  `scripts/build-hist-places.mjs` が検査・変換し、`data/hist-places.json` に **6032地点・10165件の
  年代付き名称記録**を収録する。Pleiades が CC BY 3.0 を明示し、集落型・代表点・日付付き名称を
  持ち、出典を取得した年まで続く名称の記載がないレコードが対象。**現代名の不記載は廃絶の証拠ではない。**
