...
**宇宙探索（`js/space.js` ＋ `js/ephemeris.js`、#R197）。** 「最もズームアウトしてそれよりズームできなくなった時だけ、
画面上に宇宙を探索するボタン」。ボタンの条件は**推測ではなく状態** `camera.zoom <= camera.getMinZoom() + 0.06` で、
`moveend` の比較1回以外は何も走らない。押されるまで WebGL コンテキストもテクスチャも星表も**確保しない**。
