...
- **本体は `index.html`（934行・84 KB）＋ `css/`（3本）＋ `js/`（195本・9.4 MB）＋ `src/`（8本）。**
...
### 4.4 記事 → 出来事 (Event) のクラスタリング — `js/news-cluster.js`

Atlas の `research.events`（「最近の出来事をまとめて」）は、読み込み済みの記事一覧ではなく
**出来事の一覧**を返す。1つの出来事＝同じ出来事を報じているとみられる複数の記事。
**この束ね方の実装はこの1本だけ**で、Atlas はここを `groupNewsEvents()` で呼ぶ
（`js/atlas-console.js` の `case 'events'` は窓と範囲を選び、描いて書くだけ）。

**決定論**（ネットワーク無し・乱数無し・時計も持たない——「何時間前か」は呼び出し側が渡す）。
2記事が同じ出来事とみなされるのは、次の**3つが独立に成り立つ**ときだけ:

| 条件 | 規則 |
|---|---|
| 場所 | **どちらも精密な地点**（`city` / `flashpoint`）なら **≤150 km**。片方でも**代表点**（国・地域、および解決できなかった記事の散布点・Publisher ピンの媒体HQ）なら、**同一の点であること**を要求する |
| 時刻 | 発表時刻の差 **≤48 h** |
| 文面 | 見出しトークンの Jaccard **≥0.30**。トークンは3文字以上の語からストップワードを除き、CJK は文字バイグラム（**ひらがなだけのバイグラムは文法なので除く**） |

- ⚠ **代表点は「場所」ではない。** 国の代表点に載った2記事は「同じ場所にある」のではなく
  「同じ名前で整理されている」だけなので、**距離は何の証拠にもならない**。
  同一点であっても文面の条件は満額のまま適用される。
- ⚠ **束ねる判断は `analysis.subjectLoc`（主題）を見る。** ピンの表示位置（`analysis.loc`）は
  Publisher モードで媒体HQに書き換わる**表示上の選択**であって、出来事が何であるかを変えてはならない。
- 推移的（union-find）なので、A↔C が直接は届かなくても A↔B↔C で1つの出来事になる。
...
- **会話前のプリセット送信文は「事実で選ばれる候補群」であって、差し込み式の定型文ではない**
  ...
  ③ **Chronos の位置**。選択は重み→鍵の順で**決定的**で、同じ事実なら同じ 4 つが同じ順で出る。
...
- **レイヤーを切り替えてもカメラは動かない。例外は `js/layer-home.js` の表だけ**。
  ...
  （EU members / U.S. presidential elections / Ukraine frontline）を **セッション中1回だけ**果に収める。
  ...
  果の場所は**可能な限り測る**——EU は `window.IntMapEuFC()`、ウクライナは `window.IntMapUkrFrontFC()`。
  ☠ EU は**各加盟国の最大の陸塊だけ**を囲む。域外領土を含めた外接矩形はグアドループから
  レユニオンまで伸び、画面のほとんどが海になる。
...
- ⚠ **`ui.zh-hans.js` は手で書かない**（`scripts/zh-hans.mjs` の生成物。繁体を直してから再生成する）。
  ...
  ⚠ **地名の外名もこの表にしかない**——`tw` は字体だけを変換するので、`紐西蘭`/`玻里尼西亞` のような語は
  表に書かない限り**字体だけ簡体化された台湾表記**（`纽西兰`/`玻里尼西亚`）で出る。
