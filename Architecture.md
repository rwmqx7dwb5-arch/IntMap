...
- **都市名ラベルも時計に従う**（`js/hist-cities.js` の `window.IntMapHistCities`・記録は
  `scripts/histcities/` → `data/hist-cities.json`・**4011都市／5754の歴史名**・113か国）。
...
古代）から。⚠ **9 言語完備を要求するのは手書きの行だけ**——上流にその言語の歴史名が無いことは
普通で（韓国語は導出 4,803 span 中 1 件、繁体字は 0 件）、無いものを英語で埋めれば捏造になる。
