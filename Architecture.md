...
js/
  ...
  osm-facilities.js                 (#R255/#R258) 「実測された施設」**6レイヤー** `IntMapFacilities`。#R255 で新設した
                                    ...
                                    **国別コロプレスではない点レイヤー**を1つずつ入れるためのもの
                                    ...
                                    (#R259) **さらに6つ、合計12レイヤー**——`osmair`＝空港・航空施設
...
- **レイヤー欄の分類**（`reorganizeLayerPanel()` の `GROUPS`。`GROUPS` に無い行は末尾の
  ...
  - ⚠⚠ (#R259) **「Others」と「Beta」が名指しで許可され、棚が4つ増えた**——「追加すべきと思う
...
- (#R259) **データセンター層のパネル `#dc-panel`**（`.tool-panel`）。この地図の主要層（貿易・電力構成・
...
- (#R259) **地形編集・水流**: ⑴ **粗い段（3×/9×/27×）が返す横断は「決定」であって「描画」ではない**——
