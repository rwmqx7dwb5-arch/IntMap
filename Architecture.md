...
- (#R261) **データセンター層のパネル `#dc-panel`**（`.tool-panel`）。この地図の主要層（貿易・電力構成・
  作物・海流・警報・潮汐）はどれも「今画面に何があるか」を答えるパネルを持つが、この層は凡例しか
  持たなかった。件数（収録／OSM の内訳）・**公表容量は必ず「N件／全M件」と併記**（実測 318件のうち
  公表は2件で 3.20 GW。裸の合計は総量だと誤読される）・分類別内訳（そのままフィルタ）・容量順の上位
  （クリックで飛ぶ）。⚠ **推定は1つもしない。** `IntMapDataCenters.stats()` が同じ数字を返すので、
  Atlas とテストはパネルの再計算ではなく同じ値を読む。
