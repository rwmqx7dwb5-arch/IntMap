...
- ⚠⚠⚠ **年スライダーは「年」ではなく「位置」を持つ**（`js/hist-scale.js` の `IntMapHistScale.rail`）。
  ...
  **到達範囲を伸ばすことで、すでにあった精度を奪ってはならない**ので、レールは 0〜1000 の位置を運び、
  年は位置の**区分線形関数**になっている。折れ点（1500 で 25%・1850 で 50%）は好みではなく、
...
- ⚠ **スナップショットへの丸め（`nearest`）は、1850 年以上では代替でしかない**（`js/time-borders.js`）。
  ...
  上流（aourednik/historical-basemaps）が公開しているのは **36 枚・西暦 100 年から**で、
  シリーズはそこまで広げてある。⚠ **これは一覧を手で足したのではなく、上流のディレクトリを読んで
  得たもの**（2026-09-10 実測。増えたときも同じ読み方で取り直す）。
  ⚠ 誤差は**その系列自身の粒度**になった: 1000 年より前は 100 年刻み、以降は上流が
  1279/1492/1530/1715/1783 のように細かく持つ年もあり、最大でおよそ 50 年。西暦 1〜99 年は
  最古の 1 枚（100 年）が答える。
