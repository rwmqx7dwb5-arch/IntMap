...
- **Atlas（自然言語コンソール, beta, #R42/#R42b)** — 自然言語で指示するとAIが**JSONアクション計画**を返し、実ディスパッチャが
  実行。**IntMapのほぼ全動作**を網羅（約30アクション、すべて既存コードに対応）: 移動/ズーム/投影/3D/ベース地図/方位リセット、
  レイヤー切替/グリッド/国情報、天気/AIブリーフ/比較/Isolate/ピン/計測/ウィジェット/スクショ/共有/タブ/タイムトラベル、
  テーマ/言語/単位設定、そして `countryStats` 上の**実分析**（ランキング・比率・**回帰残差relate** 例「GDP per capitaの割に
  HDIの低い国」→該当国を `countries` でハイライト）。**複合指示**にも対応（各アクションを順次await実行）。起動: ツールバー
  `⌖` / 右クリック / **Ctrl・⌘+K**。
