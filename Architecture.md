...
- **Atlas（自然言語コンソール, beta, #R42〜#R52)** — 自然言語で指示するとAIが**JSONアクション計画**を返し、実ディスパッチャが
  ...
  **プレイグラウンド(world/pandemic/quiz)**/**ニュース切替(主題/発信元/保存/翻訳)**/**ログイン/寄付/フィードバック/バグ報告**（#R52）/
  ...
  - **#R149 画像入力（ビジョン）** — Atlas入力欄に**画像をペースト/ドラッグ/添付**でき、テキストと一緒にAIへ送信（クライアントで `compressImage`→JPEG data-URL、最大4枚＝ai-proxy `MAX_IMAGES`。転送/サーバ経路は衛星比較で実証済の `input_image` を流用＝ai-proxy無変更）。画像のみ送信時は既定で「説明＋地点マッピング」を要求。
