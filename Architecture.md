...
- **凡例の大きさが変わったら積み直す——誰が変えたかを問わず、置き場も問わない。** `tileLegends()` が見つけた凡例は全部
  1 つの `ResizeObserver`（`watchLegendSize`）に登録され、大きさの変化は `js/runtime.js` の `frame`
  （鍵 `legends.reflow`）で**次のフレームに 1 回**の積み直しになる。–／▢ で開閉した・中身が遅れて
  届いた・選挙の年で行数が変わった、のどれでも同じ規則で直る。⚠ 積み直しを観測コールバックの中で
  しない（同じ深さの兄弟の大きさを変えると ResizeObserver のループ誤りになる）。⚠ 収束する——
  `tileLegends()` は値の変わらない書き込みをしないので、2 回目の呼び出しは何も変えない。
...
- **出典の行が添えるライセンスは、表示言語で「既に述べたか」を判定し、表示言語で述べる。**
  `js/reference-data.js` の `useText`（出典ページとアプリ内の出典ダイアログの両方が呼ぶ）は、行が
  `lic` を持ち、説明文と名前がまだそれを述べていないときだけ末尾に ` — <ライセンス>` を足す。
  ライセンスの**同一性は値**で、その値の言語ごとの名前は `js/locales/pages.<code>.js` の
  `licenceName[値]`（`sourceUse` と同じ文書・同じキー単位の英語への落ち方）。判定は**表示言語の名前・
  英語名・値そのもの**のどれかが既に現れていれば「述べた」で、名前は末尾の版番号を除いて比べ、
  名前の境界で止める（`CC BY` は `CC BY-NC-SA` の中では一致しない。`CC-BY-4.0` の `-4.0` は版）。
  ⚠ **`licenceName` に載るのは、出版者が名づけていない値だけ**（`Public domain` のような状態、
  出版者の名称に IntMap が付けた注記）。`CC BY 4.0`・`ODbL 1.0`・`政府標準利用規約 2.0` は出版者の
  名称なので載せず、どの言語でも綴りのまま示す——載っていないことがその答え。IntMap の言葉なので
  en + jp で書き（`CONSTITUTION.md` §7）、残り 7 言語は英語に落ちる。⚠ そのため 7 言語では、説明文が
  自国語で「パブリックドメイン」に当たる語を述べている 4 行に、英語の末尾がなお重なる（凍結中の既知の穴）。
  検査は `tests/licence-stated-once-checks.test.mjs`（全行 × 全言語。名前は語彙から引き、検査に綴りを書かない）。
