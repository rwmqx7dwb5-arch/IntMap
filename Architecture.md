...
- **歴史的な政体名は、記録をまたいで 1 つの表 `data/histnames.json` が答える。**
  ...
  実測 2026-09-14: 地図が名前を引く 2 記録で **828 名前**を尺度で決め（era 684・cshapes 144）、
  識別子で 283 QID、**101 の説明文**、合わせて 5,980 の訳語。
  ⚠ **`lanes` はこれより広い母集合を数える**——era・cshapes に加えて base lane の 30 行も含む
  表全体で、label/alias が 687・en.wikipedia のリダイレクトが 171・識別子が 283、訳語 6,124 件。
  2 つの数は別の母集合についてのもので、どちらも文書とゲートが同じ定義で照合している。
  どのストアが答えたかは文書自身の `lanes` が持つ。
  era の名前は 520 → 684 行になり、**描かれるラベルに占める割合は 22.5% → 26.4%**
  （de 12.0→14.3 / es 13.1→15.3 / fr 14.1→17.0 / jp 20.3→23.6 / ko 18.4→20.7 /
  ru 20.3→23.3 / zh 21.1→24.2%）。残りは上流の製図者だけが書いた綴りで、訳す出典がまだ無い。
