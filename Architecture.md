...
- **本体は `index.html`（988行・96 KB）＋ `css/`（3本）＋ `js/`（285本・13.5 MB）＋ `src/`（14本）。**
...
`find_capability(query)` が**レジストリの全 138 を検索**し（返るのは撤去済み 1 を除く **137** から）、**得点したものを全部** schema 付きで返し（**打ち切り無し**——説明文は 46 ブロック共有なので、能力ごとに引くと同じブロックが繰り返される。**まとめて 1 回引いて重複を落とす**：実測 67,600 → 24,519 B・1 文字も切らずに）、`run_capability(id, args)`
...
- Cesiumへ渡すベクタタイルの面はMercator座標でタイル範囲に切り、低ズームでは半球未満の部分へ分ける。穴を保持し、辺を補間して球面上の水域が陸地を覆うことを防ぐ。
...
- ⚠ **描かれる線は多角形の輪郭ではない**（`data/border-coast.js`・`scripts/build-border-coast.mjs`）。
  ...
  持たないので、その辺は何の境界でもない（実測: `hist-eras` の 8,814 本中 904 本がこれで、
...
- **歴史的な政体名は、記録をまたいで 1 つの表 `data/histnames.json` が答える。**
  ...
  実測 2026-09-11: **669 名前**を尺度で決め、識別子で 283 QID、**101 の説明文**、合わせて 5,088 の訳語。
  描かれる feature 比の被覆（改訂前 → 後）:
...
- **都市名ラベルも時計に従う**（`js/hist-cities.js` の `window.IntMapHistCities`・記録は
  `scripts/histcities/` → `data/hist-cities.json`・**6367都市／8968の歴史名**・125か国）。
...
「これから書かない」を「もう書いたものを消してよい」と読むと、今日 100% ある 421/463/7,336/6,000 行が
**1 削除ずつ無言で腐る**——それは `CONSTITUTION.md` §0 の 3 が禁じる縮小である。
`tests/i18n-coverage-floor.json` が各言語・各面の**現在の行数**を持ち、門はそれを下回ることを拒む:
