...
- **範囲人口（#R118 / #R123）** — 面積測定・半径円・**自由描画(Draw)ツール**のパネル、およびAtlas `population` アクションから、**WorldPop 100mグリッド(2020)** の実グリッド集計で「囲んだ範囲の人口」を算出（`IntMapPopArea`、リング80頂点に間引き、非同期タスクAPI）。#R123: WorldPopタスクのポーリングを~2分・バックオフ・errorステータス即中断へ延長（大面積で36sでは不足＝「読み込み中で止まる」の主因）。Drawの閉領域(loopRings)はMultiPolygonで合算。
