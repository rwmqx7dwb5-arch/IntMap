...
### #R174 補足（#R173 が壊したフライトシムの復旧 / 傾きの視点固定がズームまで飲んでいた / 立体の高度単位 / 軌跡を消すのはダブルクリック / ドローン航法）
...
**③ 「Live air traffic でズームインすると軌跡が消える」— 消しているのはダブルクリック**

ズーム倍率やリボン幅の話ではなかった。**ダブルクリックは地図のズーム操作**であり、MapLibre は自分の `dblclick` の前に
**通常の `click` を2回**配る。その click が機体ピッカーの「何もない所をクリック＝選択解除」に落ちていた。

**実測（スタブ ADS-B）**: ホイールで z11→z12.86 は選択も6本の軌跡も保持。同じ場所で**ダブルクリック1回**→
`selected: null / legs: 0`。

→ ①ダブルクリックの**2回目**（`originalEvent.detail>=2`）は無視、②「何もない所」の解除は 320ms 遅らせ、`dblclick` が
来たらキャンセル。機体に当たったクリックは従来どおり即時。ついでに、**軌跡のリボン幅は縮尺から作られるのに zoom で
作り直していなかった**（グリフだけ作り直していた）ので、`zoomend` で軌跡も作り直すようにした。
