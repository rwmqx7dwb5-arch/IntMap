...
| 部品 | ファイル | 何の正本か |
|---|---|---|
| Capability Registry | `js/atlas-capabilities.js` | **137 能力**。ID・別名（**440 綴り**＝ID＋別名の重複を除いた実測。**照合は camelCase を語に割ってから**——割らないと `myLocation` は「my location」で引けず、実測 143 綴り中 60 がどの言語からも届かなかった）・分類・副作用（`writes`＝競合キー）・生成物・危険度・確認要否・**必要な対象**・遅延モジュール・観測器・検証器 |
| 能力の説明文 | `js/atlas-catalog-text.js` | 46 ブロック。**各ブロックがどの能力を説明しているか**を持つ。`find_capability` が要求されたときだけ返す |
| 引数の schema | `js/atlas-schemas.js` | **137 能力ぶんの引数定義**。型・列挙・範囲と、`required` / `anyOf`（「地点 か 緯度経度」）|
...
`find_capability(query)` が**レジストリの全 137 を検索**し（返るのは撤去済み 1 を除く **136** から）、**得点したものを全部** schema 付きで返し（**打ち切り無し**——説明文は 46 ブロック共有なので、能力ごとに引くと同じブロックが繰り返される。**まとめて 1 回引いて重複を落とす**：実測 67,600 → 24,519 B・1 文字も切らずに）、`run_capability(id, args)`
...
Layers ▸ Tools ▸ Playground から起動。**数理は `js/pandemic-model.js`、地図と HUD は
`js/playground.js`** で、この境界は動かさない——`playground.js` は区画（S/E/I/R/V/D）に対して
算術をしてはならない。

- **モデル**: 国ごとに 1 組の区画を持つ**確率的 SEIR メタ個体群**。潜伏と感染期はそれぞれ 2 段の
  Erlang（指数分布 1 段だと滞在時間の裾が現実と合わない）。遷移は `rate × 人数` ではなく
  **ハザード `1 − e^(−dt/τ)`** なので、`τ = 0` でも 1 日に人口以上が動くことはない。
  期待値が 30 人未満の遷移は Poisson で引くので、**小さな流行はひとりでに消えることがある**。
- **乱数は種を取る**（mulberry32）。同じ種・同じ設定なら**同じ流行**が再現する。詳細設定で種を
  ...
  無いことまで測る）。
- **人口は保存する。** `S+E+I+R+V+D` は各国の初期人口に等しく、`invariant()` がそれを毎 step
  検査できる。国際的な再流入も**必ず S から取る**。
...
- **シナリオは 2 つ**。「未知の病原体」は誰も免疫を持たない世界、「現在の世界」はその病気が
  2026 年に実際に持っている**初期免疫・ワクチン・治療法**から始める（麻疹の MCV1 は世界の子どもの
  84%）。⚠ **免疫ゼロの世界を実在の病名で呼ばない**ためにこの区別がある。
...
- **国際伝播は距離と発展度の代理指標による重力型の近似**であって、**実際の航空路線でも旅客数でも
  ない。** UI もそう名乗る（「予測ではありません」を常時表示）。実路線・人口重みづけ・隣接は未実装。
...
- 地図の点は**赤＝感染性・橙＝潜伏中**で、HUD が両方の人数と**1 点が何人ぶんか**を明示する。
  点は実在の都市に置き、**ゆらぎを加えたあとも国の多角形の中にあるか検査する**
