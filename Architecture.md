...
js/
  ...
  render-scale.js                   ⚠⚠ (#R227) **武装が `idle` だけだったので、いちばん重い時間帯に効かなかった。**
                                    MapLibre の `idle` は「保留タイルが無く描くものも無い」ときにしか来ず、
                                    #R226 の実測では起動から**約35秒** `armed:false`。#R202 自身の測定は
                                    「遅いのは最初のジェスチャで桁違い」（sweep1 15.9 fps 対 sweep3 39.2 fps）
                                    ＝助けが要る唯一の窓に助けが来ていなかった。→ **扉を3つに**：`load`（＋rAF2回
                                    ＋猶予 900 ms）／**最初の実ジェスチャ**（`dragstart`/`zoomstart`/…）／`idle`。
                                    ⚠ #R202 の安全装置は一つも外していない——`canDraw()` を通らないと武装せず、
                                    比の適用はレンダラのイベントの外へ `setTimeout` で逃がしたまま
                                    （DPR3・390×844 で GL がハードクラッシュしたのはそこ）。`state().armedBy` で
                                    どの扉から入ったか分かる。
                                    (#R202／#R221 で正した) **動いている間だけ解像度を落とす `IntMapRenderScale`**——
