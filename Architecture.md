...
### #R204 補足（門の資格は「値段」／真っ暗はスプラッシュ／JPEGの復号はワーカーへ／入れ子は緯度帯でよかった）
...
**「起動時に地図が真っ暗」の正体は地図ではなくスプラッシュだった。** #R203 は**地図の画素**を測って正常と判断し、
夜側だと結論した（それは正しかった）。今回はタイルホストを**1リクエスト1.8秒**に落として測り直した——
これが「場合がある」の実体（冷たい/遅い回線）。**地図は 1.0 秒で [81,98,110] に描けている**のに、
`#boot-splash` が `var(--bg-color)`（ダークではほぼ黒）で**最大14秒**（`js/app-body.js` の 28×500 ms）覆っていた。
→ 覆いを**独立要素 `.boot-back`** に分け、アプリが最初のフレームを描いた時点で `__imBoot.live()` が
**不透明 → 0.5 のスクリム**にする。**ロゴ・プログレスバー・文言はそのまま**で、`done()` の条件は #R190 の
契約のまま一切早まらない（#R186「完全に準備完了なるまで」も #R190「終了後も読み込みが終わっていない」も維持）。
実測 `style` 8,102 ms → `live` 9,491 ms。
