...
- **気象・災害警報は、その機関が発令した単位で塗る**（13フィード・47か国。§7.1）。
...
- **パネルは「いま何が起きているか」から始まる。** 国 × 災害でまとめた上位を〈色・国・災害・階級・区域数〉で
  並べ、取得先の一覧（`Source status`）とその下の `Diagnostics`（置けなかった区域）は畳んである。
...
- **↺ 再生**は時計・水・各水源の供給量を 0 に戻し、**同じ地形と同じ水源のまま** t=0 から流す。
  ⚠ 連続水源の `m3` は「これまでに供給した量」なので**それも 0 に戻す**（戻さないと1時間分を1ステップで注ぐ）。
  ⚠ `editDirty()` を通すこと。
