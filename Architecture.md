...
- **本体は `index.html`（988行・96 KB）＋ `css/`（3本）＋ `js/`（284本・13.5 MB）＋ `src/`（14本）。**
...
| 部品 | ファイル | 何の正本か |
|---|---|---|
| Capability Registry | `js/atlas-capabilities.js` | **138 能力**。ID・別名（**440 綴り**＝ID＋別名の重複を除いた実測。**照合は camelCase を語に割ってから**——割らないと `myLocation` は「my location」で引けず、実測 143 綴り中 60 がどの言語からも届かなかった）・分類・副作用（`writes`＝競合キー）・生成物・危険度・確認要否・**必要な対象**・遅延モジュール・観測器・検証器 |
...
| 引数の schema | `js/atlas-schemas.js` | **138 能力ぶんの引数定義**。型・列挙・範囲と、`required` / `anyOf`（「地点 か 緯度経度」）|
...
`find_capability(query)` が**レジストリの全 138 を検索**し（返るのは撤去済み 137 を除く **136** から）、**得点したものを全部** schema 付きで返し（**打ち切り無し**——説明文は 46 ブロック共有なので、能力ごとに引くと同じブロックが繰り返される。**まとめて 1 回引いて重複を落とす**：実測 67,600 → 24,519 B・1 文字も切らずに）、`run_capability(id, args)`
...
**ファイル台帳の正本は [`docs/FILES.md`](docs/FILES.md)。** `js/` だけで 284 本あり、1行説明を
...
- **下限は西暦 1 年**（`IntMapTime.min`）。⚠ **この数を書き写さない**——スライダーの `min`・入力の
...
- ⚠ **スナップショットへの丸め（`nearest`）は、1689 年以上では代替でしかない**（`js/time-borders.js`）。
  ...
  **代替として残してある**（束が読めなかったときだけ動く）。
  ...
  ⚠ **名前の無い上流ポリゴンは描くがラベルを出さない**（6,892 件）。深い枚ではそれが named より
  面積が大きく（bc123000 で 18,345 deg² 対 3,210）、捨てれば地図の大半が消え、名前を付ければ捏造。
- **その政体名の 9 言語は `data/histeras-names.json`（Wikidata・CC0 1.0）が持つ。**
  上流は 1 つの `NAME` 欄しか持たないので、束の名前は**英語 1 つ**である
  （10,212 feature に**3,028 の綴り**——訳す単位は feature ではなく名前）。この表はその綴りを鍵に
  **英語以外の 8 言語**を運び、`js/time-borders.js` の `erFC` が `_i18n` として feature に載せる。
  ⚠ **その先の判断は増えていない**——`tagSame` は 1850–1885 の記録に対して既に
  `_i18n[lg]` → `_eraLocName` → 上流の英語、の順で読む。この束が同じ列に並んだだけ。
  ⚠ **採否は綴りでは決めない。** 候補は英語のラベルと別名に**一致する Wikidata 項目すべて**で、
  順位も先頭採用も無い（`scripts/histeras/harvest.mjs`）。採るのは、**座標を述べているなら
  その座標が地図の描く形の中にあり**、**存続期間を述べているならそれがその名前の出る枚に届く**
  もの（`scripts/histeras/match.mjs`。期間の許容幅は**その辺りのスナップショット間隔**から導く
  ——1492 年の周りは数十年、紀元前は十万年単位）。同じ綴りに複数が答えて決め手が無ければ**採らない**。
  ⚠ **書いていない言語は書かない**（`a` が「誰が書いたか」のビットマスク）。実測 2026-09-11:
  **513 名前・2,766 の訳語**。手書きの表（`_ERA_LOC` ほか）が答える名前とは**重ならない**
  ——重なれば同じ判断が 2 か所になる。両者を合わせた被覆は、描かれる feature 比で
  jp 11.8%→31.4% / zh 4.6%→24.8%。⚠ **残りが英語なのは埋め忘れではない**——
  上流が名前ではなく英語の説明文（`Australian aboriginal hunter-gatherers` など）を書いている
  ぶんは、どの言語のどの出典にも存在しない。
...
- **都市名ラベルも時計に従う**（`js/hist-cities.js` の `window.IntMapHistCities`・記録は
  `scripts/histcities/` → `data/hist-cities.json`・**6222都市／8784の歴史名**・125か国）。
...
同一性の規則が要る**——250 m 以内のノードは同じ場所として束ねる。この規則は使う前に OHM 自身の
