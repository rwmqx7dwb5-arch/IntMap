...
- **100本以上のデータレイヤー**。`js/data-layers.js` の `GROUPS` が **18 の棚に 135 行**を配り、
...
- **気象・災害警報は、その機関が発令した単位で塗る**（11フィード・45か国＋GDACS。§7.1）。
...
tests/
  ...
  r<n>-checks.test.mjs            ラウンドごとに追加された Node の回帰検査（120本）
  *.spec.js                       ブラウザ回帰（67本）
.github/workflows/
...
**その機関が発令した単位で塗る。国全体の薄塗りは「位置を特定できなかった区域」の意味しか持たない**
（`js/world-packs.js`）。
...
| ドイツ | DWD `maps.dwd.de` WFS `Warnungen_Landkreise`（ACAO *）＋ `warnings.json`（州名） | **郡 (Landkreis) ポリゴン**・州でまとめる |
| ノルウェー | MET Norway `api.met.no/weatherapi/metalerts/2.0/current.json`（ACAO *） | 警報ごとのポリゴン |
| ヨーロッパ35機関 | MeteoAlarm（`alerts-relay` が要約）＋ Eurostat NUTS 2/3（60M・ACAO *） | CAP の `<polygon>`。無ければ CAP が名乗る地域名に対応する NUTS 区域 |
| 中国 | CMA（`alerts-relay`）＋ Natural Earth 50m admin-1 | **省**（警報 id の GB/T 2260 区画コード） |
| オーストラリア | BoM（直接）＋ 同上 | **州** |
| フィリピン | PAGASA `publicalert.pagasa.dost.gov.ph`（Atom＋CAP・`alerts-relay`） | **州**（洪水情報の CAP が州ごとの `<polygon>` を持つ） |
| 香港 | HKO（直接） | 香港全域＝発令単位そのもの |
| その他 | GDACS | 事象の点＋国全体の薄塗り（**別の尺度・別の色**） |

自前フィードは **11本**（上表の 10 か国 ＋ MeteoAlarm）で、MeteoAlarm が EUMETNET の残り 35 か国を運ぶ。

- **境界データの費用は測ってある**（どれも1セッション1回・レイヤーがオンで、かつその国に警報があるときだけ
  取りに行く）: Natural Earth 50m admin-1 **657 KB**（中国・豪）、Eurostat NUTS 2＋3 の 60M
...
- `drawnISO` は**実際にソースに載った地物から毎回作り直す**（`publish()`）。「自分の単位を描いている国」は
  表ではなく**測定**なので、フィードが図形を出し始めた日に自動で塗らなくなる。
  ⚠ **手書きの対象一覧を作らない**——その一覧に国が増える日に嘘になる。
- `agencyTier(iso)` は「**その国の区域のうち、地図上に位置を特定できなかったもの**の最悪階級」を返す。
  「その国のどこかにある最悪の階級」ではない（それだと1つの郡の特別警報で国全体が紫になる）。
  香港だけは全域が発令単位なので、その階級がそのまま入る。
- `PLACED[iso] = [置けた数, 機関が発表した数]`。差はパネルが必ず印字する（**黙って落とさない**）。
- 名前の突き合わせは**両側に同じ正規化**（`_norm`：アクセント畳み・記号除去・単位語の除去）を通し、
  `_alias()` が `|` `/` と括弧つきの別綴りを索引に入れる。`lookupUnit()` は完全一致の次に
  **語の接頭辞・接尾辞**（4文字以上）を試す。⚠ **片側だけの正規化はコイン投げ**になる。
...
- ⚠ **2つのスケールは1色も共有してはならない**（`tests/r270-checks.test.mjs` が検査する）。
- ⚠ **鍵は、それが鍵である対象から色を取る**——`keyRows(col,name)` はパレットと名前を一緒に受け取る。
  命名関数だけ差し替えられて色が固定される形は書かない。
...
- ⚠ **全フィードが自分の時計を持つ。** `seenAt(feed, iso)` が**そのペイロードの一番新しい時刻**を記録し、
  パネルが機関ごとに年齢を印字する。**年齢は常に印字する。** 閾値は「最近か」ではなく
  「**まる1週間何も出していないか**」（33.5時間前の多日間警報は現行である）。
...
- ⚠ **上流の「悪い日」より短い制限時間は、生きたフィードをランダムに落とす。** 上流の期限は 45 秒＋1回の
  ...
  `busy` は自分しか守らない）。
- ⚠ ライブ取得は全て `cache:'no-store'`（付けないと60秒タイマがキャッシュを読み直す）。
...
- タップは `grouped()` の3段（admin-1 → sub → 市区町村）。同じ災害の組合せを持つ市区町村は1行にまとめる
  （**markup にも予算がある**）。
- ⚠ ロシアは機械可読な公開フィードが無く GDACS だけ。**凡例にそう書く**（空の地図に「平穏」を含意させない）。

### 7.2 レイヤー欄の分類

`js/data-layers.js` の `GROUPS` が **18 の棚に 135 行**を配る。`GROUPS` に無い行は末尾の
...
| `lyrGrpMaritime` | Ocean & shipping / 海洋・船舶 |
| `lyrGrpTerrain` | Terrain & land cover / 地形 |
| `lyrGrpNature` | Nature & land cover / 自然・土地被覆 |
| `lyrGrpDemo` | Population & economy / 人口・経済 |
| `lyrGrpHazard` | Hazards / 災害 |
...
- ⚠ **`✕` のような記号は文字ではなく幾何で描く**（`window.IntMapClearGlyph()` — `js/map-ui.js` に1つだけ）。
  U+2715 はこのアプリが指定するどの family も持たない。位置は**それが属する入力欄の矩形**から決める
  （`top:50%` は位置指定の祖先に解決されるため、包み箱と入力欄の高さが違うとずれる）。
...
- **`cont` / `rate` は水源自身の属性**（最後に置いた水源だけを太らせない）。継続の水源は緑＋輪、
  1回きりは青い点。▶ は継続の水源が無ければ無効で、理由を title に書く。
...
- **パネルは1本の列。** `.tw-body` は `scrollbar-gutter:stable`、スクロールバーの幅は `_squareColumn()` が
  **実測**して `.tw-head` / `.tw-foot` の `padding-right` にインラインで書く
  （⚠ **インラインの padding にスタイルシートは勝てない**）。行は 44 px 1種。
