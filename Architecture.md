...
- **時刻はマスタークロック `window.IntMapTime` 1本**。⚠ **2つ目の時計を作らない。**
  ⚠ ただし**気象モデルの予報時刻はこの時計ではない**——`window.IntMapECMWF` が持つ軸で、
  各気象レイヤーの凡例が自分の選択 UI を持つ（`docs/MAP-LAYERS.md` §7.10）。両方向とも結線されて
  いないので、予報を1時間動かしてもニュース・国境・昼夜境界は動かない。Chronos の「予報」タブは
  その軸を**直接**動かす第2の見え方で、アプリ共通の瞬間には触れない。
...
- **パネルを閉じても経路は消えない。** 経路を消すのはパネル内の「経路を消去」と Atlas の
  「経路を消して」だけで、どちらも `IntMapRouting.clear()` を通る。
