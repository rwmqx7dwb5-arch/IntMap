...
- **Atlas（自然言語コンソール, beta, #R42〜#R52)** — 自然言語で指示するとAIが**JSONアクション計画**を返し、実ディスパッチャが
  ...
  **レイヤーカタログ（約129件）＋操作カタログ（約140件、名前+#id）＋モジュールカタログ（約20件）**をシステムプロンプトで渡すため正確に指定可能。
  ...
  - **#R83 バッチ: ワークスペース自動タイル・Atlas経路/SV/各種シミュ・比較着色・プレビュー・自動ON整理**（詳細は DEV-NOTES R83）:
    ...
    カメラ修正: 機首上げ→視点も上（符号修正）、ロールは実バンクに一致（±78°クランプ撤廃で背面表示可）、コックピットは機体近傍中心。
    地形は`_terrRead`でDEM未読込を判定（0m誤判定の幻の山/墜落を解消）。機体差5種（セスナ172は実C172係数、他はスケール）。
    UI+キー(F/G/Space/C/V/P/R・1-5)、dispatch `flightSim`。**未実装（別フェーズ）**: 専用3D描画・高精度DEM/滑走路/ILS・気象風・
    機体システム(FBW/FADEC等)・ATC/AI機・音響・故障・実測値検証＝MapLibreに自由カメラAPIが無く描画は当面MapLibre）。⑫**レイヤープレビュー**: `setView` を
