...
- **本体は `index.html`（988行・96 KB）＋ `css/`（3本）＋ `js/`（275本・13.0 MB）＋ `src/`（14本）。**
...
### 7.3d 波（海況） — `js/waves.js`

**実装の正本は [`docs/MAP-LAYERS.md`](docs/MAP-LAYERS.md) §7.14**（配色の由来と実測、補間、粒子、
凡例、描けない条件）。ここに残すのは契約だけ:

- **有義波高の色の場＋平均波向きの粒子**を1レイヤーで描く（行 `dl-waves`・地図の layer `im-waves`）。
  **本体は遅延モジュール**（`IntMapLazy.need('waves')`）で、eager なのはレイヤー欄の行だけ。
- **データは既存の `.om` 経路**（§7.3b）。新しい配信元も Edge Function も無く、SDK にも何も足していない。
  提供モデルは `js/wx-models.js` が `roles:['wave']` で宣言し、**凡例のピッカーはレジストリから発見する**
  （モジュールにもレイヤーにもモデル id を書かない）。⚠ **波モデルは既存レイヤーの代替ではない**
  ——波の変数しか持たないので、`roles` が気温などのピッカーから締め出す。
- ⚠ **ラスタは SDK のタイルラスタライザを通らない。** SDK の格子補間は全経路 2×2 バイリニアで、
  ラスタライザは Blob URL Worker の中にあり差し替えられない。`js/waves-gl.js` が MapLibre の
  custom layer として 4×4 バイキュービックで描く。**データの読みは今までどおり `js/wx-ecmwf.js`。**
- **平面と球（globe）の両方で描く**——ラスタは MapLibre 自身の `projectTile` プレリュード経由、
  粒子は光線と単位球の交差で逆投影する。⚠ ここは Windy に**合わせていない**唯一の点（向こうは
  `globeNotSupported`）。理由と実測は [`docs/MAP-LAYERS.md`](docs/MAP-LAYERS.md) §7.14。
- ⚠ **配色の写しは `js/waves-palette.js` 1本だけ**（windy.com の波スケールに準拠。出典は
  `js/reference-data.js`、凡例にもそう出る）。⚠ **Windy のタイルもデータも使っていない**——
  使ったのは配色の定義だけで、値は Open-Meteo 経由の ECMWF / NOAA。
- ☠ **利用者に見せる文言は `displayed` からしか作らない**（§7.3b と同じ規約）。
