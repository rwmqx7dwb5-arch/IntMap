...
| ヨーロッパ35機関 | MeteoAlarm（`alerts-relay` が要約） | CAP の `<polygon>`、無ければ **Eurostat NUTS 2/3（20M）** |
...
- **国の3状態は3つの見え方を持つ。** ⑴ フィードが無い国＝**灰色斜線**（`fill-pattern`・`wp-alert-hatch`）、
  ...
  **「警報なし」と「データなし」は別。**
- ⚠ **灰色は「読んだうえで何も無い」という主張なので、読む前には塗らない。** MeteoAlarm も WMO 登録簿も
  **輪番で読む**ので、対応国でもまだ読めていない瞬間がある。その間は**斜線**（＝この地図は何も述べていない）
  で、タップすると「未取得」と**言葉で**言う。`readState(iso)` が `none` / `loading` / `ok` / `error` を返し、
  `washTier` はそれを見る。パネルの `unread` は**0 に落ちなければならない数**である。
...
- **ツール選択は上部にピン留めされる。** `.tw-tools` は `.tw-body` の**兄弟**（`.tw-foot` と同じ作り）で、
  head → tools → body → foot の順。⚠ **`position:sticky` はスクローラの中では効かない**——ピン留めは
  CSS ではなく **DOM の親子関係**である（不均衡な `</div>` 1個でピン留めが黙って失われる）。
