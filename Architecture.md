  ...
  **レイヤーカタログ（約129件）＋操作カタログ（約140件、名前+#id）＋モジュールカタログ（約20件）**をシステムプロンプトで渡すため正確に指定可能。
  ...
  - **#R94 タイムマシン＝時空カーネル `window.IntMapTime`（形骸化していた時刻スライダーを IntMap 全体の時空OSに）**（詳細は DEV-NOTES R94）:
    ...
    **国all×年で逐次取得**（並列だとWBがIP単位で弾くため直列）し `countryStats` に**オーバーレイ**→国タブ・コロプレス・ホバー・国カードが**その年の世界**を表示、
    「Now」で現在へ復元。誠実性: WB年次は**1960年始まり**（それ以前は最新値＋明示バナー）、年次系列の無い指標には誤った年を付さない（#R270 以降 HDI は UNDP の 1990–2022 を持つので**年で変わる**。民主主義指数(EIU) は依然として公開系列が無く、版を明示するだけ）。
...
CLAUDE.md                       (#R257) Claude Code が**毎セッション自動で読む**恒久指示（作業の進め方・
                                ...
                                （このリポジトリは public）。(#R260) §11 に**作業終了処理**を追加——
...
js/
  ...
  world-packs.js                    (#R211) 世界データ層（貿易・エネルギー・警報・潮汐・作物）。160 KB。
                                    (#R273) 警報は**13フィード＋MeteoAlarm 35機関**（+台湾 CWA・
                                    +ニュージーランド MetService／どちらも relay の CAP索引リーダ経由）。
                                    **GDACS は撤廃**。日本は**市町村**、未対応国は**灰色斜線**、
                                    配色は**各国公式 / IntMap換算**の2モード、更新は30秒。
                                    (#R271) 警報は**11フィード**（+DWD 独・MET Norway 諾／どちらも直接・
                                    ポリゴン付き）。描くのは**発令単位**——日本は気象庁自身の class10 幾何、
                                    欧州は CAP のポリゴンか NUTS、中国は省、豪は州。国全体の薄塗りは
                                    「位置を特定できなかった区域」の意味だけになった。
                                    (#R268) 警報は9フィード（+BoM 豪 / INMET 伯・ポリゴン付き / HKO 香港）、
                                    欧州37機関は毎tick数か国ずつ取り切る、ライブ取得は全て `cache:'no-store'`、
                                    タップは `grouped()` の3段（admin-1 → sub → 市区町村）。
  precip-annual.js                  (#R266) 年降水量。CHELSA V2.1 bio12 の 1 km 平年値（8192² メルカトル、
                                    ...
                                    ⚠⚠⚠ (#R258) **フローは1本の矢印**。⑴ `wp-trade-arrow`（110 px ごとに矢頭を
                                    撒く symbol 層）は**削除**——「誰が線に複数矢印つけろって言ってんねん」。
                                    ⑵ トグルは**矢印一式**（軸 `wp-trade-arc`・頭 `wp-trade-tip`・相手国名）を
                                    切る＝`setVis(LYR,on&&arrows)`。矢頭だけ消すと方向の無いフローの絵になる。
                                    ⑶ 「後付けで浮いている」の実体は**暗い縁取り**・`0.98` 対 `0.78` の不透明度・
                                    **軸が頭の下を同じ終点まで走っていること**の3つ。いまは同じ色・不透明度 1 で、
                                    **軸は頭の base で終わる**（`trimEnd`／`line-cap:'butt'`／`icon-anchor:'top'`）。
                                    ⚠ 切る長さは**画素**なので**レンダラの投影に訊く**（`GE().coords.project`）。
                                    メルカトルのメートルは画面中心でしか合わない——実測 z4・globe で中心は
                                    4,892.9 対 4,892.0 m/px と一致するのに、45.5 px の頭に対し隙間は 10.1 px
                                    （USA）だった。弧は必ず中心から離れた所で終わる。`moveend` で作り直す。
  ...
  seismic-site.js                   (#R263) **場址項は周波数の関数である `window.IntMapSiteAmp`**。従来の
                                    `√(ρ_src·β_src/(ρ·Vs30))` は 1/4 波長増幅を**深さ 30 m で 1 点評価した値**で、
                                    軟弱地盤で 1.5 Hz・岩盤で 6.3 Hz の答えを**全周波数**に当てていた。ここでは
                                    上位 30 m を Vs30、その下を CRUST1.0 の実層構造（両者の間は log-log の**べき乗**
                                    補間＝新しい定数を1つも導入しない）とした速度柱から `A(f)` を積分する。
                                    ⚠ **高周波の極限は従来値と完全に一致する**（30 m 層が定数 Vs30 なので、1/4 波長が
                                    30 m 以下の全 f で `β̄=Vs30`）。変わるのは**その下の周波数だけ**＝盆地効果。
                                    ⚠ 速度反転は作らない（Vs30 を下回らないようクランプ）。純粋な算術（DOM・
                                    レンダラ・fetch・アプリ状態なし）で Node から検証できる。
  seismic-subfault.js               (#R263) **有限断層を subfault に切る `window.IntMapSubfault`**。k⁻² すべり
                                    （Herrero & Bernard 1994）／**総モーメント厳密保存**（Σ μAD = M₀、実測 1e−16）／
                                    破壊時刻 |r−r_hypo|/Vr（Vr=0.75β）／ライズタイム τ=2.03e−9·M₀^(1/3)
                                    （Somerville et al. 1999）。粗さの唯一の自由度 σ=0.648 は
                                    **公表統計から導いた**（平均すべりの1.5倍以上の面積＝22 %）。位相は断層形状を
                                    種にした決定的生成なので**同じ断層なら常に同じすべり分布**。純粋な算術。
  earth-structure.js                (#R263) **ここはどういう場所か `window.IntMapEarth`**。同梱の3つの全球データ
                                    （`data/crust1.bin.gz`＝CRUST1.0・`data/slab2.bin.gz`＝Slab2・
                                    `data/tectonics.bin.gz`＝PB2002）を読み、震源の**テクトニクス区分**を自動判定する
                                    （interface／intraslab／active-crustal／stable-continental）。判定則は
                                    Atkinson & Boore (2003) のもので、彼らが深さ 50 km で代用した部分を
                                    **Slab2 の実面**に置き換えてある。区分ごとに**公表されたパラメータ一式**を返す
                                    （安定大陸＝AB06 の ENA モデル一式：Δσ 14 MPa・κ 0.005 s・Q=893·f^0.32（下限1000）・
                                    幾何減衰 R⁻¹·³/R⁺⁰·²/R⁻⁰·⁵）。⚠ 幾何減衰の折れ点は**その土地の Moho 深さ**に
                                    比例させる（各公表値はその基準地殻厚で厳密に再現される）。⚠ **ACTIVE_KM=150 km
                                    だけは出典のない閾値**でありファイル冒頭に明記してある。
                                    ⚠ スラブ内地震に固有の応力降下量は**与えていない**（全球の公表値が1つに定まらない
                                    ため。特定の地震に合わせて係数を作ることは禁止されている）。
  ...
  map-ui.js                         (#R166) 地図まわりのUI 8本。レイヤーレジストリ `IntMapLayers`・右サイドバー
                                    ...
                                    ⚠ **誰が前にいるか**（#R253–#R258）。開いたサイドバーは 2600、いま触っている
                                    パネルは `.im-front` で 2650（`panelOf()`＝**最初の position 付き祖先**を
                                    レイアウトに訊く。地図のシェルは名前で除外）。「なんらかの操作」は
                                    pointerdown・**wheel**・**focusin**・**keydown**（#R258）——受動的な信号は
                                    上げるだけで降ろさない。⚠ (#R258) `position:relative/sticky` でも
                                    **自分の z-index を持つ**パネルは panel と認める（従来は `panelOf` が null を
                                    返し、その pointerdown が**使っているパネルを降格させていた**）。
                                    ⚠⚠⚠ (#R258) **帯より上に自分の z-index を持つパネルは、この機構では覆えない**——
                                    そしてそれは CSS 1枚ではなく**機構**だった。#R47 のウィンドウ・マネージャ
                                    （`js/window-manager.js` の `bringToFront`）が、触られた浮遊ウィンドウ全部に
                                    **4300 から始まる `z-index` をインラインで**書いていた（`makeDraggable` が
                                    登録するので**ドラッグ可能なパネルは全部**対象）。`.im-front` は `!important`
                                    なので触っている間だけ勝ち、**サイドバーをクリックしてクラスが外れた瞬間に
                                    インラインの 4301 が効いて窓が前に戻る**（実測: `#compare-window` 4301 対
                                    サイドバー 2600）。#R47 が頼まれたのは**窓どうしの**前後＝相対順序なので、
                                    カウンタは**カードの帯 2200 から始まりサイドバーの直下 2599 で止まる**
                                    （`WIN_Z_BASE`/`WIN_Z_CAP`）。`#compare-window` の CSS 側 4000 も 2200 へ。
                                    (#R258/#R261) ツール欄 `TOOLS`＝**レイヤーでないシミュレーション13個**
                                    （地震／津波／地形・水流／放射性プルーム／見通し線／到達圏／日照・影／星空、
                                    (#R261) ＋ドローン飛行計画／浸水・津波ハザード／公共交通の到達圏／
                                    電波到達範囲／地球リプレイ）。それぞれ `IntMapOS` のアクション。
                                    `registerSimTools()` は `toolsBlock()` から呼ぶ——モジュール生成時に
                                    `window.IntMapOS` はまだ存在しない。
                                    ⚠ (#R261) 後の5つは **`js/atlas-console.js` からしか呼ばれていなかった**——
                                    ボタンもメニューも右クリックも無い。#R258 は**右クリックメニューにあるもの**を
                                    集めたので、その1段外側が丸ごと残っていた。`⛰ Slope / aspect` は
                                    **レイヤー**なので入れていない。
                                    (**#R264**) **ツールのカードはタイルカードと同じ振る舞いになった**——
                                    `.lst-tools` は `display:flex` の縦並びで**間隔はタイル格子と同じ 8px**
                                    （実測: 修正前は 0,0,0 px）、`.lst-toolrow.on` は `.lst-tile.on` と**同一の宣言**
                                    （アクセント枠＋1px リング。`:hover` と**同じ詳細度**なので `:hover` の**後**に
                                    書く）。**開いているツールの行が光り、もう一度押すと閉じる。**
                                    ⚠ タイルはチェックボックスを持つがツールカードは何も持たないので、状態は
                                    **モジュールに訊く**——各行に `mod:'IntMapX'` を1つ書き、`_toolOn()` が
                                    `isOpen()`（無ければ `state().open`）を、`_toolOff()` が `close()` を呼ぶ。
                                    **キャッシュしない**ので、✕ や Atlas で閉じられた窓とも食い違わない。
                                    再同期は `pointerup`（ツール欄が実在するときだけ）**と、`OS.exec` が
                                    返した Promise**——13個のうち**8個は遅延チャンク**で、開くまで**秒**かかる。
                                    ⚠ **時間を当てにするな**（実測 本番 R264: パネルは `display:flex` で
                                    `state().open` も true なのに**行は消えたまま**、次にどこかで
                                    ポインタを離すまで光らなかった）。`OS.exec` は**コマンドが返したものを
                                    そのまま返す**ので、遅延ツールではそれが**到着の Promise** そのもの。
                                    実測（修正後・ポインタ操作なし）: **500 ms で点灯**。
                                    ⚠ この行のために **`close()`／`isOpen()` を持っていなかった 7 モジュールに
                                    追加した**（`js/sims.js` の RF・公共交通・浸水・地球リプレイ・放射性プルーム、
                                    `js/viewshed.js`、`js/map-tools.js`）。パネルを持たない2つ（プルーム・公共交通）は
                                    **自分が描いたもの**を答える。既存の ✕ は同じ `close()` を呼ぶよう付け替えた。
                                    ⚠⚠ **`sim.radiation` の行は #R258 以来ずっと死んでいる**——
                                    `IntMapRadiation.openPanel()` を呼ぶが、そのモジュールに `openPanel` は
                                    **一度も存在しない**（実測: `OS.exec('sim.radiation')` は false）。
                                    このラウンドの範囲外なので**直さずに報告**した（13行のうちこの1行だけ、
                                    Atlas がプルームを描いたときにしか光らない）。
  ...
  runtime.js                        (#R234) **IntMap Runtime** `window.IntMapRuntime`。全体で1本の camera 購読・
                                    ...
                                    詳細は §9.0。
  water-dynamics.js                 ⚠⚠⚠ (#R265) **時間をかけて進む水 `window.IntMapWaterDynamics`**。
                                    2次元浅水方程式の**局所慣性形** — Bates, Horritt & Fewtrell (2010),
                                    *A simple inertial formulation of the shallow water equations for efficient
                                    two-dimensional flood inundation modelling*, J. Hydrology **387**(1–2) 33–45
                                    （LISFLOOD-FP の定式化）。安定化は **q 中心化**（de Almeida, Bates, Freer &
                                    Souvignet 2012, WRR **48**, W05528・θ = 0.7）——局所慣性形は Fr → 1 で不安定に
                                    なる（de Almeida & Bates 2013, WRR 49:4833）ため。摩擦は**マニング n = 0.035**
                                    （Chow 1959 表5-6 の自然河道・氾濫原の中央値）、Δt は CFL `α·Δx/√(g·h)`・α = 0.7。
                                    質量は**厳密に保存**し（各セルの流出をそのセルが持つ量で頭打ちにし、同じ数を
                                    受け側に足す）、深さは負にならない。境界は**壁ではなく落ち口**——ただし
                                    **外向きに地面が下がっている辺だけ**（向きを見ない最初の版は上流側の辺からも
                                    水を捨て、一様斜面の定常水深が 0.046 m／等流水深 1.611 m になった）。
                                    ⚠ **レンダラも DOM も持たず `window` への代入が1つだけ**なので、
                                    `tests/r265-checks` が Node で物理そのものを走らせる: 一様斜面の定常水深＝
                                    **マニング等流水深と比 1.0000**（S₀ = 0.0005〜0.02）／静水は **max|q| 厳密に 0**／
                                    質量保存 **5×10⁻¹⁰**／14.9 km の洪水波は **2.26 時間**（等流流速と運動学的波速の間）。
                                    ⚠ `storedM3` は**描かれる量**（深さ 2 cm 以上）、**保存量は `totalM3`**。
                                    ⚠ 摩擦則はツール全体で**この n 1つ**——下流トレースの断面も同じ値を読む
                                    （#R255「上流のものに合わせる」の規則）。
  ...
  terrain-water.js                  (#R268) リセットは `resetTerrainNow()` の1本（ボタンと Atlas が共用）。
                                    盛る／削るの着色は `tintEdits` でトグル。basin の拡張は DEM タイルの
                                    ブロック単位（LRU に収まる大きさ・null は1回だけ再試行・残りは
                                    `basinVoid` に数えてパネルに印字）。
  terrain-water.js                  (#R176) **地形編集＋水流 `IntMapTerrainWater`**。実 DEM をブラシで盛る／削る、
                                    ...
                                    ⚠⚠⚠ (#R267) **プライオリティフラッドはもう「水」ではない。**
                                    上の解は**地面についての解析**——どの窪地が溢れ、どこから、どれだけ——として
                                    残っており、パネルの「決壊・越流」はこれが出す。**画面に出る水は例外なく
                                    `js/water-dynamics.js` の浅水場**で、上流も下流も同じ1つの場である。
                                    ⚠⚠⚠ (#R267) **上流と下流で「モデル」も「描画」も1つになった。**
                                    「上流から下流まで全部同じモデル、描画にしろと言っている。」——同じ指示は
                                    **#R211・#R255・#R267 の3回**出ている。前2回は両者を**似せた**（同じ配色・
                                    同じ矩形セル・同じ摩擦定数）が、**別々のまま**だった:
                                    　　矩形の中＝浅水方程式を時間積分した深さ場
                                    　　矩形の外＝生 DEM を歩いて作った**折れ線**を、マニング断面で太らせ、
                                    　　　　　　　∫ds/c の**ラベル**を添えて **t = 0 で全部描く**
                                    実測（本番ビルド・クリック1回だけ）: 甲府盆地→駿河湾 99.2 km が **到達 184.8 日**
                                    （平均 0.0062 m/s）・最長の直線区間 **1,120 m（最細の48セル）／起伏 5.9 m**、
                                    アルプス→ポー 362.9 km が **566.4 日**・直線 **4,249 m（215セル）**、
                                    西シベリア 135.6 km が **432.8 日**・直線 **2,267 m（159セル）**。
                                    実河川は 0.5〜2 m/s。**報告された2つの症状は、どちらもこの後半だけの性質**——
                                    直線は折れ線の弦であり、到達時間は**別の摩擦則で別の幾何**を積分した第2の時計。
                                    → **後半を削除**した。`B`（basin）は **G と同じ格子**（同じセル寸法・同じ DEM
                                    レベル・同じ原点）で、**水が実際に走った方向へセル単位で継ぎ足される**
                                    （`grow()`——状態ベクトルをそのまま大きい格子へ写すので**補間ゼロ・質量は厳密に
                                    保存**。tests/r267 ①）。描画も**1枚のラスタ**（`tw-water` のみ。`tw-flowimg` は
                                    廃止）。**折れ線が1本も無いので「地形を無視する直線」は構造的に発生し得ない**——
                                    面越しにしか水は動かないので、**濡れたセルには必ず「先に濡れた隣」がある**。
                                    それを数えるのが `jumpCells()` で、**solve のたびに測り**、0 でなければパネルが言う。
                                    実測（甲府盆地・60,000 m³/s の連続水源・実 DEM・格子拡張2回）: 14回の読みで
                                    **jumps は全部 0**。
                                    到達時間は `tArr`＝**描画したのと同じ積分で先端に水が届いた時刻**
                                    （閾値は「水が有るか」——描画閾値で刻むと、2 cm/セル 以上落ちる斜面で
                                    上流セルが 1 cm のまま下流を 2 cm 超に満たすため 33,450 セル中1件が誤検出になる）。
                                    実測: 先端 19.4 km・到達 **5.0 時間**＝**1.09 m/s**。
                                    実測（#R265 当時・甲府盆地・5×10⁶ m³ を1回配置）: 先端は **60秒 2,276 m →
                                    10分 2,810 m → 50分 3,526 m → 1.8時間 3,914 m**——
                                    **#R265 以前のモデルは最終地点を t = 0⁺ で描いていた。**
                                    ⏭ は**同じモデルを水が動かなくなるまで進める**（別モデルの答えを写さない）。
                                    予算はステップ数と**実時間の両方**——格子が育つとステップの値段が変わるので
                                    ステップ数だけでは予算にならない（実測 96 時間分が10分で戻らなかった）。
                                    **予算に当たったら「まだ動いている」と明示**（実測 6.2 秒で打ち切り）。
                                    格子の上限は `basinMaxCells()`（PC 1.6M・携帯 360k セル）で、
                                    当たったら縁は元どおり流出境界に戻り、**その旨をパネルが言う**。
                                    ⚠⚠⚠ (#R267 追記) **継続の水源は「流量」であって「小包」ではない。**
                                    区間ぶんの水を積分の前に `addVolume` へ一度に渡していたので、
                                    **本番実測**で 60,000 m³/s を30分進めると「**max depth 21,290.1 m**」
                                    ——1.08×10⁸ m³ を 71 m セル1つで割った値そのもの。数ステップで
                                    流れ去り、できる洪水はおおむね正しいが、**21 km の水柱は存在しない**
                                    ものであり、絵にも読み出しにも出ていた。→ ソルバの `advance()` が
                                    **毎ステップ `onStep(dt)` を呼び**、蛇口は `rate·dt` だけ入れる
                                    （パネルが出す累計 `m3` も**同じ量を同じ時刻に**進む）。
                                    実測（同一条件・同一総量 1.08×10⁸ m³）: **21,424 m → 11.9 m**。
                                    ▶ は「進めるものがあるか」で決まる（蛇口が無くても**動いている水**があれば動く）。
                                    時計は**1つ**＝モデルを先に進め、**実際に積分できた秒数**で経過時刻も蛇口も進める
                                    （刻み上限に当たったティックは数えて印字する）。1点に置いた水量は**池**として入る
                                    （種セルから最も低い隣を取り続ける体積制限フラッドフィル。水位は単調増加、
                                    最終水位は標高順の累積和で厳密。実測 誤差 0.000%）。水源は `_fed` を持ち
                                    **1回だけ届く**。`clearWater()`／`undo()`／矩形の再構築は浅水の状態も作り直す。
                                    ...
                                    ⚠⚠⚠ (#R255) **上流と下流は1つの計算になった**——「上流から下流まですべて
                                    同じ計算・描画方法にしろ。上流のものに合わせろ」→確認済み「計算そのものを
                                    上流に統一する」。#R211 が揃えたのは色と描画プリミティブで、中身の問いは
                                    別のままだった: 作業グリッド＝**置かれた水の m³** を優先度フラッド＋
                                    MFD(1.1)＋カスケード窪地で流す／下流＝`channelChain` が単位寄与の MFD 集積
                                    （＝**集水面積**）を求めていた。ルーティングを **`routeWater(surf,NX,NY,cellM,own)`**
                                    に括り出し、**両方がこれを呼ぶ**。窓は `windowRoute()` が「そこへ届いた水」を
                                    入力として同じ関数に渡し、経路は `mainOut`（**流量の最大分配先**）を辿る。
                                    窓から窓へ渡るのは m³（`through` の実測値）なので、飲み込む窪地があれば
                                    そこで終わる——別ルールではなくルーティングがそう言う。
                                    ⚠ 呼び出しは**2箇所**（本流と広域エスカレーション）。片方だけ直すと
                                    トレースは 13 窓・88 km 走ってから `end='error'` になる（実測）。
                                    ⚠ (#R255) **水を置いても画面は動かない**（`build(opt.center)` で作業矩形だけ
                                    移す）／**ツールを外しても注水は止まらず時計もリセットしない**（止めるのは
                                    ⏸・全消去・元に戻す・閉じるだけ）／**パネルは本体スクロール＋下部スティック**
                                    （共通操作と経過時刻は常に見える。⚠ #R245 の教訓どおり body と foot は
                                    **兄弟**で、`</div>` 1つで sticky は無効になる）。
                                    ⚠⚠ (#R255) **彫った地形が本物の地形になる**——「盛る、削るはそれに合わせて
                                    実際の標高や3D表示も対応させろ。堤防・ダムも同様」。編集量は `editField()` に
                                    1本化し、(a) 読み出し系は `window.IntMapElevEdit` フック経由で
                                    `js/map-readout.js` の `demElevAt`/`demElevBilinear` が加算（座標表示・
                                    断面図・可視領域・日射がまとめて追従）、(b) 3D は terrarium タイルに同じ差分を
                                    足す `imapterr://` プロトコルの raster-dem に `setTerrain` を差し替える
                                    （基図タイルはデコード済みで保持するので、ストローク1回の代償は再エンコードのみ・
                                    通信ゼロ）。実測: 読み出し 1000→1117.96 m、`getTerrain().source` が
                                    `terrain-dem`→`tw-dem-1`、閉じると戻る。⚠ 再メッシュは **`editDirty()` から**
                                    駆動する——UI ハンドラに付けると Atlas の `brush()` が漏れる（実測）。
                                    ⚠⚠⚠ (#R258) **その terrain ソースは1回しか作らない**。#R255 は編集のたびに
                                    `tw-dem-1`、`-2`… と**新しいソース**を作って `setTerrain` を呼んでいた——
                                    terrain の差し替えはメッシュを丸ごと捨てて中心標高を計算し直すので、
                                    それが「盛る・削るたびに3D表示がリセットされる」の正体だった。いまはソースも
                                    `setTerrain` も1回だけで、編集は**タイル URL の版（`?v=<editStamp>`）**を
                                    `setSourceTiles` で差し替えるだけ。実測: ブラシ5ストロークで `setTerrain` **1回**・
                                    ソース **1個**・zoom / pitch のずれ **0**。
                                    ⚠⚠⚠ (#R258) **堤防の天端は幅いっぱい平ら**。`crest·(1−d²)` の放物線断面は
                                    **堤防の縁でちょうど 0** になるので、天端高 8 m と打ってもセルによっては 4.1 m
                                    しか上がらず、水は「8 m でない場所」を越えていた。さらに `halfW` の下限が
                                    **1セル**で、斜めの堤防は**角でしか接しないセルの列**＝`routeWater` の**8近傍**
                                    フラッドがその角を通り抜ける。いまは天端は平ら・肩だけ落とす、下限 **1.5 セル**、
                                    中心線の標本は **1/3 セル**刻み。実測（135.7 m セル）で中心線13点すべて 8.00 m、
                                    断面 0/0/1.32/6.71/**8**/6.27/1.14/0/0。
                                    ⚠⚠ (#R258) **`addLevee` は `editDirty()` を呼んでいなかった**——この入口で
                                    作った堤防はソルバにも標高にも 3D にも存在しなかった（実測: 中心線で全点 0.00 m）。
                                    共有リンクからの復元も同じ。
                                    ⚠⚠ (#R258) **湖の横断は「渡った経路」を描く**。窓より広い平地の脱出
                                    (`flatOutlet`) が**出口の1点だけ**を積んでいたので、入った岸から出る岸まで
                                    **1本の弦**が引かれ、間の地形を完全に無視していた（「直線で地形を完全無視する
                                    クソ区間」）。フィルを**幅優先＋親リンク**にして経路そのものを返す。
                                    ⚠⚠ (#R258) **パネルは地震シミュレータと同じ grouped inset list**（下記 #R237）、
                                    **時計はフッタ**——▶/⏸・時間倍率・経過時刻は「ここに水」のパラメータ欄の中に
                                    あり、ブラシを持つと丸ごと消えていた。時計はツールではなくシミュレーションのもの。
                                    ⚠ ツール選択は 2×2（実測: 帯は各 73 px しか与えないのに「🧱 堤防・ダム」は
                                    96 px、「💧 ここに水」は 88 px 必要で、4つ中2つが切れていた）。
  ...
  seismic.js                        ⚠⚠⚠ (#R240) **描いた震源域は t=0 から波の絵に届く。**
                                    ...
                                    **t=400 s まで1つも描かれていなかった**（実測: t=30/90/200 s は ring×4 のみ）。
                                    ...
                                    震央がまだ無いときだけ `open()` が `'epi'` を武装する。
                                    ⚠⚠ (**#R264**) **過去・最近の地震を読み込んだら武装は解除する。**
                                    「震央を置く」は**地図をタップする**のと**カタログから読み込む**の2通りがあり、
                                    `applyEvent()`／`applyReal()` はどちらも解除していなかった——だから
                                    **公表された震源を持つ地震に対して、地図の上のピル `#sq-hud` が
                                    「② 震央を置く」と言い続け**、次に地図をタップすると黙って震央が動いた。
                                    解除は**ローダの中**で行う（HUD は `clickMode` の**読み出し**であって
                                    第二の判断を持たせない＝#R239）。`clearEvent()` は従来どおり再武装する。
                                    ⚠ `applyReal` は `render()` を呼ばない（既存ノードを書き換え `.sq-out` を
                                    掴んだままなので呼べない）ので、`close()` と同じく `_hud()` を直接叩く。そして本文スクローラの
                                    **外側に固定フッタ**があり、3手順のトラック・次にすることの1行・**主動作**を持つ。
                                    ...
                                    (#R176/#R258) **地震シミュレーター `IntMapSeismic`**（#R258 で「地震波シミュレーター」から改称・9言語）。到達時刻は **IASP91**
  ...
  vs30-mask.js                      (#R223) **同梱の場址項 `IntMapVs30`**。(#R263) `data/vs30.png` は
                                    **7200×3600＝0.05°・4.89 MB** になった（携帯は `data/vs30-phone.png`＝
                                    **3600×1800＝0.1°・1.30 MB**。判定は app-body の `_imPhoneGPU` と同じ
                                    「粗いポインタで細かいポインタが無い」）。⚠ **格子は `data/vs30.json` から読む**——
                                    定数で持つと再生成のたびに読み違える（#R263 は実際に一度そうなった）。
                                    0.25°→0.05° は**新しい取得を1枚も増やしていない**：z7 タイル（1,223 m）を
                                    516 標本ずつ 1 セルに潰していたのを 20 標本ずつにしただけ。8bitグレー・
                                    各セルの**陸地部分の平均 Vs30**（0＝陸の標本なし、1..255＝150〜1500 m/s の線形）。
  ...
  datacenters.js                    (#R254/#R258) 「データセンター・AIインフラ」レイヤー `IntMapDataCenters`。
                                    ⚠ (**#R265**) **表示範囲内の集計は削除した**（「表示範囲内のものを表示する機能は
                                    いらない」）。#R261 が足し #R264 が凡例行へ移した件数・公表容量とその分母・
                                    分類別内訳・容量順の上位リストは、`dcStats`／`dcRender`／`mountSummary`／
                                    `unmountSummary`／`stats()` ごと**隠さずに削除**され、`moveend` の再描画も無い。
                                    **他は何も減っていない**——詳細カード `#dc-detail`（浮くのはこれ**だけ**）、
                                    色キー、**#R258 の分類フィルタ**（凡例の行がそのままスイッチ・式は1つ）、
                                    表示範囲の OpenStreetMap 取得（`refresh()`＝レイヤーがデータを得る手段）。
                                    (#R258) 内蔵表 **260 → 318**（AI 拠点 16→36・TOP500 14→36・接続拠点 15→31）。
                                    **MW は事業者が公表している場合だけ**——無公表の計画には所在地・年・出典だけを
                                    与え容量は空のまま（標準指示4）。点の大きさは**公表 MW の平方根**（面積で比べる。
                                    無公表はクラス既定のまま＝大きい点は必ず「これだけ公表されている」の意）。
                                    凡例の各行は**そのクラスの絞り込み**（`toggleKey` → `setFilter`）。
                                    座標の誤り6件を公表地へ修正（SoftBank 堺・Naver Gak Sejong・Nscale Glomfjord・
                                    G42 Abu Dhabi・Eclairion Essonne・AI Growth Zone Ravenscraig）。
                                    ...
                                    ⚠ (#R255) 詳細カードは `left`/`top` を**自分で置く**。`.country-popup` は
                                    `position:absolute` で座標を持たないので、置かずに body へ追加すると
                                    文書末尾＝画面下端の外に出る（実測 y=900／viewport 900 = 「押しても
                                    詳細が出ない」）。ヘッダはドラッグハンドル。
  osm-facilities.js                 (#R255/#R258) 「実測された施設」**6レイヤー** `IntMapFacilities`。#R255 で新設した
                                    4カテゴリ（政治・統治／軍事・安全保障／医療・衛生／IT・技術インフラ）に
                                    **国別コロプレスではない点レイヤー**を1つずつ入れるためのもの
                                    （「単に国別で色分けする奴だけじゃなくて、ガチな奴も」）。
                                    `osmdiplo`＝外交公館（`amenity=embassy` / `office=diplomatic`）、
                                    `osmmil`＝軍事施設（`military=*` / `landuse=military`）、
                                    `osmhealth`＝医療機関（`amenity=hospital|clinic|doctors|pharmacy`）、
                                    `osmtelecom`＝通信基盤（`telecom=exchange…` / 通信鉄塔）。
                                    (#R258) **新カテゴリ「エネルギー・資源」(`lyrGrpEnergy`) に2つ**——
                                    `osmpower`＝発電所・送変電（`power=plant|substation`、および
                                    `generator` の風力・太陽光・原子力・水力・地熱。燃料別に色分けし、
                                    カードは `plant:output:electricity` / 燃料 / 電圧 / 運営者をタグのまま出す）、
                                    `osmextract`＝鉱山・採石場・油井（`landuse=quarry` /
                                    `man_made=mineshaft|adit|petroleum_well` / `industrial=mine`）。
                                    ⚠ このカテゴリには**新しい行しか入れていない**——#R233 の7つと #R254 の61件は
                                    読者が名指しで並べた表であり、「再編」はそれを黙って覆す許可ではない。
                                    (#R261) **さらに6つ、合計12レイヤー**——`osmair`＝空港・航空施設
                                    （`aeroway=aerodrome|terminal|heliport|control_tower`。カードは ICAO/IATA・
                                    滑走路長・標高）、`osmport`＝港湾・ターミナル（`harbour` / `landuse=port` /
                                    `amenity=ferry_terminal` / `man_made=crane`）、`osmwater`＝上下水道施設
                                    （`man_made=water_works|wastewater_plant|pumping_station|water_tower`）、
                                    `osmedu`＝大学・研究機関（`amenity=university|college|research_institute` /
                                    `man_made=observatory`）、`osmemg`＝緊急対応拠点（消防・警察・救急・山岳救助）、
                                    `osmspace`＝宇宙基地・地上局（`aeroway=spaceport` / `man_made=launch_pad|
                                    satellite_dish` / 電波望遠鏡）。⚠ **1行だけの棚だった「Space & orbit」
                                    「Indicators」、新設の「交通・輸送」「社会・教育」、指標18件だけだった
                                    「医療・衛生」に、クリックできるものが入った**のがこの6つの目的。
                                    **1つのエンジンに6つのタグ集合**——取得・キャッシュ・レイヤー・カード・
                                    凡例は1度だけ書き、`SETS` で切り替える。取得は `js/datacenters.js` と
                                    同じミラー競走の Overpass、表示範囲ごとにキャッシュ、ODbL 表記つき。
                                    カードは OSM オブジェクト自身のタグだけを出し、無い欄は**推定せず省略**
                                    （出典リンクは当該オブジェクト）。行 id は `fac-dl-<id>`
                                    （`js/data-layers.js` の `rowFor()` がこの接頭辞を知っている）。
                                    ⚠ 軍事レイヤーはカードに「公開情報の記録であり画像判読の推定を含まない」
                                    と明記する。
  ...
  map-readout.js                    (#R169) 地図の読み出し系（座標/標高/レイヤー値/コンパスの表示・DEMサンプラ・
                                    ...
                                    ⚠⚠⚠ (#R265) **公表されている標高データには穴があり、穴は −32,768 m の地面
                                    ではない。** AWS terrarium は**中身が全部 RGB(0,0,0) の不透明タイル**を返す
                                    ことがある（実測 `terrarium/14/9101/5896.png` は 256×256・α=255・65,536 画素
                                    すべて 0,0,0、同じ地点が z12 では **85.61 m**）。サヴァ川氾濫原まわりでは
                                    **z14 タイル 49 枚中 14 枚**がこれで、琵琶湖・デスバレー・西シベリア・東京湾・
                                    レマン湖は 0 枚——地形・水流の「たまに直線になる」の正体だった（§7）。
                                    ① 復号は `DEM_NODATA_BELOW = −12,000 m`（チャレンジャー海淵 −10,935 m の下、
                                    符号化の最小 −32,768 の上）を割る値と α=0 の画素を **NaN** にする。
                                    ② `demElevAt` は NaN を見たら**ピラミッドを1段ずつ降りて**親タイルで答える
                                    （上限4段・z6 まで）。穴のあるタイルが復号された瞬間に**その親を要求する**。
                                    ③ `demElevBilinear` と `demSnapshot` の行サンプラは**穴でない隅だけで重みを
                                    正規化**する（穴が無いときの式は不変＝震度場・津波の値は動かない）。
                                    ④ どの段でも埋まらなかった標本は数え、`demVoidStats()` から読める（#R185）。
                                    実測: 同じ矩形・同じ z14 で −100 m 未満 **2,240 → 0**、最低標高 68 m。
                                    ⚠ `js/dem-source.js`（3D terrain / 陰影起伏がタイルを MapLibre へ直接渡す経路）
                                    は**この保護の外**——#R265 の依頼の範囲外なので直していない。
...
- `alerts-relay` … (#R266) 各国気象機関の警報フィードの **ACAO 付与＋要約**（`--no-verify-jwt`・秘密なし）。
  (#R273) **CAP索引サービスは1本の関数** `summariseCAP(key)`（`?cap=tw` / `?cap=nz`）——RSS/Atom の
  索引→各 CAP ファイル→`<area>` ごとに `<polygon>`・severity・機関自身の色語（`acol`）。国を1つ足す
  費用が `CAPSRC` の1行になる。実測、台湾 NCDR は 1,029 件のうち **157 件が中央氣象署**（残りは水道
  公社・鉄道・消防）。⚠ 南アフリカ SAWS は索引は 200 だが各 CAP の実体が **521 Web server is down**
  なので**接続していない**。
  **2ホストだけの厳格 allowlist**: `feeds.meteoalarm.org`（欧州37機関の MeteoAlarm）と `www.nmc.cn`
  ...
  ⚠ (#R271) **`areas` を併せて返す**——EMMA_ID（無ければ地域名）で重複を畳んだ**地域ごとの行**で、その
  地域で有効な最悪階級・災害名の一覧・CAP が持っていれば `<polygon>` を持つ。従来の要約は1警報の全区域を
  カンマ結合した1つの文字列だったので、**地図が塗れる単位が payload に存在しなかった**（スペインは
  15,170 の区域エントリが約180の区域に畳まれる）。上限は1国 400 区域で、`areaTotal` が実数を述べる。
  ⚠ (#R271 追記2) **`?ph=1` でフィリピン（PAGASA）**。Atom の索引から**地域ごとの最新の1件**を採り
  （GFA は番号が上がるたびに前を supersede する）、その CAP を最大24件読んで**州ごとの行**に落とす。
  ⚠ **「フィリピン責任領域（PAR）」という区域は名前で落とす**——海を含む 16°×17° の矩形で、発令単位では
  ない。⚠ **`expires` を過ぎた速報はここで落とす**（パネルが印字する件数は「現在有効な数」でなければ
  ならない）。実測: 51 エントリ → 地域ごとの最新 9 件 → **20州**（Zambales/Bataan/Bulacan が Extreme、
  La Union/Ilocos Sur が Severe）。上流の予算は他と同じ **45 秒**（#R269）。
...
### (#R273) この節に効く現状 — 警報レイヤーの全面再設計・レイヤー分類の大規模再編・×の統一

- **気象・災害警報は「どこで／何が／どれほど危険か／情報は新鮮か」の4つに答える。**
  GDACS は**完全に撤廃**した（`js/world-packs.js` にコードは1つも残っていない・tests/r273 ① が
  `loadGDACS` / `gdacsapi` / `GDACSCOL` / `'gdacs'` 等の**書かれていた構文**で確認する）。
  撤廃したからこそ、**未対応国を「静かな国」に見せない**仕組みが必要になった——それが下の灰色斜線。

  | | 対応国 | 描画単位 |
  |---|---|---|
  | 日本 | 気象庁 `bosai/warning/data/r8/map.json`（官署ごとの最新1件） | **市町村（class20）**。幾何は国土交通省 国土数値情報 N03（1,897件・629 KB…1.5 MB・ACAO *）を **JIS X 0402** で引く。class20 コードの上5桁がその JIS コードで、実測 1,805 中 **1,774 が直接一致**、残る31（政令指定都市・離島）は**区の合併**で解決 |
  | 米国 / カナダ / ブラジル / ドイツ / ノルウェー | NWS / ECCC / INMET / DWD WFS / MET Norway | 各機関の**警報ポリゴン**（DWD は郡・INMET は範囲） |
  | 欧州35機関 | MeteoAlarm（relay 要約） | CAP の `<polygon>`、無ければ **NUTS 2/3（20M）**——60M から上げた |
  | 中国 / オーストラリア | CMA（relay）/ BoM | **省 / 州**（Natural Earth 50m admin-1） |
  | フィリピン / **台湾** / **ニュージーランド** | PAGASA / **中央氣象署（NCDR 経由）** / **MetService** | **CAP索引を読む1本の関数**（relay `?cap=`）。台湾はポリゴンが無い区域を **郷鎮市区**（g0v 378件・629 KB）で引く |
  | 香港 | HKO | **全域が発令単位** |

  - **国の3状態は3つの見え方を持つ。** ⑴ フィードが無い国＝**灰色斜線**（`fill-pattern`・
    `wp-alert-hatch`）、⑵ フィードがあり何も出ていない＝**灰色**（気象庁自身の `#c8c8cb`）、
    ⑶ 発令中＝その階級の色。**「警報なし」と「データなし」は別**という要求そのもの。
  - ⚠ **国は「発令単位で描く」か「薄塗り」かのどちらかで、両方はしない。** #R271 は置けなかった
    区域のために描画済みの国も薄塗りしていた——実測、日本を市町村で描くと 1,490 のうち **11**
    置けないだけで**日本全土が着色**された。置けなかった数は**言葉で**印字する（`placedLine`）。
  - **配色は2モードで、既定は各機関の公式配色**（`localStorage['im.alertPal']`）。
    ...
    ② **IntMap換算** … 独自の4段階。**どの機関の配色とも1色も共有しない**うえ、パネルが
  ...
  - **種別は地図の上に文字で出る。** 区域は災害名を持ち、同じ区域に複数出ていれば `+N`。
    z5 未満は短縮形（`hzs`）、以上は正式名（`hz`）——⚠ **`text-field` に `['step',['zoom'],['get',…]]`
    は書けない**（MapLibre が style 時に `this.expression.evaluate is not a function` で落ちる。実測、
    ページごと落ちた）ので **2レイヤーの `minzoom`/`maxzoom`** で表現する。
    短縮形は**頭字語ではない**——階級語を落として災害語を残す（`Moderate thunderstorm warning` →
    `Thunderstorm`。頭字語だと `MTW+4` になり、読み手に鍵が無い）。
  - **塗りは薄い。** 既定の不透明度は **0.38**（`opacities['wpalerts']`・他レイヤーと同じ
    `_registerLayerOpacity` の**共通スライダー**が動かす。専用のボタンは作らない）。階級は
    **輪郭線**（色＋太さ）が持つので、塗りを透かしても答えは残る。
  ...
  - **パネルは「いま何が起きているか」から始まる。** 国×災害でまとめた上位を
    〈色・国・災害・階級・区域数〉で並べ、**取得先の一覧は畳んである**（`Source status`）。
    さらにその下に `Diagnostics`（置けなかった区域）。数字は**全ソースで「区域数」に統一**した。
  - 出典の注記は**一文**（従来は18か国を名指しする段落だった）。

- **レイヤー分類を14行動かし、見出しを1つ改名した**（`js/data-layers.js` の `GROUPS`）。
  「あきらかに不適切なレイヤーが**大量に**ある。**大規模に**再編しろ」＋（過去の名指しリストも
  動かしてよいか訊いて）「**全部動かしてよい**」。適用した規則は1つ——
  **レイヤーはそれが測っている主題に属する**（作った計器でも、対象の場所でも、出てきた表でもない）。
  1人当たりGDP→経済／HDI→社会／若年出産率→人口／製薬拠点・貧困率・ジニ係数・女性労働参加率・
  ハイテク輸出→経済／クリーン調理燃料→エネルギー／栄養不足→農業・食料／EEZ→政治／
  傾斜・方位→地形／ライブカメラ→交通／3D建物→**最上段の常設スイッチ**（描き方であって統計ではない）。
  `lyrGrpDemo` は中身に経済が無くなったので**9言語すべてで「人口・人口動態」に改名**。
  ⚠ **指示で beta へ降格されたもの（#R40 の ECMWF 系）は昇格していない**——「beta」は品質の判断、
  これは主題の判断。

- **閉じる × は U+00D7 ひとつに統一した。** 実測 `measureText` 16px で U+2715 は Inter・
  Noto Sans JP・system-ui・sans-serif・Arial の**全部で 13.07px**（＝どれも持たず記号フォント落ち）、
  U+00D7 は 10.59/16.00/10.95/16.00/9.34（＝Inter が持っている）。アプリには**両方が混在**しており
  U+2715 が **46ファイル 177箇所**。#R270 が検索欄2つだけ SVG に替えたのがその混在の直近の形。
  `window.IntMapClearGlyph()` が1つの定義で、tests/r273 ⑨ が js/ と css/ の**全ファイル**を掃く。

- **言語分布レイヤーのセルビア語系は1つの色相**（`js/layer-packs.js` の `FAM_COL`）。
  実測、#R271 の一意化の後でも セルビア語 `#31a354`（緑）・クロアチア語 `#33d2e1`（水色）・
  ボスニア語 `#6b6ecf`（青紫）で、**英語とアラビア語ほど無関係**だった。5標準を同一色相・明度差で
  並べ、生成パレットからは**予約**して他言語に渡らないようにした。

- **地形編集・水流**（`js/terrain-water.js`）。
  - **計算解像度は読み手が選ぶ**（`RES_D=[384,512,768,1024]`・既定 **512**・`im.twRes` に保存）。
    解像度変更は `build({keep:true})` で**同じ作業矩形**のまま作り直す。セル寸法と格子数はパネルが印字。
  - **↺ 再生** … 時計・水・各水源の供給量を 0 に戻し、**同じ地形と同じ水源のまま** t=0 から流す。
    ⚠ 連続水源の `m3` は「これまでに供給した量」なので**それも 0 に戻す**（戻さないと `feedSim` が
    1時間分を1ステップで注ぐ）。⚠ `editDirty()` を通す——この4ラウンドで4回、通し忘れた入口が出ている。
  - パネルの**操作の高さは 36px 1種**（従来 29/34/38 の3種）、**節の中の箱の間隔は 10px 1種**
    （ツール選択とその下のカードが**隙間0で接していた**）、`<details>` の見出しも **44px の行**。

### (#R271) この節に効く現状 — 警報は「発令単位」で描く・レイヤー分類の再編・地形パネルの1本の列

- **気象・災害警報は、その機関が発令する単位で塗る。国全体は「位置を特定できなかった区域」の意味に
  なった**（`js/world-packs.js`）。

  | 国・地域 | 出典 | 描く単位 |
  |---|---|---|
  | 日本 | 気象庁 `bosai/warning/data/r8/map.json` ＋ **気象庁自身の `geojson/class10s.json`**（153区域・ACAO *） | **一次細分区域（class10）**。市区町村はタップの中 |
  | 米国 | NWS | 警報ポリゴン |
  | カナダ | ECCC（直接） | 警報ポリゴン |
  | ブラジル | INMET（直接） | 警報ポリゴン |
  | **ドイツ** | **DWD `maps.dwd.de` WFS `Warnungen_Landkreise`（ACAO *）＋ `dwd.de/…/warnings.json`（州名）** | **郡（Landkreis）ポリゴン**・州でまとめる |
  | **ノルウェー** | **MET Norway `api.met.no/weatherapi/metalerts/2.0/current.json`（ACAO *）** | **警報ごとのポリゴン** |
  | **ヨーロッパ35機関** | MeteoAlarm（relay が要約）＋ **Eurostat NUTS 2/3（60M・ACAO *）** | **CAP の `<polygon>`。無ければ CAP が名乗る地域名に対応する NUTS 区域** |
  | **中国** | CMA（relay）＋ Natural Earth 50m admin-1 | **省**（警報 id の GB/T 2260 区画コード） |
  | **オーストラリア** | BoM（直接）＋ 同上 | **州** |
  | **フィリピン** | **PAGASA `publicalert.pagasa.dost.gov.ph`（Atom＋CAP・ACAO 無し＝relay）** | **州**（洪水情報の CAP が州ごとの `<polygon>` を持つ） |
  | 香港 | HKO（直接） | 香港全域＝発令単位そのもの |
  | その他 | GDACS | 事象の点＋国全体の薄塗り（**別の尺度・別の色**） |

  - **境界データの費用は測ってある**（どれも1セッション1回・レイヤーがオンで、かつその国に警報がある
    ときだけ取りに行く）: Natural Earth 50m admin-1 **657 KB**（中国・豪）、Eurostat NUTS 2＋3 の 60M
    **434 KB ＋ 1.24 MB**（欧州）、気象庁 class10s **288 KB**（日本）。60秒ごとの更新で再取得はしない。
    ⚠ 20M の NUTS は 2.4 MB で、これは国〜地域のズームで読むものなので 60M を選んでいる。
  - `drawnISO` は**実際にソースに載った地物から毎回作り直す**（`publish()`）。「自分の単位を描いている
    国」は表ではなく**測定**なので、フィードが図形を出し始めた日に自動で塗らなくなる。#R270 の
    `GEOM_FEEDS` という手書きの表は**削除**した。
  - `agencyTier(iso)` は「**その国の区域のうち、地図上に位置を特定できなかったもの**の最悪階級」を返す。
    「その国のどこかにある最悪の階級」ではない——それが、1つの郡の特別警報で国全体が紫になっていた原因。
    香港だけは全域が発令単位なので、その階級がそのまま入る。
  - `PLACED[iso] = [置けた数, 機関が発表した数]`。差はパネルが必ず印字する（#R185「黙って落とすな」）。
    実測（同一分・ライブ）: フランス 93/93・イタリア 18/20・オランダ 12/15・ポーランド 316/354・
    スペイン 141/233・ギリシャ 6/16・オーストリア 30/116・中国 31/34省・オーストラリア 5/5州。
  - 名前の突き合わせは**両側に同じ正規化**（`_norm`：アクセント畳み・記号除去・単位語の除去）を通し、
    `_alias()` が `|` `/` と括弧つきの別綴りを索引に入れる。`lookupUnit()` は完全一致の次に
    **語の接頭辞・接尾辞**（4文字以上）を試す——「Dolnośląskie Province Głogowski County」は県、
    「Sur de Ourense」は Ourense に落ちる。
  - 実測（修正後・本番ビルド）: 国全体の薄塗りは **香港1件のみ**（従来は仏独白墺伊克ボ香の7か国が赤、
    豪が紫）。描かれた地物は **366件**（日本 class10 39・ドイツ 162・中国 21省・カナダ 21・米国 13・
    豪 5州・ノルウェー 2・欧州の地域 11・GDACS の点 98 ほか）。
- **`alerts-relay` の MeteoAlarm 要約は「地域ごとの行」を持つ**（§6.2）。従来は1警報の全区域を
  カンマ結合した1つの文字列だったので、**地図が塗れる単位が payload に存在しなかった**。
- **言語分布・宗教分布の色の凡例は、1分類＝1色**（`js/layer-packs.js` `paletteOf(n)`）。手で選んだ30色を
  使い切ったあとは**黄金角 137.508°** で色相を進め、明度・彩度を3通り循環させ、**既出の色なら明度を
  ずらして必ず一意にする**。実測: 89言語 → **89色・重複0**（従来は30色を3周し、クロアチア語は
  イタリア語と同じ見本だった）。`IntMapCulture.palette(n)` / `.colourOf(k,cat)` が公開する。
- **レイヤー分類を大規模に再編した**（`js/data-layers.js` `GROUPS`）。20行が移動し、棚が1つ増えた。
  移動: 民主主義指数・汚職指標 → 政治・統治／平均寿命 → 医療・衛生／エネルギー構成 → エネルギー・資源／
  オーロラ予測 → 宇宙・軌道／夜間光 → 人口・経済／人口増加率・65歳以上・出生率(WB)・都市・農村・
  人口密度(WB)・難民受け入れ → 人口・経済／土地被覆・エコリージョン・植生指数・森林面積率 →
  **自然・土地被覆（新設 `lyrGrpNature`）**／3D建物 → テクノロジー・インフラ／タイムゾーン →
  最上段の常設スイッチ（`lyrGrpIndic` は空・キーは保持）。
  ⚠ **1つの id は1つのグループにしか書けない**（`order.push` は移動）——tests/r271 が全 id の一意性と、
  全グループキーが9言語すべてに見出しを持つことを検査する。
- **地形編集・水流パネルは1本の列**（`js/terrain-water.js`）。`.tw-body` は `scrollbar-gutter:stable`、
  スクロールバーの幅は `_squareColumn()` が**実測**して `.tw-head` / `.tw-foot` の `padding-right` に
  インラインで書く（インラインの padding にスタイルシートは勝てない）。見出し `.tw-cap` の内側余白は
  行と同じ 11 px、`.tw-blk` は `min-height:44px`。実測: カードの右端・ボタンの右端・状態行の右端が
  **同一**。
- **新しい水源を置いても、既にある水はリセットされない**（`js/terrain-water.js`）。作業矩形の外を
  押したときは `padsToReach()` → `extendToPoint()` が**盆地を広げて**そこに届く（`sim.grow` は水深を
  そのまま運ぶ）。予算 `basinMaxCells()` を超えるときだけ従来どおり `rebuildAround`（＝リセット）に
  落ちる。Atlas の `addSource()` も同じ経路を通る。実測: 矩形の 11.7 km 外に2つ目の水源を置いても
  湿潤セル 67→70・時計 3600 s のまま・作業矩形の bbox は不変・盆地は 384→513 セルへ拡張。
- **レイヤー検索の ✕ は、それが属する入力欄の矩形から置く**（`window.IntMapPlaceClear`、`js/map-ui.js`）。
  `top:50%` は**位置指定の祖先**に対して解決されるので、48 px の包み箱の中の 36 px の入力欄では
  4 px 下にずれる。実測（修正後）: 中心のずれ **0.0 px**、右の余白 9 px。検索欄は2つあるので定義は1つ。

### (#R270) この節に効く現状 — 警報の2つの尺度・昼夜オフの大気・凡例の年セレクタ

- **気象・災害警報は2つの尺度を持ち、2つの凡例を持つ**（`js/world-packs.js`）。
  ① **各機関の発令階級** `TIERCOL` = 注意報 黄 `#ffcc00` / 警報 赤 `#ff3b30` / 特別警報・危険警報 紫
  `#a335ee`、機関が発令する単位（都道府県・zone・州）に描く。
  ② **GDACS の事象レベル** `GDACSCOL` = 緑 `#3d9a3d` / 橙 `#f08c00` / 赤 `#e02b1d`、**国全体**を薄く塗る。
  ⚠ **鍵は、それが鍵である対象から色を取る**——`keyRows(col,name)` はパレットと名前を一緒に受け取る。
  `tierKey(names)` のように命名関数だけ差し替えられる形は**もう書けない**（#R270 の欠陥そのもの）。
  ⚠ **2つのスケールは1色も共有してはならない**（tests/r270 が検査する）。
- **発令区域そのものを描いている国は、国全体を塗らない。** `GEOM_FEEDS` = `jma` / `nws` / `eccc` /
  `inmet`。`washTier(iso)` は **0**（塗らない）／**1–3**（その国の機関の階級：CMA・BoM・HKO・MeteoAlarm の
  一覧フィード）／**11–13**（GDACS のレベル）を返し、choropleth はその1つのフィールドを2つのレンジで塗る。
  ⚠ 薄塗りのアルファは**色に入れる**（`TIERWASH`/`GDACSWASH` の `rgba`）——不透明度スライダは
  `fill-opacity` を1つのスカラで上書きするので、そこに置いた濃さは残らない。
- **昼夜表示がオフでも大気は同じ**（`js/theme-sky.js`）。`_sunElevAtCentre()` は
  ⑴ 衛星写真でなければ `null`（#R241「標準マップでは大気はなし」——`atmosphere-blend` は 0 のまま）、
  ⑵ 昼夜オフなら **90°**（`_aimSun()` がカメラ中心の真上へ向けた光の**読み戻し**。ターミネータは
  画面に出ない）、⑶ それ以外は実際の太陽高度。`_relAzimuth()` も ⑵ では 0 を返す。
  ⚠ **「昼夜オフ」は「太陽の位置が不明」ではない。** `null` を返すと `_limbOwnsRim()` が false になり、
  この app 自身の大気（js/limb-layer.js）が**そもそも点かない**。
- **年は層の上にもある**（`window._legendClockYear` — js/data-layers.js）。1人当たりGDP・人口密度・
  合計特殊出生率・国防費・国防費対GDP・HDI・貿易フロー・エネルギー構成・作物の凡例に年セレクタがあり、
  **`window.IntMapTime` を読んで書く**。⚠ **2つ目の時計は作らない**——行は自分の年を持たない。
...
- **HDI は UNDP の年次系列**（`data/hdi-series.json`、`scripts/build-hdi.mjs`、193か国 × 1990–2022）。
  ...
  1990 より前は `null`（データなし）、最後の公表年より後はその列。凡例の「<年> UNDP」はこの値で、
  **タイマーではなく `_imReapplyChoros`**（＝重ね合わせの後に走る再描画）から書き換わる。
- **世界銀行の塗り分けの凡例はグラデーション帯**（`js/wb-layers.js` `rampKey`）。停止は**値の位置**に
  置く（`interpolate` は値について線形）。タイルのサムネイルも同じランプを層から読む
  （`IntMapWB.rampOf`）——`js/layer-previews.js` の `WBP` にある複製は、その層が読めない間だけの答え。
- **言語分布は1カテゴリ1色**（`js/layer-packs.js`）。旧ユーゴの4標準を1色にまとめる `LANG_ONE_COLOUR` は
  **削除**した——同じ見本が3行に付くと、その色に付く名前は最初の行のもの（＝「セルビア語」）になり、
  **見本が区別できない鍵は鍵ではない**。
- **地形編集・水流のパネルは他の浮遊ウィンドウと同じ帯にいる**（`registerWindow`、2200–2599）。開く位置は
  `placeClear()` が**地図を覆っているものの矩形を実測**して決める（サイドバーは開閉するので定数にできない）。
  読者が動かした後は動かさない。行は **44 px 1種**、`<details>` もカード。
- **レイヤー検索の ✕ は文字ではなく幾何**（`window.IntMapClearGlyph()` — js/map-ui.js に1つだけ）。
  U+2715 はこの app が指定するどの family も持たず、全部プラットフォームの記号フォントに落ちる
  （実測: 送り幅が全 family で 9.8027px）。`type=search` のネイティブ ✕ は CSS で消してある。

### (#R268) この節に効く現状 — 年を変えられるレイヤー・警報の対応国・地形編集

- **地形編集のリセットは1本**（`js/terrain-water.js` `resetTerrainNow()`）。パネルの「地形をリセット」も
  Atlas の `resetTerrain` もこの1本を呼ぶ。⚠ `sculpt` と `levees` を空にするだけでは**何も起きない**——
  `editField()` は `editStamp` でメモ化されており、ソルバ（`surface()`）・座標読み出しのフック
  （`IntMapElevEdit`）・3-D 地形の raster-dem タイルは全部その場を読む。**`editDirty()` を通らない
  「地面を変える道」を作ってはならない**（#R255・#R258・#R268 で3回同じ抜けが起きた）。
- **盛る／削るの着色はトグル**（`tintEdits`・`localStorage: intmap_tw_tint`）。切っても `sculpt`・堤防・
  ソルバ・標高読み出し・地形タイルは1ビットも変わらない——**描画の被せだけ**。
- **basin の拡張は DEM タイルのブロック単位**（`js/terrain-water.js` `growBasin`）。1タイル1点で温め、
  ブロックは DEM の LRU に丸ごと収まる大きさ（携帯 100 / デスクトップ 280 タイル ⇔ LRU 140 / 560）、
  キャッシュに載っている間に bed を埋め、`null` は**1回だけ**狙って再試行。⚠ `demAt` の `null` は
  「地面が無い」ではなく「**キャッシュに無い**」ことがある。読めなかったセルは bed が NaN になり、
  ソルバは NaN に触れる面を全部閉じる＝**未取得タイルの矩形の辺で水が止まる**（＝報告された「直線」）。
  残った穴は `basinVoid` に数え、**パネルが件数を印字する**。
- **年・日付を選べるレイヤー**（「年を変えることに意味があるものは全部」）:
  | レイヤー | 期間 | どこから |
  |---|---|---|
  | 世界銀行系 67本 | 1990→ 全系列 | `wbSeries()`（#R266） |
  | 年降水量 | 平年値 / 1981–2020 | `data/precip-year.json`（#R266） |
  | ケッペン気候 | 4期 | `window.KOPPEN_PERIODS` |
  | 米大統領選挙 | 全回 | `data/us-elections.json` |
  | 海流 | 平均＋12か月 | `ocean-currents-months.bin.gz` |
  | 気温／SST／積雪／AOD／降水 | 日付・月 | `layerDates` + `_refreshLegendDates()` |
  | 歴史的国境 | 年 | `cshapes` + `IntMapTime` |
  | **GIBS 科学ラスタ6本** | **実測した期間** | **`data/gibs-range.json`（#R268）** |
  | **夜間光** | **2012 / 2016** | **`NIGHTSAT_EPOCHS`（#R268）** |
  | **土地被覆 ESA** | **2020 / 2021** | **`WC_EPOCHS`（#R268）** |
  | **人口密度 1km グリッド** | **2000 / 2005 / 2010 / 2015 / 2020** | **`POPGRID_EPOCHS`（#R268 追記）** |
  | 1人当たりGDP・合計特殊出生率・人口・平均寿命・軍事費 | マスタークロックの年 | `js/time-countries.js`（#R94/#R200） |
  | **HDI（UNDP）** | **1990–2022（UNDP 自身の年次系列）** | **`data/hdi-series.json`（#R270）** |
  | 民主主義指数（EIU）・汚職腐敗指標（TI CPI） | **公開系列が無い／ライセンスが許さない＝版を明示するだけ** | #R94 は持っていない年のラベルを貼らない |
  ⚠ **#R268 は HDI を「年系列が無い」に分類し、それは事実の誤りだった**（UNDP は 33 列の完全な時系列を
  公開しており、app はその1列を持っていた）。⚠ **そして #R270 以降、これらは全部「凡例の年セレクタ」から
  も変えられる**——`window._legendClockYear` がマスタークロックを読み書きする（2つ目の時計は作らない）。
  ⚠ **GIBS の期間は測る**（`scripts/probe-gibs-range.mjs`）。範囲外は 404・範囲内は 200 なので**実リクエスト
  の二分探索**で求め、`data/gibs-range.json` に書く。捏造した範囲のピッカーは**空の海を「データ」と称する**。
  実測でわかったこと: `MODIS_Terra_NDVI_8Day` は**ローリング窓**（2025-02-18〜）、
  `AMSRU2_Soil_Moisture_SCA_Day` は **2025-09-01 で終了**（＝既定の「今日−2日」では何も描いていなかった）。
  8日合成は**期間の開始日にしか配信されない**ので、日付は期間へスナップする。
- **気象・災害警報のフィードは9つ**（`js/world-packs.js`）:
  | 国・地域 | 出典 | 形 |
  |---|---|---|
  | 日本 | 気象庁 **`…/bosai/warning/data/r8/map.json`**（#R269） | 都道府県ポリゴン（**地方 → 市区町村**の3段でタップ） |
  | 米国 | NWS | 警報ポリゴン（州でまとめる） |
  | カナダ | ECCC（直接） | 警報ポリゴン（州でまとめる） |
  | **ブラジル** | **INMET（直接・ACAO は当方 origin）** | **警報ポリゴン（州でまとめる）** |
  | 欧州37か国 | MeteoAlarm（relay 経由・要約） | 国を塗り、タップで地域別 |
  | 中国 | 中国気象局（relay 経由） | 国を塗り、タップで省別 |
  | **オーストラリア** | **BoM（直接）** | **国を塗り、タップで州別** |
  | **香港** | **香港天文台（直接）** | **国を塗り、タップで信号別** |
  | その他 | GDACS | 事象の点＋国の塗り |
  ⚠ **候補は実際に叩いてから足す**。今回 NZ MetService（ACAO が他ホスト）・インド IMD（401「IP を
  whitelist しろ」）・アルゼンチン SMN（503）・METMalaysia／SAWS／CONAGUA／BMKG（401/404/500）は
  **読めないことを確認して入れなかった**。**WMO SWIC**（`severeweather.wmo.int` の GeoServer WFS）は
  51機関 4,028件のポリゴンを本当に返すが **35 MB（無フィルタでは 420 MB）・`ACAO: null`** で、
  60秒周期には載らない——定期ジョブで要約を持つ形にすれば載る（**未実装**）。
  ⚠ 欧州37機関は**毎tick数か国ずつ**取り切る（従来は4か国だけ取得し、残り33はタップ待ちだった）。
  ⚠ **ライブ取得はすべて `cache:'no-store'`**。JMA の `area.json`（区域コードと名前の定義）と県境
  ジオメトリは年単位でしか変わらないので除く。パネルは**最終取得時刻**を印字する。
- ⚠ **(#R269) 気象庁のコード表は「気象庁のもの」を置く。** `JMA_CODE` は コード → 要素 ＋ **レベル**
  （20 注意報 / 30 警報 / 40 危険警報 / 50 特別警報）で、これは気象庁自身の警報ページが持っている
  オブジェクトを読み出したもの。階級は `jmaTier(level)` が**レベルだけ**から決める——**コードの範囲**で
  決めていた旧実装は、注意報を軒並み赤い警報として塗っていた。⚠ **指定河川洪水予報の表（20/21/22 →
  氾濫注意報…）は入れない**——濃霧/乾燥/なだれと衝突し、配信ファイルも別。
- ⚠ **(#R269) 現在の状態は「官署ごとの最新1件」。** r8 は速報の一覧（実測 287件・58官署）で、全部を
  足すと後の速報が解除した警報が復活する。落とした分は `jmaSuperseded` に数える。最新速報が
  `JMA_MAX_AGE_H`（72時間）より古ければ `loadJMA` は **throw** し、「発表中」としては出さない。
- ⚠ **(#R269) 全フィードが自分の時計を持つ。** `seenAt(feed, iso)` が**そのペイロードの中で一番新しい
  時刻**を記録し、パネルが機関ごとに年齢を印字する。**止まったフィードは失敗したフィードではない**
  ——旧 `…/data/warning/map.json` は 83 日間 200・正しい JSON・期待どおりの形で答え続けていた。
  黄色くなる閾値は「最近か」ではなく「**1週間何も出していないか**」（多日間の警報は古びていない）。
  ⚠ CMA の `2026/08/19 17:22` は `Date.parse` が NaN を返すので `split('/').join('-')` で正規化する。
- **タップは `grouped()` の3段**（admin-1 → sub → 市区町村）。すべてのフィードがこの1本を通る＝
  「どの国でも」が自動的に満たされる。⚠ **markup にも予算がある**: 平坦な一覧が 111,000 文字、素朴な
  3段入れ子が 278,000 文字、**クラス化＋「災害の組み合わせが同じ市区町村を1行にまとめる」で 28,332 文字**
  （同じ警報集合・同じ情報量で測定）。
- **凡例の名前は「表」で渡す**。`window._registerLayerOpacity(id, names, …)` の `names` は
  `LA(...)` の配列である。**文字列を渡すと `names[1]` は2文字目になり**、言語ごとに1文字ずつ選ばれる
  （年降水量の凡例が「降」だった実例）。受け側は文字列を「全言語で同じ1つの名前」として正規化する。
- **宇宙基地・地上局**: 同梱スナップショット `data/osm-space.json`（**14,965件**）と視野内 Overpass を
  **OSM id で合成**する（置き換えない。実測で**ライブ 100 件がスナップショット 102 件を上書き**していた）。
  タグ集合に `communication:satellite` を追加（全球 1,861件・うち 960件は `man_made=antenna`）。
  外交公館は `data/osm-diplo.json`（17,438件）。
- **宗教・言語のポップアップ**は棒グラフ＋**データの年**（`rec.y`＝Factbook の「(2011 est.)」から抽出。
  宗教 186/202・言語 90/196。**年が書かれていない出典には年を書かない**）。
  ⚠ **旧ユーゴの言語は `sh` にまとめない**——出典（Factbook）が Bosnian / Serbian / Croatian を別々の
  行と別々の割合で書いているのだから、地図がまとめてはいけない（#R266 の英国の規則の逆側）。
  `sh` は原文が「Serbo-Croat(ian)」と書いている行にだけ残る。**塗りは `LANG_ONE_COLOUR` で1色**。
  ⚠ `Intl.DisplayNames` は `sh` を「セルビア語 (ラテン文字)」、`cnr` を「セルビア語 (モンテネグロ)」と
  訳し、`ff` `rar` `gil` `niu` `bi` `na` `pau` `mh` `tvl` `tpi` `kl` `dz` `crp` は**全言語で生コードを返す**
  （Chromium は CLDR の modern 部分集合しか積んでいない）。これらは `LANG_FIX` が自分で名前を持つ。
- **GDP成長率は発散配色**（0 = 白・負 = 赤・正 = 青・0について対称）。検査は配色ではなく**性質**を見る。
- **年降水量**: ホバー地点の数値（`valueAt` は絵ではなく `data/precip-mm.png` の値格子を読む）を年つきで
  座標行に出す。タイルのサムネイルは**実スクリーンショット** `preview_precip.png`
  （作り直しは `node scripts/shot-layer-preview.mjs preview_precip.png`）。
- **カテゴリの移動（#R268）**: `wbforest` → 地形・土地被覆、`wbagri` と `gxsoil` → 農業、`wbpm25` → 気候・大気。
  #R233 の7行と #R254 の一覧は不動。⚠ **同じ id を2つの群に書いてはならない**（`order.push` は移動なので
  後の1つにしか出ない）——`tests/r268-checks` が検査する。
...
- **レイヤー欄の分類**（`reorganizeLayerPanel()` の `GROUPS`。`GROUPS` に無い行は末尾の
  ...
  - ⚠⚠ (#R255) **カテゴリが4つ増え、「地政学・防衛」は2つに割れた**——「政治、軍事、医療・衛生、
    IT・テックレイヤーカテゴリを追加し、レイヤーの再編や追加を行うように。それぞれのレイヤーカテゴリの
    名前は任せる。」（再編の範囲は確認済み＝**任せる**）。9言語すべてに名前がある。

    | キー | 名前（EN / JA） | 中身 |
    |---|---|---|
    | `lyrGrpPolitics` | Politics & governance / 政治・統治 | `uselect` `eu` `wbwomparl` `osmdiplo` |
    | `lyrGrpSecurity` | Defense & security / 軍事・安全保障 | `milSpend` `milSpendGDP` `nato` `ukrfront` `wbmilgdp` `wbmilppl` `wbhomicide` `osmmil` |
    | `lyrGrpHealth` | Health & sanitation / 医療・衛生 | 世界銀行の保健指標17件 ＋ `osmhealth` |
    | `lyrGrpTech` | Technology & infrastructure / IT・技術インフラ | `dc` `subcables` `rail` ＋ デジタル指標7件 ＋ `osmtelecom` |

    - **`lyrGrpGeoPol` は空になった**（キーは9言語に残す——保存済みセッションや共有リンクが名指しできる。
      行が0のグループは描画されない）。
    - **`subcables` は「海洋・船舶」から移った**。海底ケーブルは海の下にあるだけで、読み手が探すのは
      技術基盤の棚である。⚠ **1つの id は1つのグループにしか書けない**——`order.push` は要素を
      **移動**させるので、2箇所に書くと**最後のグループにしか出ない**（tests/r255 ⑧ が全グループ横断で
      重複を検査する）。
    - **`rail`（鉄道ネットワーク層）は IT・技術インフラへ**。建設されたインフラだからで、
      基本表示の「鉄道」トグル（`cb-rail2`）は別物として上に残る。
    - ⚠⚠ **`dem`（民主主義指数）・`cpi`（汚職）・`lifeexp`（平均寿命）は人口・経済に残した**。
      政治／医療に見えるが、#R233 は**この7つ**を名指しで人口・経済と決めた明示の指示であり、
      今回の「再編は任せる」はそれを黙って覆す理由にはならない。移すなら一言あればよい。
    - **新規レイヤーは「国別に色を塗るもの」ではない4つの点レイヤー**（`js/osm-facilities.js`）——
      「単に国別で色分けする奴だけじゃなくて、ガチな奴もしっかりたくさん追加して。」
      外交公館・軍事施設・医療機関・通信基盤を、表示範囲ごとに Overpass から実測取得し、
      クリックでそのオブジェクトのタグを出す（ODbL・出典リンクつき・推定なし）。
      行 id の接頭辞 **`fac-dl-`** を `rowFor()` に追加。

  - ⚠⚠ (#R261) **「Others」と「Beta」が名指しで許可され、棚が4つ増えた**——「追加すべきと思う
    レイヤーカテゴリはありますか？あれば作り、Others, Betaも含め既存レイヤーの再編のほか、新レイヤー
    （国単位で塗るだけのやつじゃなくて、モノホンのやつ。）全部任せる。」
    #R255 と #R258 が「その権限は無い」と書いて手を付けなかったのは**この2つ**であり、今回の指示は
    その2つを名指ししている。#R233 の 人口・経済 7件と #R254 のエネルギー構成の昇格は**そのまま**。

    | キー | 名前（EN / JA） | 中身 |
    |---|---|---|
    | `lyrGrpEconomy` | Economy & trade / 経済・貿易 | `trade` `industry` ＋ 経済指標12件 |
    | `lyrGrpSociety` | Society & education / 社会・教育 | 社会・教育指標14件 ＋ `osmedu` ＋ 宗教・言語分布 |
    | `lyrGrpTransport` | Transport & mobility / 交通・輸送 | `planes` `rail` `ships` `oxrail` `oxsea` `osmair` `osmport` |
    | `lyrGrpAgri` | Agriculture & food / 農業・食料 | `crops` `wbagremp` |

    - **「Others」33行はすべて世界銀行指標だった**（インフレの隣に識字率の隣に難民の隣に再生可能電力）。
      カテゴリではなく1つの表の尻尾なので、経済／社会／エネルギー／気候（CO₂総量）／農業に家族ごとに
      分けた。順序は #R254 が並べたまま。**`lyrGrpOthersReal` はキーを残して空**（#R255 が
      `lyrGrpGeoPol` にした扱い。行が0のグループは描画されない）。
    - **「Beta」は 40 → 26**。昇格したのは**完成した world-packs 層**（貿易フロー・海流・潮汐・作物・
      産業ウェブ・気象警報——いずれも独自のパネル・凡例・タイムマシン配線・出典を持ち、**指示で
      降格されたことは一度も無い**）と、単に棚が無かったもの（船舶・ダム・3D建物・OpenRailwayMap・
      OpenSeaMap・医薬品拠点・宗教/言語分布）。
    - ⚠⚠ **#R40 が「指示により降格」と記録している GIBS 8枚と ECMWF 一族は1つも昇格していない。**
      過去の指示が失効したと勝手に決めるのは、このファイルが2度警告している失敗そのもの。
    - ⚠ **`rowFor()` は `ox-` 接頭辞を知らなかった**。OpenRailwayMap / OpenSeaMap を交通へ移しても
      **何も起きず**、棚は7行のはずが5行で2つは Beta のままだった（実測）。id を行に変える表は
      新しい一族が忘れられる場所なので、tests/r261 ⑩ が検査する。
    - `planes`（実況航空機）は「海洋・船舶」から交通へ、`rail`（鉄道ネットワーク層）は
      IT・技術インフラから交通へ。`subcables` は #R255 の判断どおり IT に残る。
- ⚠⚠⚠ (#R261) **貿易フローの矢印は「軸を切る長さ＝画面の画素」「頭の向き＝地理の方位」という2つの
  空間を持っていた。** `icon-rotation-alignment:'map'` は「地面に沿って」ではなく「**地図の北から
  測って**」で、その北は**ビューポート全体で1つのスカラ**である。球面投影では中心経線を離れると
  子午線が画面上で収束するので、両者は格子収束の分だけずれる（実測 globe・z2.2 中心170°E で
  **26.3°**、z1.4 中心100°E で **130.5°**。45.5px の頭を 26.3° 傾けると底辺は 13px の軸から
  **20.7px** 離れる＝触れていない）。角度も `trimEnd` が切る長さを測った**投影空間**で読み、層は
  `'viewport'`（`icon-rotate` がそのまま画面角度になる空間）。**頭と軸は構成上同一直線**で、
  メルカトルでも球面でも、どのズーム・方位・傾きでも成立する。実測 **誤差 ≤0.02°・隙間 ≤0.55px**。
- (#R261/**#R264**) **データセンター層の「表示範囲の集計」**。件数（収録／OSM の内訳）・**公表容量は
  必ず「N件／全M件」と併記**（実測 318件のうち公表は2件で 3.20 GW。裸の合計は総量だと誤読される）・
  分類別内訳（そのままフィルタ）・容量順の上位（クリックで飛ぶ）。⚠ **推定は1つもしない。**
  `IntMapDataCenters.stats()` が同じ数字を返すので、Atlas とテストは再計算ではなく同じ値を読む。
  ⚠⚠ **#R264 で浮遊ウィンドウ `#dc-panel` をやめ、レイヤー自身の凡例ブロックの中（`.dc-sum`）に
  描くようにした**——#R261 は `.tool-panel` として浮かせたので、点をクリックすると**詳細カードと合わせて
  2つ**が地図の上に重なった（「ポップアップ二つあるのを辞めろ」）。**浮くべきなのは詳細カードだけ**
  （指で触れた点についての答えなので、その点の横に置く意味がある）。集計は**レイヤーについての答え**
  なので、レイヤーの操作が並ぶ場所＝`_registerLayerOpacity('dc2',…)` が作る凡例（色キーと分類スイッチが
  既に居る箱）に入る。宿主は**消費側（`js/layer-packs.js`）が渡す**（`mountSummary(el)`／
  `unmountSummary()`）——行を作るのは消費側なので、モジュール側でセレクタを推測しない。
  ⚠ 言語切替やパネル再編で行は作り直されるので、**ON のたびに mount し直す**。
- (**#R264**) **粗い段の精細化は、その段の一番下の段でだけ発火していなかった。** 梯子の最初の倍率は
  **3**なので、その段の1セルはちょうど `3 × 細かい標本間隔` ——そして発火条件は
  `L > REFINE_MIN_CELLS(3) × spacing` の**厳密な大なり**だった。**等しい長さは通らない**＝一番よく使わ
  れる段の脚が構成上ぜんぶ生のまま描かれていた。実測（本番ビルド・実DEM 4本）: 琵琶湖 280 m・
  デスバレー 277 m（自分の段では 1.0 セル、**最細標本では 12 セル**）。⚠⚠ そして `coarseLegs` は
  **4本とも 0** だった——**精細化が諦める閾値と同じ定数で数えていた**ので、諦めた脚を報告できるはずが
  なかった。定数を2つに分けた: `REFINE_MIN_CELLS=1.5`（**弦かどうか**＝細かい格子の1歩は √2 まで）と
  `COARSE_REPORT_CELLS=3`（**目に見える継ぎ目かどうか**＝トレースの**最細標本**に対して数える）。
  実測（修正後）: 琵琶湖 280→**176 m**・デスバレー 277→**182 m**（最細標本で 12→**7.5/7.9 セル**）、
  パンノニア 163 m・西シベリア 112 m は不変、**4本とも `coarseLegs` 0・decline 0**。
  ⚠ **`nodata` は地面の事実ではなくキャッシュの事実だった**——精細化の窓が増えると
  `js/map-readout.js` の `_demCacheTrim` が**後から来る広い窓のタイルを先に追い出す**。最初の版は
  パンノニア／西シベリアで decline 0→**5件ずつ**、最長脚が 163→**3,114 m**・112→**753 m** と**悪化**した。
  → `floodWindow` が null を返したら**その窓自身の footprint（5×5 格子）を温めて1回だけ再試行**する
  （`refineCrossing` は async になった）。それでも駄目なら本物の欠落として数えて印字する。
  ⚠ `_dbgTrace().straight` は**報告された症状そのもの**（方位が 3° 以内で続く脚の連なり＝直線区間）を
  測る。**その区間の起伏 `reliefM` も返す**——平らな湖面では最小上昇経路は**定義上まっすぐ**（どの向きも
  同じコストなら最短が勝つ）ので、起伏の無い直線は欠陥ではなく水面である。
- (#R261) **地形編集・水流**: ⑴ **粗い段（3×/9×/27×）が返す横断は「決定」であって「描画」ではない**——
  各脚を細かい格子上の**最小上昇経路**（Dijkstra、回廊は粗いセル1つ分）で歩き直す `refineCrossing()`。
  実測 琵琶湖の最長脚 **3,563 m → 297 m**（自分の段では 1.4 セルだが、**描かれる側の最細標本では
  150 セル**——#R250「見るべきは比」）。⚠ 断念は `refineDecline{span,nodata,unreachable}` として印字。
  ⚠ 横断の**先**のタイルを `warmCrossing()` で温めないと `floodWindow` が null を返して黙って諦める
  （実測 `nodata` 36件）。⑵ **`build()` は `sculpt` と undo スナップショットを新しい格子へ再標本化する**
  （`regridField()`）——作業矩形の外に水を置くと `rebuildAround`→`build()` で**利用者が彫った地形だけ**が
  消えていた。実測 矩形を 1/3 東へずらす再構築で **436セル・undo 24段が生存**（従来 0）。運んだ量は
  `G.carriedEdits` として印字。⑶ **`cont`/`rate` は水源自身の属性**——注水は `sources[last]` だけを
  太らせていたので、2つ目を置くと1つ目が黙って止まっていた。継続は緑＋輪（`tw-src-ring`）、1回きりは
  青い点。▶ は継続の水源が無ければ無効で、理由を title に書く。
...
- **携帯が余分に待たないもの (#R218)**：
  - **作物レイヤーは「場所」を鍵にする**。要求 bbox をビューポートそのものにすると、どんな小さなパンでも
    ...
    - ⚠⚠⚠ **(#R255) 上の2つは症状を直していた。形そのものが原因だった**——「移動やズームですぐに
      描画がずれたり地図が**黒に**おかしくなる。」 実測（Playwright・実クロミウム）: 車輪でズームインした
      直後の画面の平均輝度 **8.3**（＝ほぼ真っ黒）、10秒後にようやく正常。世界視点から z6 のインドへ跳ぶと
      画像ソースの `coordinates` は **[[−180,83]…[180,−58]]＝全世界**のまま5秒以上、2048px の世界ラスタが
      12°の視野に約50pxのブロックで引き伸ばされる（`x/crop-4-back.png`）。
      **1枚の image ソースを視野ごとに取り直す限り、移動から到着までの間はいつも「地理的には正しく縮尺だけ
      違う絵」が出る。** 世界セルを都市まで拡大すれば1画素が画面を覆い、その画素がランプの暗端
      （`#03230f`＝rgb 3/35/15）なら画面は黒い。
      **だから raster タイルソースにした**（`imapcrop://{z}/{x}/{y}` プロトコル。`js/sat-proto.js` と同じ
      `scene.addProtocol` 契約）。レンダラが今いるズームのタイルを要求し、子が届くまでだけ親を出す——
      z0 のタイルを z14 に広げることはない。`maxzoom:4` は **GAEZ の5分メッシュ（約9km）が決める**
      （512px タイルの z4 ＝ 0.044°/px ＝ 2.6分角で、既にデータの倍細かい）。
      再彩色済み PNG をタイル単位でキャッシュし、同じタイルへの同時要求は1本にまとめる。
      実測: 設定→パン→戻る→ズームインで FAO へのリクエスト **98 → 8**（distinct 11、戻りとズームは 0）、
      同じ操作の平均輝度は **23〜29 で安定**（黒フレーム無し）。
      ⚠ `_cellBox` / `_dirty` / `moveend` ごとの再取得は**この形と一緒に消えた**。
      ⚠⚠⚠ (#R258) **「激しく点滅する」の正体は、このレイヤーが自分を restyle する閉ループだった。**
      実測: カメラを完全に止めたまま **10 秒で remove/addSource/add が 88 回**、`styledata` は
      レイヤー ON で **3 秒に 28 回**・OFF で **0 回**。`paint(true)` がソースを消して足し直す →
      レンダラが `styledata` を撃つ → `onRestyle` が `drawKey=''` にして `paint(true)` → …。
      `raster-fade-duration:180` なので絵はフェードインを完了する前に毎回壊れる。他のパックが同じ
      `styledata` で無事なのは、ハンドラが `ensureLayers()` を呼び**既にあればスタイルに触らずに返る**から。
      #R72 の修理（基図を替えるとレイヤーが落ちる）はハンドラの存在理由なので残し、**本当にレイヤーが
      消えているときだけ**作り直す。実測 28 → **0**。凡例の長い注記は `noteBlock()` で `<details>` に畳んだ。
