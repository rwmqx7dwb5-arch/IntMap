...
- **2つの翻訳テーブル**: `ui`（289 のキー付き文字列）と `inline`（2,061 の呼び出し側インライン文字列を**英語原文をキーに**引く）。最初の5言語は inline を持たず、翻訳が `L(en,jp,de,ru,es)` の**引数**として call site にある。⚠ **「n/a (positional)」は「完了」の意味ではない**——引数を5つ渡していない呼び出しは、その先の言語で英語になる。
...
js/
  ...
  theme-sky.js                      (#R199 で `js/app-body.js` から233行分離／中身は #R99–#R196)
                                    ...
                                    **常に true** なので、Map ベースマップの地球には最初から大気が無かった。
                                    ...
                                    ⚠ **`atmosphere-blend` のランプは中間が平坦になった。** maplibre は内部で
                                    このプロパティに `projectionTransition`（＝globeness、z12までに0）を掛けるので、
                                    ランプ自身の減衰は**二重**で、画面上の大気を z4→z11 で 43.7→23.9 と半減させていた
                                    （「ある程度までズームインすると途端に見えなくなってしまう」）。
                                    z0 の値（衛星 0.55／暗い地図 0.80／明るい地図 0.15＝#R187/#R205 の実測）は不変。
