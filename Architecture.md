...
そのターンに渡すのは**中核 9 ツールとその schema**（約 7 千文字）と、**能力 ID だけの索引**
（`CAPS.index()`。カテゴリ別・撤去済みを除く全件・約 2.5 千文字）。索引は**レジストリから導出**するので、
...
- 地方区分の有効期間は OHM の終了日を含まない。年月精度の端は期間境界へ正規化し、原表記と精度を `dates` に別途保持し、選択時の補足に出典の原表記を表示する。生成時の既定下限は時計の正本から導出するが、未知開始の既存表示境界は広げない。詳しくは `docs/MAP-LAYERS.md` §7.7。
