  ...
  **レイヤーカタログ（約129件）＋操作カタログ（約140件、名前+#id）＋モジュールカタログ（約20件）**をシステムプロンプトで渡すため正確に指定可能。
  ...
  - **#R83 バッチ: ワークスペース自動タイル・Atlas経路/SV/各種シミュ・比較着色・プレビュー・自動ON整理**（詳細は DEV-NOTES R83）:
    ...
    **視点を機体そのものに置き**、方位/ピッチ/ロールを機体軸から直接算出。`setMaxPitch(179)`+`setCenterClampedToGround(false)`で**宙返り/急上昇/背面でも垂直を越えて機首を追い**（ピッチ0.5–179°実測）、
    ...
    地形は`_terrRead`でDEM未読込を判定し、開始/リセットは初回の確実な地形読取まで待って安全高度へ整定（0m誤判定の幻の山/「読込即墜落」を解消）。機体差5種（セスナ172は実C172係数、他はスケール）。
    UI+キー(F/G/Space/C/V/P/R・1-5)、dispatch `flightSim`。**未実装（別フェーズ）**: 専用3D描画・高精度DEM/滑走路/ILS・気象風・
    機体システム(FBW/FADEC等)・ATC/AI機・音響・故障・実測値検証・通常飛行域外(深いスピン)の非線形空力＝描画は当面MapLibre（専用3Dシーンは別フェーズ。視点点カメラ自体は#R95で実現済み））。⑫**レイヤープレビュー**: `setView` を
  ...
  - **#R94b タイムマシン追随: 比較(Compare)の同期＋忠実な旧国家（ソ連・ユーゴ・チェコスロバキア…）**（詳細は DEV-NOTES R94b）:
    ...
    当時の**国境**はクロックが駆動する「過去の国境」レイヤー（aourednik。1960→1994の間欠なので1960スナップがソ連期を被覆、1994以降は後継国）。年移動は6月中旬で判定＝1991=ユーゴ／2000=セルビア・モンテネグロ／2007+=全て分離。
