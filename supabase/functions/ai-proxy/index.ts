...
const TASK_MAX_OUTPUT: Record<string, number> = {
  ...
  map_report: 3200,
  analysis: 2400,
  analysis_structured: 3400,   // (#R339) the SAME answer as `analysis`, plus the claims, their metrics and the evidence ids that make each figure checkable. The prose is not longer; the structure around it is what costs.
  free_text: 1800,
  json_extract: 1200,
  brief: 1800,
...
const TASK_REASONING: Record<string, string> = {
  atlas_plan: "medium",
  analysis: "medium",
  analysis_structured: "medium",   // (#R339) same bottleneck as `analysis`; the schema does not make the thinking easier
  map_report: "low",
  free_text: "low",
  json_extract: "low",
  brief: "low",
...
const JSON_TASKS = new Set(["map_report", "analysis_structured", "json_extract", "geo_verify", "geo_resolve", "research_map", "vision_read"]);   /* (#R156) vision_read returns a strict JSON object (contentClass/answer/checks/places) · (#R339) analysis_structured returns the AnswerEnvelope */
...
/* ══ (#R339) THE ANSWER ENVELOPE — the shape an ANALYSIS must arrive in ═══════════════════════════
...
Deno.serve(async (req) => {
 try {
  ...
  const responseSchema = task === "map_report" ? MAP_REPORT_SCHEMA
    : task === "analysis_structured" ? ANSWER_SCHEMA   // (#R339) server-owned, mirrored by js/atlas-answer-contract.js
  ...
  try {
    ...
    if (provider === "openai") {
      ...
      if (effortHint === "high" && (task === "atlas_plan" || task === "analysis" || task === "analysis_structured" || task === "vision_read")) effort = "high";   // (#R117/#R156/#R339) complexity hint (vision reading small text + maths earns "high")
      try {
    ...
    // (#R339) 5a) A structured answer that will not parse is a TYPED failure, refunded like any
