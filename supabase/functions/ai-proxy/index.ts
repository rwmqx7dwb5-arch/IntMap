...
const cors = {
  ...
  /* (#R315) x-intmap-turn — the turn key. It is a HEADER because the quota is consumed before the
...
/* (#R315) ONE USER TURN = ONE USE. Atlas finishes one request with up to three calls (planner +
...
Deno.serve(async (req) => {
 try {
  ...
  /* (#R315) The turn key travels in a HEADER, not in the JSON body, because the body has not been
  ...
  /* ⚠ (#R315) A REFUND RELEASES THE CHARGE **AND** THE TURN. Refunding the use while leaving the
  ...
  try {
    ...
    return json({
      ...
      /* (#R315) whether THIS call consumed a use. The UI shows the count honestly instead of
