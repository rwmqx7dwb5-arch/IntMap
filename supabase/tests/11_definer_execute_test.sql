...
--    privileges, so such a function must stay executable — is_admin() today).
...
select is(
  ...
  '',
  'no callable SECURITY DEFINER function in public is executable by anon unless an anon-facing RLS policy calls it');
