...
`refresh-news` / `routing-relay` / `sv-cov` / `volcano-feed` / `who-don`）。16 本すべてが
...
`news-ingest.js`・`volcano-parse.js`・`who-don-extract.js`）で、import した関数の中に CLI がバンドルする。`[functions._shared]` を書いてはならない。
