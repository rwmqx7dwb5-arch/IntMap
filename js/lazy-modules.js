...
export function makeLazyModules(HOST) {
  return (function () {
    ...
    const PUBLISHES = {
      ...
      analysisEdu: '__imAnalysisEdu', warLayer: '__imWarFronts',   waves: 'IntMapWaves',   /* (#R572) the wave layer's BODY — its Layers row (js/data-layers.js) is eager, and this is the renderer, the palette and the forecast plumbing, which a session that never ticks 波 does not download */   /* (#R349) the war layer's BODY — its Layers row (js/war-fronts.js) is eager, this is not */
    ...
    function fetchModule(name) {
      switch (name) {
        ...
        case 'routeUi': return import('./routing-ui.js');   case 'photoGeo': return import('./photo-geo.js');   case 'shakeMap': return import('./shakemap.js');   case 'netHealthLive': return import('./net-health-live.js');   /* (#R565) same line, same reason */   /* (#R527) same line, same reason */
    ...
    function mount(name) {
      ...
      switch (name) {
        ...
        case 'routeUi': window.IntMapRouteUI=window.IntMapModules.routeUi(IM_HOST); return true;   case 'photoGeo': window.IntMapPhotoGeo=window.IntMapModules.photoGeo(IM_HOST); return true;   case 'shakeMap': window.IntMapShakeMap=window.IntMapModules.shakeMap(IM_HOST); return true;   case 'netHealthLive': window.IntMapModules.netHealthLive(IM_HOST); return true;   /* (#R565) publishes window.__imNetHealth from inside its own factory, like warLayer */   /* (#R527) */
        ...
        case 'analysisEdu': window.IntMapModules.analysisEdu(IM_HOST); return true; case 'warLayer': window.IntMapModules.warLayer(IM_HOST); return true;   /* (#R349) */ case 'waves': window.IntMapWaves=window.IntMapModules.waves(IM_HOST); return true;   /* (#R572) */
