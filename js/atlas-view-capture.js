 ...
 */
import { makeViewGround } from './atlas-view-ground.js';   /* (#R583) the measurements that turn a picture into an answerable question — and the sentence that says when it is not one */

export function makeViewCapture(deps) {
  deps = deps || {};
  var GE = deps.GE, L = deps.L, esc = deps.esc, snapshot = deps.snapshot, waitIdle = deps.waitIdle;
  /* (#R583) `overpass` is the ONE optional dep: js/screenshot.js builds this factory for the save
  ...
  /* ══ THE FRAME LEDGER — captureFrame / urls / promptBlock / reset / SENT ═══════════════════════
    ...
    /* ── (#R583) WHAT THE RENDERER PUT ON SCREEN ──────────────────────────────────────────────────
    ...
    /* ── (#R583) WHAT OPENSTREETMAP SAYS IS IN THE FRAME ──────────────────────────────────────────
    ...
    async function captureFrame(a) {
      ...
      /* ⚠⚠⚠ (#R583) ASK THE MAP WHAT IS THERE — the step whose absence produced 「八田フランテ館」.
    ...
    function promptBlock() {
      ...
      take.forEach(function (fr, i) {
        var f = fr.facts, bits = [];
        /* ⚠⚠⚠ (#R583) NOT toFixed(2). Two decimals is 1.1 km square at this latitude, so at the zoom
        ...
        /* ⚠⚠⚠ (#R583) THE GROUND TRUTH FOR THIS FRAME — what the map can actually say is in it, and
