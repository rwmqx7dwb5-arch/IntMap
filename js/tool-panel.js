  ...
  function updateToolPanel(){
    ...
    if(HOST.toolMode==='measure'){
      ...
      body=shapeRow
        ...
        +`<div class="v3d-band"><span class="tp-sub" id="v3d-unit-lbl" style="margin:0;min-width:0;">${_L('Altitude band above sea level','高度の範囲（海抜）','Höhenband über NN','Диапазон высот над уровнем моря','Franja de altitud sobre el nivel del mar')}</span>`
        +`<select id="v3d-unit" class="v3d-unitsel" aria-labelledby="v3d-unit-lbl">${V3D_UNITS.map(u=>`<option value="${u}"${st.unit===u?' selected':''}>${u}</option>`).join('')}</select></div>`
        ...
        +`<div class="v3d-style"><span class="v3d-slbl" id="v3d-op-lbl">${HOST.t('opacity')}</span>`
        +`<input type="range" id="v3d-op" aria-labelledby="v3d-op-lbl" min="0.05" max="0.95" step="0.05" value="${st.opacity}" style="flex:1;min-width:0;accent-color:var(--primary-color);"></div>`
    ...
    /* ⚠ SEC: the circle's id used to be pasted into an onclick STRING; it is a data-attribute now,
       wired the same way as every other control in this panel (see js/app-body.js for the full note). */
    p.querySelectorAll('[data-radiusdel]').forEach(b=>{ b.onclick=()=>window.removeRadiusItem(b.getAttribute('data-radiusdel')); });
