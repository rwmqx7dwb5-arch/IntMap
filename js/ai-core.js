...
window.IntMapModules.aiCore=function(HOST){
  ...
  /* ══ (#R315) WHICH LANGUAGE THE MODEL MUST ANSWER IN — moved here from js/app-body.js ═══════
  ...
  /* (#R315) NOT the daily limit — this one request asked the model more times than a request may.
  ...
  async function aiCallServerFull(prompt, system, imgs, opts){
    ...
    if(opts){
      ...
      /* ══ (#R315) THE TURN KEY — ONE USER REQUEST, ONE USE ══════════════════════════════════════
    ...
    if(r.status===429){ let j=null; try{ j=await r.json(); }catch(_){} if(j&&typeof j.used==='number') aiSetUsage(j.used, j.limit); else { HOST.aiUsage.date=aiToday(); HOST.aiUsage.used=aiDailyLimit(); }
      /* (#R315) two different 429s. "limit" = the day's free uses are gone. "turn_calls" = THIS one
    ...
    try{ window._aiLastCharged=(j&&typeof j.charged==='boolean')?j.charged:null; }catch(_){}   /* (#R315) did THIS call consume a use */
