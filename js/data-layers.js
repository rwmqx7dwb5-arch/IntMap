...
/* ══ ⚠⚠⚠ (#R309) WHAT "Base map & labels" CONTAINS, AS ONE LIST ═══════════════════════════════════
   ...
   the section the panel actually drew (13 rows at the time; nine cb-* + two, since #R468):
   ...
   now — `cb-countries` went to `IntMapHiddenLayerRows` in #R468);
...
/* ⚠⚠ (#R468) TWO ROWS LEFT THIS LIST, IN OPPOSITE DIRECTIONS — see `IntMapHiddenLayerRows` below.
...
/* ══ ⚠⚠⚠ (#R468) ROWS THAT KEEP THEIR CHECKBOX AND LOSE THEIR ROW ═════════════════════════════════
...
/* ══ ⚠⚠⚠ (#R468) 基本表示 IS THREE EXCLUSIVE CHOICES NOW, NOT A LIST OF ELEVEN SWITCHES ═════════════
...
/* (#R40) Spanish layer-group names */
      ...
      try{
        ...
        const GROUPS=[
          ...
          /* ══ ⚠⚠ (#R439) FOUR ECMWF ROWS PROMOTED OUT OF 「その他 (beta)」 ═══════════════════════
             ...
             ⚠⚠ (#R468) …AND THEN CAPE WAS NAMED: 「ベータからはCAPE不安定度レイヤーを気象に昇格。」 The
          ...
          /* ⚠ (#R468) TWO ROWS LEFT THIS SHELF AND NEITHER WENT TO ANOTHER ONE.
          ...
          /* ⚠⚠ (#R468) 「人口密度（国別）を昇格。」 — `pop`, one of the rows #R233 demoted, is back on
        ...
        /* ⚠ (#R468) THE TIME-ZONE ROW LEFT THIS BLOCK — 「基本表示の『タイムゾーン（現在時刻）』レイヤーは、
        ...
        /* ⚠⚠⚠ (#R468) A ROW THE PANEL NEVER DRAWS IS STILL A ROW HERE ═══════════════════════════════
        ...
        /* ══ ⚠⚠ (#R468) 「以下に指定されたレイヤー以外は、『その他N件』と、各カテゴリの中で畳むように」 ══
        ...
        if(otherRows.length){
          ...
          /* ⚠ (#R468) Beta is NOT folded — the reader's category list does not name it, and every row
    ...
    /* ══ ⚠⚠⚠ (#R468) 等高線 IS A SWITCH INSIDE THREE LEGENDS, NOT A ROW OF ITS OWN ═══════════════════
    ...
    /* ⚠ (#R468) ONE listener for both halves: the switches follow the checkbox whoever pressed it
    ...
    /* (#R152) contour DENSITY slider — the layer's own control, in the legend (R16 rule). Dragging rebuilds
       ...
       ⚠ (#R468) It follows the switch into the three host legends, and is hidden while contours are off —
    ...
    function ensureContourDensity(el){ try{
      ...
      /* (#R468) directly under the contour switch it belongs to — not under the HOST layer's opacity row */
    ...
    function toggleLayer(id,on){
      if(on){
        ...
        /* ⚠ (#R468) …EXCEPT 等高線, which is no longer a layer with a row: it is a switch inside the
