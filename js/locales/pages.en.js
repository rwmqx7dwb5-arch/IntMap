...
,
  ...
  sourceUse: {
    "UNESCO World Heritage Centre": "The World Heritage List itself — the 1,273 inscribed properties and the 6,009 component points this layer draws — as the UNESCO World Heritage Centre publishes it. Whether a property is on the List in Danger TODAY is not taken from that same feed: its danger column has not been maintained since 2014 (measured), so that one fact is read from Wikidata and both are credited. Names are shown in the languages UNESCO itself publishes the List in (English, French, Spanish, Russian, Chinese, Japanese); it publishes no German, Korean or Traditional Chinese List, so those readers see the English name.",
    ...
    "WHO Disease Outbreak News": "Every WHO Disease Outbreak News item from 1996 to today — 3,195 of them — as an event layer on the map. The pathogen, the country, the date of the event and WHO’s publication date are WHO’s own structured fields: the country comes from joining WHO’s taxonomy to WHO’s country list for an ISO code, not from matching names, and 3,016 of the 3,195 (94.4 %) resolve to one. The archive is bundled with the app (data/who-don.json.gz, built by scripts/build-who-don.mjs); only the newest items are read by the page straight from WHO’s API, which needs no relay because WHO allows CORS. Case and death counts are the one thing WHO does not hold structured — they exist only in the prose of each report, so supabase/functions/who-don reads them out of it and stores them, and anything it could not read is shown as «not extracted» rather than as zero. The 179 items that are not written about a single country («Yellow fever – Global», «Cholera – Multi-country») are listed but not placed on the map: dropping them on the centroid of a WHO region would claim a precision WHO does not publish.",
    ...
    "ECMWF WAM — ocean wave model (significant wave height, mean direction and period; via Open-Meteo, CC-BY-4.0)": "The sea state on the Waves layer when the ECMWF model is chosen — significant wave height, mean wave direction and mean wave period from ECMWF's WAM ocean wave model. IntMap reads the 0.25° grid straight from Open-Meteo's public mirror on AWS Open Data (no key), three-hourly out to 15 days; the reader switches between this model and NOAA GFS Wave on the map. Licensed CC-BY-4.0.",
    "NOAA NCEP GFS Wave — ocean wave model (significant wave height, mean direction and period; via Open-Meteo)": "The same three quantities — significant wave height, mean wave direction and mean wave period — from NOAA NCEP's GFS Wave model, the second of the two the Waves layer offers. Read as a 0.25° grid from Open-Meteo's public mirror on AWS Open Data (no key), three-hourly out to 16 days. NOAA open data.",
    "Wave-height colour scale — windy.com (the published twelve-stop scale only; no Windy tiles, forecasts or requests)": "Only the colours are taken from here. The wave-height ramp reproduces windy.com's published twelve-stop scale for 0–12 m, so that a reader who already reads the sea state on Windy can read it the same way here. Nothing else of Windy's is used: no tiles, no forecast, no API — not one request ever goes to Windy (their tiles need a token valid for 48 hours, and their terms do not allow third-party use). The wave figures themselves come from ECMWF WAM and NOAA GFS Wave above.",
    ...
    "IODA — Internet Outage Detection and Analysis (Georgia Tech)": "Country- and region-level internet outage alerts from BGP, active probing and a network telescope",
    "RIPE Atlas": "Global network of measurement probes; their live connected / disconnected state",
    "RIPEstat (RIPE NCC)": "Routing statistics per country — how many ASNs and prefixes are visible in RIPE routing data",
    ...
    "OurAirports — airport size class & scheduled service (runway geometry at run time; per-country air connectivity at build time; public domain)": "Two different things read the same public-domain airport register. Runway search and the flight simulator fetch runways.csv / airports.csv in the browser, only when you actually run a search, for the REAL threshold coordinates and lengths of each runway (js/map-extras.js, then cached in IndexedDB). The pandemic simulator reads a bundled derivative instead — data/airports.json, one number per country: the large airports with scheduled service plus a quarter of the medium ones (3,269 airports counted, 1,680.5 in total), which is what weights where an outbreak is carried next. ⚠ THIS IS AIRPORT INFRASTRUCTURE, NOT TRAFFIC. OurAirports publishes a size class and whether an airport has scheduled service; it publishes no routes, no flight frequencies and no passenger numbers, and no other source in IntMap has them either. So the simulator's international spread never calls this a real air-traffic network: it is «how much scheduled airline infrastructure this country has», dated and attributable, and the model says so on screen. Public domain; the bundled file is rebuilt by scripts/build-airports.mjs.",
    "OpenFlights — airline route database (country-pair route counts for the pandemic simulator; snapshot of June 2014, no longer updated; ODbL 1.0)": "The per-country-pair route counts that weight which country the pandemic simulator's outbreak reaches next (data/mobility.json is the derived database, rebuilt from this one). ⚠ IT IS A SNAPSHOT OF JUNE 2014 AND IS NO LONGER UPDATED, and what it counts is ROUTES — not seats, not flight frequencies, not passenger numbers. It is mixed with the distance model rather than used on its own, so an outbreak still reaches countries that have no direct flight between them. ODbL 1.0, share-alike: the derived database is named and carries the same terms.",
    "WHO Global Health Observatory — UHC service coverage index & IHR SPAR (CC BY-NC-SA 3.0 IGO)": "Each country's own capacity to respond, in the pandemic simulator. The UHC service coverage index (SDG 3.8.1) sets how fast care is overwhelmed and therefore the fatality rate; IHR SPAR (health-emergency management capacity) sets how quickly border measures come in and how well a vaccination programme is actually carried out. ⚠ A COUNTRY THAT IS NOT IN THE TABLE FALLS BACK TO AN ESTIMATE FROM GDP PER CAPITA — per country, not for the world at once, so every measured country keeps its measured value. CC BY-NC-SA 3.0 IGO, so this credit is required rather than optional.",
...
"Smithsonian GVP": "Bundled with the app (data/volcanoes_gvp.json + data/volcano-detail.json.gz, built by scripts/build-volcanoes.mjs from the GVP WFS): the whole Holocene catalog with its join key, and behind it every eruption GVP records for the volcanoes in that catalog, with its VEI and dates, the volcano type, landform, tectonic setting and dominant rock type, the geological summary, the photograph, and the population living within 5, 10, 30 and 100 km. Everything the volcano card says about the past is read from this file, offline. The catalog is the Holocene list plus the volcanoes an observatory publishes a current alert level for: Yellowstone, Long Valley, Coso Volcanic Field and Isanotski Peaks are older than the Holocene and come from the GVP Pleistocene catalog, so that a volcano the USGS reports on every day can be drawn.",
    ...
    "historical-basemaps (aourednik)": "Historical borders — the beta overlay layer, and the automatic fallback for the map’s own past-year borders (nearest snapshot) whenever a bundled record fails to load. Below the day-exact bands — CShapes from 1886, OpenHistoricalMap from 1850 to 1885 — this snapshot series is the answer, and it reaches as far as AD 100: thirty-six frames, a century apart over most of the range, with the years upstream holds more finely (1279, 1492, 1530, 1715, 1783) in between. 1815 is one of them — one of the frames that answer the years before 1850.",
    ...
    "令制国 raster — Asukana/Ryoseikoku (CC0 1.0)": "The EXTENT of Japan's classical provinces (令制国), published under CC0 1.0 as 663 web-mercator zoom-10 tiles of 256×256 integer province codes. IntMap VECTORISES that raster and ships the result (scripts/build-hist-kuni.mjs → data/hist-kuni.js); one cell is 0.001373° of longitude, about 112 m at 36° N. WHY IT IS NEEDED: OpenHistoricalMap holds only 53 of the 68 classical provinces — 陸奥, 出羽, 信濃, 越後, 上野, 下野, 美濃, 飛騨, 若狭, 越前, 加賀, 能登, 越中, 佐渡 and 隠岐, the whole of 東山道 and 北陸道, exist as no relation at all; OSM proper holds 53 as well, and the union of the two is 54 of 68. Two datasets do carry all 68, and neither can be redistributed from a public repository: one is non-commercial, the other's licence contradicts itself. So IntMap derives those provinces itself. What ships is 15 units, 6,425 rings and 82,199 vertices, and the provinces OpenHistoricalMap already holds are NOT emitted — every build checks. The derivation is IntMap's; the record is theirs, and a derived work still says whose.",
    "Wikidata (CC0 1.0)": "Where the NAMES of those fifteen provinces come from. An item of the classical-province class (Q860290) carries a coordinate (P625), and which vectorised area that coordinate falls inside decides the name: there is no hand-written table of names and no hand-written table of coordinates anywhere in it. Each province's start and end (P571 / P576) are read as well, so only the provinces in force in the raster's year, 1868, are used. Where no classical province's coordinate falls inside an area, the name is settled by one hop from an item that states which province it succeeded (P1365) — Aomori Prefecture → the 1869 province of 陸奥 → the classical 陸奥. The province names in all nine languages are Wikidata labels too.",
    ...
    "Bundesamt für Strahlenschutz (BfS) — ODL-Messnetz (Datenlizenz Deutschland – Namensnennung – Version 2.0)": "Read live for the measured-radiation layer: the ambient dose equivalent rate H*(10) at the roughly 1,700 probes of Germany's ODL network, published as hourly means in µSv/h and normalised once, server-side, to nSv/h. About a year of history is available, so the layer follows the clock. ⚠ BfS recalculated the whole network on 1 July 2025, so a series that crosses that date carries a step of 14–25 % that is a change of calibration, not of radiation.",
    "原子力規制委員会 — 放射線モニタリング情報 (Nuclear Regulation Authority, Japan; 政府標準利用規約 2.0)": "Read live for the measured-radiation layer: the current reading at the monitoring posts Japan's Nuclear Regulation Authority publishes through RAMIS — about 5,400 distinct stations, in µSv/h, normalised to nSv/h. Fetched through the app's own relay; because the interface returns every sample it holds in the window, only the most recent one per station is kept.",
    "Säteilyturvakeskus (STUK) via Ilmatieteen laitos open data (CC BY 4.0)": "Read live for the measured-radiation layer: H*(10) at the roughly 240 posts of Finland's external radiation monitoring network, delivered as ten-minute means by the Finnish Meteorological Institute's open data and normalised to nSv/h. Its published archive starts in 2010, so this is the network that still answers when the clock is moved far into the past.",
    "US EPA RadNet": "Read for the measured-radiation layer: hourly means in nSv/h from the fixed RadNet stations in the United States. ⚠ RadNet publishes one CSV file per station per year, so the United States is not part of the default single bulk request — the app asks for it in slices of five stations. The collection time is kept exactly as the source wrote it, in local wall-clock time with no zone.",
    "RIVM — Nationaal Meetnet Radioactiviteit (CC0 1.0)": "⚠ This is the annual mean for 2011, not a current reading. For the 151 posts of the Dutch national radioactivity monitoring network, RIVM publishes an annual average dose rate in nSv/h (RIVM's own live display is being rebuilt). IntMap keeps these in a set separate from the current values, and the current-value colour scale does not paint them — so a reading taken twenty minutes ago and an average fifteen years old are never shown as the same thing.",
