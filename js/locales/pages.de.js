...
window.IntMapPageI18N.define('de', {
  ...
  sourceUse: {
    "UNESCO World Heritage Centre": "Die Welterbeliste selbst — die 1.273 eingeschriebenen Stätten und die 6.009 Bestandteile, die diese Ebene zeichnet — so, wie das UNESCO-Welterbezentrum sie veröffentlicht. Ob eine Stätte HEUTE auf der Roten Liste steht, stammt jedoch nicht aus demselben Feed: dessen Gefährdungsspalte wird nachweislich seit 2014 nicht mehr gepflegt, deshalb wird diese eine Angabe bei Wikidata abgefragt und beides ausgewiesen. Namen erscheinen in den Sprachen, in denen die UNESCO die Liste selbst veröffentlicht (Englisch, Französisch, Spanisch, Russisch, Chinesisch, Japanisch); auf Deutsch, Koreanisch und in traditionellem Chinesisch veröffentlicht sie sie nicht — dort steht der englische Name.",
    ...
    "WHO Disease Outbreak News": "Alle WHO Disease Outbreak News von 1996 bis heute — 3.195 Stück — als Ereignisebene der Karte. Erreger, Land, Ereignisdatum und Veröffentlichungsdatum der WHO sind strukturierte Felder der WHO selbst: das Land stammt nicht aus einem Namensabgleich, sondern daraus, dass die Klassifikation (taxon) der WHO mit deren eigener Länderliste zu einem ISO-Code verknüpft wird — 3.016 der 3.195 (94,4 %) lösen sich zu einem Land auf. Das Archiv liegt der App bei (data/who-don.json.gz, erzeugt von scripts/build-who-don.mjs); nur die neuesten Meldungen liest die Seite direkt aus der API der WHO, wofür kein Relais nötig ist, weil die WHO CORS erlaubt. Einzig Fall- und Todeszahlen hält die WHO nicht strukturiert vor — sie stehen nur im Fließtext jeder Meldung, weshalb supabase/functions/who-don sie dort ausliest und speichert; was nicht gelesen werden konnte, erscheint als «nicht extrahiert» und nicht als Null. Die 179 Meldungen, die nicht über ein einzelnes Land geschrieben sind («Yellow fever – Global», «Cholera – Multi-country»), stehen zwar in der Liste, aber nicht auf der Karte: sie auf den Schwerpunkt einer WHO-Region zu setzen, würde eine Genauigkeit vorgeben, die die WHO nicht veröffentlicht.",
    ...
    "ECMWF WAM — ocean wave model (significant wave height, mean direction and period; via Open-Meteo, CC-BY-4.0)": "Der Seegang der Wellen-Ebene, wenn das ECMWF-Modell gewählt ist — signifikante Wellenhöhe, mittlere Wellenrichtung und mittlere Wellenperiode aus dem Ozeanwellenmodell WAM des ECMWF. IntMap liest das 0,25°-Gitter direkt aus dem öffentlichen Spiegel von Open-Meteo auf AWS Open Data (ohne Schlüssel), dreistündlich bis 15 Tage voraus; auf der Karte wird zwischen diesem Modell und NOAA GFS Wave umgeschaltet. Lizenz CC-BY-4.0.",
    "NOAA NCEP GFS Wave — ocean wave model (significant wave height, mean direction and period; via Open-Meteo)": "Dieselben drei Größen — signifikante Wellenhöhe, mittlere Wellenrichtung und mittlere Wellenperiode — aus dem Modell GFS Wave von NOAA NCEP, dem zweiten der beiden Modelle der Wellen-Ebene. Gelesen als 0,25°-Gitter aus dem öffentlichen Spiegel von Open-Meteo auf AWS Open Data (ohne Schlüssel), dreistündlich bis 16 Tage voraus. Offene Daten der NOAA.",
    "Wave-height colour scale — windy.com (the published twelve-stop scale only; no Windy tiles, forecasts or requests)": "Übernommen sind ausschließlich die Farben. Die Wellenhöhen-Skala bildet die von windy.com veröffentlichte zwölfstufige Skala für 0–12 m unverändert nach, damit wer den Seegang schon auf Windy liest, ihn hier genauso liest. Sonst wird nichts von Windy verwendet: keine Kacheln, keine Vorhersage, keine API — es geht keine einzige Anfrage an Windy (ihre Kacheln verlangen ein 48 Stunden gültiges Token, und ihre Bedingungen erlauben die Nutzung durch Dritte nicht). Die Wellenwerte selbst stammen von ECMWF WAM und NOAA GFS Wave.",
    ...
    "IODA — Internet Outage Detection and Analysis (Georgia Tech)": "Warnungen zu Internetausfällen auf Landes- und Regionsebene aus BGP, aktiver Messung und einem Netzteleskop",
    "RIPE Atlas": "Weltweites Netz von Messsonden und ihr aktueller Verbindungszustand",
    "RIPEstat (RIPE NCC)": "Routing-Statistik je Land — wie viele ASNs und Präfixe in RIPE-Routingdaten sichtbar sind",
...
'Smithsonian GVP': 'Mit der App ausgeliefert (data/volcanoes_gvp.json und data/volcano-detail.json.gz, von scripts/build-volcanoes.mjs aus dem WFS des GVP erzeugt): der gesamte Holozän-Katalog mit seinem Verknüpfungsschlüssel und dahinter jeder Ausbruch, den das GVP für die Vulkane dieses Katalogs verzeichnet, mit VEI und Datumsangaben, dazu Vulkantyp, Landform, tektonische Lage und vorherrschendes Gestein, die geologische Zusammenfassung, das Foto und die Bevölkerung im Umkreis von 5, 10, 30 und 100 km. Alles, was die Vulkan-Infokarte über die Vergangenheit sagt, wird offline aus dieser Datei gelesen. Der Katalog ist die Holozän-Liste plus die Vulkane, für die ein Observatorium eine aktuelle Warnstufe veröffentlicht: Yellowstone, Long Valley, das Vulkanfeld Coso und die Isanotski Peaks sind älter als das Holozän und stammen aus dem Pleistozän-Katalog des GVP, damit auch ein Vulkan gezeichnet werden kann, über den der USGS jeden Tag berichtet.',
    ...
    'historical-basemaps (aourednik)': 'Historische Grenzen — die Beta-Overlay-Ebene und der automatische Ersatz für die Grenzen vergangener Jahre der Karte selbst (nächstgelegene Momentaufnahme), sobald ein mitgelieferter Datensatz nicht geladen werden kann. Unterhalb der taggenauen Bänder — CShapes ab 1886, OpenHistoricalMap von 1850 bis 1885 — antwortet diese Reihe von Momentaufnahmen, und sie reicht bis ins Jahr 100 n. Chr.: sechsunddreißig Aufnahmen, über den größten Teil des Bereichs im Abstand von hundert Jahren, dazwischen die Jahre, die das Ursprungs-Repository feiner führt (1279, 1492, 1530, 1715, 1783). 1815 ist eine davon — eine der Aufnahmen, die die Jahre vor 1850 beantworten.',
    ...
    'Maddison Project Database 2020 (Bolt & van Zanden)': 'Maßgebliches HISTORISCHES reales BIP (konstante internationale Dollar von 2011) und Bevölkerung ab 1850 für die Zeitmaschine — der Länderreiter, die Choroplethen und der Ländervergleich, wenn man vor die Untergrenze der Weltbank (1960) reist, sowie für aufgelöste Staaten (die frühere UdSSR, Jugoslawien und die Tschechoslowakei sind eigenständige Maddison-Einheiten).',
    ...
    "Bundesamt für Strahlenschutz (BfS) — ODL-Messnetz (Datenlizenz Deutschland – Namensnennung – Version 2.0)": "Wird für die Ebene der gemessenen Strahlung live gelesen: die Umgebungs-Äquivalentdosisleistung H*(10) an den rund 1.700 Sonden des deutschen ODL-Messnetzes, als Stundenmittel in µSv/h veröffentlicht und serverseitig einmal auf nSv/h normiert. Etwa ein Jahr Historie ist abrufbar, die Ebene folgt also der Uhr. ⚠ Das BfS hat das gesamte Netz zum 1. Juli 2025 neu berechnet; eine Reihe über dieses Datum hinweg enthält einen Sprung von 14–25 %, der eine Änderung der Kalibrierung ist und keine der Strahlung.",
    "原子力規制委員会 — 放射線モニタリング情報 (Nuclear Regulation Authority, Japan; 政府標準利用規約 2.0)": "Wird für die Ebene der gemessenen Strahlung live gelesen: der aktuelle Messwert der Messstellen, die Japans Nuclear Regulation Authority über RAMIS veröffentlicht — rund 5.400 einzelne Stationen, in µSv/h, auf nSv/h normiert. Abgerufen über die eigene Relay-Funktion der App; da die Schnittstelle alle Messwerte des Zeitfensters zurückgibt, wird je Station nur der jüngste behalten.",
    "Säteilyturvakeskus (STUK) via Ilmatieteen laitos open data (CC BY 4.0)": "Wird für die Ebene der gemessenen Strahlung live gelesen: H*(10) an den rund 240 Stationen des finnischen Überwachungsnetzes für externe Strahlung, als Zehn-Minuten-Mittel über die offenen Daten des Finnischen Meteorologischen Instituts geliefert und auf nSv/h normiert. Sein veröffentlichtes Archiv beginnt 2010 — dieses Netz antwortet also noch, wenn die Uhr weit in die Vergangenheit gestellt wird.",
    "US EPA RadNet": "Wird für die Ebene der gemessenen Strahlung gelesen: Stundenmittel in nSv/h von den festen RadNet-Stationen in den Vereinigten Staaten. ⚠ RadNet veröffentlicht eine CSV-Datei je Station und Jahr, deshalb gehören die USA nicht zum voreingestellten Sammelabruf — die App holt sie in Blöcken von fünf Stationen. Die Erfassungszeit bleibt genau so, wie die Quelle sie geschrieben hat: als lokale Uhrzeit ohne Zeitzone.",
    "RIVM — Nationaal Meetnet Radioactiviteit (CC0 1.0)": "⚠ Das ist der Jahresmittelwert 2011 und kein aktueller Messwert. Für die 151 Messposten des niederländischen nationalen Messnetzes veröffentlicht das RIVM eine mittlere Jahresdosisleistung in nSv/h (die eigene Live-Anzeige des RIVM wird derzeit neu aufgebaut). IntMap hält diese Werte in einem von den aktuellen Werten getrennten Satz, den die Farbskala der aktuellen Werte nicht einfärbt — damit ein vor zwanzig Minuten gemessener Wert und ein fünfzehn Jahre alter Mittelwert nie als dasselbe erscheinen.",
    "Hong Kong Observatory — ambient gamma radiation (data.gov.hk)": "Wird für die Ebene der gemessenen Strahlung live gelesen: das Tagesmittel der Umgebungs-Gammadosisleistung an den elf Messstationen des Hong Kong Observatory, entnommen aus dem Bulletin des Vortages (µSv/h, auf nSv/h normiert).",
    "SaveEcoBot / data.gov.ua — radiation monitoring (CC BY)": "⚠ In der Quellenliste geführt, aber derzeit steht kein Wert daraus auf der Karte. Die offenen ukrainischen Strahlungsdaten bestehen aus Stationskoordinaten und einem Massenarchiv der Messungen; einen öffentlichen Endpunkt mit aktuellen Werten gibt es nicht (die eigene API von SaveEcoBot verlangt einen Schlüssel). Eine Koordinatenliste ohne Messwerte ist keine Dosiskarte, deshalb bleibt die Ukraine leer statt ausgefüllt — und die Quelle steht hier, damit die Lücke benannt und nicht verschwiegen wird.",
  }
});
