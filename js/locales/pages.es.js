...
window.IntMapPageI18N.define('es', {
  ...
  sourceUse: {
    "UNESCO World Heritage Centre": "La propia Lista del Patrimonio Mundial — los 1273 bienes inscritos y las 6009 partes componentes que dibuja esta capa — tal como la publica el Centro del Patrimonio Mundial de la UNESCO. En cambio, si un bien está HOY en la Lista en Peligro no se toma de ese mismo canal: se ha comprobado que su columna de peligro no se actualiza desde 2014, así que ese único dato se pide a Wikidata y se citan ambas fuentes. Los nombres se muestran en los idiomas en que la propia UNESCO publica la Lista (inglés, francés, español, ruso, chino y japonés); no la publica en alemán, coreano ni chino tradicional, por lo que esos lectores ven el nombre en inglés.",
    ...
    "WHO Disease Outbreak News": "Todas las Disease Outbreak News de la OMS desde 1996 hasta hoy —3.195— como capa de eventos del mapa. El patógeno, el país, la fecha del evento y la fecha de publicación de la OMS son campos estructurados de la propia OMS: el país no sale de una coincidencia de nombres, sino de cruzar la clasificación (taxon) de la OMS con su propia lista de países para obtener un código ISO, y 3.016 de los 3.195 (94,4 %) se resuelven a un país. El archivo histórico va incluido en la aplicación (data/who-don.json.gz, generado por scripts/build-who-don.mjs) y solo lo más reciente lo lee la página directamente de la API de la OMS, sin retransmisión, porque la OMS permite CORS. Lo único que la OMS no guarda estructurado son los casos y las defunciones: están solo en el texto de cada informe, así que supabase/functions/who-don los extrae de ahí y los guarda, y lo que no ha podido leer se muestra como «no extraído», nunca como cero. Los 179 informes que no hablan de un solo país («Yellow fever – Global», «Cholera – Multi-country») aparecen en la lista pero no en el mapa: situarlos en el centroide de una región de la OMS aparentaría una precisión que la OMS no publica.",
    ...
    "ECMWF WAM — ocean wave model (significant wave height, mean direction and period; via Open-Meteo, CC-BY-4.0)": "El estado del mar de la capa Olas cuando se elige el modelo del ECMWF: altura significativa de la ola, dirección media y período medio del modelo de oleaje WAM del ECMWF. IntMap lee la malla de 0,25° directamente del espejo público de Open-Meteo en AWS Open Data (sin clave), cada 3 horas hasta 15 días; en el mapa se alterna entre este modelo y NOAA GFS Wave. Licencia CC-BY-4.0.",
    "NOAA NCEP GFS Wave — ocean wave model (significant wave height, mean direction and period; via Open-Meteo)": "Las mismas tres magnitudes — altura significativa de la ola, dirección media y período medio — del modelo GFS Wave de NOAA NCEP, el segundo de los dos que ofrece la capa Olas. Se lee como malla de 0,25° del espejo público de Open-Meteo en AWS Open Data (sin clave), cada 3 horas hasta 16 días. Datos abiertos de la NOAA.",
    "Wave-height colour scale — windy.com (the published twelve-stop scale only; no Windy tiles, forecasts or requests)": "De aquí solo se toman los colores. La escala de altura de ola reproduce la paleta publicada por windy.com, de doce tramos para 0–12 m, para que quien ya lee el estado del mar en Windy lo lea aquí igual. No se usa nada más de Windy: ni teselas, ni pronóstico, ni API; no se envía ni una sola petición a Windy (sus teselas exigen un token válido 48 horas y sus condiciones no permiten el uso por terceros). Las cifras de las olas vienen del ECMWF WAM y del NOAA GFS Wave anteriores.",
    ...
    "IODA — Internet Outage Detection and Analysis (Georgia Tech)": "Alertas de cortes de internet por país y región a partir de BGP, sondeo activo y un telescopio de red",
    "RIPE Atlas": "Red mundial de sondas de medición y su estado de conexión en vivo",
    "RIPEstat (RIPE NCC)": "Estadísticas de enrutamiento por país: cuántos ASN y prefijos son visibles en los datos de RIPE",
...
'Smithsonian GVP': 'Incluido con la aplicación (data/volcanoes_gvp.json y data/volcano-detail.json.gz, generados por scripts/build-volcanoes.mjs a partir del WFS del GVP): todo el catálogo del Holoceno con su clave de unión y, detrás, todas las erupciones que el GVP registra para los volcanes de ese catálogo, con su VEI y sus fechas, el tipo de volcán, la forma del relieve, el contexto tectónico y la roca dominante, el resumen geológico, la fotografía y la población que vive en un radio de 5, 10, 30 y 100 km. Todo lo que la ficha del volcán dice del pasado se lee de este archivo, sin conexión. El catálogo es la lista del Holoceno más los volcanes para los que un observatorio publica un nivel de alerta actual: Yellowstone, Long Valley, el campo volcánico de Coso y los picos Isanotski son más antiguos que el Holoceno y proceden del catálogo del Pleistoceno del GVP, para que también pueda dibujarse un volcán del que el USGS informa cada día.',
    ...
    'historical-basemaps (aourednik)': 'Fronteras históricas: la capa beta de superposición y el respaldo automático de las fronteras de años pasados del propio mapa (la instantánea más cercana) cuando un registro incluido en la app no se puede cargar. Por debajo de las franjas con precisión de día (CShapes desde 1886 y OpenHistoricalMap de 1850 a 1885), la respuesta es esta serie de instantáneas, que llega hasta el año 100 d. C.: treinta y seis instantáneas, separadas por un siglo en la mayor parte del recorrido, y entre ellas los años que el repositorio de origen guarda con más detalle (1279, 1492, 1530, 1715, 1783). 1815 es una de ellas: una de las instantáneas que responden por los años anteriores a 1850.',
    ...
    "Bundesamt für Strahlenschutz (BfS) — ODL-Messnetz (Datenlizenz Deutschland – Namensnennung – Version 2.0)": "Se lee en directo para la capa de radiación medida: la tasa de equivalente de dosis ambiental H*(10) en las cerca de 1.700 sondas de la red alemana ODL, publicada como medias horarias en µSv/h y normalizada una sola vez, en el servidor, a nSv/h. Hay alrededor de un año de historial, de modo que la capa sigue al reloj. ⚠ El BfS recalculó toda la red el 1 de julio de 2025: una serie que cruce esa fecha tiene un escalón del 14–25 % que es un cambio de calibración, no de la radiación.",
    "原子力規制委員会 — 放射線モニタリング情報 (Nuclear Regulation Authority, Japan; 政府標準利用規約 2.0)": "Se lee en directo para la capa de radiación medida: el valor actual de los puestos de vigilancia que la Autoridad de Regulación Nuclear de Japón publica a través de RAMIS: unas 5.400 estaciones distintas, en µSv/h, normalizadas a nSv/h. Se obtiene mediante el propio relé de la aplicación; como la interfaz devuelve todas las muestras de la ventana, de cada estación solo se conserva la más reciente.",
    "Säteilyturvakeskus (STUK) via Ilmatieteen laitos open data (CC BY 4.0)": "Se lee en directo para la capa de radiación medida: H*(10) en los cerca de 240 puestos de la red finlandesa de vigilancia de la radiación externa, servido como medias de diez minutos por los datos abiertos del Instituto Meteorológico Finlandés y normalizado a nSv/h. Su archivo publicado empieza en 2010, así que es la red que sigue respondiendo cuando el reloj se lleva muy atrás.",
    "US EPA RadNet": "Se lee para la capa de radiación medida: medias horarias en nSv/h de las estaciones fijas de RadNet en Estados Unidos. ⚠ RadNet publica un archivo CSV por estación y año, por lo que Estados Unidos no forma parte de la petición conjunta predeterminada: la aplicación lo pide en bloques de cinco estaciones. La hora de recogida se conserva tal como la escribió la fuente, en hora local y sin zona horaria.",
    "RIVM — Nationaal Meetnet Radioactiviteit (CC0 1.0)": "⚠ Esta es la media anual de 2011, no una lectura actual. Para los 151 puestos de la red nacional neerlandesa de vigilancia radiológica, el RIVM publica una tasa de dosis media anual en nSv/h (su propia visualización en directo está en reconstrucción). IntMap los guarda en un conjunto separado de los valores actuales, y la escala de color de los valores actuales no los pinta: así, una lectura de hace veinte minutos y una media de hace quince años nunca se muestran como lo mismo.",
    "Hong Kong Observatory — ambient gamma radiation (data.gov.hk)": "Se lee en directo para la capa de radiación medida: la media diaria de la tasa de dosis gamma ambiental en las once estaciones de vigilancia del Observatorio de Hong Kong, tomada del boletín del día anterior (µSv/h, normalizada a nSv/h).",
    "SaveEcoBot / data.gov.ua — radiation monitoring (CC BY)": "⚠ Se mantiene en la lista de fuentes, pero por ahora ningún valor suyo aparece en el mapa. Los datos abiertos ucranianos constan de las coordenadas de los puestos y un archivo masivo de mediciones; no existe un punto de acceso público que devuelva valores actuales (la propia API de SaveEcoBot exige una clave). Una lista de coordenadas sin mediciones no es un mapa de dosis, así que Ucrania se deja en blanco en lugar de rellenarla, y la fuente se conserva aquí para que el hueco quede dicho y no oculto.",
  }
});
