...
(function () {
  ...
  let _metric = null;   /* which scale the live layer was built for — see the rebuild note in draw() */
  ...
  async function draw(a) {
    ...
    const STOPS = { cases: [0, 0.02, 0.1, 0.4], deaths: [0, 0.0005, 0.005, 0.02] }[metric];
    ...
    const paint = () => {
      ...
      if (GE().layers.has(LYR) && _metric !== metric) { try { GE().layers.remove(LYR); } catch (_) {} }
    ...
    return { ok: true, html: drawHtml(last, features.length, metric), meta: { day: last.day, metric: metric, countries: features.length, source: SRC, layer: LYR } };
  }
  ...
  function runMeta(r) {
    return { day: r.day, asked: r.asked, preset: r.presetKey, scenario: r.scenario, seed: r.seed,
      ...
      assumed: r.described.filter(d => d.origin !== 'caller').map(d => ({ key: d.key, value: d.value, unit: d.unit, from: d.origin })),
  ...
  function runHtml(r) {
    ...
    const asmp = assumed.length ? '<p style="opacity:.75;font-size:.9em">' + esc(L('Not specified, so taken from the model:', '指定が無かったため、モデルの値を使用:', 'Nicht angegeben, daher aus dem Modell übernommen:', 'Не задано, поэтому взято из модели:', 'No especificado, tomado del modelo:')) + ' '
      + assumed.map(d => esc(d.key) + ' = ' + esc(String(d.value)) + (d.unit ? esc(d.unit) : '') + ' <span style="opacity:.7">[' + esc(d.from) + ']</span>').join(' · ') + '</p>' : '';
  ...
  function drawHtml(r, n, metric) {
    ...
    return esc(L('Day {d} of the run is on the map — {n} countries, shaded by {m}.', '実行の {d} 日目を地図に表示しました（{n} か国・{m} で濃淡）。', 'Tag {d} des Laufs liegt auf der Karte — {n} Länder, eingefärbt nach {m}.', 'День {d} прогона нанесён на карту — {n} стран, оттенок по {m}.', 'El día {d} está en el mapa: {n} países, sombreados por {m}.')
      .replace('{d}', String(r.day)).replace('{n}', String(n)).replace('{m}', what));
  }
