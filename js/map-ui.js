  ...
  window.IntMapLayerSidebar=(function(){
    ...
    function css(){ const st=document.createElement('style');
      ...
      st.textContent=':root{--lsr-w:min(300px,92vw);}'   /* (#R154/#R159/#R160) smaller default width ("もう少し小さく") — 430→380→340→300 */
        ...
        /* (#R468) the gap moved onto the collapsible body so the section can be closed without the
        ...
        /* ══ (#R468) 「その他N件」 — the fold inside a category ═══════════════════════════════════════
        ...
        /* (#R468) the three exclusive 基本表示 choices. `role=radio` + the switch the rows already use;
        ...
        /* ══ ⚠⚠⚠ (#R468) 「フロストガラス時に、『表示中のレイヤー』の背景の色が濃すぎ。」 ═══════════════
      ...
      const walk=(el)=>{ for(const ch of el.children){ if(skipIn(ch)) continue;
        ...
        if(ch.matches&&ch.matches('label')){ const cb=ch.querySelector('input[type=checkbox]');
          if(cb){ const sp=ch.querySelector('span:not(.lyr-sw):not(.lfc-sw):not(.lsr-thumb)');
            ...
            /* ⚠ (#R468) a row whose checkbox is in `IntMapHiddenLayerRows` is NOT a row of this browser.
            ...
            /* (#R468) 「以下に指定されたレイヤー以外は、『その他N件』と、各カテゴリの中で畳む」 — the mark is
    ...
    /* (#R468) 「その他N件」 — the count is part of the sentence, so it is substituted, not appended. */
    ...
    function buildTiles(host){ host=host||sb; if(!host) return; const bodyEl=host.querySelector('.lsr-body'); if(!bodyEl) return;
      ...
      rows.forEach(r=>{ const secName=r.sec||basics;
        if(secName!==curSec){ curSec=secName;
          ...
          /* (#R468) the three exclusive choices stand where the eleven switches used to — see `modeRows` */
          if(curRow) modeRows(curGrid); }
        const t=tileFor(r,curRow);
        /* (#R468) 「カスタム」 is the only mode that shows the eleven, and `filterTiles` is what shows
      ...
      /* ══ ⚠⚠ (#R468) 「その他N件」 — ONE DISCLOSURE PER CATEGORY, AFTER ITS NAMED ROWS ═══════════════
      ...
      root.querySelectorAll('.lst-grid').forEach(g=>{
        ...
        /* ⚠ (#R468) the basics section carries no count: its answer is one of three choices, and a
      ...
      syncModes();   /* (#R468) marks the live mode and runs filterTiles, which is what shows/hides the eleven */
      filterTiles(host); }
    ...
    const SIM_TOOLS=[
      ...
      /* ══ ⚠⚠⚠ (#R261) FIVE MORE, AND THEY HAD NO UI DOOR AT ALL ═════════════════════════════════════
         ...
         not. #R468 deleted that layer outright — 「⛰ 傾斜・斜面方向レイヤーは完全削除。」 — so there is
    ...
    /* ⚠ (#R468) the tools section's collapse state shares `_secClosed` with the layer categories, under
    ...
    function toolsBlock(){ registerSimTools();
      ...
      TOOLS.forEach(t=>{
        ...
        body.appendChild(b);   /* (#R468) into the collapsible body, not the wrapper */
      });
      return wrap;
    }
    ...
    function filterTiles(host){ host=host||sb; if(!host) return; const qi=host.querySelector('.lsr-search input');
      ...
      root.querySelectorAll('.lst-grid').forEach(g=>{ let vis=0;
        /* ══ ⚠⚠ (#R468) THREE THINGS DECIDE ONE `display`, AND THEY DECIDE IT HERE ═══════════════════
      ...
      /* ⚠⚠ (#R468) …AND A SEARCH FORCES THE TOOLS SECTION OPEN, now that it can be closed.
    ...
    function open(){ build();   /* (#R107) mobile allowed — the right-sidebar layer panel now works on phones too (overlay, no map push) */
      ...
      try{ const have=sb.querySelectorAll('.lst-tile[data-lid]').length;   /* (#R468) the layer tiles, not the three mode rows */
    ...
    function mountInto(container){
      ...
      try{
        const have=host.querySelectorAll('.lst-tile[data-lid]').length;   /* (#R468) as above */
...
window.IntMapModules.layerPresets=function(HOST){
  ...
  (function(){
    ...
    /* ⚠⚠ (#R468) DERIVED, because a hand-written copy of this membership had already drifted. #R309
