/* ============================================================================
 *  IntMap · The wave layer's renderer — window.IntMapWavesGL  (#R572)
 * ----------------------------------------------------------------------------
 ...
 *      THE FLAT PICTURE IS UNCHANGED — verified pixel by pixel (#R572): 0 differing pixels at z1,
...
(function (root) {
  ...
  var RASTER_FS = [
    ...
    /* ══ ⚠⚠⚠ NO HALF-TEXEL HERE. THE HALF-TEXEL IS `tap`'S, AND IT WAS BEING PAID TWICE ═══════════
       ...
       here moved the entire field half a grid cell — MEASURED (#R572) by lighting ONE node and
  ...
  var UPDATE_FS = [
    ...
    /* ══ A BEARING BECOMES A SCREEN DIRECTION THROUGH THE JACOBIAN, NOT THROUGH A SMALL STEP ══════
       ...
       MEASURED (#R572, globe at z1, 223 particles compared against MapLibre's own `project()` of
    ...
    /* ══ ⚠⚠⚠ THE SPEED IS IN `A`. IT WAS BEING READ OUT OF `R`, WHICH IS THE WAVE HEIGHT ═══════════
       ...
       MEASURED (#R572, headless Chromium/SwiftShader, z3 over the North Atlantic, uniform synthetic
    ...
    /* ══ ⚠⚠⚠ THE "IS IT GOING ANYWHERE" TEST IS ASKED PER SECOND, NOT PER FRAME ═════════
       ...
       MEASURED (#R572, headless Chromium/SwiftShader) at dt = 8.2 ms the cut sat at 0.81 of full
  ...
  /* ⚠ `uSide` IS A UNIFORM OF ITS OWN AND THAT IS THE POINT. It used to share `uVP2.zw` with the
     ...
     (about 0.002) while the side coordinate's is in PIXELS (about −4.4). MEASURED (#R572): with
  ...
  var DRAW_VS = [
    ...
    /* ══ ⚠⚠⚠ THE STATE'S Y RUNS DOWN THE SCREEN, AND THIS IS WHERE IT WAS BEING READ AS UP ═══════
       ...
       MEASURED (#R572, z3 over the North Atlantic, 4,096 slots, the live particles unprojected
  ...
  function create(opts) {
    ...
    function setData(d) {
      ...
      /* ══ ⚠⚠⚠ GROUP VELOCITY, NOT PHASE VELOCITY ═══════════════════════════════════════════════
         ...
         until #R572. Two independent reasons, and both matter:
    ...
    function renderParticles(map) {
      ...
      /* ══ WHAT THE INTEGRATOR ACTUALLY DELIVERS, MEASURED RATHER THAN ASSUMED (#R572) ═════════════
      ...
      for (var k = 0; k < W.blocks; k++) {
        ...
        /* ⚠ THE ROW STRIDE IS THE TEXTURE'S, NOT THE BLOCK'S. `1/(stateW*blocks)` squeezed a
           ...
           MEASURED (#R572): the offset `uVP0.w` was already `(k*stateH + 0.5)/stateW`, i.e. right;
    ...
    var api = {
      ...
      /* ══ THE MEASUREMENT HOOK ═══════════════════════════════════════════════════════════════════
         ...
         MEASURED that way first (#R572), and it is what hid the deficit this hook then found.
      ...
      layer: function (id) {
        return {
          ...
          /* ══ ⚠⚠⚠ MapLibre 5 DOES NOT HAND A MATRIX HERE, AND THE OBVIOUS FIELD IS THE WRONG ONE ══
             MEASURED (#R572, maplibre-gl 5.24.0, headless Chromium/SwiftShader):
