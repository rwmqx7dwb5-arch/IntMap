...
window.IntMapGazetteer=(function(){
  ...
  function _rowsFrom(doc){
    ...
    for(const r of (doc&&doc.rows)||[]){
      ...
      /* (#R600) v3 tail. A v2 file — a warmed cache from before the rebuild — has none of these,
      ...
      /* ⚠ (#R600) …AND FOUR MORE, APPENDED FOR THE SAME REASON THE SEVENTH AND EIGHTH WERE.
  ...
  function warm(){
    ...
    _worldPromise=(async()=>{
      try{
        ...
        /* (#R600) `placeKinds` is the publisher's own description of each feature code, resolved at
  ...
  /* ══ (#R600) TWO VIEWS OVER ONE LIST, BECAUSE THEY ARE TWO DIFFERENT QUESTIONS ════════════════
  ...
  function index(){
    ...
    /* (#R600) …the MATCHABLE view: this index is a bag-of-words matcher, so a row a curated entry
