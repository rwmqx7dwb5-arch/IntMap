...
window.IntMapGazetteer=(function(){
  ...
  function _rowsFrom(doc){
    ...
    for(const r of (doc&&doc.rows)||[]){
      ...
      /* (#R572) v3 tail. A v2 file — a warmed cache from before the rebuild — has none of these,
      ...
      /* ⚠ (#R572) …AND FOUR MORE, APPENDED FOR THE SAME REASON THE SEVENTH AND EIGHTH WERE.
  ...
  function warm(){
    ...
    _worldPromise=(async()=>{
      try{
        ...
        /* (#R572) `placeKinds` is the publisher's own description of each feature code, resolved at
  ...
  /* ══ (#R572) TWO VIEWS OVER ONE LIST, BECAUSE THEY ARE TWO DIFFERENT QUESTIONS ════════════════
  ...
  function index(){
    ...
    /* (#R572) …the MATCHABLE view: this index is a bag-of-words matcher, so a row a curated entry
