...
export const fetchViaProxy = (() => {
  ...
  const DIRECT_TIMEOUT_MS = 6000;     /* (#R448) the host itself, for the callers that may read it */

  /* ⚠⚠⚠ (#R448) THE CLOCK HAS TO COVER THE BODY, AND IT DID NOT. `clearTimeout` ran in a `.finally`
  ...
  /* (#R448) …and the same question once more for the callers that want DATA rather than a document:
   ...
   *   opts.direct    try the host ITSELF first, before the relays (#R448) — for hosts a browser may
   *                  read. A CORS refusal costs nothing: it rejects before a byte moves.
   *   opts.signal    the caller's AbortSignal — Stop, or a superseding turn (#R448)
   *
   * ⚠⚠ (#R448) `opts.signal` IS NOT DECORATION. Atlas builds an AbortController for every turn and
  ...
  return async function fetchViaProxy(url, opts) {
    ...
    /* ⚠ (#R448) EVERY attempt this call makes is registered here — the direct one, the racers and
    ...
    try {
      /* (#R448) the host itself, when the caller says a browser is allowed to read it */
      if (o.direct) {
        try {
