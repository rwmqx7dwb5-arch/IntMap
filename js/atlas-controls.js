...
export function makeAtlasControls(HOST, CTX) {
  const L=CTX.L, R=CTX.R, _ctlTogHtml=CTX._ctlTogHtml, esc=CTX.esc, note=CTX.note, warn=CTX.warn;
    ...
    /* (#R465) every name the sweep writes is STAMPED as its own, so a language change can take back
    ...
    /* ⚠ (#R465) THE SWEEP NAMES ONLY WHAT HAS NO NAME (`:not([aria-label])`), so the first language an
