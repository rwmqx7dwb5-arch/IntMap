...
window.IntMapModules.mapReadout=function(HOST){
  ...
  /* ══ ⚠⚠⚠ (#R568) THE CEILING WAS NOT A CEILING ═══════════════════════════════════════════════
  ...
  /* ══ THE TWO CEILINGS, AND WHY THEY ARE THOSE NUMBERS ══════════════════════════════════════════
     ...
     ⚠ EXPIRES if either of those budgets moves. tests/r568-checks ⑨ re-derives this from them, so
  ...
  /* HOW MUCH ONE BUILD MAY PIN, on top of the browsing budget. 正本 is js/app-body.js, beside the
     ...
     about; tests/r568-checks re-derives it from those two numbers so the pair cannot drift apart.
  ...
  function _demStart(key){
    ...
    /* ⚠ A REQUEST THAT OUTLIVED ITS ENTRY MUST NOT RESURRECT IT. If the trim dropped this key while
       ...
       otherwise cancelling work would be a way of putting data back (#R568). */
  ...
  function demElevAt(lng,lat,onReady,zArg,_step){
    ...
    if(c===undefined){
      /* (#R568) ADMIT, then trim, then pump. The old order asked for the tile immediately and
  ...
  /* (#R221) `hold` pins every tile this call warms so the trim cannot take them back.
     (#R568) THE PIN IS A LEASE NOW, and every call takes one:
  ...
  function warmDEMTiles(points, z, timeoutMs, onProgress, hold){
    ...
    if(!own&&!named) _demAnon.add(token);   /* hold:true (the pre-#R568 shape) → releaseDEMHold() closes it */
  ...
  /* (#R568) with a token, releases THAT lease; with none, releases every anonymous lease — which is
     what `warmDEMTiles(...,true)` opens, so the pre-#R568 call shape still means what it meant. */
