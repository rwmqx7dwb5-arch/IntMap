...
window.IntMapModules.mapReadout=function(HOST){
  ...
  /* ══ ⚠⚠⚠ (#R671) THE CEILING WAS NOT A CEILING ═══════════════════════════════════════════════
  ...
  function _demStart(key){
    ...
    /* ⚠ A REQUEST THAT OUTLIVED ITS ENTRY MUST NOT RESURRECT IT. If the trim dropped this key while
       ...
       otherwise cancelling work would be a way of putting data back (#R671). */
  ...
  function demElevAt(lng,lat,onReady,zArg,_step){
    ...
    if(c===undefined){
      /* (#R671) ADMIT, then trim, then pump. The old order asked for the tile immediately and
  ...
  /* (#R221) `hold` pins every tile this call warms so the trim cannot take them back.
     (#R671) THE PIN IS A LEASE NOW, and every call takes one:
  ...
  function warmDEMTiles(points, z, timeoutMs, onProgress, hold){
    ...
    if(!own&&!named) _demAnon.add(token);   /* hold:true (the pre-#R671 shape) → releaseDEMHold() closes it */
  ...
  /* (#R671) with a token, releases THAT lease; with none, releases every anonymous lease — which is
     what `warmDEMTiles(...,true)` opens, so the pre-#R671 call shape still means what it meant. */
