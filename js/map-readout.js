...
window.IntMapModules.mapReadout=function(HOST){
  ...
  /* ══ ⚠⚠⚠ (#R566) THE CEILING WAS NOT A CEILING ═══════════════════════════════════════════════
  ...
  /* ══ THE TWO CEILINGS, AND WHY THEY ARE THOSE NUMBERS ══════════════════════════════════════════
     ...
     ⚠ EXPIRES if either of those budgets moves. tests/r566-checks ⑨ re-derives this from them, so
  ...
  /* HOW MUCH ONE BUILD MAY PIN, on top of the browsing budget. 正本 is js/app-body.js, beside the
     ...
     about; tests/r566-checks re-derives it from those two numbers so the pair cannot drift apart.
  ...
  function _demStart(key){
    ...
    /* ⚠ A REQUEST THAT OUTLIVED ITS ENTRY MUST NOT RESURRECT IT. If the trim dropped this key while
       ...
       otherwise cancelling work would be a way of putting data back (#R566). */
  ...
  function demElevAt(lng,lat,onReady,zArg,_step){
    ...
    if(c===undefined){
      /* (#R566) ADMIT, then trim, then pump. The old order asked for the tile immediately and
  ...
  /* (#R221) `hold` pins every tile this call warms so the trim cannot take them back.
     (#R566) THE PIN IS A LEASE NOW, and every call takes one:
  ...
  function warmDEMTiles(points, z, timeoutMs, onProgress, hold){
    ...
    if(!own&&!named) _demAnon.add(token);   /* hold:true (the pre-#R566 shape) → releaseDEMHold() closes it */
  ...
  /* (#R566) with a token, releases THAT lease; with none, releases every anonymous lease — which is
     what `warmDEMTiles(...,true)` opens, so the pre-#R566 call shape still means what it meant. */
