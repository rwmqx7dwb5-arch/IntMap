...
window.IntMapModules.atlasQuery = function (HOST) {
  ...
  /* ══ WHICH GEONAMES RECORDS ARE A CITY ═══════════════════════════════════════════════════════
     ⚠⚠⚠ (#R580) `cities` USED TO MEAN «every GeoNames record whose feature CLASS is P», and that
  ...
  async function cityRows() {
    ...
    for (let i = 0; i < raw.length && rows.length < SCAN_CAP; i++) {
      ...
      /* ⚠ (#R580) THE ROW ID IS THE GEONAMES ID, not this loop's index. `c1234` meant «the 1234th
      ...
      rows.push({ id: r[8] ? ('geonames:' + r[8]) : ('c' + i), geonameId: r[8] || '', featureCode: fcode,
        /* ⚠ (#R580) DISPLAY NAME ≠ MATCHING SURFACE. `r[4]` is GeoNames' `asciiname`, which is a
    ...
    /* ⚠ (#R580) THE THREE COUNTS ARE DIFFERENT FACTS and were printed as one. «147,924 evaluated»
  ...
  /* ⚠⚠ (#R580) `source` NAMES THE DATASET; `origin` NAMES WHAT WAS DONE TO IT. Rule ② got the
  ...
  const COLUMNS = [
    ...
    /* ⚠ (#R580) the VALUE is the ISO-2 code (so `country == 'CN'` still matches); `countryLabel`
  ...
  async function run(spec) {
    ...
    /* ⚠⚠ (#R580) «EVALUATED» IS COUNTED HERE, NOT ABOVE. It used to be the size of the whole base
    ...
    /* ══ (#R580) WHAT THIS RUN RESOLVED — the identity the turn's result list dedupes on ═════════
  ...
  /* (#R580) `origin` as a sentence a reader can act on. It is deliberately about HOW THE NUMBER
  ...
  function rowProvenance(r) {
    const bits = [];
    /* ⚠ (#R580) THE ASCII TRANSLITERATION IS NOT PROVENANCE AND DOES NOT GO HERE. The first draft
       put it in, reasoning that it stays «reachable»; tests/r580.spec.js ③ then failed, because it
  ...
  function fmt(c, v) {
    if (v == null) return '—';
    /* ⚠ (#R580) THE COLUMN'S OWN FORMATTER RUNS FIRST, for text columns too. It used to be reached
  ...
  function tableHtml(res) {
    ...
    /* ⚠⚠ (#R580) THE NAME CELL AND THE COUNTRY COLUMN WERE BOTH PRINTING THE SAME CODE — 「Wuzhong
    ...
    const body = res.rows.map((r, i) => '<tr style="border-top:1px solid rgba(128,128,128,0.14);">'
      ...
      /* (#R580) …and WHICH RECORD this row is, on the row itself. A population is only checkable
  ...
  function methodHtml(res) {
    ...
    /* ⚠⚠ (#R580) FOUR COUNTS, NOT ONE. «都市 147,924 · 件を評価 → 34 · 件が該当» said one number
    ...
    for (const s of res.sources) {
      ...
      /* (#R580) …and HOW, not only WHERE FROM (see `origin` at the column registry). */
