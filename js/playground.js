...
window.IntMapModules.playground=function(map,HOST){
  ...
  (function(){
    ...
    window._openPlayground=function(){
      ...
      const modes=[
        {svg:SV.globe,bg:'linear-gradient(135deg,#0a84ff,#34c759)',t:(HOST.lang==='jp'?'サテライトドロップ':HOST.lang==='de'?'Satellite Drop':HOST.lang==='ru'?'Satellite Drop':HOST.lang==='es'?'Satellite Drop':'Satellite Drop'),d:(HOST.lang==='jp'?'衛星写真からスタート地点を推理する地理ゲーム。ランダムな地点へ飛び、現在地を当てる。':HOST.lang==='de'?'Ein Satelliten-Geografiespiel: irgendwo auf der Erde abgesetzt — errate deinen Standort.':'A satellite where-am-I geography game — dropped somewhere on Earth, guess your location.'),go:()=>{ ov.remove(); window._pgWorldExplorer&&window._pgWorldExplorer(); }},
        {svg:SV.plane,bg:'linear-gradient(135deg,#5e5ce6,#0a84ff)',t:jp()?'フライトシミュレーター':'Flight Simulator',d:jp()?'6自由度の本格フライトモデル。機体と空港を選び、実際の地形の上を滑走路から離陸・着陸。':'A full 6-DOF flight model — pick an aircraft and airport, then take off and land over the real 3-D terrain.',go:()=>{ ov.remove(); try{ window.IntMapFlightSim&&window.IntMapFlightSim.setup&&window.IntMapFlightSim.setup(); }catch(_){} }},
