    ...
    async function dispatch(a){ if(!a||!a.type) return R(true,'');
      switch(a.type){
        ...
        case 'nightSky': case 'starsFromHere': case 'skyFromHere': case 'stargazing': {
          ...
          const st=NS.state(); return R(true, note('✨ '+L('Sky from','星空：','Himmel von','Небо от','Cielo desde')+' '+ll.lat.toFixed(3)+'°, '+ll.lng.toFixed(3)+(st.last?' — '+st.last.starsDrawn.toLocaleString()+' '+L('stars above the measured skyline','個が実測した稜線の上に','Sterne über der Skyline','звёзд над горизонтом','estrellas sobre el horizonte'):''))); }
