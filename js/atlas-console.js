...
import { makeNewsCluster } from './news-cluster.js';   /* (#R336) research.events — the ONE deterministic article→event grouper, with the measurements behind every constant */
...
window.IntMapModules.atlasConsole=function(HOST){
  ...
  return (function(){
    ...
    /* (#R336) ↳ js/news-cluster.js — article→EVENT grouping for research.events. No HOST and no deps: it is pure, which is what lets tests/r336-checks run the shipped function over a fixture. */
    ...
    async function dispatch(a){ if(!a||!a.type) return R(true,'');
      switch(a.type){
        ...
        case 'events': case 'newsEvents': case 'groupNews': { /* (#R76) vision §6 / stage 4 — the loaded news
          ...
          (#R336) THE GROUPING ITSELF IS js/news-cluster.js — the one implementation, with the production
          ...
          if(plc&&!WORLD_RE.test(plc)){ if(DEIXIS_RE.test(plc)) ctx=await geocode(plc); else { try{ ctx=await placeExtent(plc); }catch(_){} if(!ctx){ try{ ctx=await geocode(plc); }catch(_){} } }
            ...
            /* (#R336) the area filter asks the SAME question the grouper does — where the story IS, not where
          ...
          const evs=groupNewsEvents(items,{agoH:_agoH,fallbackH:hrs});   /* (#R336) ↳ js/news-cluster.js — the rules, the constants and the measurements that set them */
          ...
          /* (#R336) the count is the number of articles the events are ACTUALLY built from, not the number
          ...
          /* (#R336) the numbers in this sentence are READ from the grouper, so the explanation cannot describe
             a rule the code no longer applies (#R76's copy still said «place ≤150 km» after the rule changed). */
          const _evR=L('place','位置','Ort','место','lugar')+' × ≤'+EVENT_RULES.HOURS+' h × '+L('headline similarity','見出し類似','Titelähnlichkeit','сходство заголовков','similitud de titulares')+' ≥'+Math.round(EVENT_RULES.SIM*100)+'%';
          html+='<div style="font-size:10px;color:var(--text-muted);margin-top:6px;line-height:1.5;">'+L('Grouping is mechanical on the loaded IntMap feed ('+_evR+'). A country-level reference point is not a place, so articles that merely file under the same country still have to match on wording. One group = reports that likely cover the same occurrence; "first report → latest" shows how coverage moved. For source disagreements or deeper analysis, ask e.g. "analyze event 2".','グループ化は読み込み済みニュースに対する機械的クラスタリング（'+_evR+'）です。国レベルの代表点は「同じ場所」とは見なさないので、同じ国に分類されただけの記事は見出しが一致しない限り束ねません。1グループ=同一の出来事を扱うとみられる報道で、「最初の報道→最新」で経過が分かります。報道間の相違や深掘りは「2番の出来事を分析して」のように聞いてください。','Mechanische Gruppierung ('+_evR+'); ein Länder-Referenzpunkt gilt nicht als Ort. Für Analysen: "analysiere Ereignis 2".','Механическая группировка ('+_evR+'); точка-представитель страны местом не считается. Для анализа: «проанализируй событие 2».','Agrupación mecánica ('+_evR+'); un punto representativo de país no cuenta como lugar. Para análisis: "analiza el evento 2".')+'</div>';
          ...
          /* (#R336) …and the structured half of the same honesty: research.events declares produces='map,explanation',
