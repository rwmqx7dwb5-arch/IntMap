...
window.IntMapModules.atlasConsole=function(HOST){
  ...
  return (function(){
    ...
    /* The tools as compact JSON \u2014 name, one line of purpose, and the schema its arguments must match.
       js/atlas-toolsurface.js builds them; `find_capability` reaches the other hundred-odd. */   function _capIndex(){ try{ return String(CAPS.index()||''); }catch(_){ return ''; } }   /* (#R581) the registry's OWN index of every capability id, derived from js/atlas-capabilities.js. Without it SYS() named the door (find_capability) and not one thing behind it, so every decision Atlas took BEFORE deciding to search was taken about an IntMap with nine tools in it. ⚠ ON THIS LINE because the kernel has no headroom — same reason as the GLOSS import above. */
