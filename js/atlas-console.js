...
window.IntMapModules.atlasConsole=function(HOST){
  ...
  return (function(){
    ...
    const ARENDER = makeAtlasAnswerRender(); const { renderAnswer, answerPlainText, answerCSS } = ARENDER;   /* (#R583) the whole surface too, because _atlCompose needs demoteProseLinks */
    ...
    function _atlCompose(ai){ try{
      ...
      let head=say?('<div style="margin-bottom:6px;">'+mdMini(say)+'</div>'):''; try{ const _cr=COMPOSE.recordsFor(keep); if(_cr.length&&head) head=COMPOSE.linkProse(head,_cr); if(head) head=ARENDER.demoteProseLinks(head,_curPlanCites,_cr); }catch(_){}   /* (#R511) the names in the answer get the numbers the markers carry — from the records THIS reply drew, read off its own results. ⚠⚠⚠ (#R583) AND AN ANCHOR IN THAT PROSE CLAIMS INTMAP FETCHED THE PAGE: the structured path builds every link from the registry, this one renders what Atlas wrote, which is how 「(mapion.co.jp)」 shipped as a live link to a page nothing in the turn had requested. The rule, and the set of hosts this turn actually retrieved, are js/atlas-answer-render.js. */
