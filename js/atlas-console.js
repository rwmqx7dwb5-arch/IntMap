  ...
  return (function(){
    ...
    async function dispatch(a,dctx){ if(!a||!a.type) return R(true,''); switch(a.type){
        ...
        case 'radiation': case 'fallout': case 'dispersion': case 'plume': case 'radiationSim': {
          ...
          if(!ll){ try{ ll=window.IntMapRadiation.resolveSite&&await window.IntMapRadiation.resolveSite(_place); }catch(_){} }   /* (#R578) the nuclear gazetteer is now the discovered registry (data/npp.json) and resolving is async — the rung below this one already awaited */
          ...
          /* ⚠⚠⚠ (#R568 ⑩) THE ANNUAL DOSE IS NO LONGER EXTRAPOLATED HERE. It used to be «rate × 8766 × 0.5», i.e. «this dose rate holds for a year» — false for every nuclide and absurd for I-131. The model integrates against decay AND weathering and returns `firstYearMSv`; this side only prints it (docs/RADIATION-MODEL.md §6). */ let h='<div style="font-weight:600;">☢ '+esc(ll.name||a.place)+' — '+L('radioactive dispersion & fallout','放射性物質の拡散・降下','radioaktive Ausbreitung & Fallout','рассеивание и выпадение','dispersión y lluvia radiactiva')+'</div>'
            ...
            +'</div>';   h+=await radiationChain(ll);   /* (#R578) the measured half of the chain — the body is in js/atlas-controls.js, because this file shrinks only by MOVING (tests/r419 ⑨d, tests/r511 ⑨) */
