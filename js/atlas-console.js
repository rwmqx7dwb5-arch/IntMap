...
import { makeAtlasAnswerPipeline } from './atlas-answer-pipeline.js';   /* (#R342) the analysis answer as a contract: evidence registry -> one call -> audit -> at most one repair -> degrade */
import { makeAtlasAnswerRender } from './atlas-answer-render.js';   /* (#R342) every link on screen is built from the registry, never from the model's prose */
...
window.IntMapModules.atlasConsole=function(HOST){
  ...
  return (function(){
    ...
    /* (#R342) the answer contract, in the shape tests/r175 ③ requires of every js/ module: ONE
    ...
    /* ⚠ (#R342) THE SAME NUMBERS, AS EVIDENCE INSTEAD OF PROSE. `_statsData` renders the country
    ...
    /* ⚠ (#R342) THE ANSWER PIPELINE, REACHABLE FROM THE BROWSER. #R313's addendum is the reason this
    ...
    async function dispatch(a){ if(!a||!a.type) return R(true,'');
      switch(a.type){
        ...
        case 'analyze': case 'research': case 'synthesize': { const q=String(a.question||a.query||a.text||'').trim();
          ...
          /* ══ (#R342) THE ANSWER IS A CONTRACT, NOT A STRING ══════════════════════════════════
        ...
        case 'answer': { let _ah='<div style="font-size:14px;line-height:1.68;">'+mdMini(a.text||'')+'</div>';   /* (#R149) if the answer NAMED mappable places, pin them (unless the plan already pinned) so a location-rich reply always delivers map value */
          ...
          { const _acit=_curPlanCites.slice();   /* ⚠ (#R342) from the planner call that produced THIS answer. It used to be window._aiLastCitations, read at RENDER time — so a second Atlas turn finishing in between handed this reply the other turn's sources, under the heading 「Web検証済みソース」. */
    ...
    function ensureStyle(){ if(styled) return; styled=true; const s=document.createElement('style');
      s.textContent=atlasPanelCSS()+answerCSS;   /* (#R342) citation pills, the lead line, limitations, the degraded banner */
    ...
    function _atlCompose(ai){ try{
      ...
      /* ══ ⚠ (#R342) A COURTESY THAT DID NOT LAND IS NOT A FAILED TURN ═══════════════════════════
    ...
    /* (#R342) one door for every planner call, so the citations of THIS turn are captured beside the
    ...
    async function runActions(ai, say, acts, gen){
      ...
      for(const a of acts){ if(gen!=null&&gen!==_runGen){ cancelled=true; break; }
        ...
        try{ a.__impact=_PLANNER.goalImpact(_curProfile,a); }catch(_){ a.__impact='primary'; }   /* (#R342) primary = an output the user asked for; secondary = a courtesy Atlas added */
    ...
    async function run(q,imgs,files){ q=String(q||'').trim(); imgs=(Array.isArray(imgs)?imgs:[]).filter(u=>typeof u==='string'&&/^data:image\//.test(u)).slice(0,4);   /* (#R149) optional pasted/attached images (vision) */
      ...
      _curProfile=_profile;   /* (#R342) goalImpact() needs the REQUEST to tell a map move the user asked for from one Atlas added itself */
