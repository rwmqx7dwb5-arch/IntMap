...
import { ATLAS_BUDGETS, settleWithin, lateNote, makeFetchJSON, newTurnController } from './atlas-deadlines.js';   /* (#R448) the turn's clocks — Atlas had two private, unbounded copies of the relay ladder */
...
window.IntMapModules.atlasConsole=function(HOST){
  ...
  return (function(){
    ...
    /* (#R448) the clocks, the bounded gather and the evidence fetcher live in js/atlas-deadlines.js — this file has a SHRINK-ONLY ceiling, and #R199's rule is that a subject moves OUT rather than the ceiling moving up */
    const { EVIDENCE_BUDGET_MS, GATHER_BUDGET_MS, WEB_BUDGET_MS } = ATLAS_BUDGETS;
    /* ⚠ read at CALL time — `run()` installs the controller when a turn starts, so a signal captured here would belong to no turn */
    const turnSignal = () => { try{ return _abortCtl?_abortCtl.signal:undefined; }catch(_){ return undefined; } }, _fetchJSON = makeFetchJSON(turnSignal);
    ...
    async function _buildResearchAnswer(o){ o=o||{}; const topic=String(o.topic||'').trim(), place=String(o.place||'').trim();
      ...
      await settleWithin(jobs,GATHER_BUDGET_MS);   /* (#R448) bounded — a source that has not answered by now is one this brief goes without, not one it waits behind */
        ...
        case 'analyze': case 'research': case 'synthesize': { const q=String(a.question||a.query||a.text||'').trim();
          ...
          if(wantD('web')!==false){ const topic=(ctx&&ctx.name)||placeStr||'';
            ...
            jobs.push((async()=>{ let any=false;
              /* ⚠⚠ (#R448) GOOGLE NEWS IS A DIFFERENT HOST, SO IT STARTS NOW RATHER THAN AFTER GDELT.
                 All three were awaited in one file, so own-language news waited out every GDELT attempt
                 first — and the file exists for GDELT's per-IP limit, which says nothing about
                 news.google.com. ⚠ The ladder also stops asking GDELT once WEB_BUDGET_MS is spent. */
    ...
    async function run(q,imgs,files){ q=String(q||'').trim(); imgs=(Array.isArray(imgs)?imgs:[]).filter(u=>typeof u==='string'&&/^data:image\//.test(u)).slice(0,4);   /* (#R149) optional pasted/attached images (vision) */
      ...
      _abortCtl=newTurnController(_abortCtl);   /* ⚠⚠⚠ (#R448) a second question REPLACES the first — it used to be overwritten WITHOUT being aborted, so a turn nobody would see kept running. js/atlas-deadlines.js has the measurement */
      _setGoBusy(true);
      try{
