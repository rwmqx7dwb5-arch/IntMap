  ...
  return (function(){
    ...
    function _agentPrompt(req, q){ let p=''; const ctx=stateContext(); if(ctx) p+='[CURRENT MAP STATE]\n'+ctx+'\n\n';
      ...
      try{ const steps=[]; ((req&&req.messages)||[]).forEach(m=>{
        ...
        if(steps.length) p+='[THIS TURN SO FAR \u2014 IntMap\'s mechanical record. It did not correct, substitute or reinterpret anything; those decisions are yours.]\n'+POLICY.turnMechanics.fence.wrap(steps.join('\n'))+'\n\n';   /* ⚠ (#R801) FENCED: this is where a fetched page, a headline or a geocoder's answer first stands in the same string as the reader's words. The fence (js/atlas-policy.js) says who is speaking; js/atlas-agent.js `turn.externalContentSeen` is the same fact as a flag */
    ...
    async function run(q,imgs,files){ q=String(q||'').trim(); imgs=(Array.isArray(imgs)?imgs:[]).filter(u=>typeof u==='string'&&/^data:image\//.test(u)).slice(0,4);   /* (#R149) optional pasted/attached images (vision) */
      ...
      const _ranActions=[];
      const _runOne=async(action,turn)=>{ action.__externalContent=!!(turn&&turn.externalContentSeen);   /* (#R801) stamped UNCONDITIONALLY from the agent's turn record, so a `__externalContent` the model wrote into its arguments is overwritten rather than read; `__` keys never reach the executor's arguments (below) nor `callKey` */ const before=((ai.__atlResults||[]).length);
      ...
      const _model=async(req)=>{ try{ PROG.phase(ai,(req&&req.final)?'write':'think'); }catch(_){}   /* ⚠ (#R723) THE PLANNER WAIT IS THE LARGEST PART OF A TURN AND IT WAS THE PART WITH NO ROW. Measured on a real six-operation turn: 57.2 s total, 13.6 s of it inside operations — the other 43 s was eight round-trips to the model, and the trace said nothing about any of them. onStep fires AFTER the reply, so it can only CLOSE a thinking row; this is the only place that knows one has started. */ const env=await askAIJSONEnvelope(_agentPrompt(req,q),_sys,_atlTurnImgs(VFRAMES.urls(),_atlRecallImgs),{task:'atlas_turn',files:_atts.files,docs:_atts.docs,schema:TURN_SCHEMA,webMode:'auto',effortHint:_cplx?'high':undefined,turnId:_turnKey,signal:(_abortCtl?_abortCtl.signal:undefined)});   /* ⚠ (#R493) THE THIRD ARGUMENT WAS `null` AND IS NOW THE FRAMES — the vision channel js/ai-core.js has had since #R149 and supabase/functions/ai-proxy turns into `input_image`. Nothing new is built for it: from the step after an `inspect`, the model is reading the reader's actual screen. */
        ...
        return AGENT.readReply(env&&env.data, env&&env.text, aiParseJSON, env&&env.meta); };   /* (#R801) THIS call's meta (webUsed/webAttached), not window._aiLastMeta — the #R350 reason, one reader over */
      try{
