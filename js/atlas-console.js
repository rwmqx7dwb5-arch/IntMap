...
window.IntMapModules.atlasConsole=function(HOST){
  ...
  return (function(){
    ...
    async function _tryMapResearch(place, items, opt){ opt=opt||{}; let ext=null, name=String(place||'').trim();
      ...
      let rendered=false, method='', pinsDrawn=false;   /* (#R799) `rendered` is true for a CAMERA move too, so the pin declaration needs its own witness — the paint call itself */
    ...
    async function dispatch(a,dctx){ if(!a||!a.type) return R(true,''); switch(a.type){
        ...
        case 'impact': case 'impactAnalysis': case 'nearbyCritical': { /* (#R75) vision §11 — WHERE an event's impact
          ...
          return R(true, html, _PINNED(null,okP)); }   /* (#R799) the facilities and cities this analysis pinned, declared by the painter — `okP` is the same witness the sentence above prints */
        case 'events': case 'newsEvents': case 'groupNews': { /* (#R76) vision §6 / stage 4 — the loaded news
          ...
          return R(true, html, _PINNED({meta:{code:'OK',category:'ok',retryable:false,produced:(_evMapped?['map','explanation']:['explanation']),userGoalSatisfied:true,partial:!_evMapped}},_evMapped)); }   /* (#R799) …and WHICH events are pinned, so re-grouping the same top events reads as `already_there` rather than `not_rendered` */
