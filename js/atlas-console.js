  ...
  return (function(){
    ...
    async function dispatch(a){ if(!a||!a.type) return R(true,'');
      switch(a.type){
        ...
        case 'tool': { const n=String(a.name||'').toLowerCase(); const id=/radius/.test(n)?'btn-tool-radius':/draw/.test(n)?'btn-tool-draw':/measur|dist|area/.test(n)?'btn-tool-measure':/grid/.test(n)?'btn-tool-grid':null; if(id){ const ok=clickId(id); return R(ok, ok?note('✓ '+esc(a.name||'')):warn('⚠')); } return doControl({target:a.name}); }
        ...
        case 'volume3d': case 'volume': { const ll=await geocode(a.place); if(!ll) return R(false, warn('⚠ '+esc(a.place||'')));
          ...
          const km=Math.max(0.2,Math.min(500,+a.km||5)), base=+a.base, top=+a.top;
          if(!isFinite(base)||!isFinite(top)) return R(false, warn('⚠ '+L('Need a base and a top altitude in metres','下端と上端の高度（メートル）が必要です','Basis- und Obergrenze in Metern nötig','Нужны нижняя и верхняя высота в метрах','Se necesitan altitud inferior y superior en metros')));
          ...
          try{ if(typeof setTool==='function') setTool('volume'); if(typeof HOST.measurePoints!=='undefined') HOST.measurePoints=round?[]:ring.slice();
            ...
            /* the panel rebuild re-pushes measurePoints for the click-vertex shape; a circle owns its own ring */
            if(round) V.setRing(ring); }catch(_){}
          try{ map.flyTo({center:[ll.lng,ll.lat],zoom:Math.max(map.getZoom(),10),pitch:Math.max(map.getPitch(),55)}); }catch(_){}
          const st=V.state();
          return R(!!st.points, st.points?note('🧊 '+esc(ll.name||a.place||'')+' · '+Math.round(base).toLocaleString()+'–'+Math.round(top).toLocaleString()+' m · '+V.fmtVolume()):warn('⚠')); }
