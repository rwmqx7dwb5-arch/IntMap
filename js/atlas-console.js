...
window.IntMapModules.atlasConsole=function(HOST){
  ...
  return (function(){
    ...
    const _CLEARED=(...kinds)=>{ const d={}; kinds.forEach(k=>{ d[k]=[]; }); return {meta:{painted:d}}; };   /* (#R747) the surfaces a clear declares EMPTY, in the same vocabulary — js/atlas-capabilities.js PAINT_GOAL reads it */
    ...
    async function dispatch(a,dctx){ if(!a||!a.type) return R(true,''); switch(a.type){
        ...
        case 'recallAttachment': case 'recall_attachment': case 'recallFile': case 'reopenAttachment': { const _r=ATTACH_LOG.find(_curTurn,a&&a.name); if(!_r){ const _n=ATTACH_LOG.names(_curTurn); return R(false,warn(L('No attachment called that. In this conversation: '+(_n.join(', ')||'none'),'その名前の添付はありません。この会話にあるのは: '+(_n.join('、')||'なし'),'Kein Anhang mit diesem Namen. In diesem Gespräch: '+(_n.join(', ')||'keine'),'Вложения с таким именем нет. В этом разговоре: '+(_n.join(', ')||'нет'),'No hay ningún adjunto con ese nombre. En esta conversación: '+(_n.join(', ')||'ninguno')))); } if(_r.kind==='image') _atlRecallImgs.push(_r.dataUrl); else if(_r.kind==='doc'){ if(_atlRecallAtts) _atlRecallAtts.docs.push({name:String(_r.name||'file'),mime:String(_r.mime||''),b64:String(_r.b64||'')}); } else if(_atlRecallAtts) _atlRecallAtts.files.push({name:String(_r.name||'file'),text:String(_r.text||''),truncated:!!_r.truncated}); return R(true,note(L('Brought back '+_r.name+' — it is in front of you on the next step.','「'+_r.name+'」を取り戻しました。次の一手で目の前にあります。',_r.name+' wurde zurückgeholt — beim nächsten Schritt liegt es vor dir.','Вложение '+_r.name+' возвращено — оно перед вами на следующем шаге.','Se recuperó '+_r.name+' — lo tendrás delante en el siguiente paso.')),{exec:{recalled:_r.name,kind:_r.kind}}); }   /* ⚠⚠⚠ (#R773) 取り戻したものは**次のモデル呼び出しのチャネルに載る**（画像は vision、PDF は文書、テキストは添付チャネル）。tool の結果テキストに入れないのは #R493 と同じ理由——プロンプト本文に置いた data URL は画像ではなく数十万文字の base64 である。 */
