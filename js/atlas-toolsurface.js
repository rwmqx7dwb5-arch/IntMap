...
export function makeAtlasToolSurface(deps) {
  return (function () {
    ...
    var CORE = [
      ...
      { name: 'query_data', cap: 'data.query', desc: 'FILTER AND JOIN the datasets IntMap ships and answer with the ACTUAL ROWS — a table, pinned on the map, with the source of each column printed under it. This is the action for any question with several conditions at once ("cities above 3000 m with more than 500,000 people", "M6+ quakes with a nuclear plant within 100 km"), and for any question a shipped table can answer. Prefer it over `research` whenever the answer is rows rather than prose: it computes from the data instead of describing what would have to be checked. The tables, every column, the spatial and near forms and worked examples are in the catalogue entry for "query". An empty result is an answer — do not re-run the question as prose.' },
      { name: 'research', cap: 'research.analyze', desc: 'Answer a question from live sources with citations. Use for anything current, contested or beyond your own knowledge — but when the answer is ROWS from a dataset IntMap ships, use `query_data` instead. This renders its own sourced answer to the reader. Use for anything current, contested or beyond your own knowledge. This renders its own sourced answer to the reader.' },
