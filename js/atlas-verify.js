...
export function makeAtlasVerify(HOST, CTX) {
  ...
  const PINPASS_BUDGET_MS = 20000;
    ...
    /* ⚠ (#R799) …AND RECOMPOSED AFTERWARDS, because the decomposition is a Latin device and this
    ...
    /* ⚠⚠⚠ (#R799) A NAME IS NOT ITS POSSESSIVE, AND A PERIOD IS NOT ALWAYS A SENTENCE END.
    ...
    function _atlExtractPlaces(text){ let t=String(text||'').replace(/`[^`]*`/g,' ').replace(/https?:\/\/\S+/g,' ').replace(/\[([^\]]*)\]\([^)]*\)/g,'$1');
      /* a phrase never bridges a sentence boundary ("Italy. The Colosseum") — #R726 got that by keeping
         '.' out of the class entirely, and #R799 keeps the SAME property while letting an abbreviation's
      ...
      /* (#R799) `\.?` before each join is the abbreviation period of ① above; a period that is NOT one
    ...
    function _atlMappingVerdict(spots){ const mapped=[],unplaced=[],ambiguous=[];
      ...
      /* ⚠ (#R799) …AND THE SAME IS TRUE OF 「IS THIS SHAPED LIKE A NAME AT ALL」. 「not placed (couldn't
    ...
    /* ══ ⚠⚠⚠ (#R799) THE STORE THAT ALREADY HOLDS EVERY COUNTRY'S NAME IN THE READER'S LANGUAGE ══════
    ...
    function makePinReplyPlaces(D) {
      ...
      try{
        ...
        for(const it of struct.concat(textCands)){
          ...
          /* (#R799) the app's own country store, in all nine languages, before any request goes out. ⚠ It
          ...
          /* ⚠ (#R799) A COUNTRY DOES NOT NARROW ITSELF. The model fills `country` with the country's own
