...
window.addEventListener('DOMContentLoaded', () => { const _imAppBoot = () => {
  ...
  function computeFilteredNews(){ const q=searchVal(), at=window._newsAreaTest;
    ...
    /* (#R210) ★ Saved is no longer only what the LIVE feed still carries — see js/news-ui.js
       ⚠ (#R384) …and the article snapshot is never poured into an EVENT surface (link-keyed, no `_event`).
       ⚠ (#R384) Event mode の違いは★の在り処・検索の当たる範囲・カテゴリの3つだけで、記事モードの述語は1ビットも変わらない。 */
  ...
  /* (#R384) EVENT MODE — `news_events` の経路。⚠ すぐ上の `USE_SERVER_NEWS` とは**別のスイッチ**で、
