...
window.addEventListener('DOMContentLoaded', () => { const _imAppBoot = () => {
  ...
  function updateI18n(){
    ...
    /* ⚠ (#R465) a relabel of `#opt-tz-auto` stood here — an id index.html lost in #R18, so it found nothing and 「Local (System Default)」 stayed English all session. The combo is rebuilt from `intmap-lang` now (wireTzSearch). */
  ...
  (function wireTzSearch(){ const inp=document.getElementById('setting-tz-search'), sel=document.getElementById('setting-tz'), res=document.getElementById('tz-results'); if(!inp||!res) return;
    ...
    window.addEventListener('intmap-lang',()=>{ try{ populateTimezones(res.classList.contains('show')?inp.value:''); }catch(_){} });   /* ⚠ (#R465) the language can change while this list is up — the <select> is in the same dialog, so nothing ever re-ran populateTimezones(). Rebuilding is safe: the value (`prev`) AND the filter are carried across, so a half-typed search survives. */
  })();
  ...
  /* (#R180) the ENGINE line — see the note at its call site below. ⚠ (#R465) it has a NAME now because opening Settings was the only moment it could be right, and the language changes under it. */
