...
window.addEventListener('DOMContentLoaded', () => { const _imAppBoot = () => {
  ...
  window.IntMapModules.facilities(IM_HOST); window.IntMapModules.industryWeb(IM_HOST); window.IntMapModules.oceanCurrents(IM_HOST);   /* (#R213/#R216) the industry ownership web (js/industry-web.js) and the ocean currents (js/ocean-currents.js). BOTH after worldPacks: they borrow that module's panel/row toolkit. (#R255) …and the four surveyed-facility layers (js/osm-facilities.js) join them ON THIS LINE on purpose: the shell budget (tests/r168 #8) stood at 8,200 of 8,200, and #R254 already recorded that hitting it means spending nothing rather than raising it. */ window.IntMapModules.precipAnnual(IM_HOST); window.IntMapModules.outbreaks(IM_HOST);   /* (#R590) WHO Disease Outbreak News — ON THIS LINE for the same shell budget, and after worldPacks for the same toolkit reason as the two above. */
