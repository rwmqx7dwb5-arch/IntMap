...
window.addEventListener('DOMContentLoaded', () => { const _imAppBoot = () => {
  ...
  const _imgConcurrency=/Mobi|Android|iPhone|iPad/.test(navigator.userAgent)?16:256;   function _tileCacheMax(){ if(!_imPhoneClass()) return _hiDPITiles?2048:8192; const dm=+navigator.deviceMemory; return (isFinite(dm)&&dm>4)?1024:640; }   /* (#R568) the resident-tile budget. ⚠ AN UNKNOWN deviceMemory IS NOT EVIDENCE OF HEADROOM — WebKit does not implement it, so it is undefined on every iPhone and undefined&&… selected the LARGER number: we were not told meant more than 4 GB. The numbers are #R21s; which one an unknown answer picks is what changed. Why, and what it costs: js/map-readout.js */
  try{ window.__imImgConcurrency=_imgConcurrency; }catch(_){}   try{ window.__imDemStore=()=>IM_READOUT.demStoreStats(); }catch(_){}   /* (#R568) what the DEM tile store is holding, for js/perf-hud.js — the renderer's own tile count says nothing about it */
