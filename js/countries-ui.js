...
/* ══ ⚠⚠⚠ (#R443) THE OTHER HALF OF THE SAME ROW — AND ITS VOCABULARY ALREADY EXISTED ═══════════
   ...
   (#R452 acted on that measurement: the `subregion` and `region` fallback lines are gone, along
...
/* ══ ⚠⚠⚠ (#R452) THE CARD WAS ASKING A SERVER THAT NO LONGER EXISTS ═══════════════════
   ...
   ON THE SCREEN rather than at whether the code compiles: tests/r452-checks.test.mjs runs this
    ...
    P=fetch(FACTS_URL).then(function(r){ if(!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
      ...
      .catch(function(e){
        /* ⚠ THE FAILURE IS RECORDED, NOT DISCARDED — and the promise is released so the next card
           ...
           rows tests/r452-checks.test.mjs ⑥⑧ look for. */
...
window.IntMapModules.countriesUi=function(HOST){
  ...
  /* ⚠ (#R452) SAME NINE FIELDS, SAME PRECEDENCE, A SUPPLIER THAT ANSWERS — see the ⚠⚠⚠ box at
