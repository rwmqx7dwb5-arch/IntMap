...
/* ══ ⚠⚠⚠ (#R443) THE OTHER HALF OF THE SAME ROW — AND ITS VOCABULARY ALREADY EXISTED ═══════════
   ...
   (#R453 acted on that measurement: the `subregion` and `region` fallback lines are gone, along
...
window.IntMapModules.countriesUi=function(HOST){
  ...
  /* ⚠ (#R453) SAME NINE FIELDS, SAME PRECEDENCE, A SUPPLIER THAT ANSWERS — see the ⚠⚠⚠ box at
  ...
  function renderStats(q){
    ...
    let html=`<div class="stats-toolbar"><select class="stats-sort-sel" aria-label="${HOST._sfL('Sort by','並べ替え')}" onchange="setStatsSort(this.value)">${IND.map(([k,l])=>`<option value="${k}"${HOST.statsSort===k?' selected':''}>${l}</option>`).join('')}</select><button type="button" class="stats-sort-dir" onclick="toggleStatsSortDir()" title="${HOST.t('sortDir')}">${dirLbl}</button>${filterBtn}</div>${_sfPanel}`;
