...
export function makeAtlasSources(HOST, CTX) {
  const _fetchJSON=CTX._fetchJSON, askAIJSON=CTX.askAIJSON, countryStats=CTX.countryStats, nm=CTX.nm;
    ...
    /* ⚠⚠⚠ (#R452) THIS WAS A THIRD COPY OF THE RELAY LADDER, AND IT WAS THE PRE-#R212 ONE.
    ...
    async function _wikiSummary(topic){ const t2=String(topic||'').trim(); if(!t2) return null;
      const langW=wikiLang(HOST.lang);   /* (#R318) nine languages, from the registry's BCP-47 tag */
    ...
    /* ⚠ (#R577) A RAW OVERPASS QUERY, RACED OVER THE SAME MIRRORS. `look_at_map` asks «what named
    ...
    async function wikidataPOIs(kindStr,box,iso3){
      ...
      const wikiHost=wikiLang(HOST.lang);   /* (#R318) 'zh-hans.wikipedia.org' does not exist — wikiLang() strips the script subtag */
