...
window.IntMapModules.satelliteDetail=function(HOST){
  ...
  const clock=(ms)=>{ try{ return new Date(ms).toLocaleTimeString(HOST.lang==='jp'?'ja-JP':'en-GB',{hour:'2-digit',minute:'2-digit',second:'2-digit'}); }catch(_){ return ''; } };
  function inTxt(ms){ const s=Math.max(0,Math.round((ms-Date.now())/1000));
    if(s<60) return s+(HOST.lang==='jp'?' 秒後':' s');
    const m=Math.floor(s/60); if(m<60) return m+(HOST.lang==='jp'?' 分後':' min')+' '+String(s%60).padStart(2,'0')+(HOST.lang==='jp'?' 秒':' s');
    return Math.floor(m/60)+(HOST.lang==='jp'?' 時間':' h')+' '+String(m%60).padStart(2,'0')+(HOST.lang==='jp'?' 分後':' min'); }
  function ageTxt(h){ if(h==null||!isFinite(h)) return '';
    if(h<1) return Math.round(h*60)+(HOST.lang==='jp'?' 分前':' min');
    if(h<48) return h.toFixed(1)+(HOST.lang==='jp'?' 時間前':' h');
    return (h/24).toFixed(1)+(HOST.lang==='jp'?' 日前':' d'); }
  ...
  function bodyHTML(){
    ...
    if(passBusy) passHTML='<div class="tp-hint">'+S(L('Searching the next 24 hours…','今後24時間を計算中…','Suche in den nächsten 24 Stunden…','Ищем в ближайшие 24 часа…','Buscando en las próximas 24 horas…'))+'</div>';
      ...
      if(pass.none) passHTML='<div class="tp-hint">'+S(L('This satellite does not rise above the horizon here in the next 24 hours.','今後24時間、この地点では地平線の上に出ません。','Er geht hier in den nächsten 24 Stunden nicht auf.','В ближайшие 24 часа он здесь не восходит.','No se eleva sobre el horizonte aquí en las próximas 24 horas.'))+'</div>';
        ...
        +row(L('Duration','継続時間','Dauer','Длительность','Duración'), pass.durationS?(Math.floor(pass.durationS/60)+(HOST.lang==='jp'?' 分 ':' min ')+String(pass.durationS%60).padStart(2,'0')+(HOST.lang==='jp'?' 秒':' s')):'');
    ...
    return '<div class="acp-badges">'+badge+sun+'</div>'
      ...
      +row(L('Period','周期','Umlaufzeit','Период','Periodo'), f.periodMin==null?'':(f.periodMin.toFixed(2)+(HOST.lang==='jp'?' 分':' min')+' · '+(1440/f.periodMin).toFixed(2)+(HOST.lang==='jp'?' 周/日':' rev/day')))
