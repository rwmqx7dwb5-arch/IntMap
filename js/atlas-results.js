/* ============================================================================
 *  IntMap · ATLAS — the ONE shape every operation returns  (#R318)   window.IntMapActionResult
 * ----------------------------------------------------------------------------
...
export function makeAtlasResults(HOST) {
  return (function () {
    ...
    var MESSAGES = {
      ...
      /* (#R768) NOT a failure of the request — the page was not drawing, so the move could not happen
         or be seen. It names what the reader can do about it, because that is the only thing that will. */
      'atlas.code.not_rendering':    LA('The map was not drawing — IntMap is in the background, so the view could not move. Bring IntMap to the front and ask again.', '地図が描画されていませんでした——IntMap が背面にあるため、表示を動かせませんでした。IntMap を前面にして、もう一度お尋ねください。', 'Die Karte hat nicht gezeichnet — IntMap ist im Hintergrund, die Ansicht konnte sich nicht bewegen. Hol IntMap nach vorn und frag erneut.', 'Карта не отрисовывалась — IntMap в фоне, поэтому вид не сдвинулся. Откройте IntMap на переднем плане и спросите снова.', 'El mapa no estaba dibujando: IntMap está en segundo plano y la vista no pudo moverse. Pon IntMap al frente y vuelve a preguntar.'),
      'atlas.code.already_clear':    LA('There was nothing of that kind to remove — the map is already in the state asked for.', '消す対象はありませんでした——地図はすでに求められた状態です。', 'Es gab nichts dergleichen zu entfernen — die Karte ist bereits im gewünschten Zustand.', 'Удалять было нечего — карта уже в запрошенном состоянии.', 'No había nada de ese tipo que quitar: el mapa ya está en el estado pedido.'),
      'atlas.code.already_there':    LA('The map was already showing what you asked for.', '地図はすでに、ご要望の表示になっていました。', 'Die Karte zeigte bereits das Gewünschte.', 'Карта уже показывала то, что вы просили.', 'El mapa ya mostraba lo que pediste.'),
