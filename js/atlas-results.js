/* ============================================================================
 *  IntMap · ATLAS — the ONE shape every operation returns  (#R314)   window.IntMapActionResult
 * ----------------------------------------------------------------------------
...
export function makeAtlasResults(HOST) {
  return (function () {
    ...
    var MESSAGES = {
      'atlas.result.completed':      ['Done.', '完了しました。', 'Erledigt.', 'Готово.', 'Hecho.'],
      'atlas.result.completed.named': ['{what} — done.', '{what} — 完了しました。', '{what} — erledigt.', '{what} — готово.', '{what} — hecho.'],
      'atlas.result.running':        ['Still running…', '実行中です…', 'Läuft noch…', 'Выполняется…', 'En curso…'],
      'atlas.result.running.progress': ['Still running — {done} of {total}.', '実行中 — {total} 件中 {done} 件。', 'Läuft — {done} von {total}.', 'Выполняется — {done} из {total}.', 'En curso — {done} de {total}.'],
      'atlas.result.partial':        ['Partly done — {done} of {total}.', '一部のみ完了 — {total} 件中 {done} 件。', 'Teilweise erledigt — {done} von {total}.', 'Выполнено частично — {done} из {total}.', 'Parcialmente hecho — {done} de {total}.'],
      'atlas.result.failed':         ['That did not happen.', 'これは実行されませんでした。', 'Das ist nicht passiert.', 'Это не выполнено.', 'Eso no ocurrió.'],
      'atlas.result.cancelled':      ['Cancelled.', '中止しました。', 'Abgebrochen.', 'Отменено.', 'Cancelado.'],
      'atlas.result.superseded':     ['Replaced by your newer request.', '新しい依頼に置き換えられました。', 'Durch die neuere Anfrage ersetzt.', 'Заменено более новым запросом.', 'Reemplazado por tu nueva petición.'],
      'atlas.result.needs_input':    ['I need one more thing before I can do that.', 'これを実行するには、もう1つ必要です。', 'Dafür fehlt noch eine Angabe.', 'Для этого нужно ещё кое-что.', 'Falta un dato para poder hacerlo.'],

      /* codes — why a status is what it is. These are the honest sentences the old `warn('⚠')` hid. */
      'atlas.code.not_rendered':     ['It was calculated but nothing was drawn on the map.', '計算はできましたが、地図には何も描かれませんでした。', 'Berechnet, aber nichts auf der Karte gezeichnet.', 'Рассчитано, но на карте ничего не нарисовано.', 'Se calculó, pero no se dibujó nada en el mapa.'],
      'atlas.code.not_visible':      ['It is on the map but currently hidden.', '地図上にはありますが、いまは表示されていません。', 'Auf der Karte, aber gerade ausgeblendet.', 'На карте, но сейчас скрыто.', 'Está en el mapa, pero oculto ahora.'],
      'atlas.code.no_change':        ['Nothing on the map changed.', '地図は何も変わりませんでした。', 'Auf der Karte hat sich nichts geändert.', 'На карте ничего не изменилось.', 'Nada cambió en el mapa.'],
      'atlas.code.unavailable':      ['That is not available right now.', 'いまは利用できません。', 'Das ist gerade nicht verfügbar.', 'Сейчас это недоступно.', 'Eso no está disponible ahora.'],
      'atlas.code.unknown_capability': ['I do not have that operation.', 'その操作は持っていません。', 'Diesen Vorgang habe ich nicht.', 'У меня нет такой операции.', 'No tengo esa operación.'],
      'atlas.code.bad_args':         ['The values given for that operation were not usable.', 'その操作に渡された値が使えませんでした。', 'Die übergebenen Werte waren unbrauchbar.', 'Переданные значения непригодны.', 'Los valores dados no eran utilizables.'],
      'atlas.code.ambiguous_target': ['Several things match — which one?', '複数該当します。どれですか。', 'Mehrere Treffer — welcher?', 'Несколько совпадений — какое?', 'Hay varias coincidencias, ¿cuál?'],
      'atlas.code.timeout':          ['It did not finish in time.', '時間内に終わりませんでした。', 'Es wurde nicht rechtzeitig fertig.', 'Не завершилось вовремя.', 'No terminó a tiempo.'],
      'atlas.code.threw':            ['It stopped with an error.', 'エラーで停止しました。', 'Es ist mit einem Fehler gestoppt.', 'Остановлено из-за ошибки.', 'Se detuvo con un error.'],
      'atlas.code.needs_confirm':    ['That one needs your confirmation first.', 'これは先に確認が必要です。', 'Dafür ist zuerst deine Bestätigung nötig.', 'Сначала нужно твоё подтверждение.', 'Eso necesita tu confirmación primero.'],

      /* input requests — what is missing, said plainly */
      'atlas.input.point':           ['Tap the point on the map you mean.', '地図上で対象の地点をタップしてください。', 'Tippe den gemeinten Punkt auf der Karte an.', 'Коснитесь нужной точки на карте.', 'Toca en el mapa el punto que quieres.'],
      'atlas.input.polyline':        ['Draw the line on the map.', '地図上に線を描いてください。', 'Zeichne die Linie auf der Karte.', 'Нарисуйте линию на карте.', 'Dibuja la línea en el mapa.'],
      'atlas.input.polygon':         ['Draw the area on the map.', '地図上に範囲を描いてください。', 'Zeichne den Bereich auf der Karte.', 'Нарисуйте область на карте.', 'Dibuja el área en el mapa.'],
      'atlas.input.choice':          ['Choose one:', '1つ選んでください:', 'Wähle eines:', 'Выберите одно:', 'Elige uno:'],
      'atlas.input.text':            ['Tell me the value to use.', '使用する値を教えてください。', 'Nenne mir den zu verwendenden Wert.', 'Укажите значение.', 'Dime el valor a usar.'],
      'atlas.input.number':          ['Tell me the number to use.', '使用する数値を教えてください。', 'Nenne mir die Zahl.', 'Укажите число.', 'Dime el número a usar.'],

      /* goal validation — the sentence a turn ends on when the goal was not met */
      'atlas.goal.map_missing':      ['You asked for this on the map, and it is not on the map yet.', '地図上でのご要望ですが、まだ地図に出ていません。', 'Auf der Karte gewünscht — dort ist es noch nicht.', 'Вы просили на карте — там этого ещё нет.', 'Lo pediste en el mapa y aún no está en el mapa.'],
      'atlas.goal.explanation_missing': ['You asked for an explanation, and I only operated the map.', 'ご要望は説明でしたが、地図を操作しただけでした。', 'Du wolltest eine Erklärung — ich habe nur die Karte bedient.', 'Вы просили объяснение — я лишь управлял картой.', 'Pediste una explicación y solo manejé el mapa.'],
      'atlas.goal.action_missing':   ['You asked me to do something, and I only wrote about it.', 'ご要望は操作でしたが、文章で答えただけでした。', 'Du wolltest eine Aktion — ich habe nur darüber geschrieben.', 'Вы просили действие — я только написал о нём.', 'Pediste una acción y solo escribí sobre ella.'],
      'atlas.goal.targets_missing':  ['Not every target was reached: {names}', 'すべての対象には届きませんでした: {names}', 'Nicht jedes Ziel wurde erreicht: {names}', 'Достигнуты не все цели: {names}', 'No se alcanzaron todos los objetivos: {names}']
    };
    ...
    API.text = function (key, params, L) {
      ...
      if (!row) return '';
      var s;
      try { s = L ? L.apply(null, row) : row[0]; } catch (_) { s = row[0]; }
