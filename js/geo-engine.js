...
function _m(){ return window.__imap||null; }
  ...
  /* (#R177) the camera geometry — the mercator projection, the eye position for a camera, the
     pitch that saturates and the zoom floor on a sphere — is js/camera-math.js. It moved out
     under this project's standing rule for the shell's line ceiling (see that file's header).
     `gGuard` could not go with it: it is the one piece that asks the RENDERER whether a camera is
     reachable, so it stays here and is handed to the solvers as their `guard` argument. */
  function gGuard(t){
  ...
  return { id:'maplibre', capabilities:MAPLIBRE_CAPS,
    ...
    setPaint(id,p,v){ const m=_m(); if(!m) return; const Y=m.getLayer(id);
    ...
    commandConfig(patch){
      if(patch&&typeof patch==='object'){
        if('on' in patch) CMD.on=!!patch.on;
        if('detail' in patch) CMD.detail=!!patch.detail;
        if('phase' in patch) CMD.phase=String(patch.phase);
        if(patch.skip&&typeof patch.skip==='object') for(const k of CMD_OPS) if(k in patch.skip) CMD.skip[k]=!!patch.skip[k];
      }
      return { on:CMD.on, detail:CMD.detail, phase:CMD.phase, skip:Object.assign({},CMD.skip) };
    },
    sceneStats(){ const m=_m(); if(!m) return null;   /* ⚠⚠ (#R568) counts WITHOUT copying the style, and a `tiles` that answers null rather than 0 when the tile holder is neither of the two below (5.x tileManagers · ≤4.x sourceCaches). The measurements, and why each one is shaped this way, are in js/perf-hud.js's header — the shell has a line ceiling (tests/r168 #8) and this file is in it. */
    ...
    setFeatureState(f,s){ const m=_m(); if(!(m&&m.setFeatureState)) return;
      ...
      m.setFeatureState(f,s); }, removeFeatureState(f,k){ const m=_m(); if(m&&m.removeFeatureState){ if(k!==undefined) m.removeFeatureState(f,k); else m.removeFeatureState(f); } },
    ...
    setFilter(id,f){ const m=_m(); try{ if(!m) return; const Y=m.getLayer(id);
      ...
      if((CMD.on||CMD.skip.filter)&&skipProp(_cmd,'filter',id,Y.filter,f)) return;
      m.setFilter(id,f); }catch(_){} },
  ...
  function engineFacade(A){
   return {
    ...
    render:{ resize:()=>A().resize(), triggerRepaint:()=>A().triggerRepaint(), canvas:()=>A().getCanvas(),
      ...
      commandConfig:(patch)=>A().commandConfig?A().commandConfig(patch):null },
