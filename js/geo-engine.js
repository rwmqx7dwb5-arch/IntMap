...
function _m(){ return window.__imap||null; }
  ...
  return { id:'maplibre', capabilities:MAPLIBRE_CAPS,
    ...
    sceneStats(){ const m=_m(); if(!m) return null;   /* ⚠⚠ (#R569) counts WITHOUT copying the style, and a `tiles` that answers null rather than 0 when the tile holder is neither of the two below (5.x tileManagers · ≤4.x sourceCaches). The measurements, and why each one is shaped this way, are in js/perf-hud.js's header — the shell has a line ceiling (tests/r168 #8) and this file is in it. */
