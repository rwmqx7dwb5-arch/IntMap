...
window.IntMapModules.radiation=function(HOST){
  ...
  window.IntMapRadiation=(function(){
    ...
    /* ══ (#R574) THE NUCLEAR REGISTRY — DISCOVERED, NOT TYPED ═══════════════════════════════════
       This was twelve hand-written regular expressions with twelve hand-written coordinates. It is
       the shape `.agents/rules/no-ad-hoc-hardcoding.md` §1 names first — an embedded list of names —
       and it failed in both of the ways that rule predicts:
         · IT WENT SILENT ON THE THIRTEENTH. Ask about any of the world's other ~630 nuclear sites
           and the ladder fell through to a general-purpose geocoder that does not know them.
         · IT WAS WRONG AND NOTHING COULD TELL. Measured this round: the typed Bushehr coordinate
           (52.23°E) is 130.8 km from the actual reactor (50.887°E). Wikidata and OSM agree with
           each other and disagree only with the number that had been typed here. A list nobody can
           check is a list that quietly rots.
       So the sites are now DISCOVERED from an open source and rebuilt by a script —
       `data/npp.json`, generated by `scripts/build-npp-registry.mjs` from Wikidata (CC0), carrying
       names in the nine UI languages plus the aliases people actually type. docs/RADIATION.md §7.
       ⚠ AND THE MATCH IS A MEASURE, NOT A TABLE (#R515). A query is scored against every name the
       registry holds; below the floor it REFUSES rather than handing back the nearest stranger,
       because a plume drawn from the wrong place is worse than no plume. */
    let _nppP=null, _nppIdx=null;
    /* strip the words that decorate a plant's name in each language without identifying it — a
       reader typing 「福島第一原発」 and one typing "Fukushima Daiichi Nuclear Power Plant" mean the
       same site, and neither spelling is more correct. */
    const NPP_GENERIC=/(原子力発電所|原子力発電|発電所|原発|원자력\s*발전소|核电站|核電廠|核能發電廠|nuclear\s*power\s*(plant|station|complex)?|power\s*(plant|station)|generating\s*station|nuclear|npp|reactor|kernkraftwerk|kkw|centrale\s*nucl[ée]aire|central\s*nuclear|атомная\s*электростанция|аэс|станция|site)/gi;
    function nppNorm(s){
      /* ⚠ the combining-mark range is written as escapes on purpose: as literal marks it is a run of
         invisible characters that any editor, diff or copy-paste can silently eat. */
      return String(s||'').toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g,'')
        .replace(NPP_GENERIC,' ')
        /* \u26a0 THE LONG-VOWEL MARK IS A TRANSLITERATION CHOICE, NOT PART OF THE NAME. \u300c\u30de\u30e4\u30fc\u30af\u300d and
           \u300c\u30de\u30e4\u30af\u300d are one Russian word written by two people; so are \u300c\u30e9\u30fb\u30a2\u30fc\u30b0\u300d and \u300c\u30e9\u30a2\u30fc\u30b0\u300d.
           Measured against the registry: four of the spellings the old hand-written table carried
           failed for exactly this \u2014 a separator or a \u9577\u97f3\u7b26 one side wrote and the other did not.
           This is a rule about writing systems, not a special case for four names. */
        .replace(/[\u30fc\u2010-\u2015\u2212]/g,'')
        .replace(/[^\p{L}\p{N}]+/gu,' ').trim();
    }
    /* the same string with every separator removed. Latin names are compared WITH their spaces (the
       spaces are word boundaries and carry information); CJK and transliterated names are written
       with and without them by different people, so both forms are scored and the better taken. */
    const nppTight=(s)=>String(s).replace(/\s+/g,'');
    /* Dice coefficient over character bigrams — the same measure #R515 put behind geocoding, for the
       same reason: it degrades sensibly across scripts and transliterations, where a regex cannot. */
    function nppBigrams(s){ const b=new Map(); for(let i=0;i<s.length-1;i++){ const g=s.slice(i,i+2); b.set(g,(b.get(g)||0)+1); } return b; }
    function nppDice(a,b){
      if(!a||!b) return 0; if(a===b) return 1;
      if(a.length<2||b.length<2) return a===b?1:0;
      const A=nppBigrams(a), B=nppBigrams(b); let hit=0, na=0, nb=0;
      for(const v of A.values()) na+=v; for(const v of B.values()) nb+=v;
      for(const [g,v] of A) if(B.has(g)) hit+=Math.min(v,B.get(g));
      return (2*hit)/(na+nb);
    }
    function nppLoad(){
      if(_nppP) return _nppP;
      /* the registry is ~300 kB (measured 2026-09-09) and nobody who has not asked about a nuclear site should pay for
         it, so it is fetched on the first question and never at boot. */
      _nppP=fetch('data/npp.json').then(r=>r.ok?r.json():null).then(j=>{
        if(!j||!Array.isArray(j.plants)) return null;
        _nppIdx=[];
        for(const p of j.plants){
          if(typeof p.lat!=='number'||typeof p.lon!=='number') continue;
          const names=[p.name];
          if(p.labels) for(const k of Object.keys(p.labels)) names.push(p.labels[k]);
          if(Array.isArray(p.alias)) for(const a of p.alias) names.push(a);
          const keys=[]; const seen=new Set();
          for(const n of names){ const k=nppNorm(n); if(k&&!seen.has(k)){ seen.add(k); keys.push(k); } }
          if(keys.length) _nppIdx.push({p,keys});
        }
        return j;
      }).catch(()=>null);
      return _nppP;
    }
    /* THE FLOOR. Observation (2026-09-09): over the 638-site registry, the twelve spellings the old
       table carried all score ≥0.62 against their own site, while the best WRONG match any of them
       ...
       floor. CANONICAL: this constant and tests/r574-npp-checks.test.mjs. */
    ...
    /* ⚠ AN AMBIGUOUS NAME IS NOT A MATCH, AND IT IS NOT A MISS EITHER. 「福島原発」 names Fukushima
       ...
       first is the #R515 failure with extra steps. So a near-tie between DIFFERENT sites is
       reported as one — the answer still comes back (refusing outright would lose a name the old
       table could resolve) but it says what else it matched, and Atlas can say so too. */
    const NPP_TIE=0.04;
    function nppScore(key,k){
      if(key===k) return 1;
      /* a query that STARTS a name is much stronger evidence than one that merely appears inside it:
         「запор」 is how a person begins to type Запорожская, while a three-letter substring in the
         middle of a name is a coincidence. Measured: without this rung «запор» scored 0.509 against
         its own plant — below the floor — purely because the query is short. */
      const pre=(key.indexOf(k)===0||k.indexOf(key)===0);
      const short=Math.min(key.length,k.length), long=Math.max(key.length,k.length);
      let s=nppDice(key,k);
      /* ⚠ HOW MANY CHARACTERS COUNT AS EVIDENCE DEPENDS ON THE WRITING SYSTEM. Two Han characters
         carry about as much as four Latin letters — 「福島」 names a prefecture, «fu» names nothing —
         so a fixed minimum is a rule about Latin pretending to be a rule about names. Measured:
         with a flat minimum of 3, 「福島原発」 (which normalises to 「福島」 once 原発 is stripped)
         matched nothing at all. */
      const CJK=/[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}]/u;
      const minEvidence=(CJK.test(k)?2:3);
      if(short>=minEvidence&&long){
        if(pre) s=Math.max(s,0.55+0.45*(short/long));
        else if(key.indexOf(k)>=0||k.indexOf(key)>=0) s=Math.max(s,0.9*(short/long)+0.1);
      }
      return s;
    }
    function nppMatch(q){
      const k=nppNorm(q); if(!k||!_nppIdx) return null;
      const kt=nppTight(k);
      let best=null, bestS=0, second=null, secondS=0;
      for(const e of _nppIdx){
        let s=0;
        for(const key of e.keys){
          const v=Math.max(nppScore(key,k), nppScore(nppTight(key),kt));
          if(v>s) s=v;
        }
        if(s>bestS){ secondS=bestS; second=best; bestS=s; best=e.p; }
        else if(s>secondS&&e.p!==best){ secondS=s; second=e.p; }
      }
      if(!best||bestS<NPP_FLOOR) return null;
      const tied=(second&&(bestS-secondS)<NPP_TIE)?second:null;
      return {lng:best.lon,lat:best.lat,name:best.name,id:best.id,country:best.country,
        status:best.status,netMW:best.netMW,units:best.units,score:+bestS.toFixed(3),
        ambiguous:!!tied, alt:tied?[{name:tied.name,id:tied.id,score:+secondS.toFixed(3)}]:[]};
    }
    /* ⚠ ASYNC NOW. The callers in js/atlas-console.js already await a geocode on the next rung of
       the same ladder, so this costs nothing there; a synchronous door would have forced the whole
       registry to be eager. */
    async function resolveSite(q){ q=String(q||'').trim(); if(!q) return null; await nppLoad(); return nppMatch(q); }
    /* the registry, for anything that wants the sites themselves rather than a name lookup */
    async function plants(){ await nppLoad(); return _nppIdx?_nppIdx.map(e=>e.p):[]; }
    async function nearestPlant(lat,lon){
      await nppLoad(); if(!_nppIdx) return null;
      const cos=Math.cos(lat*Math.PI/180); let best=null, bd=Infinity;
      for(const e of _nppIdx){ const dy=(e.p.lat-lat)*111.32, dx=(e.p.lon-lon)*111.32*cos, d=Math.sqrt(dy*dy+dx*dx);
        if(d<bd){ bd=d; best=e.p; } }
      return best?Object.assign({km:bd},best):null;
    }
    /* ── the chain: a modelled plume, and what the instruments around it are ACTUALLY reading ────
       This is the join the round exists for. It does NOT change the model — measurement and model
       stay separate claims — it puts them side by side so a reader can see whether the world agrees
       with the hypothesis. Returns [] when the measured layer has never been loaded, which is an
       honest "nothing to compare", not a claim that the readings are normal. */
    async function observedAround(lat,lon,km){
      try{
        const ok=await window.IntMapLazy.need('radiationLayer');
        if(!ok||!window.IntMapRadiationObs) return [];
        if(!window.IntMapRadiationObs.state().stations) await window.IntMapRadiationObs.load(null);
        return window.IntMapRadiationObs.near(lat,lon,km||200);
      }catch(_){ return []; }
    }
    ...
    return { run, clear, isOpen, openPanel, closePanel,
      ...
      plants, nearestPlant, observedAround,   /* (#R574) the registry and the measured-radiation join; resolveSite is on the line above and is now async */
