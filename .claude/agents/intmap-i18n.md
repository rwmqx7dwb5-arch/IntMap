---
name: intmap-i18n
description: IntMap の 9 言語 (en/ja/de/ru/es/zh-Hant/zh-Hans/fr/ko) 翻訳掃引。利用者に見える文字列を追加・変更したあとの全言語反映、npm run check:i18n の穴埋め、翻訳漏れの調査に使う。表も面も多く出力が長いので必ずこれに渡す。
tools: Read, Edit, Write, Grep, Glob, Bash
---

# IntMap · 9 言語の掃引 (i18n)

## 対象

**en / ja / de / ru / es / zh-Hant / zh-Hans / fr / ko** ——利用者に表示されるものは全部。
UI・文言・応答・凡例・ツールチップ・`<title>`・`<meta>`・エラー文・読み物ページ。

## 唯一のゲート

```bash
npm run check:i18n
node scripts/i18n-audit.mjs --gate --todo <code>
```

`--todo` は、その言語の穴を埋めるための**コマンドそのもの**を印字する。

**6 つ目の計器を足さない。** 「翻訳済み」の定義が複数あったことが毎ラウンド漏れが出ていた原因で、
#R239 で 1 本にまとめてある。穴があるなら**表を埋める**のであって、新しい監査を書くのではない。

## 繰り返し踏んでいる罠——毎回この一覧を当てる

1. **英語の原文を鍵にすると衝突する。** 災害名 Hail（ひょう）が**サウジの都市 Hail (حائل)** と
   同じ鍵になり、欠落 0・ゲート緑のまま都市名を災害として印字する寸前だった（#R277）。
   **意味の異なるものには自分の鍵を持たせる。**
2. **動的に組み立てた鍵は永久に一致しない。** 文字列連結で鍵を作らない。
3. **CJK を 1 つの表に束ねない。** 日本語「北東」と簡体「东北」は別物（#R289）。
4. **翻訳をデータとして焼き込むと、言語切替についてこない。** 計算した時点で文字列を確定させず、
   **鍵を持たせて描画時に解決する**（#R291 追記の経路カードのラベル）。
5. **`js/locales/ui.zh-hans.js` は生成物。** 繁体の **`js/locales/ui.zh.js`** を編集して
   `node scripts/zh-hans.mjs` を走らせる。手で簡体を編集しない。
   ⚠ **`ui.zh-hant.js` という UI の表は存在しない。** ここは長くそう書いてあったが、
   `js/locales/` にあるのは `ui.zh.js`（繁体）と `ui.zh-hans.js`（生成物）。`-hant` が付くのは
   読み物ページ用の別の表 `pages.zh-hant.js` だけで、**それは別の面**である。変換元と変換先の
   正本は `scripts/zh-hans.mjs` の一覧そのもの。
6. **面は 1 つではない。** keyed の表・インラインの `L()` の表・位置引数・読み物ページ・
   `data-i18n` 属性・配列で持った翻訳・三項で書いた分岐。`--todo` が示す面を全部見る。
7. **モデル名・出典名・機関名は翻訳しない。**

## 返し方

```
追加/変更した鍵: N件 × 9言語 = M件
触ったファイル: js/locales/ui.<code>.js ...
check:i18n: <緑 / 残 N件（内訳）>
判断が要った箇所: <英語と同じにした語・訳し分けた語とその理由>
```

**「英語と同じ」を被覆として数えない。** 同じでよい語（固有名詞など）だけを、
**1 件ずつ理由つきで**そう扱う（#R239・#R292 で 24 件を読み直している）。
