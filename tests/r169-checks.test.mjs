...
const MODULES = {
  ...
  mapReadout:       { file: 'js/map-readout.js', k: 'IM_READOUT', /* (#R221) demTilePoints + releaseDEMHold: the intensity field asks for one point per DEM TILE and PINS what it warms — see js/map-readout.js */ exports: ['_demZoomForSpan', 'demElevAt', 'demElevBilinear', 'demSnapshot', 'demTilePoints', 'demVoidStats', 'demZoomForMap', 'releaseDEMHold', 'fetchBathymetry', 'fmtElevVal', 'fmtLL', 'handleMapClick', 'refreshGrid', 'renderCoordReadout', 'setGrid', 'showMeasureTip', 'updateCompass', 'updateCoord', 'updateLayerReadout', 'warmDEMTiles'], /* (#R568) what the DEM tile store is actually retaining. Read through IM_READOUT (js/app-body.js publishes it as window.__imDemStore for js/perf-hud.js), so it costs the app shell no lines — tests/r168 #8 has no headroom. */ direct: ['demStoreStats'] },
