...
test('#R251 (deep) every language: nothing on screen is English while IntMap holds a translation of it', async ({ page }) => {
  ...
  /* ══ ③ (#R474) THE FAVOURITES HEADING DRAWS EXACTLY ONE STAR, IN EVERY LANGUAGE ═════════════════
     ...
     bytes that shipped. tests/r474-checks.test.mjs gates the TABLES on that reasoning (a ceiling of
  ...
  expect(favStar.ok, `#R474 ③ starring a layer: ${favStar.why}`).toBe(true);
  ...
  /* ══ ③ (#R474) — 数える。①の掃引が言語を切り替えたその状態で読んだ見出しを、ここで判定する ══ */
  ...
  expect(favSeen.length, `#R474 ③ every language was read (${favReport})`).toBe(langs.length + 1);
  expect(favSeen.filter((f) => favStarsIn(f.text || '').length !== 1).map((f) => f.lang),
    `#R474 ③ the favourites heading draws exactly ONE star in every language:\n${favReport}\n`).toEqual([]);
  for (const f of favSeen) {
    expect(favStarsIn(f.text)[0], `#R474 ③ ${f.lang}: the star is the markup's ⭐, not a ★ the translation brought`).toBe('⭐');
    ...
    expect(favStrip(f.text), `#R474 ③ ${f.lang}: the heading is that language's own translation`).toBe(f.own);
    expect(String(f.own || '').length, `#R474 ③ ${f.lang} has a favLayers row of its own`).toBeGreaterThan(0);
  }
