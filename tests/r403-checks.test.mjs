...
test('R403 ③ a roster that is missing a name, and a count that is wrong, both go RED', async () => {
  await withTreeLock(async () => {
    ...
    /* 数だけを間違える（名前は12本のまま） */
    const num = anchorRe('**Edge Functions は 16 本**');
...
test('R403 ④ a legitimately PARTIAL list of functions is not read as the roster', async () => {
  await withTreeLock(async () => {
    ...
    assert.ok(/を共有するのは12本\*\*（`ais-feed`/.test(arch),
    ...
    assert.ok(/All sixteen are declared there now/.test(rd('docs/SECURITY-ARCHITECTURE.md')),
...
const NEW_RULES = [
  ...
  { rule: 'relay-guard', file: 'Architecture.md', why: 'a shared-module count that no longer matches the imports',
    /* (#R510) ais-feed made it ten; (#R574) radiation-feed made it twelve. The MUTATION is what
    ...
    from: '`_shared/relay-guard.js` を共有するのは12本', to: '`_shared/relay-guard.js` を共有するのは5本' },
