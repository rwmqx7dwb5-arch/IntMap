...
/* (#R585) DISCOVERED, NOT TYPED. This was a hand-written array, and its own comment said what was
...
test('every Edge Function in the tree is declared in config.toml, and _shared is not one', () => {
  ...
  const dirs = readdirSync(join(ROOT, 'supabase/functions'), { withFileTypes: true })
    ...
    .map((d) => d.name).sort();
  /* ⚠ (#R585) THIS LINE USED TO COMPARE DISK TO A TYPED ARRAY. The array is now derived from disk
