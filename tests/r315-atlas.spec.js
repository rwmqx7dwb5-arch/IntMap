/* ============================================================================
 *  R315 — the half of the kernel's browser proof that needs Atlas itself
 * ----------------------------------------------------------------------------
 *  These three assertions all require js/atlas-console.js — 719 kB fetched on demand (#R224) — so
 *  they cannot live in tests/r315.spec.js, which stands in the sixty-four-second gate. Same round,
...
test('R315-atlas ①: loading Atlas binds the registry to the engine, and the same kernel is used', async () => {
...
test('R315-atlas ②: the built app sends a relevant catalogue instead of all of it', async () => {
...
test('R315-atlas ③: the button and the command leave the same state', async () => {
  const r = await page.evaluate(async () => {
...
test('R315-atlas ④: Atlas loaded without console errors', async () => {
