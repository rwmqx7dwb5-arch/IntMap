/* ============================================================================
 *  R438 — Windy's own colours for three more fields, and the isobars become a switch
 * ----------------------------------------------------------------------------
...
test('R438 ① the three fitted tables are complete, ordered, opaque and in the reader’s unit', () => {
...
test('R438 ①b the resampled ramps land on their anchors exactly', () => {
...
test('R438 ② the ramps are registered under the family names the resolver resolves to', () => {
...
test('R438 ③ the new ramps are memoised and built lazily, not at parse time', () => {
...
test('R438 ④ the contour interval is declared once and converted through FIELD_UNITS', () => {
...
test('R438 ⑤ ec-isobars has no row and no legend box, and follows ec-slp', () => {
...
test('R438 ⑤b the sub-layers are raised over their parent from every path that rebuilds one', () => {
...
test('R438 ⑥ the retired isobar id still resolves — share link, alias and a verb of its own', () => {
...
test('R438 ⑦ the particle preference is per layer, and ec-temp keeps its old key', () => {
...
test('R438 ⑧ the legend’s model picker is contained by the legend', () => {
  ...
  const blk = DL.slice(DL.indexOf('(#R438) THE MODEL PICKER'), i);
...
test('R438 ⑨ the promoted rows are in 気候・気象 and in no other list', () => {
