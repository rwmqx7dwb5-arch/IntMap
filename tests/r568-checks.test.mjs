/* ============================================================================
 *  #R568 — 「スマホでズームやホバーが遅い。操作によってはブラウザが落ちる」
 * ----------------------------------------------------------------------------
...
test('① 480 tiles asked for at once are 24 requests, not 480', () => {
  ...
  /* ⚠ THE PEAK, MEASURED WITHOUT ASKING THE STORE ANYTHING. Counting the Image objects the module
     actually constructed is a question the pre-#R568 code answers too — and it answered 480. */
...
test('② after 480 unpinned tiles have all landed, the cache is back inside its ceiling', () => {
  ...
  /* ⚠ …AND THE SAME FACT MEASURED THROUGH THE PUBLIC DOOR, so the question is one the pre-#R568
...
test('⑪ a phone that never reported its memory takes the smaller tile budget', () => {
  ...
  assert.equal(make(true, false, undefined), 640, 'WebKit reports nothing — every iPhone took 1024 before #R568');
