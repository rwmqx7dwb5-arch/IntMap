...
test('R320 ③a: the module catalogue names the modules that load on demand', () => {
  ...
  assert.match(read('js/lazy-modules.js'), /publishes: \(n\) => PUBLISHES\(n\)/,   /* (#R799) a view over the registry */
...
test('R320 ③c: every lazy module the loader publishes is a name the catalogue can offer', () => {
  /* (#R799) the manifest is the registry's `publishes` column */
...
test('R320 ④: the capability audit is still green, and still asks twenty-three questions', () => {
  ...
  /* ⚠ THIS NUMBER IS A GUARD, NOT A POLICY ([[intmap-ceiling-guards-are-not-policies]]): it is here so a
     ...
     added argument-schemas and required-arguments; #R799 added catalogue-subject, which asks of every
