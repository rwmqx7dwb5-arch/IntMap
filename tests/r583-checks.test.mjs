/* ============================================================================
 *  R583 — 「これなに」 must not be answered by inventing a name  (tests)
 * ----------------------------------------------------------------------------
...
test('R583 ①: the coordinate handed to the model resolves finer than one pixel of the frame', () => {
...
test('R583 ①: at the zoom that produced the wrong answer, the coordinate resolves the building', () => {
...
test('R583 ②: a feature far larger than the view does not outrank one the view contains', () => {
...
test('R583 ②: a feature outside the frame is not reported at all', () => {
...
test('R583 ②: an unnamed footprint is never offered as an identification', () => {
...
test('R583 ②: the tags survive, so Atlas can tell a road from a building', () => {
...
test('R583 ②: the reported list is a head of a measured order, and says how much it dropped', () => {
  const many = [];
...
test('R583 ③: finding nothing produces a POSITIVE statement, never an empty block', () => {
...
test('R583 ③: a failed lookup is never reported as «nothing is there»', () => {
...
test('R583 ③: the block names the failure mode instead of asking for care in general', () => {
...
test('R583 ④: a link to a host the turn never fetched stops being a link', () => {
...
test('R583 ④: the sentence is never deleted — Atlas loses no capability to this rule', () => {
...
test('R583 ④: a host the turn actually fetched keeps its link', () => {
...
test('R583 ④: citation pills are built from the registry and are not this rule\'s business', () => {
...
test('R583 ⑤: a world view is not asked what single object is in it', () => {
  assert.equal(G.framedBudget({ south: -60, west: -180, north: 70, east: 180 }).ok, false);
});

test('R583 ⑤: the frame that produced the wrong answer is well inside the budget', () => {
  assert.equal(G.framedBudget({ south: 35.148, west: 136.842, north: 35.155, east: 136.8515 }).ok, true);
});

test('R583 ⑤: skipping the lookup is reported as «not looked up», never as an absence', () => {
...
test('R583 ⑥: relations are asked for only where the schema says they are areas', () => {
...
test('R583 ⑥: the bbox is written at full precision and the timeout is bounded', () => {
