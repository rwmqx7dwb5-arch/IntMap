...
/* ══ R348 — THE ANSWER CONTRACT, MEASURED IN THE BROWSER ══════════════════════════════════════
...
test('R348 ①: the structured answer renders through the real reply pipeline, and a model URL is not a link', async () => {
  const r = await page.evaluate(() => {
...
test('R348 ②: the answer stylesheet is on the page, not merely in the module', async () => {
...
test('R348 ③: a fabricated host cannot enter the registry in the browser either', async () => {
  const r = await page.evaluate(() => {
