...
test('R256 ①: the crop layer only rebuilds when its layer is actually gone', () => {
...
test('R256 ②: the along-the-shaft arrowhead repeater is gone, and removed from a live style', () => {
  const s = read('js/world-packs.js');
...
test('R256 ②b: the switch takes the whole arrow, and the head is not pasted on', () => {
  const s = read('js/world-packs.js');
...
test('R256 ②c: the shaft is trimmed in the renderer’s own projection', () => {
  const s = read('js/world-packs.js');
...
test('R256 ③: the night sky opens in the standing view', () => {
...
test('R256 ④: the sculpted terrain source is created once and re-tiled in place', () => {
...
test('R256 ⑤: a levee has a flat crest and is wide enough for an 8-connected flood', () => {
  const s = read('js/terrain-water.js');
...
test('R256 ⑤b: every door that changes the ground marks it dirty', () => {
  const s = read('js/terrain-water.js');
  ...
  assert.match(s, /editDirty\(\);\s+\/\* \(#R256\) a restored levee is a change to the ground/,
    'and so does a levee restored from a share link');
});

/* ── ⑥ the straight section that ignored the terrain ────────────────────────────────────────── */
test('R256 ⑥: crossing a lake draws the crossing, not a chord', () => {
  const s = read('js/terrain-water.js');
...
test('R256 ⑦: the terrain panel is a grouped inset list with a pinned clock', () => {
  const s = read('js/terrain-water.js');
...
/* ── ⑧ who is in front ─────────────────────────────────────────────────────────────────────── */
test('R256 ⑧: the front-most band covers every panel, including the compare window', () => {
...
test('R256 ⑧b: click-to-front orders the windows inside the band, not above it', () => {
...
test('R256 ⑨: every non-layer simulation is a row in the tools list', () => {
...
test('R256 ⑨b: 地震波シミュレーター is 地震シミュレーター in every language', () => {
...
test('R256 ⑩: Energy & resources is a category with two surveyed layers in it', () => {
...
test('R256 ⑪: the curated table grew, and a size on the map means a published number', () => {
  const s = read('js/datacenters.js');
  const rows = (s.slice(s.indexOf('const DC=['), s.indexOf('\n  ];', s.indexOf('const DC=['))).match(/^\s*\[-?[0-9]/gm) || []).length;
  assert.ok(rows >= 310, 'the curated table holds ' + rows + ' entries (it was 260 before #R256)');
