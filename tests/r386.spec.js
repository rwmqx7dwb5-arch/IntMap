...
/* ⚠ (#R386) THIS FILE IS THE ROUND'S GATE SPEC, so it carries only the two claims that must hold
   ...
   differences, the detail view, and the Atlas state provider — are in tests/r386-detail.spec.js,
   which the whole suite runs and the gate does not (scripts/tiers.mjs · #R337's shape). */
test('R386 ① 出来事が読み込まれ、記事ではなく出来事が一覧の項目になる', async ({ app }) => {
  const page = app.page;
...
test('R386 ② カテゴリは一覧と地図の両方に効く（述語が 1 本しかないので）', async ({ app }) => {
  const page = app.page;
  await loadEvents(page);
