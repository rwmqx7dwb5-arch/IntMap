/* ============================================================================
 *  #R258 — the session has an END, and the end is written down
 * ----------------------------------------------------------------------------
...
test('#R258 ① CLAUDE.md has §11 作業終了処理 and still has 本ファイル自体の保守', () => {
...
test('#R258 ② CLAUDE.md still carries each clause of the finish procedure', () => {
  const md = read('CLAUDE.md');
  const rules = [
    ['作業完了後に必ず実行',     '作業終了処理'],
...
test('#R258 ③ the finish report has a line for each outcome', () => {
  const md = read('CLAUDE.md');
...
test('#R258 ④ the mirror is one-way, PC → USB', () => {
  const md = read('CLAUDE.md');
...
test('#R258 ⑤ CONSTITUTION.md and Architecture.md record the finish procedure', () => {
...
test('#R258 ⑥ tests/r258-checks.test.mjs is wired into `npm run test:checks`', () => {
  ...
  assert.ok(pkg.scripts['test:checks'].includes('tests/r258-checks.test.mjs'),
