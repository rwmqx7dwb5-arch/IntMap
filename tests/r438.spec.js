/* ============================================================================
 *  R438 — 凡例の中の4つが、ブラウザで実際にそうなっていること
 * ----------------------------------------------------------------------------
 *  ⚠⚠⚠ **どれもソースの形では言えない。**
 *    ①「モデルの選択欄が凡例から突き出ている」は<b>幅の話</b>である。CSS を足したことは
 *      `tests/r438-checks` が読めるが、「はみ出していない」は**描かれた矩形にしか訊けない**
 ...
 *    #R438 で入口を付け替え、スタック順の表明を足してある）。
 * ==========================================================================*/
import { test, expect } from './helpers/app.js';

test('R438 the pressure legend: a picker that fits, an isobar switch that works, Windy’s key, and the promoted rows', async ({ app }) => {
  const page = app.page;
