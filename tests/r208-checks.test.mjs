...
function locator() {
  ...
  /* ⚠ (#R572) THE MATCHABLE VIEW, WHICH IS WHAT js/news-context.js HANDS OVER. The build no longer
...
test('R208 ⑦b: the legacy regex index is capped; the growth goes to the locator that can index it', () => {
  ...
  /* ⚠ (#R572) the rows the cap is applied TO are the matchable ones — this index is itself a
  ...
  assert.ok(/const w=worldMatchable\(\);/.test(gz), 'and it is fed the matchable view (#R572)');
  ...
  assert.ok(/worldMatchable,/.test(gz), '…and the matchable view for the locator (#R572)');
  const nc = read('js/news-context.js');
  ...
  assert.ok(/const w=GZ\.worldMatchable&&GZ\.worldMatchable\(\);/.test(nc),
    'that array being the matchable view — the curated coordinate still wins (#R572)');
});
