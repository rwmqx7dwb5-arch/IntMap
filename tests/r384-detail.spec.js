/* ============================================================================
 *  R384 — 出来事の詳細・媒体間の相違・Atlas の状態（門ではなく全体スイートの側）
 * ----------------------------------------------------------------------------
...
/* ⚠ (#R384) 門は tests/r384.spec.js の 2 件だけで、こちらは全体スイートが走らせる 3 件。
   分けたのは時間のためであって、弱いからではない——「媒体間で数量が食い違っている点を保持して
   表示できる」は依頼の 6 つの Done when の 1 つで、ここが唯一それを測っている。 */
test('R384 ③ 媒体ごとの相違は、別々の系列が違う数を言ったときだけ出る', async ({ app }) => {
  const page = app.page;
...
test('R384 ④ 詳細は、誰がいつ報じたか・同系列・初報・組み立て方を出す', async ({ app }) => {
  const page = app.page;
  await loadEvents(page);
...
test('R384 ⑤ Atlas は News について観測した事実を持つ（state provider）', async ({ app }) => {
  const page = app.page;
  await loadEvents(page);
