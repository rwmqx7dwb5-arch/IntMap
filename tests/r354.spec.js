/* ============================================================================
 *  R354 — the cable graphic did not change, and the layer still draws
 * ----------------------------------------------------------------------------
 ...
 *  tests/r354-cables.spec.js, which runs nightly.
 *
 ...
 *  tests/r186.spec.js pins that default, and tests/r354-cables.spec.js ① checks
...
test('R354 the cable layer draws real routes, with the graphic it always had', async ({ page }) => {
  ...
  const s = await page.evaluate((ids) => {
    ...
    out.rendered = f.length;
    out.quality = f.length ? f[0].properties.quality : null;
    out.hasSrcProp = f.length ? !!f[0].properties.src : null;
    return out;
  }, LAYERS);
  ...
  expect(s.rendered, 'cables are actually rendered').toBeGreaterThan(20);

  /* ── it is the REBUILT dataset, not the schematic ───────────────────────── */
  expect(['verified', 'reconstructed', 'estimated'], 'every route declares its quality').toContain(s.quality);
  expect(s.hasSrcProp, 'every route declares where it came from').toBe(true);

  /* ══ THE RECORDED BASELINE — §2 lists every one of these by name as forbidden
     to change. tests/r354-checks.test.mjs holds the other half of §17: the same
