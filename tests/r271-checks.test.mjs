...
test('R271 ⑦ every layer id is in exactly one group, and the moved rows are where they were sent', () => {
  ...
  /* ⚠ (#R468) THE OLD PARSE WAS A REGEX AND IT DIED THE DAY THE TUPLE GREW A THIRD ELEMENT.
     `/\['(lyrGrp\w+)',\[([^\]]*)\]\]/` needs the id list to be followed immediately by `]]`;
     #R468 gave every shelf a count of the rows the reader named, and the pattern matched NOTHING —
  ...
  /* ⚠ (#R468) THE SLOPE ROW IS NOT ON A SHELF BECAUSE THERE IS NO SLOPE ROW. #R273 promoted it
  ...
  const code = codeOnly(s);
  /* ══ ⚠⚠ (#R468) THIS ROW WENT BACK, BY INSTRUCTION — 「基本表示の『タイムゾーン（現在時刻）』
...
test('R271 ⑦ every group key the panel uses has a heading in all nine languages', () => {
  const keys = GROUPS.map(([k]) => k);   /* (#R468) the value, not the spelling — see ⑦ above */
