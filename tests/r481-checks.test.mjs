/* ============================================================================
 *  R481 — 「DOM に在って viewport の中」は「読者に見えている」ではない
 * ----------------------------------------------------------------------------
...
test('R481 ① the credit sits outside the map container, in the map column', () => {
...
test('R481 ② the map column is a flex column and the map gives up the row', () => {
...
test('R481 ③ the base .map-credit rule is layout, not position', () => {
  const css = codeOnly(read('css/intmap.css'));
...
test('R481 ④ the attribution survives capture mode', () => {
  const css = codeOnly(read('css/intmap.css'));
...
test('R481 ⑤ nothing else in the map column was moved to make room', () => {
  const css = codeOnly(read('css/intmap.css'));
