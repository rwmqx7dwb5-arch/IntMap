// ============================================================================
//  IntMap · tests/r585-radiation-sources.test.mjs — the radiation registry, offline  (#R585)
// ----------------------------------------------------------------------------
...
test("R585 ① every provider declares the full contract", () => {
...
/* ── ③ the one conversion, including the one it must refuse ─────────────────────────────────────
   ...
   convert at all. */
test("R585 ③ toNanoSvH converts what upstreams declare and refuses what they do not", () => {
...
test("R585 ④ coordinates are validated, and a missing one keeps its reading", () => {
...
test("R585 ⑤ de-bfs parses real GeoJSON and reads the declared unit", () => {
...
/* ── ⑥ jp-nra: the same station arrives many times, and only the newest may survive ─────────────
   ...
   case.) */
test("R585 ⑥ jp-nra collapses repeated samples to the newest per station", () => {
...
test("R585 ⑥b a value under the detector's own declared floor says so", () => {
  const p = providerById("jp-nra");
...
test("R585 ⑦ fi-stuk joins values to named stations and drops NaN", () => {
...
test("R585 ⑧ hk-hko discovers its stations instead of listing them", () => {
...
test("R585 ⑨ the 2011 annual mean names itself and stays out of the live set", () => {
...
test("R585 ⑩ an unread provider is distinguishable from one that read zero", () => {
...
test("R585 ⑪ RadNet fails the budget by cost, and its chunks tile the whole network", () => {
...
test("R585 ⑫ us-epa reads its unit from the header and does not invent a timezone", () => {
  const p = providerById("us-epa");
...
test("R585 ⑬ the compact wire row still carries quantity and kind", () => {
...
test("R585 ⑭ every upstream host is one of the licensed networks", () => {
