/* ============================================================================
 *  R355 (deep) — everything about the cable layer that needs more than one boot
 * ----------------------------------------------------------------------------
 ...
 *  five tests and two boots belong. tests/r355.spec.js carries the two claims
...
const installFinder = (p) => p.evaluate(() => {
  window.__r355FindPoint = (layer, pad, step) => {
    const E = window.IntMapGeoEngine;
...
test('R355 ② OFF → ON → OFF → ON never leaves a ticked box with no layer', async () => {
  await waitForCables(page);
...
test('R355 ③ a style reload rebuilds the layer rather than losing it', async () => {
  await waitForCables(page);
...
test('R355 ④ clicking a cable opens a popup that names it', async () => {
  await waitForCables(page);
  ...
  const pt = await page.evaluate(() => window.__r355FindPoint('lyr-subcables', 8, 7));
...
test('R355 ⑤ a landing point opens its own popup and lists the cables that land there', async () => {
  await waitForCables(page);
  ...
  const pt = await page.evaluate(() => window.__r355FindPoint('lyr-subcables-pts', 6, 3));
...
test('R355 ⑥ the layer still appears when the new dataset cannot be fetched', async ({ browser }) => {
