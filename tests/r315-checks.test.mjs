/* ============================================================================
 *  R315 — the Atlas control kernel: registry, executor, results, state, planner, audit
 * ----------------------------------------------------------------------------
...
test('R315 ①a: every live dispatch spelling resolves to a canonical capability', () => {
...
test('R315 ①b: the registry adds nothing that cannot run, and misses nothing that can', () => {
  const groups = dispatchGroups(lines('js/atlas-console.js'));
...
test('R315 ①c: a capability that writes cannot be registered without observe() and verify()', () => {
  const caps = makeAtlasCapabilities({});
...
test('R315 ①d: a lazy capability is discoverable before its module exists', () => {
...
test('R315 ①e: a non-equivalent substitution is recorded as forbidden', () => {
...
test('R315 ②a: the audit is green on the tree as it stands, except where it is honestly red', () => {
...
test('R315 ②b: it goes red on an unreachable dispatch label', () => {
...
test('R315 ②c: it goes red on a capability the planner is never told about', () => {
  const caps = makeAtlasCapabilities({});
...
test('R315 ②d: it goes red when a promise can be reported before it settles', () => {
...
test('R315 ②e: it goes red on a success claimed on top of a swallowed error', () => {
...
test('R315 ②f: it goes red when a tool is allowed to take the map centre', () => {
  const caps = makeAtlasCapabilities({});
...
test('R315 ②g: it goes red when the registry truncates its own population', () => {
...
test('R315 ②h: it goes red when a withdrawal quietly ends', () => {
...
test('R315 ③: a caller cannot write a success it did not observe', () => {
...
test('R315 ③b: a legacy result that could not be verified does not become "completed"', () => {
...
test('R315 ④a: an async capability is not "completed" until its promise settles', async () => {
...
test('R315 ④b: a rejected promise is a failure, not a success', async () => {
  const { caps, exec } = kernel();
...
test('R315 ④c: a required point that was not given is asked for, never invented', async () => {
  const { caps, exec } = kernel();
...
test('R315 ④d: a route computed and not drawn is not_rendered, not success', () => {
...
test('R315 ④e: cancel and supersede reach the work, and the result says which', async () => {
  const { caps, exec } = kernel();
...
test('R315 ④f: two operations writing the same thing do not interleave', async () => {
  const { caps, exec } = kernel();
...
test('R315 ④g: unknown capabilities and bad arguments are refused, not shrugged off', async () => {
  const { caps, exec } = kernel();
...
test('R315 ⑤a: the old plan shapes all still parse', () => {
  const { P } = planner();
...
test('R315 ⑤b: unknown capabilities, dangling deps and bad $refs are REJECTED with a reason', () => {
  const { P } = planner();
...
test('R315 ⑤c: a step reads its predecessor\'s output instead of re-deriving it', async () => {
  const { k, P } = planner();
  let geocodes = 0;
...
test('R315 ⑤d: independent steps run together; a dependent one does not run after its prerequisite fails', async () => {
  const { k, P } = planner();
...
test('R315 ⑥a: a map that was asked for and not produced leaves the goal unmet', () => {
  const { P } = planner();
...
test('R315 ⑥b: a request to DO something is not satisfied by writing about it', () => {
  const { P } = planner();
...
test('R315 ⑥c: a turn with an operation still waiting is not finished', () => {
  const { P } = planner();
...
test('R315 ⑥d: repair aims at the unmet GOAL, not at the call that failed', () => {
  const { P } = planner();
...
test('R315 ⑦a: the model is told the language it must answer in, in ENGLISH, for all nine', () => {
  ...
  /* (#R315) the two helpers moved to js/ai-core.js — the transport is what carries them, and the
...
test('R315 ⑦b: the five-language tables inside Atlas are gone', () => {
...
test('R315 ⑦c: every language has a locale file and a region tag', () => {
...
test('R315 ⑦d: the capability search can rank a request written in any script', () => {
...
test('R315 ⑧a: the client stamps a turn key, and it travels in a header', () => {
  const core = codeOnly(read('js/ai-core.js'));
...
test('R315 ⑧b: the server does not trust the key it is given', () => {
...
test('R315 ⑧c: a deterministic operation needs no AI at all', async () => {
...
test('R315 ⑨a: the 58 kB catalogue moved byte-for-byte and is still whole', () => {
...
test('R315 ⑨b: the kernel shrank by moving, and the ceiling came down with it', () => {
  ...
  assert.ok(atlas < 5_270, `js/atlas-console.js is ${atlas} lines; #R315 moved the planner, the catalogue and the state out and must stay below 5,270`);
...
test('R315 ⑨c: the #R135 block moved whole, and its four tuned rows are unchanged', () => {
...
test('R315 ⑨d: the state is data, and the paragraph is derived from it', () => {
...
test('R315 ⑨e: "that one" resolves by id, and a rewound turn stops answering', () => {
  const S = makeAtlasState({});
...
test('R315 ⑨f: the prompt state is trimmed by whole sections, and says which it dropped', () => {
  const S = makeAtlasState({});
