/* ============================================================================
 *  R348 — the final answer was a STRING, so nothing could be compared with anything
 * ----------------------------------------------------------------------------
...
test('R348 ①a: the answer schema on the client and in ai-proxy are the same schema', () => {
...
test('R348 ①b: the model is given no field in which to put a URL', () => {
...
test('R348 ①c: the six meanings of 「支えている」 are an enumeration, not prose', () => {
...
test('R348 ①d: a percentage and a percentage point are different classes', () => {
...
test('R348 ①e: a figure is read out of the sentence with its unit, and a bare year is not a figure', () => {
...
test('R348 ②a: the fabricated host the reader was shown is refused', () => {
...
test('R348 ②b: every rejection has its own reason, and none of them is a shrug', () => {
  const cases = [
    ['', 'empty'],
...
test('R348 ②c: the identity of a document ignores the campaign it arrived through', () => {
...
test('R348 ②d: a URL nothing fetched cannot become a record', () => {
  const reg = EV.makeEvidenceRegistry({ callId: 'c', retrievedAt: 'now' });
...
test('R348 ③a: 「Web検証済み」 requires the hosted search to have RUN this call', () => {
...
test('R348 ③b: a citation stamped with another call has nowhere to land', () => {
  const reg = EV.makeEvidenceRegistry({ callId: 'c1', retrievedAt: 'now' });
...
test('R348 ③c: hosted_web evidence with no search this call is an audit error', () => {
...
test('R348 ④a: the correct answer passes cleanly', () => {
  const a = auditGood();
...
test('R348 ④b: every code the audit can raise is declared with a severity', () => {
...
test('R348 ⑤a: on the demand side consumption is the largest, and saying so is not an error', () => {
  const a = auditGood();
...
test('R348 ⑤b: net exports may be important without being most of GDP', () => {
  const e = goodAnswer();
...
test('R348 ⑤c: the value-added level and the industrial growth rate are not one series', () => {
  const reg = fixtureRegistry();
  const e = goodAnswer();
...
test('R348 ⑤d: the reported opening sentence does not survive the audit', () => {
  const e = goodAnswer();
...
test('R348 ⑥a: a valid answer costs exactly one model call', async () => {
...
test('R348 ⑥b: a failed audit buys exactly ONE repair, on the same turn key', async () => {
...
test('R348 ⑥c: two failures degrade in code rather than showing unverified prose', async () => {
  const bad = RAW_GOOD(); bad.claims[0].evidenceIds = [];
...
test('R348 ⑥d: a repair that is WORSE than the original is refused', async () => {
  const bad = RAW_GOOD(); bad.claims[0].evidenceIds = [];
...
test('R348 ⑥e: two answers in flight keep their own citations', async () => {
...
test('R348 ⑦a: a URL the model wrote never becomes a link', () => {
  const e = goodAnswer();
...
test('R348 ⑦b: the source cards come from the registry, and 「Web検証済み」 only from hosted_web', () => {
  const reg = fixtureRegistry();
...
test('R348 ⑦c: a figure from IntMap\'s own data still shows a citation', () => {
...
test('R348 ⑦d: a degraded answer says so, above the prose', () => {
  const e = goodAnswer();
...
test('R348 ⑧a: a map move Atlas added itself is secondary; one the user asked for is primary', () => {
  const P = planner();
...
test('R348 ⑧b: the turn summary counts PRIMARY failures, and says the rest quietly', () => {
...
test('R348 ⑧c: every action is stamped with its goal impact as it runs', () => {
...
test('R348 ⑨a: the SOURCES: line and the PLACES: trailer no longer exist', () => {
  const src = read('js/atlas-console.js');
...
test('R348 ⑨b: no Atlas answer reads the globals a concurrent call overwrites', () => {
...
test('R348 ⑨c: askAIEnvelope is exported — it was defined in #R132 and never reachable', () => {
...
test('R348 ⑨d: the proxy knows the task, budgets it, and refuses a shape the client cannot audit', () => {
  const proxy = read('supabase/functions/ai-proxy/index.ts');
...
test('R348 ⑨e: the kernel stayed under its ceiling while gaining all of this', () => {
