/* ============================================================================
 *  R570 — WHO Disease Outbreak News, in a real browser
 ...
 *
 *  ⚠ AND THE DRAWING IS ASKED OF THE RENDERER, NOT OF THE SOURCE CODE. tests/r570-checks.test.mjs
...
test('R570 ① the layer draws one circle per country, and the panel agrees with the map', async ({ app }) => {
  const page = app.page;
...
test('R570 ② the items that are not about one country are listed and NOT placed', async ({ app }) => {
  const page = app.page;
  await outbreaksOn(page);
...
test('R570 ③ the master clock moves the window, and the map follows it', async ({ app }) => {
  const page = app.page;
  await outbreaksOn(page);
