/* ============================================================================
 *  R577 — 「これなに」 must not be answered by inventing a name  (tests)
 * ----------------------------------------------------------------------------
...
test('R577 ①: the coordinate handed to the model resolves finer than one pixel of the frame', () => {
...
test('R577 ①: at the zoom that produced the wrong answer, the coordinate resolves the building', () => {
...
test('R577 ②: a feature far larger than the view does not outrank one the view contains', () => {
...
test('R577 ②: a feature outside the frame is not reported at all', () => {
...
test('R577 ②: an unnamed footprint is never offered as an identification', () => {
...
test('R577 ②: the tags survive, so Atlas can tell a road from a building', () => {
...
test('R577 ②: the reported list is a head of a measured order, and says how much it dropped', () => {
  const many = [];
...
test('R577 ③: finding nothing produces a POSITIVE statement, never an empty block', () => {
...
test('R577 ③: a failed lookup is never reported as «nothing is there»', () => {
...
test('R577 ③: the block names the failure mode instead of asking for care in general', () => {
...
test('R577 ④: a link to a host the turn never fetched stops being a link', () => {
...
test('R577 ④: the sentence is never deleted — Atlas loses no capability to this rule', () => {
...
test('R577 ④: a host the turn actually fetched keeps its link', () => {
...
test('R577 ④: citation pills are built from the registry and are not this rule\'s business', () => {
...
test('R577 ⑤: a world view is not asked what single object is in it', () => {
  assert.equal(G.framedBudget({ south: -60, west: -180, north: 70, east: 180 }).ok, false);
});

test('R577 ⑤: the frame that produced the wrong answer is well inside the budget', () => {
  assert.equal(G.framedBudget({ south: 35.148, west: 136.842, north: 35.155, east: 136.8515 }).ok, true);
});

test('R577 ⑤: skipping the lookup is reported as «not looked up», never as an absence', () => {
...
test('R577 ⑥: relations are asked for only where the schema says they are areas', () => {
...
test('R577 ⑥: the bbox is written at full precision and the timeout is bounded', () => {
