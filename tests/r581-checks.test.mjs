/* ============================================================================
 *  R581 — WHAT ATLAS CAN SEE AT THE MOMENT IT DECIDES
 * ----------------------------------------------------------------------------
...
test('R581 ① the index names every capability the registry holds, and nothing else', () => {
...
test('R581 ② withdrawn capabilities are not advertised', () => {
  const shown = indexed(caps.index());
...
/* ③ THE DEFECT, STATED AS A NUMBER. Before this round the simulators Atlas could see at decision
      ...
      IntMap has. */
test('R581 ③ every simulator IntMap has is visible before Atlas decides', () => {
  const shown = indexed(caps.index());
...
test('R581 ④ SYS() concatenates the index, and the index comes from the registry', () => {
...
test('R581 ⑤ the index stays an index', () => {
...
test('R581 ⑥ the policy gained no clause and no blocklist', () => {
