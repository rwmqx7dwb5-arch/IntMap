/* ============================================================================
 *  R572 — 複合空間クエリを、実物のドアから、実物のデータで走らせる（ブラウザで実測）
 * ----------------------------------------------------------------------------
 ...
 *  tests/r572-checks.test.mjs ⑤ が列レジストリの側から測る。
 *
...
test('R572: the cities table is a list of cities, named for a reader, and the country is printed once', async ({ app }) => {
  const page = app.page;
...
test('R572: two runs that resolved the same rows are ONE block in the reply', async ({ app }) => {
  const page = app.page;
