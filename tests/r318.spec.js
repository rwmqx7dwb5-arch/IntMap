/* ============================================================================
 *  R318 — the Atlas control kernel, in a real browser
 * ----------------------------------------------------------------------------
 *  tests/r318-checks.test.mjs proves the SHAPE of the kernel from source and from its own modules.
 ...
 *  equality — are in tests/r318-atlas.spec.js, which is deep-tier for exactly that reason.
 *
...
test('R318 ①: the registry is in the boot bundle, and it knows capabilities whose code is absent', async () => {
...
test('R318 ②: ok cannot be made true, in the built bundle', async () => {
  /* ⚠ WHETHER IT THROWS DEPENDS ON THE CALLER, NOT ON THE VALUE. Assigning to a getter-only
     ...
     is asserted is the invariant that holds either way. tests/r318-checks.test.mjs asserts the
     throw, from a module. */
...
test('R318 ③: a point-needing capability asks for the point — it does not take the map centre', async () => {
...
test('R318 ④: the state snapshot is real, and the prompt paragraph is derived from it', async () => {
  const r = await page.evaluate(async () => {
...
test('R318 ⑤: an async OS command is not recorded as done before it settles', async () => {
  const r = await page.evaluate(async () => {
    ...
    window.IntMapOS.register('test.r318.async', () => new Promise((res) => { resolveIt = () => res({ ok: false }); }), { label: 'test' });
    window.IntMapOS.register('test.r318.reject', () => new Promise((_r, rej) => { rejectIt = () => rej(new Error('boom')); }), { label: 'test' });
    window.IntMapOS.exec('test.r318.async', { source: 'test' });
    const recA = window.IntMapOS.log().slice(-1)[0];
    const duringPending = !!recA.pending;
    window.IntMapOS.exec('test.r318.reject', { source: 'test' });
  ...
  expect(r.sync, 'the log entry read back was not the command that was just run').toBe('test.r318.async');
  ...
  /* ⚠ THE TWO THAT MATTER. Before #R318 both of these read `ok:true`: the first because a Promise
...
test('R318 ⑥: the page still runs — no console errors, and the layer panel still builds', async () => {
