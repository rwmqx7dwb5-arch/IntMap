/* ============================================================================
 *  R576 — 複合空間クエリを、実物のドアから、実物のデータで走らせる（ブラウザで実測）
 * ----------------------------------------------------------------------------
 ...
 *  tests/r576-checks.test.mjs ⑤ が列レジストリの側から測る。
 *
...
test('R576: the cities table is a list of cities, named for a reader, and the country is printed once', async ({ app }) => {
  const page = app.page;
...
test('R576: two runs that resolved the same rows are ONE block in the reply', async ({ app }) => {
  const page = app.page;
