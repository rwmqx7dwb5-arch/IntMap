/* ============================================================================
 *  IntMap · #R569 — the diagnostics must not be part of what they measure
 * ----------------------------------------------------------------------------
...
test('R569 ① ?perf=1 counts; only an explicit request buys the per-call string tables', () => {
...
test('R569 ① the light arm does not serialise a single payload; the full arm does', () => {
...
test('R569 ② the per-id and per-phase tables stop growing, say so, and never lose a count', () => {
...
test('R569 ② the source memory is bounded by deletion instead, and the adapter really deletes', () => {
...
test('R569 ③ the layers switch puts every layer back the way it found it', () => {
...
test('R569 ③ a layer someone else moved while the switch was off is not overwritten', () => {
...
test('R569 ④ the counts are taken from the public API, without serialising the style', () => {
  const map = fakeMap({
...
test('R569 ④ a renderer that exposes no tile table gets «unknown», not zero', () => {
...
test('R569 ④ the fallback still answers for a renderer without getLayersOrder', () => {
  const map = {
...
test('R569 ④ the HUD prints an unobtainable quantity as n/a and labels the tile count', () => {
  const src = read('js/perf-hud.js');
