...
/* ⚠ (#R688) THE BAND THIS ROUND BUILT IS NOW A PART OF A WIDER ONE, so the claim moves from «the
   window IS 1850-1885» to «1850-1885 is still inside it and still full». The record reaches 1689
   now, and the floor is derived rather than typed — tests/r688-histborders-deep-checks.test.mjs
...
test('④ go() serves the day-exact band from the bundle, above the snapshot fallback', () => {
  /* ⚠ (#R688) THE SPELLING IS NO LONGER A LITERAL PAIR. HB_MIN is a copy of the bundle's derived
     floor, so what this asserts is that the band is declared AND that it contains #R518's window;
     that the copy equals the bundle is r688's job. */
