/* ============================================================================
 *  R575 — パンデミック・シミュレーターが実ブラウザで走り、地図が計算と同じものを見せる
 * ----------------------------------------------------------------------------
 *  数理そのものは tests/r575-checks.test.mjs が node で 13 本測っている（人口保存・再生速度から
...
test('R575: 発生地点をクリックすると engine が走り、地図と HUD が同じ流行を示す', async ({ app }) => {
  const page = app.page;
  const errors = [];
