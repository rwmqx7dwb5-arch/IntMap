/* ============================================================================
 *  IntMap · #R568 — the diagnostics must not be part of what they measure
 * ----------------------------------------------------------------------------
...
test('R568 ① ?perf=1 counts; only an explicit request buys the per-call string tables', () => {
...
test('R568 ① the light arm does not serialise a single payload; the full arm does', () => {
...
test('R568 ② the per-id and per-phase tables stop growing, say so, and never lose a count', () => {
...
test('R568 ② the source memory is bounded by deletion instead, and the adapter really deletes', () => {
...
test('R568 ③ the layers switch puts every layer back the way it found it', () => {
...
test('R568 ③ a layer someone else moved while the switch was off is not overwritten', () => {
...
test('R568 ④ the counts are taken from the public API, without serialising the style', () => {
  const map = fakeMap({
...
test('R568 ④ a renderer that exposes no tile table gets «unknown», not zero', () => {
...
test('R568 ④ the fallback still answers for a renderer without getLayersOrder', () => {
  const map = {
...
test('R568 ④ the HUD prints an unobtainable quantity as n/a and labels the tile count', () => {
  const src = read('js/perf-hud.js');
