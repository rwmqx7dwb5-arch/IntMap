// ============================================================================
//  IntMap · tests/r574-radiation-sources.test.mjs — the radiation registry, offline  (#R574)
// ----------------------------------------------------------------------------
...
test("R574 ① every provider declares the full contract", () => {
...
test("R574 ② no provider can be steered off its own host", () => {
...
/* ── ③ the one conversion, including the one it must refuse ─────────────────────────────────────
   ...
   convert at all. */
test("R574 ③ toNanoSvH converts what upstreams declare and refuses what they do not", () => {
...
test("R574 ④ coordinates are validated, and a missing one keeps its reading", () => {
...
test("R574 ⑤ de-bfs parses real GeoJSON and reads the declared unit", () => {
...
/* ── ⑥ jp-nra: the same station arrives many times, and only the newest may survive ─────────────
   ...
   case.) */
test("R574 ⑥ jp-nra collapses repeated samples to the newest per station", () => {
...
test("R574 ⑥b a value under the detector's own declared floor says so", () => {
  const p = providerById("jp-nra");
...
test("R574 ⑦ fi-stuk joins values to named stations and drops NaN", () => {
...
test("R574 ⑧ hk-hko discovers its stations instead of listing them", () => {
...
test("R574 ⑨ the 2011 annual mean names itself and stays out of the live set", () => {
...
test("R574 ⑩ an unread provider is distinguishable from one that read zero", () => {
...
test("R574 ⑪ RadNet fails the budget by cost, and its chunks tile the whole network", () => {
...
test("R574 ⑫ us-epa reads its unit from the header and does not invent a timezone", () => {
  const p = providerById("us-epa");
...
test("R574 ⑬ the compact wire row still carries quantity and kind", () => {
...
test("R574 ⑭ every upstream host is one of the licensed networks", () => {
