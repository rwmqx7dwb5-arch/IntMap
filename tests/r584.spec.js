/* ============================================================================
 *  #R584 — the browser half of the national-elections layer
 * ----------------------------------------------------------------------------
 *  tests/r584-checks.test.mjs proves the DATA is well formed and that the runtime contains no
...
test('R584 ① the layer mounts from its own row and paints every district it draws', async ({ app }) => {
...
test('R584 ② picking a country loads it, credits ITS publisher, and takes the camera there', async ({ app }) => {
  /* the reader's own act. #R584 added `goTo` to js/layer-home.js for exactly this — a selector that
...
test('R584 ③ a real press on a constituency opens that constituency’s own result', async ({ app }) => {
