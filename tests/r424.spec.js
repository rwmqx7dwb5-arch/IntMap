...
/** ⚠ (#R452) カードは**二度**描かれる——同期の一枚目と、enrich が解決したあとの二枚目。
...
/** カードの行を丸ごと読む——#R452 は行の**有無**を見るので、見出しを鍵にした表を返す。 */
const READ_ROWS = () => {
...
test('R424 1916年の一覧で、歴史の行のサブ行が現代の行と同じように訳される（カードの Region も）', async ({ app }) => {
  ...
  /* ══ (#R452) 同じカードの、衛星から届かなくなっていた行 ══════════════════════
     ...
     **5か国 5件とも**失敗し、`catch(e){}` がそれを飲んでいた——USA のカードは **16行**で、
     ...
     ⚠ tests/r452-checks.test.mjs は出荷される module を Node で実行して同じ 3 行を見ているが、
