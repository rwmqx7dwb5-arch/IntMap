...
/* ⚠ (#R576) THE LOCATOR IS FED THE MATCHABLE VIEW, SO THESE TESTS FEED IT THE SAME THING.
...
test('R198 ③a: the built table is ten times the curated one, and every row is usable', () => {
  ...
  for (const r of doc.rows) {
    ...
    /* ── (#R576) the four appended fields, each of which something downstream now depends on ── */
...
test('R198 ③b: the client turns those rows into the shape the locator already speaks', () => {
  ...
  /* ── (#R576) the tail survives the conversion, and the two views are actually different ────── */
  ...
  assert.ok(typeof GZ.worldMatchable === 'function', 'the matching view is published (#R576)');
