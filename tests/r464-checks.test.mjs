/* ============================================================================
 *  #R464 — 設定ダイアログは「開いたとき」にしか塗られていなかった
 * ----------------------------------------------------------------------------
 ...
 *    「開き直しても 1 文字も変わらない」ことは tests/r464.spec.js が本物のブラウザで測る。
 *
...
test('R464 ① 設定ダイアログを塗る側は、言語が変わったことを聞いている', () => {
...
test('R464 ② もう存在しない id を触る行が残っていない', () => {
  const files = [];
...
test('R464 ③ 命名掃引は自分の書いた aria-label に印を付け、それだけを取り返す', () => {
...
test('R464 ④ 恒久的に隠されたヘッダの言語ピルを、relabel の引き金にしていない', () => {
...
test('R464 ⑤ 衛星の鍵欄は、言語切替では貼り替えるだけで作り直さない', () => {
...
/* ── ⑥ そして実際に走らせる: 貼り替えは「チェックの側」から要約を作る ──────────────
   ⚠⚠⚠ **綴りでは足りない。** `renderCountries()` を言語切替から呼べば国名は直る——そして
     ...
     `relabel()` を動かして測る。 */
test('R464 ⑥ run it: 貼り替えは国名を表から書き直し、要約はチェックの側から作る', () => {
...
test('R464 ⑦ renderCountries() は今も確定済みの選択からチェックを書き戻す', () => {
  const src = CODE('js/news-sources.js');
