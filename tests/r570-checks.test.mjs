/* ============================================================================
 *  R570 · THE PANDEMIC MODEL IS A NUMERICAL OBJECT, AND THESE ARE ITS INVARIANTS
 * ----------------------------------------------------------------------------
...
test('R570 ①: population is conserved, every country, every step (Test 1)', () => {
...
test('R570 ②: playback speed cannot reach the model at all (Test 2)', () => {
...
test('R570 ③: immunity 0 months is «no lasting immunity», not a 1/0 rate (Test 3)', () => {
...
test('R570 ④: latent 0 days moves nobody through a compartment they are not in (Test 4)', () => {
...
test('R570 ⑤: a run does not end while people are incubating (Test 5)', () => {
...
test('R570 ⑥: cumulative infections never decrease (Test 6)', () => {
  const m = build({ scenario: 'naive' });
  m.seed(3, 800);
  let prev = 0;
  for (let d = 0; d < 500; d++) {
...
test('R570 ⑦: mobility 0 keeps the outbreak in one country (Test 7)', () => {
...
test('R570 ⑧: the same seed is the same world; a different seed is a different one (Test 8)', () => {
...
test('R570 ⑨: R₀ below 1 dies out, above 1 grows — one closed population (Test 9)', () => {
...
test('R570 ⑩: more baseline immunity, smaller outbreak — monotonically (Test 10)', () => {
  const one = [{ name: 'X', pop: 5e7, lat: 0, lng: 0, dev: 0.6 }];
...
test('R570 ⑪: a variant exists where it emerged, not everywhere at once (Test 11)', () => {
...
test('R570 ⑫: no case dot is placed where the country is not (Test 12)', () => {
...
test('R570 ⑬: the simulator UI reads the engine and no longer does its own epidemiology', () => {
