/* ============================================================================
 *  #R471 — 「既存機能は絶対に削らない」を、利用者が「確認を取れば削ってよい」に変えた回
 * ----------------------------------------------------------------------------
...
test('R471 ① 3つの正本が、確認つきの形で削除・縮小の方針を述べている', async () => {
...
test('R471 ② どの1文書からでも「確認」の段が消えたら赤い', async () => {
...
test('R471 ③ 「勝手にはしない」が消えたら赤い', async () => {
  await withTreeLock(async () => {
    for (const f of OWNERS) {
...
test('R471 ④ Atlas の但し書きは CONSTITUTION.md が正本で、そこから消えたら赤い', async () => {
  await withTreeLock(async () => {
    /* (a) 「一体として扱う」が消える */
...
test('R471 ⑤ 他の2文書が正本を名指さなくなったら赤い', async () => {
  await withTreeLock(async () => {
...
test('R471 ⑥ 錨ごと消えても緑にならない（文が消えたせいで緑、を作らない）', async () => {
  await withTreeLock(async () => {
    for (const f of OWNERS) {
