/* ============================================================================
 *  IntMap · #R462 — the document a browser kept is older than the build it names
 * ----------------------------------------------------------------------------
...
test('R462 ① a stale document recovers itself — one reload, onto the build the server has', async () => {
...
test('R462 ② a genuinely broken deploy gets a prompt, never a reload', async () => {
...
test('R462 ③ the one-reload mark stops the second attempt', async () => {
...
test('R462 ④ no storage → no automatic reload, because the mark is what stops the loop', async () => {
...
test('R462 ⑤ offline is not a redeploy — it does nothing at all', async () => {
...
test('R462 ⑥ an unreachable server gets a prompt, not a reload', async () => {
...
test('R462 ⑦ a LAZY chunk that 404s is left to #R372 — the app is alive and holding state', async () => {
  const h = run({ serverHtml: FRESH });
...
test('R462 ⑧ a third-party script that 404s is none of its business', async () => {
  const h = run({ serverHtml: FRESH });
...
test('R462 ⑨ it fires ONCE however many of the six assets 404', async () => {
  const h = run({ serverHtml: FRESH });
...
test('R462 ⑩ the recovery is INLINE in the document, and listens in the capture phase', () => {
...
test('R462 ⑪ the one-reload mark is never cleared — clearing it IS the loop', () => {
