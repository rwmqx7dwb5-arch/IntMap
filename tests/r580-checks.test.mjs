/* ============================================================================
 *  IntMap · R580 — THE CROSS-DATASET QUERY, AUDITED END TO END
 * ----------------------------------------------------------------------------
...
test('R580 ①: the cities table admits places and refuses parts of places and places that are gone', async () => {
...
test('R580 ①b: with no verdicts shipped, nothing is refused — an older file degrades, it does not lie', async () => {
...
test('R580 ②: a row prints the name chosen for a reader, not GeoNames ASCII transliteration', async () => {
  ...
  /* ⚠ AND THE TRANSLITERATION REACHES THE READER NOWHERE IN THE FRAGMENT — not as the label and
     ...
     passed, because it was asking about the CELL; tests/r580.spec.js ③ asks about the whole
...
/* ══ ③ THE COUNTRY, ONCE ═══════════════════════════════════════════════════════════════════════ */
test('R580 ③: the country code is not printed twice, and the column shows a country', async () => {
...
test('R580 ③b: the country column compares on the CODE, whatever it prints', async () => {
...
test('R580 ④: the identity of a query is what it resolved, not which columns it printed', async () => {
  const API = await engine([gzRow('Wuzhong', 7202654, 'CN', '1790842', 'PPLA2')]);
...
test('R580 ④b: the turn keeps ONE block for two runs that resolved the same rows', () => {
...
test('R580 ④c: the shipped dispatch actually hands the key to the turn', () => {
...
test('R580 ⑤: every column the planner may name declares a source AND an origin', async () => {
  const API = await engine([]);
...
test('R580 ⑤b: the method block prints the chain of counts and how each number was obtained', async () => {
  const API = await engine([
    gzRow('Wuzhong', 7202654, 'CN', '1790842', 'PPLA2'),
    gzRow('Kowloon', 2232339, 'HK', '1819609', 'PPLX'),
  ]);
...
test('R580 ⑥: the world gazetteer carries GeoNames identity, feature code and a display name', () => {
...
test('R580 ⑥b: the reported rows, in the shipped file', () => {
...
test('R580 ⑥d: the phone gets the verdicts as well as the codes', () => {
...
test('R580 ⑥c: js/ does not carry a hand-written list of GeoNames feature codes', () => {
