...
**There are sixteen Edge Functions, and this table used to list two.** `supabase/config.toml` used
to declare five and the other three carried their deploy flag only in a header comment — a deploy
flag that lives in a comment is not configuration. All sixteen are declared there now
...
`news-geo-prompt.js`, `news-ingest.js`, `radiation-sources.js`, `volcano-parse.js`) that the CLI bundles into the functions that import it.
