...
- ⚠⚠⚠ **「発令なし」は<b>区分単位</b>で決まる。** 発令中は #R271 から発令単位で描いていたのに、
  ...
  CWA の郷鎮市区、Natural Earth admin-1、geoBoundaries ADM1。つまり**灰色に塗れる単位は、
  警報が出れば色を塗れたはずの単位**であり、それが灰色を「主張」にしている。
  ⚠ 持っていない国は**国全体の灰色**のまま（`washTier` = 1）で、パネルが**その差を数えて印字する**
  （`unitLine`）。実測、レイヤー投入 80 秒で **53 か国・1,813 区分**が単位単位、99 か国が国単位。
...
- **海岸線・湖岸線**（`cb-coast` → `js/coast-line.js`）は**国境線と全く同じ手法**で描く：
  ...
  ⚠ **既定は OFF、風レイヤーを点けたとき1回だけ提案する**（`window._imCoastAuto` のラッチ）。
  ラッチであることが「既定」と「連動」を分ける——消した読み手を次の点灯で覆さない。
...
- ⚠⚠⚠ **予報時刻はアプリ共通の時計そのもので、ECMWF 専用の時計は存在しない。**
  `window.IntMapTime`（`js/app-body.js`・#R94）はこのアプリの唯一のマスタークロックで、
  ニュース・日付つき NASA ラスタ・歴史的国境・昼夜境界がすべてそれを購読している。
  ECMWF の軸はその瞬間を**モデルが公開している最寄りの時刻へ丸めたもの**（`_followClock`）であり、
  逆に軸を動かすと**その瞬間が時計へ書き戻される**（`_pushClock`）。読み手が操作するのは
  タイムマシン（`js/news-timeline.js`）の **「予報」タブ**——モデルの軸がそのままスライダーになり、
  操作盤（最初へ／1つ戻る／再生／1つ進む／現在）は風の凡例と**同じ宣言**（`IntMapWxPlayer.IC`）を読む。
  ⚠ **`data-legend-ec-time`（「ECMWF 予報時刻」）という箱はもう無い。** ECMWF レイヤーを入れると
  勝手に開く別の箱＝**2つ目の時計**だった。各レイヤーの凡例が持つのは「**この絵がいつのものか**」の
  1 行で、それは押すと共通の時計を開く**ボタン**である（`.ecl-when`）。
  ⚠ **共通の時計への書き戻しは<b>まとめて</b>行う**（220 ms のトレーリング、再生中は撃たず停止時に1回）。
  `IntMapTime.set` はアプリ中の時刻購読者を全部起こすので、ドラッグの1画素ごとや 700 ms の再生フレーム
  ごとに撃つと、読み手が通り過ぎるだけの瞬間のために十数のレイヤーを組み直すことになる。
