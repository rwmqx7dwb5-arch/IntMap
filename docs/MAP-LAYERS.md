...
- ⚠⚠ **レイヤーが手放してよいのは自分のフレームだけ**（`release(variable)`）。無条件の `release()` は、
  ...
  **いま告げようとしている時刻以外**の読み込みに限る（`fileUrl(idx)` で判定）。
  実測: 起動後の最初の1歩は必ず負ける（冷えた場は 7〜9 秒、窓は 140 ms）——`valueNow` が null を
  返し、1.5 秒後の再試行は 25.27 °C を返した。暖まった場は窓の内側で解決するので影響を受けない。
