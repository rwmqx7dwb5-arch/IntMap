...
| 国・地域 | 出典 | 描く単位 |
|---|---|---|
| 日本 | 気象庁 `bosai/warning/data/r8/map.json`（**官署 × 速報種別ごとの最新1件**） | **市町村 (class20)**。幾何は国土数値情報 N03 を **JIS X 0402** で引く（class20 コードの上5桁が JIS コード。実測 1,805 中 1,774 が直接一致、残る31は政令指定都市・離島で区の合併により解決）。全国版 `s0001` を床とし、**画面内の都道府県は `s0010`（実測 10.9 倍の頂点数）へ昇格**する |
