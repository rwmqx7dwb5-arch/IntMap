...
- **状態は3つで、見え方も3つ。**
  ...
  ⑵ **読んだうえで何も出ていない**＝**灰色**（`rgba(200,200,203,0.42)`。= 1／単位で描くときは 2）、
  ⑶ **発令中**＝その階級の色。
  ⚠⚠⚠ **斜線のタイルは<b>線だけ</b>で、下に灰色を敷かない。** 10×10 のタイルは
  `rgba(158,162,170,0.26)` の `fillRect` で始まり、その上に斜線を引いていた——つまり
  ⑴ は「灰色＋斜線」、⑵ は「灰色」で描かれ、**斜線の国は静かな国の見え方を下に着ていた**。
  「灰色塗と灰色斜線が両方ある地域がある」はこの1行のことで、2つの主張は**見た目として排他**で
  なければならない。タイルは `clearRect` から始まり、線が信号のすべてを運ぶ。
...
- ⚠⚠⚠ **気温は<b>1つのレイヤー</b>で、切り替わるのは<b>ソースだけ</b>。** 「気温」（`ec-temp`）は
  ECMWF IFS の予報と NASA **MERRA-2 の月平均再解析**の両方を答えられる。凡例の `Source` で切り替わり、
  帯・目盛・単位・説明・並びは**どちらでも同じ**——変わるのは数値の出どころと「いつの絵か」の行だけ。
  ...
  ⚠ **再解析タイルは色から値へ戻してから塗り直す**（`js/wx-reanalysis.js`・`imwxre://`）。
  GIBS の `MERRA2_2m_Air_Temperature_Monthly` は **8bit インデックス PNG**（実測 IHDR colour type 3・
  768 バイトの PLTE）で、その配色表は NASA が公開している（183 エントリ、ref 0 = 欠測、
  ref 2–181 が 220–310 K の 0.5 K 刻み）。**183 色に重複が 1 つも無い**ので色→値は全単射であり、
  各画素をケルビンの階級へ戻してから予報と同じ表で塗る。**値は NASA のもので、変わるのは配色だけ。**
