...
- **状態は3つで、見え方も3つ。**
  ...
  ⑵ **読んだうえで何も出ていない**＝**灰色**（`QUIET_COL`。= 1／単位で描くときは 2）、
  ⑶ **発令中**＝その階級の色。
...
- ⚠⚠⚠ **「発令なし」は<b>区分単位</b>で決まる。** 発令中は #R271 から発令単位で描いていたのに、
  ...
  区別できない。主張は単位ごとに真なので、塗りも単位ごとにする（`wp-alert-quiet` ＋ 境界線
  `wp-alert-quiet-line`、**発令単位の塗りの下**）。
...
- ⚠⚠ **境界線の解像度は2段。** Eurostat NUTS の**床は 20M** のまま（欧州全体が画面に入るズームでは
  20M と 10M は同じ絵で、細かくすると再タイルの費用だけが増える）。`UNIT_HIRES_Z` を越えると
...
- ⚠ **更新間隔。** tick は 20 秒、輪番の再読み込み下限は 35 秒（relay のエッジキャッシュ 60 秒に対する
  下限）、冷たい輪番のバーストは 10 枠。実測、レイヤー投入から **45 秒で 35/35 か国・93/93 加盟**を
  読み終えており、そのあと増えるのは**読み込みではなく形**である。
...
- ⚠⚠⚠ **時刻を変えても地図は空にならない。** 新しい時刻は**空いているスロット**に不透明度 0 で組まれ、
  ...
  **そのソース自身の `sourcedata` + `isSourceLoaded`** である。`once('idle')` は
