...
| 9 | ~~**Publisher モード＝選択中 Event の媒体HQだけ**（同じ媒体は1ピン）~~ → **撤去**（決定 16） | 実装されないまま、記事モード時代のトグルだけが Event の面に残っていた |
...
| 12 | **1 つの Event は IntMap の中だけで理解できなければならない** | 出荷された最初の版はカードも詳細も**要約を 1 文字も持たず**（`desc: ''` が固定）、外部記事を開かない限り何が起きたか分からなかった。⇒ 地図付きのニュース索引であって、出来事を理解する道具ではない。記事リンクは**二次導線**へ下げる（§9.2） |
| 13 | **要約は「決定論の抽出が土台・LLM は複数媒体の統合だけ」** | 実測で **1 記事から取れる文は中央値 1**・Event の 88% は単独記事なので、単独記事に LLM を通しても `description` を言い換えるだけで新しい事実は増えない（§15 の「根拠のない AI 要約」に一歩近づくだけ）。⇒ **独立 2 媒体以上が本文を持つ Event にだけ**払う |
| 14 | **日本語訳は生成も表示も停止。News は英語** | 決定 3 を**改訂する**。`translate` 段は既定で止まり（`NEWS_TRANSLATE=on` で復帰）、`js/news-events.js` は `news_event_i18n` を読まない。⚠ **表と行は消していない**——止めたのは生成と表示であって記録ではない |
| 15 | **Bloomberg は直接 RSS へ。Reuters と AP は移せない** | Google 経由の `site:` 検索は `<description>` にリンク一覧を入れるので、この 3 社の記事は**本文 0 文字**だった。Bloomberg には生きた直接 RSS が 5 本ある（実測）。Reuters と AP は **11 の URL を試して全部 401/404**——存在しない（§3.1） |
| 16 | **Subject / Publisher のピン切替を撤去し、地図は「出来事が起きた場所」1 通りにする** | 決定 9 を**取り消す**。実装されたことは一度も無く、残っていたのは記事モード用のトグルである。Event は `pubLoc` を構造上 `null` にするので、既定の面でこれを押すと **200/200 の出来事が擬似座標へ散った**（実測 2026-08-24: NPR が東経 144.9/南緯 2.0、AP News が東経 163.8/北緯 23.4＝いずれも海上）。「どこが報じているか」は詳細の **Coverage** が媒体名と時刻で答えており、地図の第二の座標系は要らない |
...
最初に投入したのは **18媒体 / 33フィード**（`20260823120100_news_sources_seed.sql`）で、
**33/33 が 200 を返し `<item>` を持つことを投入前に測ってある**（合計 1,380 件・2026-08-23）。
その後 Bloomberg を直接 RSS へ移したので（決定 15・`20260824220000_r405_news_feeds.sql`）、
**いまは 18媒体 / 38フィード（うち有効 36）**。

| collection | 本数 | 使う相手 | 実測 |
|---|---|---|---|
| `direct_rss` | 32（＋既定オフ1） | 媒体自身のセクション別 RSS | 投入時 27 本で **922 件**。全て日時と description あり。**セクションがそのままカテゴリになる**。＋Bloomberg 5 本（§3.1・80 件・本文 98%） |
| `google_news_site` | 3（うち **1 は無効**） | 無料 RSS を持たない媒体 | `news.google.com/rss/search?q=when:24h+site:<domain>` で **Reuters 98 / AP 100**。既存の `news-relay` allowlist（`/rss/search` ＋ `q`）を**変更せずに通る**。⚠ Bloomberg の行は残したまま `enabled=false`（`news_articles.feed_id` が参照している） |
...
### 3.1 「本文が来ない 3 社」— 測って分かったこと

⚠⚠⚠ **`google_news_site` の記事は本文を 1 文字も持たない。** Google は `site:` 検索の
`<description>` に**リンク一覧**を入れるので、`isLinkList()` が正しく捨てる。実測
(2026-08-24・本番 active 1,520 本):

| 媒体 | 記事 | うち本文が取れた | |
|---|---:|---:|---|
| Reuters | 246 | **0** | 見出しだけ |
| Bloomberg | 224 | **0** | 見出しだけ |
| AP News | 164 | **0** | 見出しだけ |
| CNN | 8 | **0** | 見出しだけ（下の ⚠） |
| The Guardian | 124 | 109 (88%) | |
| Yonhap | 94 | 90 (96%) | |
| France 24 | 57 | 57 (100%) | |

⇒ **Event の 48.7% が「見出し以外に読むものが無い」**（§13.4）。これは閾値でも UI でもなく、
**上流が本文を配っていない**ことが原因である。

**Bloomberg は移せる。** 直接 RSS が 5 本生きている（実測 2026-08-24・すべて 200・最新記事は
0.2 日以内・`<link>` のホストは `www.bloomberg.com` で `news_sources.domains` と完全一致）:
`markets` 20/20 ・`politics` 20/20 ・`technology` 14/13 ・`industries` 15/15 ・`economics` 10/10
（items / うち本文 40 字以上）。⚠ 採らなかったもの: `wealth` は 200 だが 0 items、`green` は
404、`business` は 20 items あるが本文を持つのは 3 本だけ。

⚠⚠⚠ **Reuters と AP には直接フィードが存在しない。** 11 の URL を実測して全部
401 / 404 / DNS 失敗だった（`apnews.com/hub/*/rss`・`apnews.com/index.rss`・`apnews.com/rss`・
`apnews.com/feed`・`reuters.com/arc/outboundfeeds/rss/`・`reuters.com/rssfeed/world`・
`reuters.com/tools/rss`・`reuters.com/rss/world`・`feeds.reuters.com/reuters/worldNews`・
`reutersagency.com/feed/`）。**この 2 社の Event は見出しだけのままである。**
⚠ Google の `CBMi…` リダイレクトを解いて発信元へ行く道も**無い**——そのページは JS の殻で、
HTML に発信元 URL が 1 つも入っていない（実測: `og:description` / `description` /
`articleBody` すべて 0 件）。⇒ **足りないことを UI に書く**のが正しい答えである（§9.2）。

⚠⚠ **登録済みの CNN フィード（`rss.cnn.com/rss/edition_world.rss`）は凍っている。**
200 と 29 items を返すが、**最新の記事は 1,071 日前**（median 1,235 日）。`rss.cnn.com` の他の
綴りも同じ（`edition.rss` 867 日・`cnn_topstories.rss` 1,210 日）。⚠ **「200 と `<item>` が
返る」は「生きている」ではない**——#R388 が overpass で踏んだ形の一般形である。無効化は
していない（範囲外）。事実は `news_source_feeds.last_error` に記録してある。
...
| `news_articles` | 正規化された記事1本（地点と**その出どころ**——§4.1） | 全員（`status='active'` のみ） | **service_role だけ** |
...
## 4.1 地点解析 — AI が第一手段、決定論エンジンがフォールバック (#R404)

**測って分かったこと (2026-08-24・本番)。** #R29 は「AI が第一手段・辞書がフォールバック」を
作り、#R40 が**読み出し**を止めた。5 か月後に測ると、止まっていたのは読み出しだけではなかった:
...
- `analyzed_by='ai'` が 0 なのは cron の不調でもモデルの質でもない——**`AI_MODEL` の 403**
  （§7 の ⚠ と同じ原因）。`refresh-news` は例外を握り潰すので、**書かれた日から一度も成功せず、
  どこにも「なぜ 0 件か」が残っていなかった。**
- そして UI が実際に読むのはこちらの経路（`news_events`）で、**そこは AI を 1 度も呼んでいなかった。**
...
1. `fetch` が届いた記事を `IntMapNewsGeo`（`js/newsgeo.js` と 1 バイト同一）で 1 度置く。
2. `locate` が**まだ AI が見ていない記事**を新しい順に最大 240 本、20 本ずつ AI に送る。
   規則の正本は `supabase/functions/_shared/news-geo-prompt.js` の 1 本で、`refresh-news` と共有する
   （返し方だけが違う——あちらの書き込み先に種別の列が無い）。
3. AI が場所を返したらそれで上書きする。**返さなかった記事は決定論エンジンの答えのまま残す。**

**列は 2 つに分ける（#R394 の形をもう一度作らない）**
...
上限を同じ記事で使い切って**新しい記事に一生届かない**。

**確度は自己申告させず、一致を測る**

模型の自己申告は較正されていないのに、`location_confidence` として画面に出る。代わりに、
独立に走っている決定論エンジンと**一致したかを測る**——

| 観測 | `subject_confidence` | `subject_reasons` |
|---|---|---|
| 決定論エンジンと 50 km 以内 | 0.95 | `ai` / `agrees-with-gazetteer` / `<n>km` |
| 決定論エンジンが答えを持たなかった | 0.8 | `ai` / `gazetteer-had-no-answer` |
| 50 km を超えて食い違った | 0.7 | `ai` / `differs-from-gazetteer` / `<相手の名前> <n>km` |

2 つの別々の仕組みが同じ場所を指したことは、後から人が確かめられる本物の証拠であり、
**食い違った行はそのまま運用者の待ち行列**になる（§11）。

**採らない返答**（`_shared/news-ingest.js` の `parseAiPlaces()`。Node のテストから直接呼べる）

- 渡していない id ／ 同じ id の 2 度目
- 実在しない座標（`|lat|>90` / `|lng|>180` / 数でない）
- **(0,0)** ——ギニア湾の海上で、模型が「わからない」を返すときの既定値でもある
- 名前が空
- 種別は `js/newsgeo.js` の `KIND_LOCAL` と**同じ語彙**でなければ採らない（採らないのは種別だけで、
  座標は採る）。⚠ 語彙がずれると `summariseEvent` の具体性が 0 になり、**AI が置いた都市が、
  決定論エンジンが置いた国に代表を譲る。**

**壊さないための 2 つの配線**

- ⚠⚠⚠ **`fetch` の upsert は、AI が置いた記事の `subject_*` を送らない。** 同じ記事は保持期間の
  72 時間ずっとフィードに居るので、送り続けると **20 分ごとに AI の座標を踏み潰す**
  （`first_seen_at` と同じ形の罠）。鍵の揃わないオブジェクトを 1 回の upsert に混ぜられないので、
  **2 本に分けて送る**。
  ⚠⚠ その「AI 済みの指紋」は**逆から訊く**。**本番で実測 (2026-08-24)**: 1 ページぶん 1,000 件の
  指紋を `.in(…)` に詰めると PostgREST への URL が **約 65,000 文字**（指紋は 64 桁）になり、
  上流が **400 Bad Request** を返して **`fetch` 段が丸ごと落ちる＝1 行も取り込めない**。
  保持期間が 72 時間なので、**AI 済みの指紋を全部もらう**ほうが安く、URL は常に短い。
- ⚠⚠⚠ **地点が変わった記事の Event は `assign` が数え直す。** 所属が動いていなくても代表地点は
  変わる。代表地点を選び直すのは `summariseEvent` を呼ぶそこ 1 か所しか無いので、入れないと
  **AI の答えは記事の行にだけ入って地図に出ない。**

**止め方** — `supabase secrets set NEWS_GEO_AI=off`（cron を残したまま止まる）。
モデルだけ変えるなら `NEWS_GEO_MODEL`。
...
- ⚠ (#R404) **この経路の AI 呼び出しはもう 1 つある**——地点解析（§4.1）。かつてここには
  「AI に送るのは代表見出しの訳だけ」と書いてあり、プライバシーポリシーもそう説明していた。
  どちらも #R404 で直してある。
...
- ⚠⚠⚠ **`AI_MODEL` を無条件に信じてはならない。** これは Atlas 用の secret 1 つを 9 本の
  Function が読む形で、**その値が指すモデルにこのプロジェクトの鍵が届く保証はない**——実測
  (2026-08-23、#R722 で再実測) では `AI_MODEL=gpt-5.6-terra` が **403** を返し続けていた。
  `ai-proxy` はそれを知っていて 403/404 のときフォールバックの段を降りる
  （**段の正本は [`Architecture.md`](../Architecture.md) の Atlas プロキシの節**。ここには書き写さない
  ——#R722 でこの 1 行が古いまま残り、2 つの文書が別の答えを述べていた）。
  **`refresh-news` にはその retry が無い**——#R334 が測った
...
- 検索欄は**独立した行**。その下は **1 行だけ**——`すべて / ★ 保存済み`（scope）と
  カテゴリ chips が**同じ行**に並ぶ
...
**地図のピンは 1 つが 1 つの出来事である。** 地物を組むのは `newsFeatureOf()`
（`js/news-feed.js`）**ただ 1 か所**で、`ev` / `evId` / `evSources` / `evArticles` / `evCat` を
積む。⚠ **押すと出来事の詳細が開く**（外部記事ではない）——§9.2 の「1 つの Event は IntMap の
中だけで理解できなければならない」は、地図から入ってきた読者にも同じだけ当てはまる。
⚠ **帯は地図の被せものの下に出さない。** 本番実測 (2026-08-24): 右上の Map/Satellite ＋
Flat/Globe/3D の卓の下に帯が潜り、角丸の箱の切れ端だけが読者に届いた——空の `text-field` とは
別の原因で**絵は同じ**である。しかも**読めない帯が場所を確保していた**ので、読めたはずの帯が
それに負けていた。⇒ `declutterNewsBands()` が場所を取る前に `elementFromPoint` で確かめる。
⚠ **帯（`news-labels`）の文字は代表見出しである。** 規則は `IntMapMapTypography.bandText()`
1 本で、記事モードもこれを読む。⚠ かつてここが空文字だったため、帯は
`icon-text-fit:'both'` に合わせる文字を持てず、**ピルの画像だけが素の大きさで描かれていた**
（実測 2026-08-24: 画面上の帯 46/46 が空の白い箱）。
...
アプリのセグメント外皮（受け皿の中で選択側が持ち上がる）、カテゴリは平たいピル。
両方を同じ「選択＝青塗り」にすると、**1 つの操作で 2 つ選ばれている**ように読める。
⚠⚠ **同じ語を 2 つの軸に使わない。** カテゴリの先頭 chip は `All topics / 全カテゴリ` である
——`All / すべて` は scope 側が使っている。
⚠⚠ **言語を変えたら、その行は全部その言語になる。** chip の語は描画時に評価されるので、
`intmap-lang` を聞いていないと**次の再取得が着地するまで**前の言語が残る（本番実測: 5.9〜6.7 秒、
scope だけ即座に切り替わり、同じ行の右半分が英語のまま）。⇒ `relabelChips()` が語だけ貼り替える。
⚠ **再描画ではない**——件数は `HOST.globalData` から来るが言語切替はそれを空にした直後なので、
再描画すると「全カテゴリ 0」だけになって行が消える。
...
### 9.2 「1 つの Event を IntMap の中だけで理解できる」

**正本は `js/news-brief.js`（`makeNewsBrief()`）。** ⚠ **表示の層に置かない**——UI と
`scripts/news-events-eval.mjs --brief` が**同じ 1 本**を呼ぶので、歩留まりを本番のデータで
測れる（§9.1 が #R386 で踏んだのと同じ穴を開けない）。

**証拠は最初から手元にあった。** `js/news-events.js` の `MEMBER_COLS` は
`news_articles(id,title,description,…)` を取っており、`description` は `members[].description`
に載っている。読者は `differences()` ただ 1 人で、本番 1,069 Event 中 **2 件**しか発火しない。
⇒ **51% の記事が持っている本文が、画面に 1 文字も出ていなかった。**

**規則は実測から決めた**（本番 758 本の `description`・2026-08-24）:

| 測ったもの | 実測 | 扱い |
|---|---|---|
| 末尾 `Continue reading...` | 109 本 | 落とす |
| 末尾が `...` で切れている | 89 本 | 最後の不完全な文を落とす |
| 先頭のデートライン `SEOUL, Aug. 24 (Yonhap) -- ` | 85 本 | 剥がす |
| **1 記事から取れる文の数** | **中央値 1**（p75=2・最大 8） | ⇒ 2〜4 文は**複数の記事から**組む |
| 文の長さ | 中央値 118 字（p10=56・p90=246） | 40 字未満・320 字超は採らない |
| 見出しの語が第 1 文に出る割合 | 中央値 **0.25** | ⇒ 重なりで主題を判定しない（低すぎる） |

⚠⚠⚠ **「1 記事あたり中央値 1 文」がこの設計の中心にある事実である。** 単独記事の Event
（88%）から「2〜4 文」は**取れない**。無いものを埋めるには書くしかなく、それは §15 が
禁じている根拠のない AI 要約になる。⇒ **取れるだけ出し、足りないことを言う。**

⚠⚠⚠ **1 系列 1 文。** 同じ通信社の速報 3 本を 3 文並べると、読者は「3 媒体が報じた」と読む。
単独系列のときに限り同じ記事から 2 文目を採る。

⚠⚠⚠ **同一配信を「2 媒体が報じた」に見せない——ただし語の重なりでは判定できない。**
別々の系列どうしの文 **526 組**を測った（2026-08-24）:

| containment | 組 | 中身 |
|---|---:|---|
| ≥0.9 | **1** | Guardian / Japan Times ＝**本物の重複**（違いは `utilising`/`utilizing` と U+2060 だけ） |
...
⇒ **閾値を下げると価値のある独立報道が消える。** 重複と呼ぶのは ①畳んだ文字列の完全一致か
②**c ≥ 0.9 かつ長さの比 ≥ 0.6** のときだけ（0.8 帯の誤りは全部「短い文 ⊂ 長い文」の形で、
NPR 79 字 / France 24 133 字 = 0.59）。⚠ 母数は薄い（**526 組中 1 組**が発火）。それでも規則を
置くのは、外したときの被害が**独立した裏付けの捏造**＝この機能が犯しうる最も重い誤りだから。

**詳細が出すもの**（すべて 1 行ごとに出典が付く）:

1. **何が起きたか** — 原文の文をそのまま、時系列で。複数媒体なら統合文が上に付く（下）
2. **誰が・どこで・いつ** — 代表地点・初出・最新・経過時間・記事数と独立媒体数
3. **主要な数字** — ⚠⚠ #R386 は数量を**食い違ったときしか**出していなかった。抽出は当たって
   いるので食い違っていないものも出す。実測: 出せる Event が **2 → 80**（本番 1,211 件中）
4. **最新の記事で更新された点** — ⚠ 初報から 1 時間以上あとの記事だけを見て、そこで初めて
   出た数量と、初めて報じた系列。何も新しくなければ**欄ごと出さない**
5. **媒体間の一致と相違** — 一致は別々の系列が同じ値を言ったとき、相違は §9.1
6. **発信元で読む** — 媒体ごとのリンク

**LLM が足すのは 1 つだけ**——複数の媒体が別々に書いた文を 1 つの説明にまとめること
（`news-ingest` の `summarise` 段）。⚠⚠⚠ **返答をそのまま採らない。** 1 文ごとに
①どの媒体の ②**原文のどの断片**を根拠にしたかを言わせ、**サーバー側でその断片が実際に
その媒体の文に在ることを確かめる**。1 文でも通らなければ**その Event の返答を丸ごと捨てる**
（部分採用しない）。捨てても損は無い——ブラウザは決定論の抽出を出し続ける。
⚠ クライアントは保存された統合文を出す前に**引用元の媒体がいまの構成記事に在るか**をもう一度
確かめる（記事が入れ替わって次の run がまだ来ていない Event のため）。
⚠ 統合文には「IntMap がまとめた」ことと**照合に使った原文**が畳んで添えてある——読者が
「この文はどこから来たのか」を自分で確かめられる状態にする。

⚠ **カードに載せるのは 1 文だけ**（`.ev-gist`・2 行で切る）。密度を維持する。
⚠ **カードの「記事を読む」は外し、詳細の「発信元で読む」へ移した。** 二次導線に下げるとは
導線を減らすことではない——カードのボタンは代表記事 1 本しか開けなかったが、詳細には
**媒体ごとに**リンクがある。⚠ **記事モードのカードからは外していない**（`appendNewsBatch()` は
両モード共用なので、Event の項目にだけ呼ばれる `decorate()` の側で外す）。
...
リンク・時刻・座標・場所・**`kind`（`'article'` / `'event'`）**、そして本文には **`synthesis` と `gist` だけ**——どちらも媒体の原文と
...
- **道**: 読む面の帯（`.nrp-bar`）が **`.nrp-atlas`（「Ask Atlas」）** を持つ。帯は
  ...
  ——電話の初回は `IntMapConsole` がまだ undefined。呼ぶのは **`askReading()`** で、`open()` ではない
  ——`open()` だけを呼んでいた間、この道は**タブを切り替えるだけ**だった。見出しを目の前にした
  読み手が、その見出しを打ち直さないと Atlas に何も訊けない＝橋は渡っているのに、渡ったことが
  読み手に知らされていない状態である。
- **到着**: `askReading()`（`js/atlas-reading.js`）は読んでいたものの上に開く——見出し／媒体・日付・
  場所の 1 行／`loc` をピンに据える（**flyTo はしない**。読み手はまだ読んでいる）／その 1 件が
  **実際に持っているもの**から導いた質問チップを 3 つまで（場所が無い記事に場所の問いを出さない・
  `kind` が `'event'` なら出典の一致を訊く）。⚠ **固定の 3 文にしない**——#R392 が `askHere` から
  固定文を外したのと同じ理由。⚠ **何も送らない**：チップは起点で、入力欄は自由のまま。
  吹き出しは同じ module の `arrive()` が 1 か所で組み、`askHere()` も同じものを呼ぶ。
  何も読んでいなければ `askReading()` は `false` を返すが、`open()` は既に済んでいる
  ＝読む面の側に「開くだけ」の第 2 の筋書きを持たせない。
...
| 段 | すること | 実測 |
|---|---|---|
| `fetch` | 32 フィード取得 → 正規化 → 媒体の帰属 → 決定論エンジンで地点の下書き → `news_articles` へ upsert | 3.5〜3.8 s・32/32 稼働・1,367 件 → 779 行 (08-23) |
| `locate` | まだ AI が見ていない記事の地点を AI に決めさせる（§4.1） | #R404 で追加。上限 240 本/run・20 本ずつ |
...
| `summarise` | **独立 2 媒体以上が本文を持つ** Event を 1 つの説明にまとめ、根拠を照合してから保存（§9.2） | 上限 30 件/run。捨てた数と理由は `notes.summarise_reject_reasons` |
| `translate` | 代表見出しを ja へ（§7） | **既定で停止**（決定 14）。`NEWS_TRANSLATE=on` で復帰。止まっている run は `notes.translate_skipped` に残る |
| `prune` | 記事 72 h・Event 30 d・判定 30 d・★は無期限（§8） | 0.1 s (08-23) |

⚠ **kill-switch は段ごとに分かれている**（`NEWS_TRANSLATE` / `NEWS_GEO_AI` / `NEWS_SUMMARY`）。
1 本の旗が AI 経路ぜんぶの門になっていると、翻訳を止めたつもりで要約も地点解析も止まる。
⚠ **`summarise` は単独媒体の Event に 1 トークンも払わない。** 実測で Event の 88% は単独記事で、
そこに LLM を通しても `description` を言い換えるだけ（決定 13）。

⚠ 段の**順序は関数が決める**（`ORDER`）——`locate` は `fetch` より後（この run で届いた記事も
AI に掛ける）で `assign` より**前**（Event は AI の座標で組まれ、代表地点もそれで決まる）、
`embed` は `assign` より前（埋め込みが無ければ第 2 段は何も足せない）、`link` は `assign` より
後（新しくできた Event も対象にする）、`summarise` は `link` より**後**（結ばれる直前の Event を
半分の記事で要約しない）。body で並べ替えても順序は変わらない。
...
- cron は **2 本**。SQL の形は `docs/AREA-MONITORS.md` §4 と同じで、秘密は**ヘッダ**で送り、
  ...
  | job | 間隔 | body |
  |---|---|---|
  | `news-ingest-tick` | `*/20 * * * *` | `{"stages":["fetch","locate","assign","link","prune"]}` |
  | `news-ingest-summarise` | `13 * * * *` | `{"stages":["summarise"]}` |

  ⚠⚠ (#R405) **`news-ingest-summarise` は新しい job ではなく、`news-ingest-translate` を
  付け替えたものである**（`20260824220000_r405_news_feeds.sql`）。理由は 3 つ——① その command には
  秘密が入っているので migration から書き直せない（段の一覧だけを置換する）、② 日本語訳は決定 14 で
  止まったので、枠を空けたまま 1 時間ごとに「何もしない run」を呼ぶ理由が無い、③ 要約は Event 1 件
  あたりに払うもので、`summary_evidence.fp` があるので**構成記事が変わったときだけ**払う。
  ⚠ **書いてあるのに存在しない cron を残さない。** 「呼ばれない段は存在しない段」（#R404）の裏返しで、
  文書だけに在る job は「走っている機構」を名乗る列と同じ嘘である。

  ⚠ (#R404) `locate` は `news-ingest-tick` の body に入っている。cron の command には秘密が入って
  いるので、migration は command を**書き直さず**、段の一覧の文字列だけを置換する
  （`supabase/migrations/20260824210000_news_geo_ai.sql`）。**呼ばれない段は存在しない段である。**
...
### 13.4 「IntMap の中だけで読めるか」の baseline (2026-08-24)

`node scripts/news-events-eval.mjs --from-db --brief`（本番の表そのもの・active 1,211 Event /
1,520 記事）。⚠ **これは Bloomberg を直接 RSS へ移す前の値である**——移したあとに同じ
コマンドで測り直せば、何が効いたかが分かる。
...
| `none`（何も読めない） | **590** | 48.7%（理由: `no_text` 603 / `unusable_text` 9） |
| **何かしら読める** | **621** | **51.3%** |
| gist の総文数 | 817 | 同一配信として落とした文 11 |
| **数字を出せる Event** | **80** | ⚠ #R386 の表示規則では **2 件**だった |
| 一致を出せる Event | 8 | |
| 「最新で更新された点」を出せる Event | 132 | |

⚠⚠⚠ **`none` の 48.7% は閾値の問題でも UI の問題でもない。** 内訳は §3.1 の表のとおり
Reuters 246 / Bloomberg 224 / AP 164 / CNN 8 が**本文 0 件**で、上流が配っていない。
Bloomberg は移せる。**Reuters と AP は移せない**（直接フィードが存在しない）。
...
| **AI 地点解析 (#R404)** | 翻訳と同じ単価・同じ batch の大きさで、新規記事は約 780 件/日 | **約 $0.15〜0.25/日 ＝ 月 $5〜8** |
...
⚠ **(#R404) 地点解析も継続課金である。** 上の見積・上限・送信されるもの（既に公表されている
見出しと要約の冒頭 160 字だけ。利用者の情報は 1 バイトも含まない）・安いやり方（未測位の記事だけに
掛ける）を示したうえで、**利用者が「全記事に AI（#R29 と同じ）」を選んだ**。上限は
1 run 240 本＝20 分ごとで 1 日 17,280 本ぶんの天井があり、実際の新規記事量（約 780 件/日）に対して
backlog は残らない。止め方は `supabase secrets set NEWS_GEO_AI=off`（cron を残したまま止まる）。
...
⚠ **「根拠のない AI 要約」と §9.2 の要約は別物である。** §9.2 が出すのは
①媒体が実際に公表した原文の文（言い換えない）と、②**独立 2 媒体以上**の Event に限って
LLM がそれらを 1 つの説明にまとめたもので、後者は**1 文ごとの根拠の断片が原文に実在することを
サーバー側で照合してから**しか保存されない（1 文でも通らなければ Event 丸ごと捨てる）。
禁じているのは**証拠を持たない作文**であって、証拠を持つ統合ではない。
⚠ **足りないときに埋めない**のも同じ規律の側にある——本文が無い Event には「無い」と書く。
