...
### `tests/r778-tdz-in-shipped-modules-checks.test.mjs` (#R778)

**本番の Atlas が何も答えられなくなっていた形を固定する。** `run()` の中で
`const`/`let` が**自分の宣言より上で読まれていないか**を parser に訊く。

- ① `run()` の body が、読む前に全部作っていること
- ② 添付台帳（#R773）が `const turn=…` の**後ろ**にあること
- ③ ⚠ **検査が本当に赤くなれること**——配備されていた並びを変異として与え、① が拒むこと

⚠ **母集団は `run()` 1 つである。** 全 `js/` を走査する版を先に書いたが、
`js/ai-core.js:340` の健全な束縛を告発した（正しい判定には本物のスコープ解析が要る）。
**真だと示せない検査は出荷しない**——全般形は DEV-NOTES #R778 の提案。
