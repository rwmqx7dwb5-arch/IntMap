...
### `tests/r780-frame-urls-one-shape-checks.test.mjs` (#R780)

**1 つの関数に 1 つの形。** `makeViewCapture().urls()` を**評価して**、フレームが
無いときも配列を返すことを測る（`null` だった頃、唯一の読み手が `.concat` を
始めた瞬間に本番の全ターンが死んだ）。

- ① フレームが無いとき `urls()` が配列であること
- ② `reset()` の後もそうであり、カーネルが毎ターン評価する式が throw しないこと
- ③ カーネルが今も 2 つの画像源を結合していること（①② が意味を持つ前提）

⚠⚠ **ログインを要らないことがこのファイルの存在理由である。** #R777 の browser spec は
「吹き出しに到達すること」を測るが、**ログイン判定が吹き出しとモデル呼び出しの間にある**ので、
未ログインの run は投げる行の 1 歩手前で必ず止まる。
