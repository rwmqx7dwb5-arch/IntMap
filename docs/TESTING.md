...
**112 measured spec files / 77.3 min** of serial browser time against a ceiling of 77.3 min; and
...
**deep** tier — **105 spec files** — is the whole suite minus core
...
tier: `npm run test:deep`, **105 spec files** against core's 7, because #R204/#R207 turned the split
...
`data/coastline.json.gz` だけなので、**全 55,348 リングを判定し直して `data/border-coast.js` と
...
（#R564。この回で印す対象が 4,830 本から 25,506 本へ一桁増え（束が育った現在は上の 55,348 リング）ので、網羅版は CI の
...
### `npm run check:histadmin` — 83.30 MB の行政区分に、初めて門を付ける (#R680)
...
- ⚠ **どの世紀にも在force の単位があること。** `check:histborders` が 36 年の窓を 1 年ずつ測るのと
  ...
  上流の事情で、OHM が育つたびに動く（実測 2026-09-11、第1級は 1 年 73・1500 年 398・1900 年 656、
  第2級は 1 年 17・1900 年 4,181）。**「その世紀に描くものがある」だけがこの束自身の事実**である。
...
日本の令制国 15 国。由来は docs/MAP-LAYERS.md §7.7）の不変条件を測る: **relation id が null**であること
...
- ③ **本当に拒む**——線のバッファは `buffer-needs-points`、凹な窓は `clip-window-not-convex`、
  日付変更線をまたぐ窓は `clip-window-crosses-antimeridian`、存在しない列の条件は `unknown-field`。
  ⚠ **0 件は答えであって失敗ではない**（`{ok:true}`）。
- ④ **返しうる拒否コードに、全部 9 言語の文がある**——⚠ **両側を構文から集める**。
  `js/gis-ops.js` / `js/gis-project.js` / `js/gis-core.js` が返す集合と `js/gis-panel.js` が
  文を持つ集合を突き合わせる（`tests/r576-checks` ⑩ と同じ形）。検査に一覧を書くと、次に増えた
  1 件が黙って落ちる。実際に**綴りが 2 つに割れていた 1 件**をこれが出した。
...
- ② **能力の一覧が母集合である**——`makeAtlasCapabilities({})` を読み込み、`category` の集合を取り、
  ...
  ことを直接訊く。⚠ 後者は**測って足した**——前者だけでは fallback を「考え中」に書き換えても
...
10 本。門の側は**壊して鳴るかを測る**（合成の root を作って `--check` を走らせる。#R680 の仕掛け）:
階層の admin_level が**集合として**不整合なら落ちる（重複・穴）／1 階層 1 レベルは通る／
世紀の穴は落ちるが「単に遅く始まる階層」は通る／**名前の天井を述べていない階層**は落ちる／
穴埋め記録から 1 単位を落とすと「1国は丸ごと」が落ちる。描画の側は**評価して測る**:
`js/map-tools.js` を実際に読み込んで Draw の線を引かせ、**描かれる線の最大の折れ角**が
スライダーが残した折れ線の半分未満であること・折れ線の各点を**通る**こと・全頂点が地球上にあること
（最初の実装は最終頂点を `[0,0]` に置いていた）。
