...
## The day-exact border record below CShapes — `npm run check:histborders` (`scripts/build-hist-borders.mjs --check`, #R518, widened #R688)
...
⚠ **(#R688) AND «A WORLD» IS NOW MEASURED AGAINST THE RECORD NEXT DOOR, not against a number
...
1. A file that has drifted from the upstream source still passes. Only a rebuild can catch that, and
   ...
   `tests/r688-histborders-deep-checks.test.mjs` names seven more that only the widened band can
...
### `npm run check:histadmin` — 25.8 MB の行政区分に、初めて門を付ける (#R680)

`scripts/build-hist-admin1.mjs --check` は `data/hist-admin1.js`（10.3 MB・第1級 4,820 単位）と
`data/hist-admin2.js`（15.5 MB・第2級 22,708 単位）の不変条件を測る。**この 2 本は #R680 まで
