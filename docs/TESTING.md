...
**111 measured spec files / 77.3 min** of serial browser time against a ceiling of 77.3 min; and
...
**deep** tier — **104 spec files** — is the whole suite minus core
...
tier: `npm run test:deep`, **104 spec files** against core's 7, because #R204/#R207 turned the split
...
`tests/r668-checks.test.mjs`（13 本）は、区分をクリックしたときの輪郭が**上流の原寸**であること
...
### `npm run check:kuni` — 上流が黙っている区分を、出荷したバイトの側から測る (#R668)
