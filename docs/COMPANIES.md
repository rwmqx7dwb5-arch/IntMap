...
## 12. 既知の限界 — 携帯での自動フレーミング

企業を開いたときの自動フレーミング（`show({fit:true})`）は、**携帯では拠点をシートの下に置く。**

実測（本番・375×812・Toyota 33 拠点）: 見えている帯（上端 130 px）に入る拠点 **0 件**、
シートの下 **30 件**、画面外 3 件。デスクトップ（1280×800）では **25 件が可視**で正しい。

原因は所有権にある。**携帯のカメラ padding は [`js/mobile-ui.js`](../js/mobile-ui.js) が持ち**、
自分のシートと lock-step で動かす（`setPadding` / `easeTo({padding})`）。企業パネルは
`.country-popup` という**別のシート**で、`mobile-ui.js` はその存在を知らない。だから
`js/company-facilities.js` がフレーム計算に使う padding（パネルの実寸 662 px）と、
レンダラが実際に使う padding（144 px）が食い違う。

⚠ **直すには `js/mobile-ui.js` に「2 枚目のシート」を教える必要があり、それはこのラウンドの
依頼の範囲外**（`CLAUDE.md` §3.2）。ここに書いてあるのは、次に触る人が同じ場所を掘り直さない
ためである。

**今できていること**（実測で確認済み）:

- **施設の行をタップする `focus(id)` は携帯でも正しい** — Toyota 本社が `(187, 89)`、
  可視帯の中、zoom 11。携帯で「その拠点を見る」経路はこちらで、これは効いている。
- **カメラの padding は退避して戻す**（パネルを閉じたあと 144 に戻ることを実測）。
  孤児 padding を残さない。
- padding の上限は**各辺 42%** ではなく**対辺の和が 85%** になった。片側だけ大きい
  （＝シートが 1 枚だけ被っている）状況を潰さないため。
- フレーミングは**パネルが組み上がってから**行う（`setTimeout 60ms`）。それ以前は
  `#co-popup` の寸法が 0 で、避けるべきものを測れていなかった。
