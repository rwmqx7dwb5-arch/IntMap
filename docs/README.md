...
| [`AGENT-SETUP.md`](AGENT-SETUP.md) | 同上／Codex | **2 製品の配線図** — 何が共通で、何が製品固有で、**何が手作業で残るか**。Codex の初期設定。**Edge Function の名簿（17 本の名前と `_shared/` の扱い）と `--use-api` の実測、および USB バックアップのスクリプトが守る不変条件（§10）もここが正本**（`AGENTS.md` は 1 行で指すだけ） | 対応する製品・その設定が変わったとき／Edge Function を足す・消すとき／`scripts/backup-usb.ps1` を書き換えるとき |
