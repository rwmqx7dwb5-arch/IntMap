...
| [`AGENT-SETUP.md`](AGENT-SETUP.md) | 同上／Codex | **2 製品の配線図** — 何が共通で、何が製品固有で、**何が手作業で残るか**。Codex の初期設定。**Edge Function の名簿（20 本の名前と `_shared/` の扱い）と `--use-api` の実測、および USB バックアップのスクリプトが守る不変条件（§10）もここが正本**（`AGENTS.md` は 1 行で指すだけ） | 対応する製品・その設定が変わったとき／Edge Function を足す・消すとき／`scripts/backup-usb.ps1` を書き換えるとき |
...
| [`DATA-GOVERNANCE.md`](DATA-GOVERNANCE.md) | データを足す人／`data/` へ書く script を触る人 | **同梱データの出自・権利・鮮度の横断的な正本**（#729）— 出自／ライセンス／鮮度／完全性／優先度／重複の**語彙**（機械の正本は `js/data-governance.js`）、governance の記録が**どこに住むか**（束自身・builder の `GOVERNANCE`・レイヤー登録の `rights` と、その 3 つである理由）、`npm run check:datagov` が**何を拒み何を拒まないか**（⚠ 沈黙を拒み、古さは拒まない）、台帳 `data/governance-ledger.json` が**免責ではない**こと、「読者がどこで辿れるか」と「Atlas が何を答えられるか」。⚠ **値を持たない**——ライセンスの表も束の一覧も無く、主題別の正本はこの表が引く | データ源を足す・変える／`data/` へ書く script を足す／出自・権利・鮮度の語彙を変えたとき |
...
| [`ELECTIONS.md`](ELECTIONS.md) | 選挙を触る人 | **国政選挙レイヤーの正本** — データの契約（選挙区と結果の結合・議席の算術）、パックの作り方、境界の「版」が選挙と別である理由、出典とライセンス（表記が再配布の条件になっているもの）、採らなかったデータ源と理由。⚠ 米大統領選レイヤー（`dl-uselect`）は別のレイヤー | 選挙・データ源・契約・収録範囲を変えたとき |
...
| [`RADIATION.md`](RADIATION.md) | 放射線を触る人 | **実測放射線の正本** — 観測網の採否とライセンス、単位と量の正規化、色の尺度の由来と失効条件、時計との連動、値を読むときの注意、**なぜ EURDEP を使わないか**（技術・ライセンス・取れてしまう迂回路）。⚠ 拡散 simulation（`js/sims.js`）とは別物であることの正本でもある | 観測網を足す・外す、上流の条件が変わる、色の段を動かすとき |
