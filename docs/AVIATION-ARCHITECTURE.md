...
⚠ **台帳は isolate の記憶であり、そこが限界である。** Supabase は冷えた isolate をよく配る
（§4.2）ので、冷えた側は視野の中心から歩き直す。**isolate をまたいで残るのは機体のほう**——
どの読み取りも共有スナップショットへ merge され `STALE_DROP_S`（15 分）残るので、歩き直した
先の空は在庫から答えられる。失うのは**順序であって被覆ではない**。台帳を残すにはスナップショットの
形式を変える必要があり、それに見合わない。
