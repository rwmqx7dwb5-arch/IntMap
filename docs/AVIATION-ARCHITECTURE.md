...
⚠ **回す角度は「地図の回転」であって、方位磁針の値でも、地面に貼ったシールの向きでもない。**
`gl.POINTS` のスプライトは**ビューポートに軸を揃えて**描かれる——地図を回してもスプライトは
回らないので、`track` をそのまま渡すと「画面に無い方位磁針」に対して機首を向けることになる。
一方、スプライトは**形も潰れない**ので、角度だけを地面へ投影すると変換の半分だけを掛けたことに
なり、**捨てるほうが情報を持っている側**になる。

頂点シェーダは**3回投影する**——機体・**東へ1歩**・**北へ1歩**。この2つの差が
地面→画面のヤコビ行列 `M = [dE dN]` で、`atan(dE.y − dN.x, dE.x + dN.y)` はその**極分解の
回転成分**（＝傾きによる異方的な潰れを割り去った `M`）である。これを `track` から引くと、
北を上にした平面地図では**報告された track そのもの**、地図を回せば**その分だけ回り**、
傾けても**動かない**。方位・透視除算・globe 投影はすべて `projectTileFor3D` の中に既にあるので、
この層で導出し直すものは何も無い。Cesium 側は `_airHeading` が同じ量をカメラのベクトルから出す
（`th − track`。Cesium の billboard の回転は逆回りなので符号が反対）。

⚠ **なぜ「地面に貼った向き」ではないのか（#R411 の実測）。** #R401 は針路の1歩先を投影して
その差を使っていた。日本上空 z6.2・400機で測ると:
...
報告された track 自体はほぼ一様（0.214）なのに、**傾けると5機に4機が同じ向きを向く**。
端から見た地面は、あらゆる方位を地平線の向きへ写すからである。極分解にすると同じ実測で
**0.249 / 0.243 / 0.244 / 0.238 / 0.236** と平坦になる。
