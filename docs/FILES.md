...
gis-atlas.js                      **GIS 層が自分で持つ Atlas の扉** `window.IntMapGis.atlas`
                                  ...
                                  入力は登録済みデータセットの id・題名・`layer:<id>`（地図のレイヤー）
                                  で、解決できないときは**何が在るかを添えて**断る。出力は登録された
                                  データセットの id ＝ **次の処理の入力**。描画は別の能力
                                  （`map.drawDataset`）——「訊かれたときだけ描く」ものは
                                  「地図に書き込む」と宣言できない（観測器が `not_rendered` と言う）
...
atlas-geo-resolve.js              Atlas — 場所・地域の解決とカメラの寄せ方。
                                  ...
                                  dispatch の拒否 8 か所がこれを通る。**渡されていなければ呼び出し側の文を
                                  そのまま返す**ので、今読める拒否は 1 つも変わらない
...
atlas-capabilities.js             **能力レジストリの正本**（#R318）— IntMap が何をできるかの唯一の一覧。
                                  143 能力 × 別名・分類・副作用・生成物・危険度・確認要否・必要な対象・
...
atlas-toolsurface.js              **道具の面**（#R406）— 中核9ツール＋`find_capability`（レジストリの全143を検索・到達可能 142）／
...
atlas-schemas.js                  **引数の schema**（#R406）— 143能力ぶんの型・列挙・範囲と `required`/`anyOf`。
...
pandemic-world.js                 パンデミックの**世界**——どの行が人の住む場所か・人口・接続（#R754）
                                  （Natural Earth ＋ 4 表を 1 つの世界にまとめる。パネルと Atlas の
                                  能力が**同じ物**を受け取るので、両者は同じ流行を見る。2 段階で、
                                  行は即座・`ready` は表が settle してから＝走らせてよい時刻）
pandemic-atlas.js                 パンデミックを Atlas から駆動する口 IntMapPandemicAtlas（#R754）
                                  （run＝計算して状態を返す／draw＝その日を地図へ。計算と描画が
                                  別の能力なのは観測器の都合＝#R743。`painted()` は自分の canvas を
                                  読み、js/atlas-capabilities.js の観測器がそれを訊く）
