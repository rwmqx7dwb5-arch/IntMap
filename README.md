...
* Watch the weather forecast itself: wind, temperature, precipitation, cloud, gusts, sea-level
  ...
  runs from now to six days ahead — every weather layer has its own discrete time control in its own
  legend, listing only the hours the model actually publishes. The wind is a WebGL particle field over
  the model's own colour surface — both read the same numbers, and the value under the cursor comes
  from that same field at that same forecast hour. Live precipitation radar loops over the last two hours.
