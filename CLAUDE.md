...
- **この工程はロックを必要としない。** `--sync` は `main` を `origin/main` へ早送りするだけで、
  **branch を切り替えず、未コミットの変更に触れない**＝**冪等**。何セッションが同時に走っても
...
- **ミラー元は必ず原本（`C:\Users\gyuuk\OneDrive\IntMap`）とする。**
  temp の worktree からミラーしてはならない——それは「原本のバックアップ」ではない。
  §5 の最終工程で原本を最新化したうえで、`node scripts/master-sync.mjs --check` が
  exit 0 を返すことを確認してから同期を開始すること。
- 同期方向は **`原本 → USB` の一方向のみ**とする。
